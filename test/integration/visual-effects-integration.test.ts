...
describe('Visual Effects Integration Tests', () => {
    ...
    describe('Complete Game Scenario Tests', () => {
        test('should handle full game session with all effects', async () => {
            ...
            for (const bubbleType of specialBubbles) {
                simulateBubblePop(bubbleType, 200, 200);
    ...
    describe('Quality Settings Integration', () => {
        test('should adapt effects to different quality levels', async () => {
            ...
            for (const quality of qualityLevels) {
                expect(results[quality].frame.averageFPS).toBeGreaterThan(20);
            }
        });
    ...
    describe('Accessibility Features Integration', () => {
        ...
        test('should provide alternative feedback for visual effects', async () => {
            // 代替フィードバック設定を有効化
    ...
    describe('Mobile Performance Integration', () => {
        ...
        test('should handle touch-optimized effects', async () => {
            mockMobileDevice();
            gameEngine.start();
    ...
    // ヘルパー関数
    function simulateFrames(count: number): Promise<void> {
    ...
    function simulateBubblePop(bubbleType: string, x: number, y: number) {
    ...
    function simulateComboEffect(comboCount: number) {
        if (gameEngine.enhancedParticleManager) {
    ...
    function simulateTouchEffect(x: number, y: number) {
        if (gameEngine.enhancedParticleManager) {

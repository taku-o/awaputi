...
interface WindowWithGameEngine extends Window {
...
test.describe('Visual Effects E2E Tests', () => {
    ...
    test.describe('Basic Effect Functionality', () => {
        test('should display particle effects when bubbles pop', async ({ page }: { page: Page }) => {
            // ゲーム開始
            ...
            const logs: number = await page.evaluate(() => {
        ...
        test('should trigger combo effects with consecutive pops', async ({ page }: { page: Page }) => {
            ...
            const comboEffects: number = await page.evaluate(() => {
                return (window as WindowWithGameEngine).gameEngine?.enhancedEffectManager?.getActiveEffectCount() || 0;
            });
    ...
    test.describe('Quality Settings Integration', () => {
        test('should change effect quality through settings', async ({ page }: { page: Page }) => {
            // デバッグインターフェースを開く
    ...
    test.describe('Mobile Performance', () => {
        test('should optimize effects for mobile viewport', async ({ page }: { page: Page }) => {
            // モバイルビューポートに設定
            ...
            const particleCount: number = await page.evaluate(() => {
                return (window as WindowWithGameEngine).gameEngine?.enhancedParticleManager?.getActiveParticleCount() || 0;
            });
        ...
        test('should handle touch interactions properly', async ({ page }: { page: Page }) => {
            ...
            const touchEffects: number = await page.evaluate(() => {
                return (window as WindowWithGameEngine).gameEngine?.enhancedParticleManager?.getActiveParticleCount() || 0;
            });
    ...
    test.describe('Performance Monitoring', () => {
        test('should track performance metrics in real-time', async ({ page }: { page: Page }) => {
            // デバッグインターフェースを開く
            await page.keyboard.press('Control+Shift+E');
            await page.waitForTimeout(500);
            // FPS表示が動作していることを確認
    ...
    test.describe('Error Handling', () => {
        test('should handle JavaScript errors gracefully', async ({ page }: { page: Page }) => {
            // JavaScriptエラーをキャッチ
            const errors: string[] = [];
        ...
        test('should recover from canvas context loss', async ({ page }: { page: Page }) => {
            ...
            await page.waitForTimeout(1000);
            // ゲームが継続して動作することを確認
            const isRunning: boolean = await page.evaluate(() => {
                return (window as WindowWithGameEngine).gameEngine?.isRunning || false;
            });
    ...
    test.describe('Developer Tools Integration', () => {
        test('should provide debug interface functionality', async ({ page }: { page: Page }) => {
            // デバッグインターフェースを開く
            await page.keyboard.press('Control+Shift+E');
            // デバッグパネルが表示されることを確認
    ...
    test.describe('Seasonal Effects', () => {
        test('should apply seasonal theme effects', async ({ page }: { page: Page }) => {
            ...
            await page.waitForTimeout(2000);
            // 季節エフェクトが利用可能かチェック
            const seasonalEffectsAvailable: boolean = await page.evaluate(() => {
                return !!(window as WindowWithGameEngine).gameEngine?.seasonalEffectManager;
            });
            if (seasonalEffectsAvailable) {
                ...
                await page.waitForTimeout(500);
                // テーマが適用されていることを確認
                const currentTheme: string = await page.evaluate(() => {
                    const gameEngine = (window as WindowWithGameEngine).gameEngine;

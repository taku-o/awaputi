...
describe('ErrorScreenshotCapture', () => {  let screenshotCapture: any,
    ...
    beforeEach(() => { }
        ...
        (global as any).document = { createElement: jest.fn(() => createMockCanvas()); }
    });
        // Window プロパティのモック
    ...
    afterEach(() => { screenshotCapture.destroy(); }'
    ...
    describe('スクリーンショット管理', () => {  beforeEach(async () => {
            // テスト用スクリーンショットを追加 }
            for (let i = 0; i < 3; i++) { }
                const error = new Error(`Test error ${i}`);
        ...
        test('特定のスクリーンショットが取得される', () => {  const allScreenshots = screenshotCapture.getScreenshots();
    ...
    describe('ストレージ情報', () => {  beforeEach(async () => {
            // テスト用データを追加 }
            for (let i = 0; i < 2; i++) { }
        ...
        test('ストレージ情報が正しく取得される', () => {  const info = screenshotCapture.getStorageInfo();

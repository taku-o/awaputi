...
describe('EnhancedAutocompleteEngine', () => {
    let mockConsole: any;
    ...
    describe('getSuggestions', () => {
        test('should get command suggestions', () => {
            ...
            expect(suggestions.some(s => s.text === 'config.set')).toBe(true as any);
        });

        test('should get alias suggestions', () => {
            const suggestions = autocompleteEngine.getSuggestions('h');

            expect(suggestions.some(s => s.text === 'help')).toBe(true as any);
            expect(suggestions.some(s => s.text === 'h' && s.type === 'alias')).toBe(true as any);
        });

        test('should get fuzzy suggestions', () => {
            const suggestions = autocompleteEngine.getSuggestions('gm');

            expect(suggestions.some(s => s.text === 'game.pause')).toBe(true as any);
        });
        ...
        test('should get argument suggestions for config commands', () => {
            ...
            expect(suggestions.length).toBeGreaterThan(0);
            expect(suggestions.some(s => s.text.includes('game.'))).toBe(true as any);
        });
        ...
        test('should handle empty input', () => {
            ...
            expect(suggestions.length).toBeGreaterThan(0);
            expect(suggestions.every(s => s.text.length > 0)).toBe(true as any);
        });
    });
    ...
    describe('analyzeContext', () => {
        test('should analyze command context', () => {
            ...
            expect(context.isComplete).toBe(true as any);
        });
        ...
        test('should handle incomplete input', () => {
            ...
            expect(context.args).toEqual([]);
            expect(context.isComplete).toBe(false as any);
        });
    });
    ...
    describe('getBooleanSuggestions', () => {
        test('should provide boolean suggestions', () => {
            ...
            const suggestions = autocompleteEngine.getBooleanSuggestions(context as any);

            expect(suggestions.some(s => s.text === 'true')).toBe(true as any);
            expect(suggestions.some(s => s.text === 'false')).toBe(true as any);
        });
        ...
        test('should filter boolean suggestions by partial input', () => {
            ...
            expect(suggestions.some(s => s.text === 'false')).toBe(true as any);
            expect(suggestions.some(s => s.text === 'true')).toBe(false as any);
        });
    });
    ...
    describe('getNumberSuggestions', () => {
        test('should provide common number suggestions', () => {
            ...
            const suggestions = autocompleteEngine.getNumberSuggestions(context as any);

            expect(suggestions.some(s => s.text === '1')).toBe(true as any);
            expect(suggestions.some(s => s.text === '10')).toBe(true as any);
            expect(suggestions.some(s => s.text === '100')).toBe(true as any);
        });
        ...
        test('should handle parameter ranges', () => {
            ...
            expect(rangeValues.some(s => parseInt(s.text) >= 1 && parseInt(s.text) <= 10)).toBe(true as any);
        });
    });
    ...
    describe('updateSettings', () => {
        test('should update settings', () => {
            ...
            autocompleteEngine.updateSettings(newSettings as any);

            expect(autocompleteEngine.maxSuggestions).toBe(30);
            expect(autocompleteEngine.fuzzyMatchThreshold).toBe(0.8);
            expect(autocompleteEngine.enableSmartSuggestions).toBe(false as any);
        });
    });

...
describe('EnhancedAutocompleteEngine', () => {
    ...
    describe('getSuggestions', () => {
        test('should get command suggestions', () => {
            ...
            expect(suggestions.some((s: any) => s.text === 'config.get')).toBe(true);
            expect(suggestions.some((s: any) => s.text === 'config.set')).toBe(true);
        });

        test('should get alias suggestions', () => {
            const suggestions = autocompleteEngine.getSuggestions('h');
            expect(suggestions.some((s: any) => s.text === 'help')).toBe(true);
            expect(suggestions.some((s: any) => s.text === 'h' && s.type === 'alias')).toBe(true);
        });

        test('should get fuzzy suggestions', () => {
            const suggestions = autocompleteEngine.getSuggestions('gm');
            expect(suggestions.some((s: any) => s.text === 'game.pause')).toBe(true);
        });
        ...
        test('should get argument suggestions for config commands', () => {
            ...
            expect(suggestions.length).toBeGreaterThan(0);
            expect(suggestions.some((s: any) => s.text.includes('game.'))).toBe(true);
        });

        test('should handle empty input', () => {
            const suggestions = autocompleteEngine.getSuggestions('');
            expect(suggestions.length).toBeGreaterThan(0);
            expect(suggestions.every((s: any) => s.text.length > 0)).toBe(true);
        });
    });
    ...
    describe('getBooleanSuggestions', () => {
        test('should provide boolean suggestions', () => {
            ...
            expect(suggestions.some((s: any) => s.text === 'true')).toBe(true);
            expect(suggestions.some((s: any) => s.text === 'false')).toBe(true);
        });
        ...
        test('should filter boolean suggestions by partial input', () => {
            ...
            const suggestions = autocompleteEngine.getBooleanSuggestions(context);
            expect(suggestions.some((s: any) => s.text === 'false')).toBe(true);
            expect(suggestions.some((s: any) => s.text === 'true')).toBe(false);
        });
    });
    ...
    describe('getNumberSuggestions', () => {
        test('should provide common number suggestions', () => {
            ...
            expect(suggestions.some((s: any) => s.text === '1')).toBe(true);
            expect(suggestions.some((s: any) => s.text === '10')).toBe(true);
            expect(suggestions.some((s: any) => s.text === '100')).toBe(true);
        });
        ...
        test('should handle parameter ranges', () => {
            ...
            const rangeValues = suggestions.filter((s: any) => s.type === 'number_range');
            expect(rangeValues.length).toBeGreaterThan(0);
            expect(rangeValues.some((s: any) => parseInt(s.text) >= 1 && parseInt(s.text) <= 10)).toBe(true);
        });
    });

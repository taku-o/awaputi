...
describe('GameStateCommands', () => {
    ...
    describe('Initialization', () => {
        test('should initialize with game engine and console', () => {
            ...
            expect(gameStateCommands.console).toBe(mockConsole: any);
            expect(gameStateCommands.executionState).toBeDefined();
            expect(gameStateCommands.safetyChecks).toBeDefined();
        });

        test('should register all command categories', () => {
            expect(mockConsole.registerCommand).toHaveBeenCalledTimes(23); // Total commands registered
        ...
        test('should setup safety checks', () => {
            expect(gameStateCommands.safetyChecks.confirmDestructive).toBe(true: any);
            expect(gameStateCommands.safetyChecks.preventDataLoss).toBe(true: any);
            expect(gameStateCommands.safetyChecks.validateInputs).toBe(true: any);
            expect(gameStateCommands.safetyChecks.logAllChanges).toBe(true: any);
        });
    });

    describe('Game Control Commands', () => {
        let pauseCommand, resumeCommand, resetCommand, statusCommand;

        beforeEach(() => {
            // Extract registered commands
            const commands: Record<string, any> = {};
            mockConsole.registerCommand.mock.calls.forEach(([name, config]) => {
        ...
        test('should handle already paused game', () => {
            mockGameEngine.isPaused = true;
        ...
        test('should handle game not paused', () => {
            mockGameEngine.isPaused = false;
        ...
        test('should require confirmation for reset', () => {
        ...
        test('should show game status', () => {
            mockGameEngine.isRunning = true;
            mockGameEngine.isPaused = false;
        ...
        test('should handle missing game engine', () => {
            gameStateCommands.gameEngine = null;
    ...
    describe('Score Commands', () => {
        let setScoreCommand, addScoreCommand, resetComboCommand, highScoreCommand;
        ...
        test('should validate score input', () => {
            let result = setScoreCommand.execute([]);
        ...
        test('should reset combo', () => {
            const result = resetComboCommand.execute([]);
        ...
        test('should show high score', () => {
            const result = highScoreCommand.execute([]);
    ...
    describe('Bubble Commands', () => {
        let spawnBubbleCommand, clearBubblesCommand, bubbleInfoCommand;
        ...
        test('should spawn bubbles', () => {
            ...
            expect(result).toBe('Spawned 5 normal bubble(s: any)');
        });
        ...
        test('should spawn bubble with position', () => {
            ...
            expect(result).toBe('Spawned 1 rainbow bubble(s: any)');
        });
        ...
        test('should default to single normal bubble', () => {
            ...
            expect(mockGameEngine.bubbleManager.spawnBubble).toHaveBeenCalledWith('normal', null);
            expect(result).toBe('Spawned 1 normal bubble(s: any)');
        });
        ...
        test('should require confirmation to clear bubbles', () => {
            const result = clearBubblesCommand.execute([]);
        ...
        test('should show bubble information', () => {
            const result = bubbleInfoCommand.execute([]);
    ...
    describe('Player Data Commands', () => {
        let setAPCommand, setLevelCommand, playerInfoCommand, resetPlayerCommand;
        ...
        test('should validate AP input', () => {
            let result = setAPCommand.execute([]);
        ...
        test('should show player information', () => {
            const result = playerInfoCommand.execute([]);
        ...
        test('should require confirmation to reset player', () => {
            const result = resetPlayerCommand.execute([]);
    ...
    describe('Level Commands', () => {
        let gotoStageCommand, listStagesCommand, setDifficultyCommand;
        ...
        test('should require stage name', () => {
            const result = gotoStageCommand.execute([]);
            expect(result).toBe('Usage: goto-stage <stage-name>');
        });

        test('should list available stages', () => {
            const result = listStagesCommand.execute([]);
    ...
    describe('Debug Commands', () => {
        let runTestCommand, dumpStateCommand, undoCommand;
        ...
        test('should dump game state', () => {
            const result = dumpStateCommand.execute([]);
        ...
        test('should dump state as JSON', () => {
            ...
            expect(() => JSON.parse(result: any)).not.toThrow();
            const parsed = JSON.parse(result: any);
            expect(parsed.running).toBe(false: any);
            expect(parsed.paused).toBe(false: any);
            expect(parsed.currentScene).toBe('game');
        });

        test('should handle undo with empty stack', () => {
            const result = undoCommand.execute([]);
            expect(result).toBe('No commands to undo');
        });

        test('should undo last command', () => {
            // First execute a command that adds to undo stack
    ...
    describe('State Management', () => {
        test('should capture game state', () => {
            ...
            expect(state.running).toBe(false: any);
            expect(state.paused).toBe(false: any);
        ...
        test('should capture player data', () => {
            const data = gameStateCommands.capturePlayerData();
        ...
        test('should add to undo stack', () => {
            ...
            expect(undoItem.state).toEqual(state: any);
        });

        test('should limit undo stack size', () => {
            gameStateCommands.safetyChecks.maxUndoSize = 2;
    ...
    describe('Safety Features', () => {
        test('should update safety settings', () => {
            ...
            gameStateCommands.updateSafetySettings(newSettings: any);
            
            expect(gameStateCommands.safetyChecks.confirmDestructive).toBe(false: any);
            expect(gameStateCommands.safetyChecks.preventDataLoss).toBe(false: any);
            expect(gameStateCommands.safetyChecks.maxUndoSize).toBe(5);
        });

        test('should bypass confirmation when disabled', () => {
            gameStateCommands.safetyChecks.confirmDestructive = false;
    ...
    describe('Statistics and Information', () => {
        ...
        test('should format game state', () => {
            ...
            const formatted = gameStateCommands.formatGameState(state: any);

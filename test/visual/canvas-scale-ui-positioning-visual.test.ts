...
describe('Canvas Scale UI Positioning Visual Tests', () => {
    ...
    function simulateCanvasSize(width: number, height: number) {
    ...
    function createRenderingSnapshot(renderSnapshot: any) {
        return {
            textElements: renderSnapshot.fillTextCalls.map((call: any) => ({
                text: call[0],
                x: call[1],
                y: call[2]
            })),
            rectangles: renderSnapshot.fillRectCalls.map((call: any) => ({
                x: call[0],
                y: call[1],
                width: call[2],
                height: call[3]
    ...
    function validateProportionalConsistency(snapshots: Record<string, any>) {
    ...
    function validateTabletLayout(snapshot: any, isPortrait: boolean) {
        // タブレットレイアウトの基本検証
        expect(snapshot.fillTextCalls.length).toBeGreaterThan(0);
        ...
        if (isPortrait) {
            ...
            expect(textElements.some((call: any) => call[2] > 50)).toBe(true); // 上部からの適切な距離
        } else {
            // ランドスケープモードでの横方向の適切な配置
            const textElements = snapshot.fillTextCalls;
            expect(textElements.some((call: any) => call[1] > 50)).toBe(true); // 左端からの適切な距離
        }
    }

...
describe('Canvas Scale UI Positioning Performance Tests', () => {
    ...
    beforeAll(() => {
        ...
        performanceMonitor = {
            ...
            getAverageTime(measureName: string) {
                const measures = this.measures.filter((m: any) => m.name === measureName);
                if (measures.length === 0) return 0;
                return measures.reduce((sum: number, m: any) => sum + m.duration, 0) / measures.length;
            },
    ...
    describe('Memory Usage and Optimization', () => {
        ...
        test('should efficiently handle coordinate system cleanup', async () => {
            ...
            for (let i = 0; i < cleanupIterations; i++) {
                // 新しい座標システムを作成
                const { ScaledCoordinateManager } = await import('../../src/utils/ScaledCoordinateManager.js');
                const testManager = new ScaledCoordinateManager(responsiveCanvasManager);
    ...
    describe('Real-world Scenario Performance', () => {
        test('should maintain 60fps during typical gameplay', async () => {
            ...
            const framesWithinBudget = frameTimes.filter(time => time <= frameBudget).length;
    ...
    // ヘルパー関数
    function simulateCanvasResize(width: number, height: number) {

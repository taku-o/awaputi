import { SettingsImportExportComponent } from '../../../src/components/SettingsImportExportComponent.js';
        ...
        currentScene: {
            accessibilitySettingsManager: {
                getExtendedAccessibilitySettings: jest.fn(() => [
                    ...
                    { key: 'accessibility.largeText', label: '大きな文字' }
                ]),
...
// Jest globals で既にErrorHandlerとLocalizationManagerがモック済み
...
describe('SettingsImportExportComponent', () => {
    ...
    describe('validateImportFile()', () => {
        test('有効なJSONファイルが受け入れられる', async () => {
            ...
            const result = await component.validateImportFile(validFile);
        ...
        test('大きすぎるファイルが拒否される', async () => {
            ...
            const result = await component.validateImportFile(largeFile);
        ...
        test('サポートされていない形式が拒否される', async () => {
            ...
            const result = await component.validateImportFile(invalidFile);
    ...
    describe('validateImportData()', () => {
        test('有効なデータが受け入れられる', async () => {
            ...
            const result = await component.validateImportData(validData);
        ...
        test('タイムスタンプがないデータが拒否される', async () => {
            ...
            const result = await component.validateImportData(invalidData);
    ...
    describe('applyImportedSettings()', () => {
        ...
        test('設定が正しく適用される', async () => {
            ...
            const result = await component.applyImportedSettings(importData);
    ...
    describe('UI操作', () => {
        ...
        test('エクスポートボタンのクリックでエクスポート処理が開始される', async () => {
            ...
            await new Promise(resolve => setTimeout(resolve, 0)); // イベントループを待つ
    ...
    describe('Requirements満足確認', () => {
        ...
        test('Requirement 5.9: ファイル検証とエラーハンドリングが実装されている', async () => {
            // ファイル検証
            const validFile = new File(['{}'], 'test.json', { type: 'application/json' });
            const validResult = await component.validateImportFile(validFile);
            ...
            const dataResult = await component.validateImportData(validData);

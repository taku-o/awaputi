...
describe('EffectsCalculator', () => {
    ...
    describe('パーティクル数計算', () => {
        ...
        test('should apply quality modifier', () => {
            ...
            const lowQualityConfig = {
                getEffectsConfig: () => ({
                    particles: { maxCount: 500, quality: 0.5 }
                })
            };
    ...
    describe('アニメーション時間計算', () => {
        ...
        test('should respect time limits', () => {
            const modifiers = { durationMultiplier: 20 }; // 非常に高い倍率
            const result = calculator.calculateAnimationDuration('bubble_pop', 1.0, modifiers);
    ...
    describe('効果強度計算', () => {
        ...
        test('should handle power modifier', () => {
            const modifiers = [{ type: 'power', value: 2 }];
        ...
        test('should handle clamp modifier', () => {
            const modifiers = [{ type: 'clamp', min: 0.5, max: 1.5 }];
        ...
        test('should handle unknown modifier type', () => {
            const modifiers = [{ type: 'unknown', value: 5 }];
            const result = calculator.calculateEffectIntensity(1.0, modifiers);
    ...
    describe('画面シェイク計算', () => {
        test('should calculate screen shake correctly', () => {
            const shake = calculator.calculateScreenShake('bubble_pop');
        ...
        test('should handle custom easing', () => {
            const shake = calculator.calculateScreenShake('bubble_pop', 1.0, { easing: 'easeIn' });
            expect(shake.easing).toBe('easeIn');
        });
        
        test('should handle unknown shake type', () => {
            const shake = calculator.calculateScreenShake('unknown');
    ...
    describe('パーティクル軌道計算', () => {
        test('should calculate linear trajectory', () => {
            ...
            const trajectory = calculator.calculateParticleTrajectory(start, end, 'linear', { 
                steps: 4, 
                gravity: 0.5, 
                wind: 0, 
        ...
        test('should calculate arc trajectory', () => {
            const start = { x: 0, y: 0 };
            const end = { x: 100, y: 0 };
            const trajectory = calculator.calculateParticleTrajectory(start, end, 'arc', { steps: 2 });
        ...
        test('should handle missing end position', () => {
            ...
            const trajectory = calculator.calculateParticleTrajectory(start, null, 'linear', { 
                steps: 2, 
                gravity: 0, 
                wind: 0, 
                randomness: 0 // ランダム性を無効化
    ...
    describe('色遷移計算', () => {
        test('should calculate linear color transition', () => {
            const startColor = { r: 0, g: 0, b: 0, a: 0 };
            const endColor = { r: 255, g: 255, b: 255, a: 1 };
            
            const midColor = calculator.calculateColorTransition(startColor, endColor, 0.5, 'linear');
            expect(midColor).toEqual({ r: 127, g: 127, b: 127, a: 0.5 });
            
            const endResult = calculator.calculateColorTransition(startColor, endColor, 1.0, 'linear');
            expect(endResult).toEqual({ r: 255, g: 255, b: 255, a: 1 });
        });
        
        test('should handle ease_in blend mode', () => {
            const startColor = { r: 0, g: 0, b: 0, a: 0 };
            const endColor = { r: 100, g: 100, b: 100, a: 1 };
            
            const result = calculator.calculateColorTransition(startColor, endColor, 0.5, 'ease_in');
        ...
        test('should handle ease_out blend mode', () => {
            ...
            const result = calculator.calculateColorTransition(startColor, endColor, 0.5, 'ease_out');
            expect(result.r).toBe(75); // 100 * (1 - (1-0.5)^2)
        });
        
        test('should clamp color values', () => {
            const startColor = { r: 200, g: 200, b: 200, a: 0.8 };
            const endColor = { r: 300, g: -50, b: 100, a: 1.5 }; // 範囲外の値
            
            const result = calculator.calculateColorTransition(startColor, endColor, 1.0, 'linear');
        ...
        test('should clamp progress value', () => {
            ...
            const overResult = calculator.calculateColorTransition(startColor, endColor, 1.5, 'linear');
            expect(overResult).toEqual({ r: 100, g: 100, b: 100, a: 1 });
            
            const underResult = calculator.calculateColorTransition(startColor, endColor, -0.5, 'linear');
    ...
    describe('エフェクト優先度計算', () => {
        ...
        test('should apply combo bonus', () => {
            const context = { isCombo: true, comboCount: 10 };
    ...
    describe('パフォーマンス最適化', () => {
        test('should provide default optimization for good performance', () => {
            const optimization = calculator.calculatePerformanceOptimization({
                fps: 60,
                particleCount: 100,
                memoryUsage: 0.3,
                cpuUsage: 0.3

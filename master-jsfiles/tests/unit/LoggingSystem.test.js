...
import { LoggingSystem, getLoggingSystem } from '../../src/core/LoggingSystem.js';
...
describe('LoggingSystem', () => {
    ...
    describe('設定変更記録', () => {
        test('設定変更を記録できる', () => {
            ...
            expect(loggingSystem.logs[0].data.category).toBe('game');
            expect(loggingSystem.logs[0].data.key).toBe('score');
            expect(loggingSystem.logs[0].data.oldValue).toBe(100);
            expect(loggingSystem.logs[0].data.newValue).toBe(200);
            expect(loggingSystem.logs[0].data.changeType).toBe('update');
        });
        
        test('設定作成を記録できる', () => {
            loggingSystem.logConfigChange('game', 'newSetting', undefined, 'value', 'TestSource');
            
            expect(loggingSystem.logs[0].data.changeType).toBe('create');
        });
        
        test('設定削除を記録できる', () => {
            loggingSystem.logConfigChange('game', 'oldSetting', 'value', undefined, 'TestSource');
            
            expect(loggingSystem.logs[0].data.changeType).toBe('delete');
        });
    });
    ...
    describe('設定競合記録', () => {
        test('設定競合を記録できる', () => {
            ...
            expect(loggingSystem.logs[0].data.category).toBe('game');
            expect(loggingSystem.logs[0].data.key).toBe('score');
            expect(loggingSystem.logs[0].data.value1).toBe(100);
            expect(loggingSystem.logs[0].data.value2).toBe(200);
            expect(loggingSystem.logs[0].data.resolvedValue).toBe(200);
        });
    });
    ...
    describe('設定履歴取得', () => {
        ...
        test('カテゴリでフィルタリングできる', () => {
            ...
            expect(history[0].data.category).toBe('game');
            expect(history[1].data.category).toBe('game');
        });
        ...
        test('キーでフィルタリングできる', () => {
            ...
            expect(history[0].data.key).toBe('score');
        });
    ...
    describe('ログエクスポート', () => {
        ...
        test('未対応の形式では空文字列を返す', () => {
            const result = loggingSystem.exportLogs('unknown');
            expect(result).toBe('');
        });
    });

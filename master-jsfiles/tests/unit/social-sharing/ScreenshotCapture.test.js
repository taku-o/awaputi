...
describe('ScreenshotCapture', () => {
    ...
    beforeEach(() => {
        ...
        // toBlobのデフォルトモック実装
        mockCanvas.toBlob.mockImplementation((callback, format, quality) => {
    ...
    describe('オーバーレイ機能', () => {
        ...
        it('オーバーレイ失敗時のフォールバック', async () => {
            screenshotCapture.screenshotOverlay.createScoreOverlay.mockRejectedValue(
    ...
    describe('画像最適化', () => {
        it('画像サイズを最適化する', async () => {
            // 大きなBlobをモック
            const largeBlobMock = new Blob(['x'.repeat(600 * 1024)], { type: 'image/png' }); // 600KB
    ...
    describe('バックグラウンド処理', () => {
        ...
        it('Web Workerを使用した処理', async () => {
            ...
            spy.mockImplementation((options, resolve, reject) => {
                resolve({ blob: new Blob(['worker-result']) });
            });
    ...
    describe('クリーンアップ', () => {
        ...
        it('待機中の要求をキャンセルする', () => {
            ...
            screenshotCapture.captureQueue.push({ reject: mockReject });
...
function createMockBlob(size = 1024, type = 'image/png') {

...
export class MockFactory {
  ...
  static setupGlobalMocks() {
    // Canvas API mock
    global.HTMLCanvasElement = jest.fn(() => MockFactory.createCanvasMock());
    global.CanvasRenderingContext2D = jest.fn();
    
    // Performance API mock
    global.performance = MockFactory.createPerformanceMock();
    
    // Storage API mock
    global.localStorage = MockFactory.createLocalStorageMock();
    global.sessionStorage = MockFactory.createLocalStorageMock();
    ...
    if (!global.console.warn) global.console.warn = jest.fn();
    if (!global.console.error) global.console.error = jest.fn();
    if (!global.console.debug) global.console.debug = jest.fn();
  }
}

...
export class ConfigurationParser extends BaseComponent {
     ...
     * @returns {Object|null} 解析された設定
     */
    parseGameBalanceFile(filePath) {
        try {
            ...
            for (const bubbleType of bubbleTypes) {
                try {
                    ...
                    for (const property of properties) {
                        try {
                            const value = this.mainController.configurationManager.get('game', `bubbles.${bubbleType}.${property}`);
                        ...
                        if (property === 'score') {
                            try {
                                const scoreValue = this.mainController.configurationManager.get('game', `scoring.baseScores.${bubbleType}`);
                    ...
                    // 特殊効果を取得
                    try {
                        const effects = this.mainController.configurationManager.get('game', `bubbles.${bubbleType}.effects`);
    ...
    validateConfigurationFile(filePath) {
        ...
        try {
            ...
            validation.valid = false;
            validation.issues.push(`Cannot read file: ${error.message}`);
        }

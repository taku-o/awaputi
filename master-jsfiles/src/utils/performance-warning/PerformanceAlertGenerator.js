...
export class PerformanceAlertGenerator {
    ...
    getAlertTemplate(violationId) {
        ...
        return templateId ? this.alertTemplates.get(templateId) : null;
    }
     ...
     */
    calculateAlertSimilarity(alertId1, alertId2) {
     ...
     */
    getEscalationFactors(violation) {
        const factors = [];
        ...
        if (violation.data) {
            ...
            if (metric === 'fps' && value < threshold * 0.5) {
                factors.push('severe_fps_drop');
            } else if (metric === 'memory_pressure' && value > threshold * 1.2) {
                factors.push('severe_memory_pressure');
            } else if (metric === 'stability_score' && value < threshold * 0.5) {
     ...
     */
    escalatePriority(currentPriority, escalationLevel = 1) {
     ...
     */
    generateAlertTitle(template, violation) {
     ...
     */
    generateAlertMessage(template, violation) {
     ...
     */
    generateAlertDetails(template, violation) {
     ...
     */
    generateAlertSuggestions(violation) {
        const suggestions = [];
    ...
    calculatePriorityDistribution() {
        ...
        this.alertHistory.forEach(alert => {
            if (distribution.hasOwnProperty(alert.priority)) {

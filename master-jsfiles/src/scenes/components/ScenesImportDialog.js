...
export class ScenesImportDialog extends ScenesBaseDialog {
    constructor(gameEngine, eventBus, state) {
        ...
        // インポートタイプ
        this.importType = 'playerData'; // 'playerData', 'gameSettings', 'achievements'
     ...
     */
    renderImportTypeSelection(context, x, y, width) {
     ...
     * @param {number} width - 幅
     */
    renderValidationResult(context, x, y, width) {
     ...
     */
    validatePlayerData(data, errors) {
     ...
     * @param {Array} errors - エラー配列
     */
    validateGameSettings(data, errors) {
     ...
     * @param {Array} errors - エラー配列
     */
    validateAchievements(data, errors) {
    ...
    /**
     * ゲーム設定をインポート
     * @param {Object} data - ゲーム設定
     */
    async importGameSettings(data) {
        const currentSettings = this.gameEngine.gameSettings || {};
    ...
    async importAchievements(data) {
        try {
            if (this.gameEngine.achievementManager && data.achievements) {
                // 実績データをマージ
                data.achievements.forEach(achievement => {
                    this.gameEngine.achievementManager.unlockAchievement(achievement.id, false);
                });
            }

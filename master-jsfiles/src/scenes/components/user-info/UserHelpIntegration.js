...
export class UserHelpIntegration {
    ...
    setupEventListeners() {
        if (this.eventBus) {
            this.eventBus.on('helpSectionChanged', (section) => {
                this.changeHelpSection(section);
            });
            
            this.eventBus.on('helpSystemToggle', (active) => {
    ...
    loadHelpContent() {
        try {
            if (this.helpSystem && this.helpSystem.loadHelpContent) {
                this.helpContent = this.helpSystem.loadHelpContent();
            } else {
     ...
     * ヘルプコンポーネントでの描画
     */
    renderHelpWithComponent(context, y, height, helpTabComponent) {
    ...
    renderHelpContent(context, x, y, width, height) {
        ...
        if (content.content && Array.isArray(content.content)) {
            ...
            this.renderWrappedHelpText(context, content.content, textX, currentY, width - padding * 2, lineHeight);
        }
    }
     ...
     * ヘルプタブのクリック処理
     */
    handleHelpTabClick(x, y, helpTabComponent) {
        const canvas = this.gameEngine.canvas;
     ...
     * ヘルプセクションを変更
     */
    changeHelpSection(sectionId) {
        if (this.helpSections.find(s => s.id === sectionId)) {
            this.currentHelpSection = sectionId;
            
            // ヘルプシステムにも通知（存在する場合）
            if (this.helpSystem && this.helpSystem.changeHelpSection) {
                this.helpSystem.changeHelpSection(sectionId);
            }

...
export class UserAchievementDisplay {
    ...
    setupEventListeners() {
        if (this.eventBus) {
            ...
            this.eventBus.on('achievementCategoryChanged', (category) => {
                this.currentAchievementCategory = category;
            });
            
            this.eventBus.on('achievementUnlocked', (achievementId) => {
     ...
     * 実績コンポーネントでの描画
     */
    renderAchievementsWithComponent(context, y, height, achievementsTabComponent) {
     ...
     * 基本実績統計の描画（フォールバック）
     */
    renderBasicAchievementStats(context, x, y, width) {
        if (!this.achievementsData) return;
    ...
    getAchievementDetails(achievementId) {
        ...
        return this.achievementsData.find(achievement => achievement.id === achievementId);
    }
     ...
     */
    getCompletionRate() {
        if (!this.achievementsData || this.achievementsData.length === 0) return 0;

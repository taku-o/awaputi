...
export class HelpTab extends TabComponent {
    constructor(gameEngine, eventBus, state) {
        ...
        // ヘルプセクション
        this.helpSections = ['overview', 'categories', 'progress', 'rewards', 'tips', 'faq'];
        this.helpSectionLabels = ['概要', 'カテゴリ', '進捗', '報酬', 'ヒント', 'FAQ'];
    ...
    changeHelpSection(section) {
        if (this.helpSections.includes(section)) {
            ...
            this.eventBus.emit('help-section-changed', {
                oldSection: this.currentHelpSection,
                newSection: section
            });
        }
    }

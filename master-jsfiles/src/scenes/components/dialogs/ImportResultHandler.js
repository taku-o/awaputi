...
export class ImportResultHandler {
     ...
     * 成功結果を描画
     */
    renderSuccessResult(context, layout, y) {
     ...
     * エラー結果を描画
     */
    renderErrorResult(context, layout, y) {
     ...
     * 成功統計情報を描画
     */
    renderSuccessStats(context, layout, y) {
     ...
     * 結果アクションを描画
     */
    renderResultActions(context, layout, y) {
        if (this.mainController.data.success) {
     ...
     * 成功時のアクションを描画
     */
    renderSuccessActions(context, layout, y) {
        context.font = '12px sans-serif';
     ...
     * エラー時のアクションを描画
     */
    renderErrorActions(context, layout, y) {
        context.font = '12px sans-serif';
        context.fillStyle = '#6C757D';
        context.textAlign = 'center';
        context.textBaseline = 'top';
     ...
     * トラブルシューティングヒントを描画
     */
    renderTroubleshootingHints(context, layout, y) {
    ...
    handleComplete() {
        const result = {
            action: 'import',
            data: {
                success: this.mainController.data.success,
     ...
     * 成功完了処理
     */
    handleSuccessCompletion(result) {
        // 成功ログの記録
     ...
     * インポート統計の生成
     */
    generateImportStats(parsedData) {
        return {
     ...
     * エラーメッセージの切り捨て
     */
    truncateErrorMessage(error, maxLength = 50) {
     ...
     * 結果フィードバックアニメーション
     */
    animateResultFeedback(resultType, duration = 1000) {
        if (!this.feedbackAnimations[resultType]) return;

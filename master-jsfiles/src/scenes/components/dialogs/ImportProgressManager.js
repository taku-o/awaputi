...
export class ImportProgressManager {
     ...
     * プログレスバーを描画
     */
    renderProgressBar(context, x, y, width, height) {
        // 背景
     ...
     * 詳細進捗を描画
     */
    renderDetailedProgress(context, layout, y) {
        const stepHeight = 25;
     ...
     * ステップアイコンを描画
     */
    renderStepIcon(context, x, y, completed, current) {
        const radius = 8;
    ...
    async handleImport() {
        ...
        try {
            ...
            this.mainController.data.error = error.message || 'データの復元中にエラーが発生しました';
     ...
     * データ検証ステップを実行
     */
    async executeValidationStep(baseProgress, stepWeight) {
        this.stepProgress = 0;
     ...
     * データ解析ステップを実行
     */
    async executeParsingStep(baseProgress, stepWeight) {
        this.stepProgress = 0;
     ...
     * データ復元ステップを実行
     */
    async executeRestorationStep(baseProgress, stepWeight) {
        this.stepProgress = 0;
     ...
     * 完了ステップを実行
     */
    async executeCompletionStep(baseProgress, stepWeight) {
        this.stepProgress = 0;
     ...
     * 進捗の手動設定
     */
    setProgress(stepIndex, stepProgress, overallProgress) {
     ...
     * 進捗ステップの追加
     */
    addProgressStep(id, label, duration) {

...
export class HelpSectionSelector {
    constructor(gameEngine, eventBus, state) {
        ...
        // セクション設定
        this.sections = ['overview', 'categories', 'progress', 'rewards', 'tips', 'faq'];
        this.sectionLabels = ['概要', 'カテゴリ', '進捗', '報酬', 'ヒント', 'FAQ'];
     ...
     * @param {number} containerX - コンテナX座標
     * @param {number} containerY - コンテナY座標
     ...
     * セクションを選択
     * @param {string} section - セクションID
     * @param {number} buttonIndex - ボタンインデックス
     */
    selectSection(section, buttonIndex) {
        if (this.currentSection !== section) {
            ...
            // イベント通知
            this.eventBus.emit('help-section-changed', {
                oldSection,
                newSection: section,
                buttonIndex
            });
    ...
    /**
     * セクション変更アニメーションを開始
     * @param {string} section - アニメーション対象セクション
     */
    ...
    /**
     * HEX色をRGBに変換
     * @param {string} hex - HEX色文字列
     * @returns {Object|null} - RGB値オブジェクト
     */
    hexToRgb(hex) {
    ...
    /**
     * 利用可能なセクション一覧を取得
     * @returns {Array<Object>} - セクション情報の配列
     */

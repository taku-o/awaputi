/**
 * AccessibilitySettingsManager.js
 * 
...
export class AccessibilitySettingsManager {
    ...
    setupValidators() {
        ...
        // テキスト長制限
        this.validators.set('textLength', (value, maxLength) => {
        ...
        // 選択肢検証
        this.validators.set('options', (value, validOptions) => {
        ...
        // コントラスト比検証（アクセシビリティ）
        this.validators.set('contrastRatio', (foreground, background) => {
        ...
        // フォントサイズ検証
        this.validators.set('fontSize', (value) => {
            const numValue = parseFloat(value);
            if (isNaN(numValue)) return { valid: false, error: '数値である必要があります' };
    ...
    setupPreviewCallbacks() {
        // ハイコントラストのプレビュー
        this.previewCallbacks.set('accessibility.highContrast', (value) => {
            if (value) {
        ...
        // 大きな文字のプレビュー
        this.previewCallbacks.set('accessibility.largeText', (value) => {
            if (value) {
        ...
        // アニメーション削減のプレビュー
        this.previewCallbacks.set('accessibility.reducedMotion', (value) => {
            if (value) {
        ...
        // 色覚サポートのプレビュー
        this.previewCallbacks.set('accessibility.colorBlindSupport', (value) => {
            if (value) {
     ...
     * 拡張された設定項目の取得
     */
    getExtendedAccessibilitySettings() {
        return [
     ...
     * 設定値の検証
     */
    validateSetting(key, value, constraints = {}) {
        this.validationErrors = [];
     ...
     * 設定のエクスポート
     */
    exportSettings(format = 'json', includeProfiles = false) {
        try {
            const exportData = {

...
export class MobileWebShareHandler {
     ...
     * 共有機能の分析
     */
    async analyzeShareCapabilities() {
        const capabilities = {
            basicShare: false,
            fileShare: false,
            maxFileSize: 0,
     ...
     */
    async testSupportedMimeTypes(capabilities) {
     ...
     * 最大ファイルサイズの推定
     */
    async estimateMaxFileSize() {
        if (!this.shareCapabilities?.fileShare) {
            return 0;
        }
        ...
        if (platformLimits && platformLimits[this.browserInfo.browser]) {
            return platformLimits[this.browserInfo.browser];
        }
    ...
    canShareData(shareData) {
        try {
            ...
            return navigator.canShare ? navigator.canShare(webShareData) : true;
        } catch (error) {
            return false;
        }
    }
     ...
     * Web Share APIでの共有
     */
    async shareViaWebShareAPI(shareData) {
        const webShareData = this.convertToWebShareData(shareData);
     ...
     * フォールバック共有
     */
    async shareViaFallback(shareData, isError = false) {
        // 利用可能なフォールバックを選択
    ...
    convertToWebShareData(shareData) {
        ...
        // 基本フィールドの設定
        if (shareData.title) {
            webShareData.title = shareData.title;
        }
        
        if (shareData.text || shareData.message) {
            webShareData.text = shareData.text || shareData.message;
        }
        
        if (shareData.url) {
            webShareData.url = shareData.url;
        }

        // ファイル共有（スクリーンショット等）
        if (shareData.files && this.shareCapabilities.fileShare) {
            webShareData.files = shareData.files;
        } else if (shareData.screenshot && this.shareCapabilities.fileShare) {
            // スクリーンショットをファイルに変換
    ...
    getSupportInfo() {
        return {
            ...
            capabilities: this.shareCapabilities,
            fallbackCount: this.fallbackHandlers.size
        };
    }
}
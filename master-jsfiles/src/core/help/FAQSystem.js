...
export class FAQSystem {
    constructor(gameEngine) {
        ...
        this.sortOrder = 'relevance'; // relevance, popularity, recent, alphabetical
    ...
    async initialize() {
        try {
            ...
            ErrorHandler.handle(error, 'FAQSystem.initialize');
        }
    }
    ...
    searchFAQs(query, options = {}) {
        try {
            ...
            const matchingFAQs = new Map(); // FAQ ID -> スコア
            ...
            for (const [faqId, faq] of this.faqDatabase) {
                ...
                // 回答での部分一致
                if (answerLower.includes(query.toLowerCase())) {
                    bonusScore += 5;
                }
    ...
    /**
     * フィルタリングとページネーションの適用
     * @param {Array} faqs - FAQ配列
     * @param {Object} options - オプション
     * @returns {Array} 処理済みFAQ配列
     */
    applyFiltersAndPagination(faqs, options) {
        ...
        return {
            faqs: filteredFAQs.slice(startIndex, endIndex),
    ...
    sortFAQs(faqs, sortBy) {
        switch (sortBy) {
            ...
            case 'recent':
                return faqs.sort((a, b) => new Date(b.lastUpdated) - new Date(a.lastUpdated));
     ...
     * ユーザーフィードバックの記録
     * @param {string} faqId - FAQ ID
     * @param {string} feedbackType - フィードバックタイプ ('helpful', 'not_helpful', 'suggestion')
     ...
     * 使用統計の更新
     * @param {string} faqId - FAQ ID
     * @param {string} action - アクション ('view', 'feedback', 'search')
     * @param {string} details - 詳細情報
     */
    updateUsageStatistics(faqId, action, details = '') {
        const statsKey = `${faqId}_${action}`;
        const currentStats = this.usageStatistics.get(statsKey) || {
            count: 0,
            lastAction: null,
            details: []
        };
        ...
        if (details) {
            ...
            if (currentStats.details.length > 10) {
                currentStats.details = currentStats.details.slice(-10);
            }
        }
     ...
     * @param {string} faqId - FAQ ID
     * @param {number} limit - 取得件数
     * @returns {Array} 関連FAQ一覧
     */
    getRelatedFAQs(faqId, limit = 3) {
        const targetFAQ = this.faqDatabase.get(faqId);
        if (!targetFAQ) return [];
...
// シングルトンインスタンス管理
let faqSystemInstance = null;

/**
 * FAQSystemのシングルトンインスタンスを取得
 * @param {Object} gameEngine - ゲームエンジン
 * @returns {FAQSystem} FAQSystemインスタンス
 */
export function getFAQSystem(gameEngine) {
 ...
 * @param {Object} gameEngine - ゲームエンジン
 * @returns {FAQSystem} 新しいFAQSystemインスタンス
 */
export function reinitializeFAQSystem(gameEngine) {
    if (faqSystemInstance) {

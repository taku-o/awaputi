...
export class WeeklyChallengeManager {
     ...
     * 今週のチャレンジをチェック・生成
     */
    async checkAndGenerateWeeksChallenges() {
        ...
        if (this.lastGenerationWeek && this.lastGenerationWeek !== currentWeek) {
            await this.archiveLastWeek();
        }
    ...
    async archiveLastWeek() {
        try {
            ...
            if (this.weeklyArchive.length > this.config.maxArchiveWeeks) {
                this.weeklyArchive = this.weeklyArchive.slice(0, this.config.maxArchiveWeeks);
            }
     ...
     * テンプレートからチャレンジを作成
     */
    async createChallengeFromTemplate(template, difficulty, week, index, weekStart, weekEnd) {
        try {
    ...
    updateWeeklyProgress(progressType, value) {
        try {
            ...
            // 連続日数の更新
            if (progressType === 'CONSECUTIVE_DAYS') {
     ...
     * 手動チャレンジ再生成（デバッグ用）
     */
    async regenerateWeeksChallenges() {
        await this.archiveLastWeek();
        this.lastGenerationWeek = null;
        this.thisWeeksChallenges = [];
        this.resetWeeklyStats();

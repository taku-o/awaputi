...
export class SimplificationManager {
     ...
     * 簡素化を適用
     */
    async applySimplification(level, options = {}) {
        try {
            // 前の簡素化を元に戻す
            ...
            if (result.success) {
                ...
                this.state.sessionMetrics.simplificationCount++;
     ...
     * 現在の簡素化を元に戻す
     */
    async revertCurrentSimplification() {
        if (!this.state.currentSimplification) return;

...
export class PanelManager {
    ...
    addPanelToUI(name, config) {
        ...
        for (let i = 0; i < existingTabs.length; i++) {
            const tabName = existingTabs[i].dataset.panel;
            const tabConfig = this.panelConfigs.get(tabName);
        ...
        try {
            ...
            if (saved) {
                ...
                for (const [name, state] of Object.entries(stateData)) {
                    this.panelStates.set(name, state);
                }
            }
        } catch (error) {

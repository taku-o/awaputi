        ...
        this.registerDocumentation('developer-console', {
            ...
            content: `
                ...
                <ul>
                    ...
                    <li><code>game.clearBubbles()</code> - すべてのバブルをクリア</li>
                </ul>
...
class TestSupportTools {
    ...
    registerDocumentation(id, doc) {
        ...
        // 検索エンジンにインデックス
        this.searchEngine.indexDocument(id, doc);
    }
    ...
    setupEventHandlers() {
        ...
        document.addEventListener('click', (e) => {
            if (e.target.closest('.doc-link')) {
                const docId = e.target.closest('.doc-link').dataset.doc;
    ...
    showCategoryDocuments(category) {
        ...
        document.getElementById('help-article').innerHTML = content;
    }
    ...
    showDocument(docId) {
        ...
        document.getElementById('help-article').innerHTML = doc.content;
    ...
    showError(message) {
        document.getElementById('help-article').innerHTML = `
    ...
    indexDocument(docId, doc) {
        ...
        tokens.forEach(token => {
            ...
            tokenIndex.set(docId, {
                docId: docId,
                title: doc.title,
                frequency: (tokenIndex.get(docId)?.frequency || 0) + 1
            });
        });
    }
...
window.debugDocs = null; // インスタンスは後で設定
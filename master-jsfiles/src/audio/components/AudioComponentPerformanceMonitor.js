...
export class AudioComponentPerformanceMonitor {
    ...
    loadConfiguration() {
        try {
            ...
            this.loggingSystem.debug('AudioPerformanceMonitor', 'Configuration loaded', {
    ...
    setupConfigWatchers() {
        try {
            // パフォーマンス設定の監視
            this.configManager.watch('performance', 'optimization.optimizationInterval', (newValue) => {
            ...
            this.configManager.watch('performance', 'optimization.adaptiveMode', (newValue) => {
                if (newValue !== undefined) {
    ...
    collectAudioMetrics() {
        try {
            ...
            if (this.audioContext) {
                this.currentMetrics.latency = this.audioContext.baseLatency * 1000 || 0;
            }
     ...
     * @returns {number} アクティブノード数
     */
    countActiveAudioNodes() {
        try {
            let nodeCount = 0;
            
            // オーディオマネージャーから取得
            if (this.audioManager && typeof this.audioManager.getActiveNodeCount === 'function') {
                nodeCount = this.audioManager.getActiveNodeCount();
            } else {
                // 推定値
    ...
    /**
     * バッファーアンダーランを検出
     * @returns {number} アンダーラン数
     */
    detectBufferUnderruns() {
        try {
            // 簡易的な検出
    ...
    calculateAverageLoad(metrics) {
        const recentMetrics = metrics.slice(-10); // 最新10個
     ...
     * @param {Array} metrics - メトリクス配列
     */
    calculatePeakLoad(metrics) {
        this.analysisData.peakLoad = metrics.reduce((max, metric) => {
    ...
    determinePerformanceGrade() {
        const avgLoad = this.analysisData.averageLoad;
     ...
     */
    checkMetricAlert(type, value, message) {
    ...
    triggerAlert(alert) {
        try {
            this.loggingSystem.warn('AudioPerformanceMonitor', `Performance alert: ${alert.message}`, {
                type: alert.type,
    ...
    optimizeCPUUsage() {
        try {
            // 音質を下げる
            if (this.audioManager && typeof this.audioManager.reducQuality === 'function') {
    ...
    /**
     * パフォーマンス履歴を取得
     * @param {number} timeWindow - 時間範囲（ミリ秒）
     * @returns {Array} パフォーマンス履歴
     */
    getPerformanceHistory(timeWindow = 30000) {
        const currentTime = performance.now();
    ...
    /**
     * アクティブなアラートを取得
     * @returns {Array} アクティブなアラート
     */
    getActiveAlerts() {
        return Array.from(this.alertSystem.alerts.values())
    ...
    getStatus() {
        return {
            initialized: true,
            monitoring: this.performanceMonitor.intervalId !== null,
            metricsCount: this.performanceMonitor.metrics.size,
            alertsCount: this.alertSystem.alerts.size,
            lastUpdate: this.performanceMonitor.lastUpdateTime,

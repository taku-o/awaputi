...
export class AudioVolumeController {
    constructor(audioContext) {
        ...
        this.fadeCurves = {
            ...
            exponential: (progress) => Math.pow(progress, 2),
            logarithmic: (progress) => Math.log10(progress * 9 + 1),
            ...
            easeInOut: (progress) => progress < 0.5 ? 2 * progress * progress : 1 - 2 * Math.pow(1 - progress, 2)
        };
    ...
    initializeEqualizer() {
        try {
            ...
            this.loggingSystem.debug('AudioVolumeController', 'Equalizer initialized with bands:', this.equalizerConfig.frequencies);
        } catch (error) {
    ...
    setupEqualizerPresets() {
        ...
        Object.entries(presets).forEach(([name, gains]) => {
            this.equalizer.presets.set(name, gains);
        });
     ...
     */
    fadeIn(gainNode, duration = this.fadeConfig.defaultDuration, curve = this.fadeConfig.defaultCurve, options = {}) {
     ...
     * @param {Object} options - オプション
     */
    fadeOut(gainNode, duration = this.fadeConfig.defaultDuration, curve = this.fadeConfig.defaultCurve, options = {}) {
        return this.performAdvancedFade(gainNode, {
     ...
     * @param {number} duration - フェード時間（ミリ秒）
     */
    exponentialFade(gainNode, startVolume, endVolume, duration) {
        return this.performAdvancedFade(gainNode, {
     ...
     * 高度フェード処理
     * @param {GainNode} gainNode - 対象のゲインノード
     * @param {Object} options - フェードオプション
     */
    performAdvancedFade(gainNode, options) {
     ...
     * @param {string} curve - フェードカーブ
     */
    crossFade(fadeOutNode, fadeInNode, duration = this.fadeConfig.defaultDuration, curve = this.fadeConfig.defaultCurve) {
        return Promise.all([
     ...
     * カスタムフェード
     * @param {GainNode} gainNode - 対象のゲインノード
     * @param {Function} customCurve - カスタムカーブ関数
     * @param {number} duration - フェード時間（ミリ秒）
     * @param {Object} options - オプション
     */
    customFade(gainNode, customCurve, duration, options = {}) {
        const fadeOptions = {
            ...options,
            duration: duration
        };
        ...
        return this.performAdvancedFade(gainNode, fadeOptions).finally(() => {
            // カスタムカーブを削除
    ...
    setEqualizerEnabled(enabled) {
        try {
            ...
            this.loggingSystem.debug('AudioVolumeController', `Equalizer ${enabled ? 'enabled' : 'disabled'}`);
        } catch (error) {
     ...
     */
    isEqualizerEnabled() {
        return this.equalizer.enabled;
    }
    
    /**
     * イコライザーバンドのゲインを設定
     * @param {number} bandIndex - バンドインデックス
     * @param {number} gain - ゲイン値（dB）
     */
    setEqualizerBandGain(bandIndex, gain) {
        try {
            ...
            if (this.equalizer.enabled) {
                this.equalizer.bands[bandIndex].filter.gain.value = clampedGain;
            }
            
            this.loggingSystem.debug('AudioVolumeController', `Equalizer band ${bandIndex} gain set to: ${clampedGain}dB`);
        } catch (error) {
    ...
    getEqualizerBandGain(bandIndex) {
        try {
            ...
            return this.equalizer.bands[bandIndex].gain;
        } catch (error) {
    ...
    setEqualizerGains(gains) {
        try {
            ...
            gains.forEach((gain, index) => {
                this.setEqualizerBandGain(index, gain);
            });
            
            this.loggingSystem.debug('AudioVolumeController', 'Equalizer gains set:', gains);
        } catch (error) {
     ...
     */
    getEqualizerGains() {
        return this.equalizer.bands.map(band => band.gain);
    }
    ...
    applyEqualizerPreset(presetName) {
        try {
            ...
            this.loggingSystem.info('AudioVolumeController', `Equalizer preset applied: ${presetName}`);
        } catch (error) {
     ...
     */
    getEqualizerPresets() {
        return Array.from(this.equalizer.presets.keys());
    }
    ...
    /**
     * イコライザーの周波数レスポンスを取得
     * @param {Float32Array} frequencies - 周波数配列
     * @returns {Object} 周波数レスポンス
     */
    getEqualizerFrequencyResponse(frequencies) {
        try {
    ...
    getEqualizerBandInfo() {
        return this.equalizer.bands.map((band, index) => ({
            index: index,
    ...
    /**
     * イコライザー状態を取得
     * @returns {Object} イコライザー状態
     */
    getEqualizerStatus() {
        return {
            enabled: this.equalizer.enabled,
     ...
     */
    getEqualizerInput() {
        return this.equalizer.gainNodes[0] || null;
    }
     ...
     */
    getEqualizerOutput() {
        return this.equalizer.gainNodes[this.equalizer.gainNodes.length - 1] || null;
    }
     ...
     * @param {Object} newConfig - 新しい設定
     */
    updateEqualizerConfig(newConfig) {
    ...
    dispose() {
        try {
            ...
            this.loggingSystem.info('AudioVolumeController', 'Audio volume controller disposed');
        } catch (error) {

...
| キー | 機能 | 説明 | 戻り動作 |
|------|------|------|---------|
| **S** | 設定画面を開く | 統一された設定画面を表示 | 自動的に元の画面に戻る |
| **H** | 標準ヘルプを開く | 一般的な操作説明とヘルプを表示 | 自動的に元の画面に戻る |
...
| 言語 | ヘルプキー | 設定キー | 備考 |
|------|-----------|---------|------|
| **日本語** | H (ヘルプ) | S (設定) | 標準QWERTY配列 |
| **英語** | H (help) | S (settings) | 標準QWERTY配列 |
| **ドイツ語** | H (hilfe) | E (einstellungen) | QWERTZ配列に最適化 |
| **フランス語** | A (aide) | P (paramètres) | AZERTY配列に最適化 |
| **アラビア語** | H (مساعدة) | S (اعدادات) | RTL対応、方向キー自動調整 |
| **ヘブライ語** | H (עזרה) | S (הגדרות) | RTL対応、方向キー自動調整 |
...
| キー | 機能 | 説明 |
|------|------|------|
| **S → アクセシビリティ** | 統合設定画面 | Sキーで設定画面を開き、アクセシビリティカテゴリでまとめて設定 |
...
- **🆕 統一アクセス**: H、S、F1、Ctrl+Hキーでどの画面からでもヘルプ・設定にアクセス可能

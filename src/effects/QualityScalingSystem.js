...
export class QualityScalingSystem {
    constructor(particleManager, performanceMonitor) {
    ...
    /**
     * 品質レベルを調整
     * @param {number} direction - 調整方向 (-2, -1, 0, 1, 2)
     */
    adjustQualityLevel(direction) {
        if (direction === 0) return;
        
        const qualityOrder = ['potato', 'low', 'medium', 'high', 'ultra', 'insane'];
    ...
    /**
     * フォールバック効果を取得
     * @param {string} originalType - 元の効果タイプ
     * @returns {string} フォールバック効果タイプ
     */
    getFallbackEffect(originalType) {
        ...
        // フォールバック効果を検索
        let fallbackType = this.fallbackEffects[originalType];
    ...
    /**
     * 調整履歴を記録
     * @param {string} from - 変更前品質
     * @param {string} to - 変更後品質

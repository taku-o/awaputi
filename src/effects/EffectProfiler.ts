...
interface ParticlePerformanceMetric {
    effectType: string;
    particleCount: number;
    duration: number;
    memoryDelta: number;
...
interface EffectPerformanceMetric {
    effectType: string;
    duration: number;
    memoryDelta: number;
    timestamp: number;
...
interface ProfilingData {
    particlePerformance: Map<string, ParticlePerformanceMetric[]>;
...
interface ParticleEffectAnalysis {
    sampleCount: number;
...
export class EffectProfiler {
    ...
    private startFrameMetrics(): void {
        this.frameMetricsInterval = window.setInterval(() => {
            if (this.isActive) {
    ...
    private captureFrameMetric(): void {
    ...
    private recordRenderingMetric(entry: PerformanceMeasure): void {
    ...
    profileParticleEffect(effectType: string, particleCount: number, duration: number): Promise<ParticlePerformanceMetric> {
        ...
        return new Promise((resolve) => {
            setTimeout(() => {
                ...
                if (!this.profilingData.particlePerformance.has(effectType)) {
    ...
    profileScreenEffect(effectType: string, duration: number): Promise<EffectPerformanceMetric> {
        ...
        return new Promise((resolve) => {
            setTimeout(() => {
                ...
                if (!this.profilingData.effectPerformance.has(effectType)) {
    ...
    private getDetailedMemoryInfo(): MemorySnapshot {
        if (performance.memory) {
            return {
    ...
    private analyzeProfilingData(): ProfileAnalysis {
    ...
    private analyzeParticlePerformance(): Record<string, ParticleEffectAnalysis> {
        ...
        for (const [effectType, metrics] of this.profilingData.particlePerformance) {
            const durations = metrics.map((m: ParticlePerformanceMetric) => m.duration);
            const memoryDeltas = metrics.map((m: ParticlePerformanceMetric) => m.memoryDelta);
            analysis[effectType] = {
    ...
    private analyzeEffectPerformance(): Record<string, EffectAnalysis> {
        ...
        for (const [effectType, metrics] of this.profilingData.effectPerformance) {
            const durations = metrics.map((m: EffectPerformanceMetric) => m.duration);
            const memoryDeltas = metrics.map((m: EffectPerformanceMetric) => m.memoryDelta);
            analysis[effectType] = {
                sampleCount: metrics.length,
                averageDuration: this.calculateAverage(durations),
                maxDuration: Math.max(...durations),
    ...
    private generateOptimizationSuggestions(analysis: ProfileAnalysis): void {
    ...
    private detectMemoryLeak(memorySamples: number[]): boolean {
    ...
    private calculateOverallScore(frameAnalysis: FrameAnalysis | null, memoryAnalysis: MemoryAnalysis | null): number {
        let score = 100;
    ...
    exportProfilingData(): ProfilerExportData {
        return {

...
interface ErrorHandler {
    handleError(error: Error, context: string | ErrorContext): void;
...
interface EffectPerformanceOptimizer {
...
export class EffectErrorHandler {
    ...
    private setupPerformanceErrorHandling(): void {
        // パフォーマンス監視と自動フォールバック
        setInterval(() => {
    ...
    private isEffectRelatedError(error: any): boolean {
    ...
    private handleEffectError(error: Error, source: string): void {
        this.errorCount++;
    ...
    private getErrorContext(): ErrorContextInfo {
        return {
    ...
    private recoverFromRenderingError(): void {
        // レンダリングコンテキストの再初期化試行
        try {
            const canvas = this.gameEngine.canvas;
            if (canvas) {
    ...
    private recoverFromParticleError(): void {
        // パーティクルシステムのリセット
        if (this.gameEngine.enhancedParticleManager) {
    ...
    private enableBasicFallback(): void {
        // 最小限のエフェクトのみ有効化
        try {
    ...
    private enableBasicEffects(): void {
        if (this.gameEngine.enhancedParticleManager) {

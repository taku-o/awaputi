...
export class EffectOptimizationAdvisor {
    ...
    analyzeAndOptimize(profilingData: ProfilingDataInput): AdvisorResult {
    ...
    private generateOptimizationPlan(analysis: AnalysisResult): OptimizationRecommendation[] {
    ...
    private getStrategiesForIssue(issue: PerformanceIssue): OptimizationRecommendation[] {
    ...
    private deduplicateRecommendations(recommendations: OptimizationRecommendation[]): OptimizationRecommendation[] {
        ...
        for (const rec of recommendations) {
            ...
            if (!unique.has(key) || this.compareSeverity(unique.get(key)!.severity, rec.severity) < 0) {
                unique.set(key, rec);
            }
        }
    ...
    private autoApplyOptimizations(recommendations: OptimizationRecommendation[]): AppliedOptimization[] {
    ...
    // 最適化戦略の実装
    private reduceParticleCount(intensity: number): OptimizationResult {
    ...
    private disableComplexEffects(intensity: number): OptimizationResult {
    ...
    private optimizeRenderFrequency(intensity: number): OptimizationResult {
    ...
    private enableAggressiveCulling(intensity: number): OptimizationResult {
    ...
    private cleanupMemory(intensity: number): OptimizationResult {
        let cleanedItems = 0;
    ...
    private estimateImpact(strategyId: string, severity: SeverityLevel): number {
    ...
    private canAutoApply(strategyId: string, severity: SeverityLevel): boolean {
        // 重要度が高すぎる場合は自動適用しない
    ...
    private logOptimization(recommendation: OptimizationRecommendation, result: OptimizationResult): void {
        const logEntry: OptimizationHistoryEntry = {
            timestamp: Date.now(),
            recommendation,
            result,

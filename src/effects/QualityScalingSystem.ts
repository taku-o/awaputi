...
interface CurrentQualitySettings extends QualityLevel {
    current: string;
}
...
export class QualityScalingSystem {
     ...
     * @param force - 強制設定
     */
    public setQualityLevel(quality: string, force: boolean = false): boolean {
     ...
     * @param count - 削除する数
     */
    private removeExcessParticles(count: number): void {
        // 優先度の低いパーティクルから削除
     ...
     */
    private getParticlePriority(particle: Particle): number {
     ...
     */
    public autoAdjustQuality(performanceData: PerformanceData): void {
        ...
        try {
            ...
            if (fps < this.performanceThresholds.criticalFPS) {
                adjustmentDirection = -2; // 大幅に下げる
            } else if (fps < this.performanceThresholds.minFPS) {
                adjustmentDirection = -1; // 下げる
            } else if (fps > this.performanceThresholds.targetFPS + 10) {
                adjustmentDirection = 1; // 上げる
            }
            
            // メモリ使用量評価
            if (memoryUsage > this.performanceThresholds.memoryThreshold) {
                adjustmentDirection = Math.min(adjustmentDirection - 1, -1);
            }
            ...
            getErrorHandler().handleError(error, 'QualityScalingSystem.autoAdjustQuality');
        }
    }
     ...
     * @returns サポート状況
     */
    public isEffectSupported(effectType: string): boolean {
        const settings = this.qualityLevels[this.currentQuality];
        ...
        // 有効な効果をチェック
        if (settings.enabledEffects.includes('*')) {
            return true;
        }
     ...
     */
    public getFallbackEffect(originalType: string): string {
        // サポートされている場合は元のタイプを返す
        ...
        while (fallbackType && !this.isEffectSupported(fallbackType)) {
            fallbackType = this.fallbackEffects[fallbackType];
        }
     ...
     * @param reason - 変更理由
     */
    private recordAdjustment(from: string, to: string, reason: string): void {
     ...
     * @returns 品質設定
     */
    public getCurrentQualitySettings(): CurrentQualitySettings {
        return {

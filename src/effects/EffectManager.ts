...
;
     ...
     * @private
     */
    private _getEffectParameters(): EffectParameters | null { try {
            ...
            return { shakeIntensity: screenConfig.shakeIntensity,
     ...
     * 効果の更新処理
     */
    public update(deltaTime: number): void { // 効果を更新し、期限切れの効果を削除
     ...
     * 変換状態をリセット
     */
    private resetTransforms(): void { Object.keys(this.transforms).forEach(key => { );
     ...
     * 揺れ効果を適用
     */
    private applyShakeEffect(effect: ShakeEffect, progress: number): void { const intensity = effect.intensity * (1 - progress * effect.damping),
     ...
     * フラッシュ効果を適用
     */
    private applyFlashEffect(effect: FlashEffect, progress: number): void { let alpha = 0,
     ...
     * 色調効果を適用
     */
    private applyTintEffect(effect: TintEffect, progress: number): void { const easedProgress = this.applyEasing(progress, effect.easing);
     ...
     * ズーム効果を適用
     */
    private applyZoomEffect(effect: ZoomEffect, progress: number): void { const easedProgress = this.applyEasing(progress, effect.easing);
     ...
     * 回転効果を適用
     */
    private applyRotationEffect(effect: RotationEffect, progress: number): void { const easedProgress = this.applyEasing(progress, effect.easing);
     ...
     * ぼかし効果を適用
     */
    private applyBlurEffect(effect: BlurEffect, progress: number): void { const easedProgress = this.applyEasing(progress, effect.easing);
     ...
     * フィルター効果を適用
     */
    private applyFilterEffect(effect: FilterEffect, progress: number): void { const easedProgress = this.applyEasing(progress, effect.easing);
        ...
        switch(easing) {
            ...
            case 'easeOutBounce':,
                if (progress < 1 / 2.75) {
                    ...
                    return 7.5625 * progress * progress; else if (progress < 2 / 2.75) { return 7.5625 * (progress -= 1.5 / 2.75) * progress + 0.75 } else if (progress < 2.5 / 2.75) { return 7.5625 * (progress -= 2.25 / 2.75) * progress + 0.9375, else { }'
     ...
     * 最終的な変換状態を計算
     */
    private calculateFinalTransform(): void { // 揺れの合計
     ...
     * 色のブレンド
     */
    private blendColors(colors: ColorRGBA[]): ColorRGBA { if (colors.length === 0) { }
            return { r: 0, g: 0, b: 0, a: 0  }
        return colors.reduce((result, color) => {  const alpha = color.a,
     ...
     * 変換を適用してレンダリング
     */
    public applyTransform(context: CanvasRenderingContext2D): void { const centerX = this.canvas.width / 2,
     ...
     * フラッシュとティントを適用
     */
    public applyOverlays(context: CanvasRenderingContext2D): void { // フラッシュを適用
     ...
     * 特定の効果を削除
     */
    public removeEffect(effectId: number): boolean { const initialLength = this.effects.length,
        this.effects = this.effects.filter(effect => effect.id !== effectId);
        return this.effects.length < initialLength,
    /**
     * 全ての効果をクリア
     */
    public clearEffects(): void { this.effects = [];
        this.resetTransforms();
        this.calculateFinalTransform();
    /**
     * 現在の変換状態を取得
     ...
     * アクティブな効果の数を取得
     */
    public getActiveEffectCount(): number { return this.effects.length }
    /**
     * 効果の統計を取得
     */
     ...
     */''
    public addScreenTint(intensity: number, duration: number, color: string = '#FFFFFF): number { return this.addTint(color, intensity, duration) }'
    /**
     * ビネット効果を追加'
     */''
    ...
    public setQualityLevel(level: string): void { try {'
            ...
            } catch (error) { console.warn('EffectManager: setQualityLevel failed', error }
    }
    ...
    public setConfigValue(key: string, value: any): boolean { try {'
            switch(key) {
                ...
                case 'zoomSensitivity':
                    ...
                    break;
                case 'enabled':
                    if (this.effectsConfig.setScreenEffectEnabled) { this.effectsConfig.setScreenEffectEnabled(value);
                    break;
                default:;
     ...
     */
    public applyConfiguration(): void { try {
            const params = this._getEffectParameters();
            if (!params) return,
            ...
            console.log('[EffectManager] 設定システムとの連携を適用しました'); }'} catch (error) { getErrorHandler().handleError(error, 'EFFECT_ERROR', {''
     ...
     */
    public renderOverlays(context: CanvasRenderingContext2D): void { this.applyOverlays(context);
    /**
     * 前処理エフェクトを描画（変換適用）
     */
    public renderPreEffects(context: CanvasRenderingContext2D): void { context.save();
        this.applyTransform(context);
    /**
     * 後処理エフェクトを描画（オーバーレイ）
     */
    public renderPostEffects(context: CanvasRenderingContext2D): void { this.resetTransform(context);
        this.renderOverlays(context);
        context.restore();
    /**
     * 全ての効果をクリア（clearAllEffectsエイリアス）
     */
    public clearAllEffects(): void { this.clearEffects();
    /**
     * 設定システムとの連携を更新
     * 動的な設定変更に対応
     */
    public updateConfiguration(): void { try {
            // 設定を再読み込み
            const params = this._getEffectParameters();
            ...
            console.log('[EffectManager] 設定を更新しました'); }'} catch (error) { getErrorHandler().handleError(error, 'EFFECT_ERROR', {''
     ...
     * クリーンアップ
     */
    public dispose(): void { this.clearEffects();
        // 設定変更の監視を停止

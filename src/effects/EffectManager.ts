...
export class EffectManager {
    ...
    public createRevivalEffect(): void {
        try {
            // 強い白いフラッシュ
            this.addFlash('#FFFFFF', 0.8, 300, 'out');
            ...
            // 画面を軽く振動させる
            this.addScreenShake(15, 500, 'random');
    ...
    /**
     * 画面振動効果を追加（addShakeエイリアス）
     */
    public addShake(intensity: number, duration: number, damping: number = 0.8): number {
        // dampingを考慮した新しいシェイクエフェクトを追加
        const effect: ShakeEffect = {
            id: this.effectId++,
            type: 'shake',
            intensity: intensity,

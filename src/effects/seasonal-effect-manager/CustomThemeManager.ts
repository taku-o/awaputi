...
export interface ThemeHistoryEntry {
    action: 'save' | 'delete' | 'import' | 'export';
    themeName: string;
    timestamp: string;
    themeId: string;
}
...
export class CustomThemeManager {
     ...
     * @returns テーマデータ
     */
    loadCustomTheme(name: string): CustomTheme | null {
     ...
     * @returns テーマ一覧
     */
    getAvailableCustomThemes(): ThemeListItem[] {
     ...
     * @returns インポート成功かどうか
     */
    importTheme(themeJson: string, newName: string | null = null): boolean {
        try {
     ...
     * @returns 複製成功かどうか
     */
    duplicateTheme(sourceName: string, newName: string): boolean {
     ...
     * @returns 履歴配列
     */
    getThemeHistory(limit: number = 10): ThemeHistoryEntry[] {
     ...
     * @returns 統計情報
     */
    getThemeStatistics(): ThemeStatistics {

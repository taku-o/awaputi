...
export interface QualityController {
    canExecuteEffect(type: 'particle', priority: 'normal' | 'important' | 'critical'): boolean;
...
export class SeasonalParticleRenderer {
    ...
    createSeasonalComboEffect(x: number, y: number, comboCount: number, theme: ThemeConfig | null): void {
        ...
        try {
            const effect = theme.effects.comboEffect || theme.effects.bubbleDestruction;
    ...
    private _updateSpecialEffects(particle: SeasonalParticle, deltaTime: number): void {
        ...
        // 点滅効果
        if (particle.twinkle && particle.twinkleSpeed) {
            particle.alpha = 0.5 + Math.sin(time * particle.twinkleSpeed) * 0.5;
        }
        
        // 揺れ効果
        if (particle.wobble && particle.wobbleAmplitude) {
     ...
     * @returns 統計情報
     */
    getEffectStats(): EffectStatistics {

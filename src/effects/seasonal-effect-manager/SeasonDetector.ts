...
export class SeasonDetector {
    ...
    constructor() {
        ...
        this.seasonCheckInterval = 1000 * 60 * 60 * 24; // 24時間
     ...
     * @returns イベント名
     */
    getActiveEvent(eventThemes: EventThemes): string | null {
        const now = new Date();
     ...
     * @returns 期間内かどうか
     */
    isDateInSeason(season: Season, date: Date = new Date()): boolean {
     ...
     * @returns アクティブイベント配列
     */
    getAllActiveEvents(eventThemes: EventThemes): string[] {
        const now = new Date();
        const currentDate = `${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`;
     ...
     * @returns 季節情報サマリー
     */
    getSeasonalSummary(eventThemes: EventThemes): SeasonalSummary {
     ...
     */
    updateSettings(settings: DetectorSettings): void {
     ...
     * @returns システム状態
     */
    getDetectorStatus(): DetectorStatus {
        return {
            currentSeason: this.currentSeason,
            currentEvent: this.currentEvent,

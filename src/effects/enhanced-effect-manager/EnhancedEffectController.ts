 ...
 */
interface LightSource extends LightSourceBase {
    id: number;
...
export class EnhancedEffectController {
     ...
     * 影効果を追加
     */
    addShadowEffect(shadowObject: ShadowObject, lightSource: LightSourceBase): number {
        try {
     ...
     * 反射効果を追加
     */
    addReflectionEffect(reflectionObject: ReflectionObject, surfaceY: number, intensity: number = 0.8, distortion: number = 0.1): number {
        try {
     ...
     * 水面リップル効果を追加（反射効果の一部）
     */
    addWaterRipple(x: number, y: number, maxRadius: number = 50, speed: number = 2, intensity: number = 1.0): void {
        try {
            // 対応する反射効果を検索して追加
     ...
     * 光源を追加
     */
    addLightSource(x: number, y: number, intensity: number, color: string, radius: number, type: LightSourceType = 'point'): number {
        try {

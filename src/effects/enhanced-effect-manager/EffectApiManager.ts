...
export class EffectApiManager {
     ...
     * レンダリング設定を更新
     */
    updateRenderSettings(newSettings: Partial<RenderSettings>): void {
        try {
     ...
     * 遷移スムージングを設定
     */
    setTransitionSmoothing(enabled: boolean, duration: number = 300): void {
        try {
     ...
     * モーションブラーを設定
     */
    setMotionBlur(x: number, y: number, intensity: number): void {
        try {
     ...
     * グリッチ効果を設定
     */
    setGlitchEffect(intensity: number, frequency: number): void {
        try {
     ...
     * 現在の設定を取得
     */
    getCurrentSettings(): { renderSettings: RenderSettings; enhancedTransform: EnhancedTransform } {
        return {

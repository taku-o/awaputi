...
export class LightingSystemRenderer {
     ...
     * 光源効果をレンダリング
     */
    renderLighting(context: CanvasRenderingContext2D, lightSources: LightSource[]): void {
        try {
     ...
     * 基本的な影レンダリング
     */
    renderBasicShadows(context: CanvasRenderingContext2D, shadowCasters: ShadowCaster[], lightSources: LightSource[]): void {
        try {
     ...
     * 高度な影レンダリング
     */
    renderAdvancedShadows(context: CanvasRenderingContext2D, shadowCasters: ShadowCaster[], lightSources: LightSource[]): void {
        try {
            // バブル専用の影レンダリング
            shadowCasters.forEach(caster => {
                if (!caster.enabled) return;
     ...
     * オブジェクトの形状に基づいた影を描画
     */
    renderObjectShadow(context: CanvasRenderingContext2D, object: ShadowObject, shadowX: number, shadowY: number, dirX: number, dirY: number): void {
        try {
     ...
     * バブル専用の高品質影
     */
    renderBubbleShadow(context: CanvasRenderingContext2D, bubble: BubbleObject, light: LightSource, distance: number): void {
        try {

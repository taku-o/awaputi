...
export class EnhancedEffectManager extends EffectManager {
    private effectController: EnhancedEffectController;
    ...
    public addWipeTransition(duration: number, pattern: string = 'horizontal', direction: string = 'left'): string {
        return this.apiManager.addWipeTransition(duration, pattern, direction);
    }
    
    public addDissolveTransition(duration: number, noiseScale: number = 1.0, threshold: number = 0.5): string {
    ...
    public addShadowEffect(shadowObject: ShadowObject, lightSource: LightSource): string {
        return this.effectController.addShadowEffect(shadowObject, lightSource);
    }
    
    public addReflectionEffect(reflectionObject: ReflectionObject, surfaceY: number, intensity: number = 0.8, distortion: number = 0.1): string {
    ...
    public updateRenderSettings(newSettings: Partial<RenderSettings>): void {
    ...
    public enableOptimization(enabled: boolean): void {
        this.apiManager.enableOptimization(enabled);
    }
    
    public setTransitionSmoothing(enabled: boolean, duration: number = 300): void {
        this.apiManager.setTransitionSmoothing(enabled, duration);
    }
    
    public setDepthOfField(intensity: number): void {
        this.apiManager.setDepthOfField(intensity);
    }
    
    public setMotionBlur(x: number, y: number, intensity: number): void {
    ...
    public setGlitchEffect(intensity: number, frequency: number): void {
    ...
    private _renderEnhancedEffects(context: CanvasRenderingContext2D, deltaTime: number): void {
        try {
            ...
            if (this.apiManager.renderSettings.enableLighting) {
                this.effectController.lightSources.forEach((light: LightSource) => {
            ...
            if (this.apiManager.renderSettings.enableShadows) {
                this.effectController.shadowCasters.forEach((shadow: Shadow) => {
            ...
            if (this.apiManager.renderSettings.enableReflections) {
                this.effectController.reflectionSurfaces.forEach((reflection: Reflection) => {
                    this.reflectionRenderer.renderReflection(context, reflection);
                });
            }
            
            // 背景効果
            this.effectController.backgroundEffects.forEach((bg: BackgroundEffect) => {
    ...
    public getCurrentSettings(): CurrentSettings {

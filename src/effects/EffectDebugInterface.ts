...
interface GameEngine {
    ...
    enhancedParticleManager?: {
        getActiveParticleCount(): number | null;
    ...
    enhancedEffectManager?: {
        getActiveEffectCount(): number | null;
    ...
    effectQualityController?: {
        getCurrentQualityLevel(): QualityLevelType;
...
export class EffectDebugInterface {
    ...
    private bindEvents(): void {
        ...
        qualitySelect?.addEventListener('change', (e: Event) => {
        ...
        particleMultiplier?.addEventListener('input', (e: Event) => {
        ...
        effectIntensity?.addEventListener('input', (e: Event) => {
            const target = e.target as HTMLInputElement;
            const value = parseFloat(target.value);
        ...
        // エフェクトトグル
        const toggleParticles = document.getElementById('toggle-particles') as HTMLInputElement;
        toggleParticles?.addEventListener('change', (e: Event) => {
            const target = e.target as HTMLInputElement;
        ...
        toggleScreenEffects?.addEventListener('change', (e: Event) => {
            const target = e.target as HTMLInputElement;
        ...
        toggleAnimations?.addEventListener('change', (e: Event) => {
            const target = e.target as HTMLInputElement;
        ...
        toggleSeasonal?.addEventListener('change', (e: Event) => {
            const target = e.target as HTMLInputElement;
        ...
        benchmarkEffects?.addEventListener('click', () => {
            this.runBenchmark();
        });
    ...
    private updateMetricsDisplay(): void {
        ...
        if (elements.fpsValue) {
            elements.fpsValue.textContent = this.metrics.fps.toFixed(1);
        }
        if (elements.particleValue) {
            elements.particleValue.textContent = this.metrics.particleCount.toString();
        }
        if (elements.effectValue) {
            elements.effectValue.textContent = this.metrics.effectCount.toString();
        }
        if (elements.memoryValue) {
            elements.memoryValue.textContent = this.metrics.memoryUsage.toFixed(1);
        }
        if (elements.renderValue) {
            elements.renderValue.textContent = this.metrics.renderTime.toFixed(2);
        }
    }
    ...
    private triggerBubbleEffect(type: BubbleEffectType): void {
    ...
    private runBenchmark(): void {
        ...
        ]).then(([particleStress, effectStress, animationStress]) => {
...
Animation Stress: ${benchmarkResults.animationStress.fps.toFixed(1)} FPS`);
    ...
    private benchmarkEffects(): BenchmarkResult {
        // スクリーンエフェクトストレステスト
        const startTime = performance.now();
    ...
    private benchmarkAnimations(): BenchmarkResult {
        // アニメーションストレステスト
        const startTime = performance.now();
...
// グローバルアクセス用（デバッグ目的）
(window as any).EffectDebugInterface = EffectDebugInterface;
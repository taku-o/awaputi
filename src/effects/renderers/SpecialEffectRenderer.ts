...
interface SpecialEffects {
    [key: string]: SpecialEffectConfig;
}
...
export class SpecialEffectRenderer {
     ...
     */
    createSpecialEffect(x: number, y: number, effectType: string, intensity: number = 1.0): void {
        try {
     ...
     */
    createExplosionEffect(x: number, y: number, intensity: number, config: SpecialEffectConfig): void {
        ...
        for (let i = 0; i < adjustedCount; i++) {
            ...
            particle.bounce = 0.2 + Math.random() * 0.3;
     ...
     * @param {Object} config - 設定
     */
    createImplosionEffect(x: number, y: number, intensity: number, config: SpecialEffectConfig): void {
        const adjustedCount = this.particleManager.adjustParticleCount(config.particleCount * intensity);
        ...
        // 第二段階: 中心からの爆発（遅延）
        setTimeout(() => {
            this.createExplosionEffect(x, y, intensity * 0.8, {
                ...config,
     ...
     * @param {Object} config - 設定
     */
    createVortexEffect(x: number, y: number, intensity: number, config: SpecialEffectConfig): void {
        const adjustedCount = this.particleManager.adjustParticleCount(config.particleCount * intensity);
     ...
     * @param {Object} config - 設定
     */
    createEnergyDischargeEffect(x: number, y: number, intensity: number, config: SpecialEffectConfig): void {
        const adjustedCount = this.particleManager.adjustParticleCount(config.particleCount * intensity);
     ...
     * プラズマ波効果（未実装メソッド）
     */
    createPlasmaWaveEffect(x: number, y: number, intensity: number, config: SpecialEffectConfig): void {
        // TODO: 将来実装予定
     ...
     * テレポート効果（未実装メソッド）
     */
    createTeleportEffect(x: number, y: number, intensity: number, config: SpecialEffectConfig): void {
        // TODO: 将来実装予定
     ...
     * 突風効果（未実装メソッド）
     */
    createWindGustEffect(x: number, y: number, intensity: number, config: SpecialEffectConfig): void {
        // TODO: 将来実装予定
     ...
     * 炎上効果（未実装メソッド）
     */
    createFireBurstEffect(x: number, y: number, intensity: number, config: SpecialEffectConfig): void {
        // TODO: 将来実装予定
     ...
     * 氷砕効果（未実装メソッド）
     */
    createIceShatterEffect(x: number, y: number, intensity: number, config: SpecialEffectConfig): void {
        // TODO: 将来実装予定
     ...
     * @param {Object} config - 設定
     */
    createMagicCircleEffect(x: number, y: number, intensity: number, config: SpecialEffectConfig): void {
        const adjustedCount = this.particleManager.adjustParticleCount(config.particleCount * intensity);
     ...
     */
    createEnergyCore(x: number, y: number, intensity: number, color: string): void {
        for (let i = 0; i < 3; i++) {
            const particle = this.particleManager.getParticleFromPool();
     ...
     */
    createMagicSymbols(x: number, y: number, intensity: number, colors: string[]): void {
        for (let i = 0; i < 6; i++) {
            const particle = this.particleManager.getParticleFromPool();
     ...
     * @param {number} intensity - 強度
     */
    createSimplifiedSpecialEffect(x: number, y: number, effectType: string, intensity: number): void {
        const config = this.specialEffects[effectType];
        if (!config) return;
    ...
    getAvailableEffects(): string[] {
        return Object.keys(this.specialEffects);
    }
     ...
     */
    getEffectInfo(effectType: string): SpecialEffectConfig | null {
        return this.specialEffects[effectType] || null;
    }
}
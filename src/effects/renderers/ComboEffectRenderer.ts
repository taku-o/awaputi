 ...
 */
interface ComboTiers {
    [key: string]: ComboTier;
}

/**
 * Combo tier with key interface
 */
interface ComboTierWithKey extends ComboTier {
    key: string;
}
 ...
 */
interface ComboBreakEffect {
    fadeParticleCount: number;
    fadeColor: string;
    fadeDuration: number;
}
...
export class ComboEffectRenderer {
    ...
    createEnhancedComboEffect(x: number, y: number, comboCount: number, comboType: string = 'normal'): void {
        try {
            ...
            if (tier.screenEffects.length > 0) {
                this.triggerScreenEffects(tier.screenEffects, comboCount);
            }
     ...
     */
    getComboTier(comboCount: number): ComboTierWithKey | null {
        for(const [key, tier] of Object.entries(this.comboTiers)) {
     ...
     */
    selectComboParticleType(tier: ComboTierWithKey, index: number, totalCount: number): string {
     ...
     * @param {number} comboCount - コンボ数
     */
    createGoldenParticles(x: number, y: number, comboCount: number): void {
        const goldenCount = this.particleManager.adjustParticleCount(5 + comboCount);
     ...
     * コンボスパークル効果（未実装）
     */
    createComboSparkle(x: number, y: number, comboCount: number): void {
        // TODO: 将来実装予定
        this.createGoldenParticles(x, y, comboCount);
    }
     ...
     * 強化スパークル効果（未実装）
     */
    createEnhancedSparkle(x: number, y: number, comboCount: number): void {
        // TODO: 将来実装予定
        this.createGoldenParticles(x, y, comboCount);
    }
    ...
    createRainbowBurst(x: number, y: number, comboCount: number): void {
        ...
        for(let i = 0; i < rainbowCount; i++) {
            ...
            particle.life = 2000 + Math.random() * 500;
            particle.maxLife = particle.life;
            particle.alpha = 0.9;
            particle.gravity = 10;
     ...
     * @param {Object} tier - コンボ段階
     */
    createComboMultiplierIndicator(x: number, y: number, comboCount: number, tier: ComboTierWithKey): void {
        // 数字パーティクル（テキスト表示の代替として視覚的表現）
        ...
        for(let i = 0; i < digitCount; i++) {
            ...
            particle.vy = -60 - Math.random() * 20;
            particle.size = 3;
            particle.color = tier.colors[0];
            particle.life = 1500;
            particle.maxLife = particle.life;
            particle.alpha = 0.9;
            particle.gravity = 10;
     ...
     * @param {number} comboCount - コンボ数
     */
    triggerScreenEffects(effects: string[], comboCount: number): void {
        // このメソッドは実際の画面効果システムと連携する必要がある
        // ここでは効果の種類を記録するのみ
     ...
     */
    createComboBreakEffect(x: number, y: number, lastComboCount: number): void {
        if (lastComboCount < 2) return; // 2コンボ未満はブレイク演出なし
        ...
        try {
            ...
            for(let i = 0; i < breakCount; i++) {
                ...
                particle.color = this.comboBreakEffect.fadeColor;
                particle.life = this.comboBreakEffect.fadeDuration;
                particle.maxLife = particle.life;
                particle.alpha = 0.6;
                particle.gravity = 20;
     ...
     */
    createComboProgressEffect(x: number, y: number, currentCombo: number, nextTierThreshold: number): void {
        if (nextTierThreshold === Infinity) return;

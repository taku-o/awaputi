 ...
 */
interface ActiveEvent extends EventTheme {
    key: string;
}
...
export class SeasonalEffectRenderer {
     ...
     */
    createSeasonalEffect(x: number, y: number, seasonType: string | null = null, eventType: string | null = null): void {
        try {
            ...
            } else if (season && this.seasonalThemes[season]) {
                this.createSeasonEffect(x, y, this.seasonalThemes[season]);
            } else {
                // フォールバック: デフォルト効果
            ...
            getErrorHandler().handleError(error, 'SEASONAL_EFFECT_ERROR', {
     ...
     */
    createSeasonEffect(x: number, y: number, theme: SeasonalTheme): void {
        // 品質チェック
     ...
     */
    createSpringEffect(x: number, y: number, theme: SeasonalTheme): void {
        // 桜の花びら
     ...
     * @param {Object} theme - テーマ
     */
    createSummerEffect(x: number, y: number, theme: SeasonalTheme): void {
        // 太陽の光線
     ...
     * @param {Object} theme - テーマ
     */
    createAutumnEffect(x: number, y: number, theme: SeasonalTheme): void {
        // 落ち葉
        ...
        for (let i = 0; i < leafCount; i++) {
            ...
            particle.color = theme.colors.primary[Math.floor(Math.random() * theme.colors.primary.length)];
            particle.life = 4000 + Math.random() * 2000;
            particle.maxLife = particle.life;
            particle.alpha = 0.8;
            particle.gravity = 15;
     ...
     * @param {Object} theme - テーマ
     */
    createWinterEffect(x: number, y: number, theme: SeasonalTheme): void {
        // 雪の結晶
     ...
     */
    createEventEffect(x: number, y: number, event: EventTheme | ActiveEvent): void {
        // 品質チェック
        ...
        // 各イベントの特殊効果
        Object.entries(event.particles).forEach(([effectName, config]) => {
            this.createEventParticleEffect(x, y, effectName, config, event);
        });
    }
     ...
     * @param {Object} event - イベントテーマ
     */
    createEventParticleEffect(x: number, y: number, effectName: string, config: ParticleConfig, event: EventTheme | ActiveEvent): void {
        const adjustedCount = this.particleManager.adjustParticleCount(config.count);
     ...
     * @param {Object} event - イベントテーマ
     */
    configureEventParticle(particle: Particle, effectName: string, event: EventTheme | ActiveEvent): void {
        const colors = [
     ...
     */
    detectActiveEvents(): ActiveEvent[] {
        const now = new Date();
     ...
     * @param {number} y - Y座標
     */
    createDefaultSeasonalEffect(x: number, y: number): void {
        const defaultCount = this.particleManager.adjustParticleCount(8);
     ...
     * @param {Object} theme - テーマ
     */
    createSimplifiedSeasonalEffect(x: number, y: number, theme: SeasonalTheme): void {
        const simpleCount = Math.max(3, Math.floor(6 * 0.5));
     ...
     * 簡略化されたイベント効果
     */
    createSimplifiedEventEffect(x: number, y: number, event: EventTheme | ActiveEvent): void {
     ...
     */
    setCustomTheme(customTheme: any): void {
     ...
     */
    getCurrentThemeInfo(): object {
        const activeEvent = this.getHighestPriorityEvent();

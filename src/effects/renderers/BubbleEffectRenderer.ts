 ...
 */
interface BubbleEffectConfigs {
    [key: string]: BubbleEffectConfig;
}

/**
 * Bubble effect options interface
 */
interface BubbleEffectOptions {
    intensity?: number;
    multiplier?: number;
    customColors?: string[];
}
...
export class BubbleEffectRenderer {
    ...
    selectParticleType(config: BubbleEffectConfig, index: number, totalCount: number): string {
        ...
        if (qualitySettings.complexityLevel >= 3) {
            ...
            return 'star';
        } else if (qualitySettings.complexityLevel >= 2) {
            // 中品質: 基本的な拡張タイプ
     ...
     * @param {Object} config - 設定
     */
    createRippleEffect(x: number, y: number, bubbleSize: number, config: BubbleEffectConfig): void {
        const rippleCount = this.particleManager.adjustParticleCount(3);
        ...
        for (let i = 0; i < rippleCount; i++) {
            ...
            particle.vx = 0;
            particle.vy = 0;
            particle.size = bubbleSize * 0.5;
            particle.color = config.colors[0];
            particle.life = 1500 + i * 200;
            particle.maxLife = particle.life;
            particle.alpha = 0.6 - i * 0.2;
            particle.gravity = 0;
            particle.friction = 1;
            particle.type = 'ripple';
            particle.scale = 0.1 + i * 0.05;
     ...
     * @param {Object} config - 設定
     */
    createDebrisEffect(x: number, y: number, bubbleSize: number, config: BubbleEffectConfig): void {
        const debrisCount = this.particleManager.adjustParticleCount(12);
     ...
     * ダスト効果（未実装）
     */
    createDustEffect(x: number, y: number, bubbleSize: number, config: BubbleEffectConfig): void {
        // TODO: 将来実装予定
        this.createDebrisEffect(x, y, bubbleSize, config);
    }
     ...
     * 火花効果（未実装）
     */
    createSparksEffect(x: number, y: number, bubbleSize: number, config: BubbleEffectConfig): void {
        // TODO: 将来実装予定
        this.createElectricArcsEffect(x, y, bubbleSize, config);
    }
     ...
     * 金属片効果（未実装）
     */
    createMetalShardsEffect(x: number, y: number, bubbleSize: number, config: BubbleEffectConfig): void {
        // TODO: 将来実装予定
        this.createDebrisEffect(x, y, bubbleSize, config);
    }
     ...
     * プリズム効果（未実装）
     */
    createPrismaticEffect(x: number, y: number, bubbleSize: number, config: BubbleEffectConfig): void {
        // TODO: 将来実装予定
        this.createSparkleEffect(x, y, bubbleSize, config);
    }
     ...
     * ブリリアンス効果（未実装）
     */
    createBrillianceEffect(x: number, y: number, bubbleSize: number, config: BubbleEffectConfig): void {
        // TODO: 将来実装予定
        this.createSparkleEffect(x, y, bubbleSize, config);
    }
     ...
     * 魔法の輝き効果（未実装）
     */
    createMagicalSparklesEffect(x: number, y: number, bubbleSize: number, config: BubbleEffectConfig): void {
        // TODO: 将来実装予定
        this.createSparkleEffect(x, y, bubbleSize, config);
    }
     ...
     * 稲妻効果（未実装）
     */
    createLightningEffect(x: number, y: number, bubbleSize: number, config: BubbleEffectConfig): void {
        // TODO: 将来実装予定
        this.createElectricArcsEffect(x, y, bubbleSize, config);
    }
     ...
     * 衝撃波効果（未実装）
     */
    createShockwaveEffect(x: number, y: number, bubbleSize: number, config: BubbleEffectConfig): void {
        // TODO: 将来実装予定
        this.createRippleEffect(x, y, bubbleSize, config);
    }
     ...
     * @param {Object} config - 設定
     */
    createMassiveExplosionEffect(x: number, y: number, bubbleSize: number, config: BubbleEffectConfig): void {
        const explosionCount = this.particleManager.adjustParticleCount(40);
     ...
     * @param {number} bubbleSize - バブルサイズ
     */
    createDiamondRefraction(x: number, y: number, bubbleSize: number): void {
        const refractionCount = this.particleManager.adjustParticleCount(12);
     ...
     * 電気パルス効果（未実装）
     */
    createElectricPulse(x: number, y: number, bubbleSize: number): void {
        // TODO: 将来実装予定
     ...
     * ボス死亡効果（未実装）
     */
    createBossDeathEffect(x: number, y: number, bubbleSize: number): void {
        // TODO: 将来実装予定

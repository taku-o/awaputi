...
interface EffectPriorities {
    critical: number;    // ゲームプレイに必須
...
export class EffectQualityController {
    ...
    // エフェクト優先度システム
    private readonly effectPriorities: EffectPriorities = {
    ...
    // エフェクト制限
    private effectLimits: EffectLimits = {
    ...
    // 現在のエフェクトカウント
    private activeEffectCounts: ActiveEffectCounts = {
        particles: 0,
        effects: 0,
        screenEffects: 0,
    ...
    private _initializeQualitySettings(): void {
        try {
            ...
            // 品質変更の監視
            this.configManager.watch<QualityLevelType>('effects.quality.level', (newValue) => {
                this.setQualityLevel(newValue);
            });

            this.configManager.watch<boolean>('effects.quality.autoAdjust', (newValue) => {
     ...
     */
    public setQualityLevel(level: QualityLevelType | null): void {
        ...
        if (!this.qualityLevels[level]) {
            this.errorHandler.handleError(
                ...
                {
                    context: 'EffectQualityController.setQualityLevel'
                }
            );
            return;
        }
     ...
     * @returns エフェクトを実行可能か
     */
    public canExecuteEffect(effectType: EffectTypeType, priority: EffectPriorityType = 'normal'): boolean {
     ...
     * @param delta - 変更量
     */
    public updateEffectCount(effectType: string, delta: number): void {
     ...
     */
    public updatePerformanceMetrics(currentTime: number, frameRate: number, memoryUsage?: number): void {
     ...
     * @returns 現在の品質レベル
     */
    public getCurrentQualityLevel(): QualityLevelType {
 ...
 */
export function getEffectQualityController(): EffectQualityController {

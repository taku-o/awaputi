...
export class MobileEffectOptimizer {
    ...
    async initialize(): Promise<boolean> {
        try {
            ...
            getErrorHandler().handleError(error as Error, 'MOBILE_OPTIMIZATION_ERROR', {
     ...
     * デバイスプロファイルの決定
     */
    private determineDeviceProfile(): void {
        ...
        if (/iPhone/.test(ua)) {
            if (/iPhone1[2-9]|iPhone[2-9][0-9]/.test(ua)) {
                performanceScore += 20; // iPhone 12以降
            } else if (/iPhone[8-9]|iPhone1[0-1]/.test(ua)) {
                performanceScore += 10; // iPhone 8-11
            }
        }
     ...
     * エフェクトマネージャーの最適化
     */
    private async optimizeEffectManager(profile: DeviceProfileConfig): Promise<void> {
        // 品質レベルの設定
        ...
        if (this.effectManager.setAdvancedEffectsEnabled) {
            this.effectManager.setAdvancedEffectsEnabled(profile.enableAdvancedEffects);
        }
        
        // ポストプロセシングの有効/無効
        if (this.effectManager.setPostProcessingEnabled) {
            this.effectManager.setPostProcessingEnabled(profile.enablePostProcessing);
        }
        
        // バッチング有効化
        if (this.optimizations.enableBatching && this.effectManager.enableBatching) {
            this.effectManager.enableBatching(true);
        }
        
        // カリングの有効化
        if (this.optimizations.enableCulling && this.effectManager.enableCulling) {
            this.effectManager.enableCulling(true);
        }
    }
     ...
     * パーティクルシステムの最適化
     */
    private async optimizeParticleSystem(profile: DeviceProfileConfig): Promise<void> {
     ...
     * アニメーションシステムの最適化
     */
    private async optimizeAnimationSystem(profile: DeviceProfileConfig): Promise<void> {
     ...
     * レンダリングの最適化
     */
    private async optimizeRendering(profile: DeviceProfileConfig): Promise<void> {
        // Canvas設定の最適化
     ...
     * 描画頻度の最適化
     */
    private optimizeRenderingFrequency(profile: DeviceProfileConfig): void {
        let targetFPS = 60;
        ...
        if (this.effectManager.setTargetFPS) {
            this.effectManager.setTargetFPS(targetFPS);
        }
    }
     ...
     * テクスチャの最適化
     */
    private optimizeTextures(profile: DeviceProfileConfig): void {
        // テクスチャ解像度の調整
        ...
        if (this.effectManager.setTextureScale) {
            this.effectManager.setTextureScale(textureScale);
        }
    }
    ...
    private startMemoryMonitoring(): void {
        if (!this.deviceInfo.performanceMemory) return;
    ...
    private startThermalMonitoring(): void {
        // 実際の熱状態APIは限定的なので、パフォーマンス指標から推定
        setInterval(() => {
    ...
    private setupTouchOptimizations(): void {
        if (!this.deviceInfo.supportsTouch) return;
     ...
     * タッチ遅延の最小化
     */
    private minimizeTouchLatency(): void {
        // passive イベントリスナーの使用を推奨
        const canvas = this.effectManager.canvas;
        if (!canvas) return;
     ...
     * ジェスチャーの最適化
     */
    private optimizeGestures(): void {
        if (this.state.deviceProfile === 'low-end' || this.state.deviceProfile === 'ultra-low') {
            const sensitivity = this.touchOptimizations.gestureSensitivity;
            this.touchOptimizations.gestureSensitivity = Math.min(sensitivity * 1.2, 2.0);
        }
    }
     ...
     * 低パフォーマンス時の処理
     */
    private handleLowPerformance(fps: number): void {
    ...
    private adaptToBatteryLevel(): void {
        if (!this.optimizations.batteryAware) return;
    ...
    private applyBatteryOptimizations(level: 'low' | 'medium' | 'high'): void {
        const optimizations: Record<string, BatteryOptimizationConfig> = {
            ...
            high: {
                particleMultiplier: 1.0,
                effectQuality: this.deviceProfiles.get(this.state.deviceProfile!)?.effectQuality || 'medium',
                targetFPS: 60
            }
        };
        ...
        const particleManager = this.effectManager.enhancedParticleManager;
        if (particleManager?.setParticleMultiplier) {
            particleManager.setParticleMultiplier(config.particleMultiplier);
        }
        
        if (this.effectManager.setQualityLevel) {
            this.effectManager.setQualityLevel(config.effectQuality);
        }
        
        if (this.effectManager.setTargetFPS) {
            this.effectManager.setTargetFPS(config.targetFPS);
        }
    }
     ...
     * レポート生成
     */
    generateReport(): OptimizerReport {
        return {
            component: 'MobileEffectOptimizer',
            state: { ...this.state },

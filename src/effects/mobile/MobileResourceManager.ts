...
interface ParticleResource extends PooledResource {
    x: number;
    y: number;
    vx: number;
    vy: number;
...
interface AnimationResource extends PooledResource {
    startTime: number;
    duration: number;
    elapsed: number;
    progress: number;
...
interface SoundResource extends PooledResource {
...
interface LoadedImageResource extends CachedResource {
    canvas: HTMLCanvasElement;
    context: CanvasRenderingContext2D;
    width: number;
    height: number;
...
interface LoadedAudioResource extends CachedResource {
    buffer: AudioBuffer;
...
interface ResourceManagerReport {
    component: string;
...
export class MobileResourceManager {
    ...
    private readonly statistics: Omit<ResourceStatistics, 'poolSizes' | 'cacheSizes' | 'memoryUsage' | 'dataUsage'> = {
        memoryAllocated: 0,
        resourcesCreated: 0,
     ...
     * メモリ機能の検出
     */
    private detectMemoryCapabilities(): void {
     ...
     * プールサイズの調整
     */
    private adjustPoolSizes(multiplier: number): void {
     ...
     * プールの事前割り当て
     */
    private preAllocatePool(type: ResourceType, count: number): void {
     ...
     * パーティクルリソースの作成
     */
    private createParticleResource(): ParticleResource {
        return {
            x: 0,
            y: 0,
            vx: 0,
     ...
     * テクスチャリソースの作成
     */
    private createTextureResource(): TextureResource {
     ...
     * アニメーションリソースの作成
     */
    private createAnimationResource(): AnimationResource {
        return {
            startTime: 0,
            duration: 0,
            elapsed: 0,
     ...
     * サウンドリソースの作成
     */
    private createSoundResource(): SoundResource {
        return {
            buffer: null,
            source: null,
            gainNode: null,
     ...
     * リソースの設定
     */
    private configureResource(resource: PooledResource, config: Record<string, any>): void {
        // 共通プロパティの設定
     ...
     * リソース状態のリセット
     */
    private resetResourceState(resource: PooledResource): void {
        // 基本プロパティのリセット
        if ('life' in resource) {
     ...
     * リソースの解放
     */
    releaseResource(type: ResourceType, resource: PooledResource): void {
     ...
     * リソースのクリーンアップ
     */
    private cleanupResource(resource: PooledResource): void {
        // Canvas リソースのクリーンアップ
        ...
        // アニメーションリソースのクリーンアップ
        if ('onComplete' in resource) {
            (resource as AnimationResource).onComplete = null;
        }
    }
     ...
     * プールの強制クリーンアップ
     */
    private forceCleanupPool(type: ResourceType): void {
        const pool = this.resourcePools.get(type);
        if (!pool) return;
     ...
     * LRU キャッシュエントリの削除
     */
    private evictLRUCacheEntry(cache: Map<string, CachedResource>): void {
     ...
     * 非アクティブリソースのクリーンアップ
     */
    private cleanupInactiveResources(type: ResourceType): void {
        const pool = this.resourcePools.get(type);
        if (!pool) return;
     ...
     * 設定の更新
     */
    updateSettings(newSettings: ResourceSettings): void {
     ...
     * レポート生成
     */
    generateReport(): ResourceManagerReport {
        return {
            component: 'MobileResourceManager',
            state: { ...this.state },
            settings: {

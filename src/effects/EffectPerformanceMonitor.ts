...
export class EffectPerformanceMonitor {
    ...
    private _initializePerformanceAPI()';
            ...
            } catch (error) { console.warn('Performance Observer not available:', (error as Error).message }
    }
    
    /**
     * フレーム開始時の測定
     */
    startFrame(): void { const now = performance.now(),
     ...
     * パフォーマンス監視の更新
     */
    update(currentTime: number): void { if (!this.monitoringEnabled) return,
     ...
     * パフォーマンスチェックの実行
     */
    private _performPerformanceCheck(currentTime: number): void { const stats = this.getPerformanceStats(),
     ...
     * メモリ使用量のチェック
     */
    private _checkMemoryUsage(): void { const memoryUsage = this._getCurrentMemoryUsage(),
     ...
     * エフェクトカリングの実行
     */
    cullEffects(effects: Effect[], viewport: Viewport): Effect[] { if (!this.optimizationSettings.cullOffScreen) {
            return effects }
     ...
     * エフェクトがビューポート内かチェック
     */
    private _isEffectInViewport(effect: Effect, viewport: Viewport, margin: number = 0): boolean { return effect.x + effect.width >= viewport.x - margin &&
     ...
     * エフェクトが画面外かチェック
     */
    private _isEffectOffScreen(effect: Effect, viewport: Viewport): boolean { return effect.x + effect.width < viewport.x ||
     ...
     * 現在のメモリ使用量を取得
     */
    private _getCurrentMemoryUsage(): number | null { if (performance.memory) {
     ...
     * クリーンアップのスケジュール
     */
    private _scheduleCleanup(effectId?: string): void { if (effectId) {
            this.cleanupQueue.push(effectId) } else {  // 古いエフェクトをクリーンアップキューに追加
     ...
     * クリーンアップキューの処理
     */
    private _processCleanupQueue(): void { while (this.cleanupQueue.length > 0) {
     ...
     * エフェクトのクリーンアップ
     */
    private _cleanupEffect(effectId: string): void { if(this.activeEffects.has(effectId) {
     ...
     * 監視の有効/無効設定
     */
    setMonitoringEnabled(enabled: boolean): void { this.monitoringEnabled = enabled }

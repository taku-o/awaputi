 ...
 */
export interface FocusGlowState {
    active: boolean;
    intensity: number;
    color: string;
    phase?: number;
 ...
 */
export interface TrailAnimation {
    trail?: TrailPoint[];
 ...
 */
export interface SparkleAnimation {
...
export class LoadingAnimationRenderer {
     ...
     * ドットローディングをレンダリング
     */
    static renderDots(context: CanvasRenderingContext2D, pos: Position, size: number, phases: number[], options: LoadingAnimationOptions): void {
        const dotCount = options.elements || 3;
     ...
     * パルスローディングをレンダリング
     */
    static renderPulse(context: CanvasRenderingContext2D, pos: Position, size: number, phase: number, options: LoadingAnimationOptions): void {
        const scale = (Math.sin(phase) + 1) * 0.5 * 0.5 + 0.5;
     ...
     * ウェーブローディングをレンダリング
     */
    static renderWave(context: CanvasRenderingContext2D, pos: Position, size: number, waveOffset: number, options: LoadingAnimationOptions): void {
     ...
     * プログレスバーをレンダリング
     */
    static renderProgress(context: CanvasRenderingContext2D, pos: Position, size: number, options: LoadingAnimationOptions): void {
...
export class InteractiveEffectRenderer {
     ...
     * ホバー効果をレンダリング
     */
    static renderHoverEffect(context: CanvasRenderingContext2D, element: ElementBounds, hoverOptions?: HoverOptions): void {
...
export class ParticleEffectRenderer {
     ...
     * 星形を描画
     */
    static drawStar(context: CanvasRenderingContext2D, x: number, y: number, size: number, points: number): void {
...
export class TransitionEffectRenderer {
     ...
     * ズーム遷移をレンダリング
     */
    static renderZoomTransition(context: CanvasRenderingContext2D, fromElement: RenderableElement | null, toElement: RenderableElement | null, progress: number): void {
     ...
     * 要素を描画（プレースホルダー）
     */
    static renderElement(context: CanvasRenderingContext2D, element: RenderableElement): void {
        // 実際の要素描画ロジックは使用側で実装
        if (element?.render) {
...
export class AnimationEffectUtils {
     ...
     * ベジェ曲線上の点を計算
     */
    static calculateBezierPoint(t: number, p0: BezierPoint, p1: BezierPoint, p2: BezierPoint, p3: BezierPoint): BezierPoint {
        const mt = 1 - t;
        const mt2 = mt * mt;
        const mt3 = mt2 * mt;
        const t2 = t * t;

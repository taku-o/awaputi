...
interface PanelManager {
    getPanelInfo(name: string): PanelInfo | null;
}
...
export class LazyLoadManager {
     ...
     * コンポーネントを登録
     */
    public registerComponent(name: string, config: Partial<ComponentConfig>): void {
     ...
     * コンポーネントを読み込み
     */
    public async loadComponent(name: string): Promise<LoadedComponent | null> {
        // すでに読み込み済みの場合
     ...
     * 実際のコンポーネント読み込み処理
     */
    private async performComponentLoad(name: string, config: ComponentConfig): Promise<LoadedComponent> {
        // 依存関係を先に読み込み
     ...
     * 読み込み済みコンポーネントを取得
     */
    private getLoadedComponent(name: string): LoadedComponent | null {
        // 読み込み済みの場合、インスタンスを返す
        ...
        return panelInfo?.instance ? {
            ...
            config: this.componentRegistry.get(name)!
        } : null;
    }
     ...
     * オンデマンド読み込み用の要素を設定
     */
    public setupLazyElement(element: HTMLElement, componentName: string): void {
     ...
     * バッチ読み込み
     */
    public async loadComponentBatch(names: string[]): Promise<(LoadedComponent | null)[]> {
     ...
     */
    public unloadUnusedComponents(): void {

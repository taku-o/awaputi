...
export class FailureHistoryManager extends BaseComponent {
    private failureHistory: FailureHistoryEntry[];
     ...
     */
    addToFailureHistory(analyses: Analysis[]): void {
     ...
     * @returns 関連する失敗履歴
     */
    findRelatedFailures(test: Test): FailureHistoryEntry[] {
        if (!test || !test.name) {
            return [];
        }
     ...
     * @returns 履歴ベースの提案
     */
    generateHistoricalSuggestions(test: Test): Suggestion[] {
     ...
     * @returns トレンド分析結果
     */
    analyzeFailureTrend(days: number = 30): TrendAnalysis {
        const now = Date.now();
     ...
     * @returns 失敗履歴
     */
    getFailureHistory(limit: number | null = null): FailureHistoryEntry[] {

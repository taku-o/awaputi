 ...
 */
export class DebugSuggestionEngine extends BaseComponent {
    private debugSuggestions: Map<string, Suggestion[]>;
    ...
    setupDebugSuggestions(): void {
        ...
        this.debugSuggestions.set('configuration_error', [{
            ...
            },
            {
                ...
                code: 'const value = config.value ?? DEFAULT_VALUE',
     ...
     * コンテキスト特化の提案を設定
     */
    setupContextualSuggestions(): void {
        this.contextualSuggestions.set('BubbleManager', {
            null_reference: [{
        ...
        this.contextualSuggestions.set('ScoreManager', {
            assertion_failure: [{
        ...
        this.contextualSuggestions.set('Canvas', {
            null_reference: [{
     ...
     * @returns 提案の配列
     */
    generateSuggestions(pattern: Pattern, test: Test): Suggestion[] {
        if (!pattern) {
            return [];
        }
     ...
     * @returns コンテキスト特化提案
     */
    generateContextualSuggestions(pattern: Pattern, test: Test): Suggestion[] {
        if (!test || !test.name) {
            return [];
        }
     ...
     * @returns テスト特化提案
     */
    generateTestSpecificSuggestions(test: Test): Suggestion[] {
        if (!test || !test.name) {
            return [];
        }
        ...
        if (testName.includes('async') || testName.includes('promise')) {
            suggestions.push({
        ...
        if (testName.includes('render') || testName.includes('canvas')) {
            suggestions.push({
     ...
     * @returns 提案テンプレート
     */
    getSuggestionTemplates(patternId: string): Suggestion[] {
        return this.debugSuggestions.get(patternId) || [];
    }
     ...
     * @returns 統計情報
     */
    getSuggestionStatistics(): SuggestionStatistics {
        ...
        for (const suggestions of Array.from(this.debugSuggestions.values())) {

...
export class DebugPanelManager extends BaseComponent {
     ...
     * @param config - パネル設定
     */
    addPanelTab(id: string, config: PanelConfig): void {
     ...
     * @param config - パネル設定
     */
    addPanelContent(id: string, config: PanelConfig): void {
        const controller = this.mainController as MainController;
     ...
     * @param panelId - パネルID
     */
    addToHistory(panelId: string): void {
        // 重複を避けるため、既存の履歴から削除
     ...
     * @returns パネル情報
     */
    getPanelInfo(panelId: string): PanelConfig | null {
     ...
     * @returns パネルMap
     */
    getAllPanels(): Map<string, PanelConfig> {

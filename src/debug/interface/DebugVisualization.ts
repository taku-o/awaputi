...
export class DebugVisualization extends BaseComponent {
    private charts: Map<string, HTMLElement>;
     ...
     * 視覚化イベントをバインド
     */
    bindVisualizationEvents(): void {
        // テーマ切り替えイベント
        document.addEventListener('themeChange', (event: CustomEvent) => {
            this.setTheme(event.detail.theme);
        });

        // パフォーマンス監視イベント
        document.addEventListener('performanceUpdate', (event: CustomEvent) => {
     ...
     */
    updatePerformanceVisualization(data: PerformanceData): void {
    ...
    applyResponsiveLayout(): void {
        const container = (this.mainController as MainController).container;
     ...
     */
    addCustomChart(id: string, title: string, renderer?: (chart: HTMLElement) => void): void {

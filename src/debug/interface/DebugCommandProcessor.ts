...
interface ShortcutConfig extends Required<ShortcutOptions> {
    shortcut: string;
    callback: (event: KeyboardEvent) => void;
}

interface ShortcutStatistics {
...
export class DebugCommandProcessor extends BaseComponent {
     ...
     * @returns ショートカット文字列
     */
    buildShortcutString(event: KeyboardEvent): string {
        const parts: string[] = [];
    ...
    handleKeyboardEvent(event: KeyboardEvent): void {
        ...
        if (shortcutConfig && shortcutConfig.enabled && this.isContextActive(shortcutConfig.context)) {
            ...
            try {
                ...
                this._handleError('shortcut execution', error as Error);
            }
        }
    }
     ...
     * @param event - キーボードイベント
     */
    handleDebugInterfaceKeyboard(event: KeyboardEvent): void {
        // ESCキーでインターフェースを非表示
        if(event.key === 'Escape') {
     ...
     * ショートカットコンテキストを初期化
     */
    initializeShortcutContexts(): void {
     ...
     * @param config - 設定
     */
    addToContext(context: string, shortcut: string, config: ShortcutConfig): void {
    ...
    getShortcutConflicts(): ShortcutConflict[] {
        ...
        for(const [shortcut, config] of Array.from(this.shortcuts)) {
        ...
        for(const [shortcut, configs] of Array.from(shortcutGroups)) {
            if (configs.length > 1) {
     ...
     * @param suspend - 停止フラグ
     */
    setSuspendShortcuts(suspend: boolean): void {
     ...
     * @param context - コンテキスト名
     */
    switchShortcutContext(context: string): void {
     ...
     * @returns ショートカットMap
     */
    getShortcutsByContext(context: string): Map<string, ShortcutConfig> {
        return this.shortcutContexts.get(context) || new Map<string, ShortcutConfig>();
    }
    ...
    removeShortcut(shortcut: string): void {
        ...
        if (this.shortcuts.delete(normalizedShortcut)) {
            ...
            // コンテキストからも削除
            for (const contextMap of Array.from(this.shortcutContexts.values())) {

...
export class BenchmarkExecutor {
    ...
    private async runWarmup(executionSession: ExecutionSession): Promise<void> {
        ...
        try {
            performance.mark(`benchmark-${name}-warmup-start`);
     ...
     */
    private async executeSingleIteration(benchmark: Benchmark): Promise<any> {
        const timeout = benchmark.timeout || this.executionConfig.defaultTimeout;
     ...
     */
    private collectPerformanceEntries(benchmarkName: string): PerformanceEntry[] {
        try {
    ...
    private async cleanupBenchmarkEnvironment(executionSession: ExecutionSession): Promise<void> {
        ...
        try {
            performance.mark(`benchmark-${name}-cleanup-start`);
     ...
     */
    getCurrentExecution(): ExecutionSession | null {

...
interface ComponentWithInitialize {
...
export class GameStateCommands extends BaseComponent {
     ...
     * 状態操作コンポーネントを取得
     */
    public getStateManipulation(): any {
    ...
    public getBubbleManager(): BubbleManager | undefined {
    ...
    public getStageManager(): StageManager | undefined {
     ...
     * ゲーム状態キャプチャ
     */
    public captureGameState(): GameState {
     ...
     * テストシナリオ実行
     */
    public runTestScenario(scenario: TestScenario): string {
        const scenarios: Record<TestScenario, () => string> = {
            stress: () => {
                const bubbleManager = this.getBubbleManager();
     ...
     * ゲーム状態フォーマット
     */
    public formatGameState(state: GameState): string {
        const lines: string[] = [];
     ...
     * プレイヤーデータバックアップ作成
     */
    public createPlayerDataBackup(): BackupData {
     ...
     */
    public logCommand(command: string, params: any): void {
     ...
     */
    public updateSafetySettings(settings: SafetySettings): void {
    ...
    public get safetyChecks(): SafetyChecks {
        const safetyValidator = this.getSafetyValidator();

...
export class DebugErrorAnalyzer {
    ...
    public analyzeErrorTrends(timeframe: number = 3600000): ErrorTrend[] {
        const cutoff = Date.now() - timeframe;
        const recentErrors = this.errorHistory.filter(e => e.timestamp > cutoff);
        const categories = new Map<string, number>();
        
        recentErrors.forEach(error => {
            const analysis = this.analyzeError(error, error.timestamp);
    ...
    public getCommonIssues(limit: number = 10): ErrorPattern[] {
        return this.patterns
            .sort((a, b) => b.frequency - a.frequency)
            .slice(0, limit);
    }

    public addCustomPattern(pattern: Omit<ErrorPattern, 'frequency'>): void {
        this.patterns.push({ ...pattern, frequency: 0 });
    }
    ...
    private generateSuggestions(patterns: ErrorPattern[], error: any): string[] {
    ...
    private calculateConfidence(patterns: ErrorPattern[], error: any): number {
        ...
        if (error.stack && patterns.some(p => p.pattern.test(error.stack))) {
            confidence += 0.3;
        }
    ...
    private calculateTrend(category: string, timeframe: number): 'increasing' | 'decreasing' | 'stable' {
        const now = Date.now();
        const halfTimeframe = timeframe / 2;
        
        const recentCount = this.errorHistory.filter(e =>
            e.timestamp > now - halfTimeframe &&
            this.analyzeError(e, e.timestamp).category === category
        ).length;

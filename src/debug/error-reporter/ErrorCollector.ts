...
export class ErrorCollector {
     ...
     * エラーの収集
     */
    public collect(error: ErrorInfo): string | null {
     ...
     * 収集すべきかどうかの判定
     */
    private shouldCollect(error: ErrorInfo): boolean {
     ...
     * 削除時の統計更新
     */
    private updateStatisticsOnRemoval(error: ErrorInfo): void {
        // カテゴリ統計
     ...
     * フィルターマッチング
     */
    private matchesFilter(error: ErrorInfo, filter: ErrorFilter): boolean {
     ...
     * フィンガープリントで取得
     */
    public getErrorsByFingerprint(fingerprint: string): ErrorInfo[] {
     ...
     * フィルター設定の更新
     */
    public updateFilters(newFilters: Partial<CollectionFilters>): void {

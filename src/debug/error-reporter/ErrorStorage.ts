...
export class ErrorStorage {
    ...
    public async store(error: StoredError): Promise<void> {
        if (!this.initialized) {
        ...
        // Enforce size limit
        while (this.cache.length > this.config.maxItems) {
            this.cache.shift();
        }
    ...
    public async clearErrors(filter?: {
        category?: string;
        severity?: string;
        olderThan?: number;
    }): Promise<number> {
    ...
    private async loadFromIndexedDB(): Promise<void> {
        // IndexedDB loading would go here
    }

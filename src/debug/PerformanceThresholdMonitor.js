...
export class PerformanceThresholdMonitor {
    ...
    checkThresholds() {
        ...
        try {
            ...
            this.errorHandler.handleError(error, 'PerformanceThresholdMonitor.checkThresholds');
        }
    }
    ...
    evaluateThreshold(name, threshold, value, timestamp) {
        ...
        // 違反統計更新
        if (violated) {
            this.updateViolationStatistics(name, severity);
        }

        // 警告生成
        if (violated && !this.isSuppressed(name, severity)) {
            this.generateAlert(name, threshold, value, severity, timestamp);
        }
    }
    ...
    setSuppression(name, severity, timestamp) {
        ...
        const suppressionDuration = severity === 'critical' ? 
            this.monitoring.criticalAlertTimeout : 
            this.monitoring.suppressionTimeout;

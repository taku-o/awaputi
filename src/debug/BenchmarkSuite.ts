...
export class BenchmarkSuite {
    ...
    private generateSummary(results: Record<string, BenchmarkResult>): BenchmarkSummary {
    ...
    // ベースライン管理
    updateBaseline(benchmarkName: string, results: BenchmarkResult): void {
        if (results.success) {
    ...
    getBaseline(benchmarkName: string): any {
    ...
    // 結果の取得
    getResults(limit: number = 10): Array<{ timestamp: number } & SuiteResult> {
    ...
    getLatestResults(): SuiteResult | undefined {
        const latest = Math.max(...this.results.keys());
        return this.results.get(latest);
    }

    // 統計とステータス
    getBenchmarkResults(): SuiteResult | undefined {

...
export class ConfigurationCommands {
     ...
     * コマンドをコンソールに登録
     */
    registerCommands(console: DeveloperConsole): void {
        // 設定値取得コマンド
     ...
     * 設定値取得
     */
    private getConfig(args: string[]): string {
        try {
            const path = args[0];
     ...
     * 設定値設定
     */
    private setConfig(args: string[]): string {
        try {
            const path = args[0];
     ...
     * 設定値リセット
     */
    private resetConfig(args: string[]): string {
        try {
            const path = args[0];
     ...
     * 設定一覧表示
     */
    private listConfig(args: string[]): string {
        try {
     ...
     * 設定検証
     */
    private validateConfig(args: string[]): string {
        try {
     ...
     * 設定エクスポート
     */
    private exportConfig(args: string[]): string {
        try {
            const path = args.length > 0 ? args[0] : null;
     ...
     * 設定インポート
     */
    private importConfig(args: string[]): string {
        try {
     ...
     * 変更履歴表示
     */
    private showHistory(args: string[]): string {
     ...
     * 設定の差分表示
     */
    private diffConfig(args: string[]): string {
        try {
            const path = args.length > 0 ? args[0] : null;
     ...
     * 設定テンプレート適用
     */
    private applyTemplate(args: string[]): string {
    ...
    private filterConfigRecursive(obj: any, prefix: string, currentPath: string, result: any): void {
     ...
     * 設定の差分を検出
     */
    private findDifferences(current: any, defaults: any, basePath: string): DiffEntry[] {

...
export class ResponsiveDebugLayout {
     ...
     * ブレークポイント変更を処理
     */
    private handleBreakpointChange(breakpoint: BreakpointName, matches: boolean): void {
        if (matches) {
     ...
     * ブレークポイントレイアウトを適用
     */
    private applyBreakpointLayout(breakpoint: BreakpointName): void {
    ...
    private setupSwipeGestures(): void {
        ...
        debugPanel.addEventListener('touchstart', (e: TouchEvent) => {
            startX = e.touches[0].clientX;
            startY = e.touches[0].clientY;
        });

        debugPanel.addEventListener('touchend', (e: TouchEvent) => {
     ...
     * 画面向きレイアウトを適用
     */
    private applyOrientationLayout(orientation: Orientation): void {
        const debugPanel = this.debugInterface.debugPanel;
     ...
     * レスポンシブCSSルールを追加
     */
    public addResponsiveStyles(): void {

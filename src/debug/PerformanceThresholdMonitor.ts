...
interface ErrorHandler { ''
    handleError: (erro;r: Error, context: string') => void ,}'
}
...
';
     ...
     * カスタム閾値設定'
     */''
    public setThreshold(name: string, config: Partial<ThresholdConfig>): void { const threshold: ThresholdConfig = {''
     ...
     * 閾値監視停止
     */'
    public stopMonitoring(): void { if (this.monitoring.intervalId) {''
            clearInterval(this.monitoring.intervalId);
...
';
    ...
    private isSuppressed(name: string, severity: 'warning' | 'critical): boolean {
        ...
        if (suppressionEnd && Date.now() < suppressionEnd) { return true; }
...
';
    ...
    private setSuppression(name: string, severity: 'warning' | 'critical', timestamp: number): void {'
        const key = `${name}_${severity}`;''
        const suppressionDuration = severity === 'critical' ?   : undefined
     ...
     * 警告キュー処理
     */
    private processAlertQueue(): void { while (this.warningSystem.alertQueue.length > 0) {
...
';
     ...
     * 提案カテゴリ分類
     */''
    private categorizeSuggestion(suggestion: string): string { const categories: Record<string, string[]> = {'', 'rendering': ['render', 'draw', 'texture', 'batch'],
...
';
     ...
     * 提案クールダウンチェック
     */
    private isSuggestionInCooldown(suggestion: Suggestion): boolean { const similar = this.suggestionEngine.suggestionHistory.filter(s => )
    ...
    private attemptAutoRecovery(alert: Alert): void { ''
        const recoveryActions: Record<string, (') => void> = {'', 'fps': (') => { ''
     ...
     * 警告確認
     */
    public acknowledgeAlert(alertId: string): void { const alert = this.warningSystem.alertHistory.find(a => a.id === alertId);
        if(alert) {
     ...
     * 古い通知クリーンアップ
     */
    private cleanupOldNotifications(currentTime: number): void { const maxAge = 5 * 60 * 1000; // 5分
    ...
    private setupEventListeners()';
        ...
        this.visibilityChangeHandler = () => { ;
            ...
            } else { this.monitoring.enabled = true; }
        };
    ...
    public updateSettings(settings: Settings): void { if (settings.checkInterval) {
        ...
        if(settings.maxNotifications) { this.warningSystem.maxNotifications = settings.maxNotifications; }
     ...
     * リセット
     */
    public reset(): void { this.warningSystem.notifications = [];

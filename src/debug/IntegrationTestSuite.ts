...
export class IntegrationTestSuite {
     ...
     * ゲームシステム統合テスト
     */
    private async runGameSystemIntegrationTests(): Promise<void> {
     ...
     * 既存システム互換性テスト
     */
    private async runExistingSystemCompatibilityTests(): Promise<void> {
     ...
     * クロスブラウザ互換性テスト
     */
    private async runCrossBrowserCompatibilityTests(): Promise<void> {
        ...
        await this.runTest(category, 'Mobile Device Compatibility', async () => {
            const debugInterface = this.gameEngine.enhancedDebugInterface;
    ...
    private async runPerformanceIntegrationTests(): Promise<void> {
        ...
        await this.runTest(category, 'Performance Monitor Integration', async () => {
            const debugInterface = this.gameEngine.enhancedDebugInterface;
    ...
    private async runErrorHandlingTests(): Promise<void> {
        ...
        await this.runTest(category, 'Invalid Input Handling', async () => {
            const debugInterface = this.gameEngine.enhancedDebugInterface;
            
            // 無効な入力に対する堅牢性テスト
            const invalidInputs = [
                () => debugInterface.switchPanel(null),
        ...
        await this.runTest(category, 'Console Error Recovery', async () => {
            const debugInterface = this.gameEngine.enhancedDebugInterface;
    ...
    private async runMemoryManagementTests(): Promise<void> {
        ...
        await this.runTest(category, 'Panel Creation and Destruction', async () => {
            const debugInterface = this.gameEngine.enhancedDebugInterface;
        ...
        await this.runTest(category, 'Event Listener Cleanup', async () => {
            const debugInterface = this.gameEngine.enhancedDebugInterface;
     ...
     * 個別テストを実行
     */
    private async runTest(category: string, testName: string, testFunction: () => Promise<string>): Promise<TestResult> {
     ...
     * テストサマリーを生成
     */
    private generateTestSummary(duration: number): TestSummary {
     ...
     * テスト結果をエクスポート
     */
    exportTestResults(): TestSummary {
        const summary = this.generateTestSummary(performance.now() - (this.startTime || 0));
    ...
    getTestStatus(): {
        ...
        const currentCategory = currentResults.length > 0 
            ? currentResults[currentResults.length - 1].category 
            : null;

        return {
            isRunning: this.testRunning,
            totalTests: Object.keys(this.testCategories).length * 4, // 概算
            completedTests: currentResults.length,
            currentCategory
        };
    }
}
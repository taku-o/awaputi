...
export class DebugKeyboardShortcutManager {
     ...
     */
    findShortcut(shortcutString: string): ShortcutData | null {
        // アクティブコンテキストから検索
     ...
     * @returns 実行したかどうか
     */
    executeShortcut(event: KeyboardEvent, shortcut: ShortcutData, shortcutString: string): boolean {
        try {
     ...
     * @returns 登録成功可否
     */
    handleConflict(normalized: string, newShortcut: ShortcutData): boolean {
     ...
     * @returns マージ成功可否
     */
    mergeShortcuts(normalized: string, existing: ShortcutData, newShortcut: ShortcutData): boolean {
        // 優先度に基づいてマージ
     ...
     * @param normalized - 正規化されたショートカット
     */
    addToGroup(group: string, normalized: string): void {
     ...
     * @param normalized - 正規化されたショートカット
     */
    removeFromGroup(group: string, normalized: string): void {
     ...
     * @param normalized - 正規化されたショートカット
     */
    addToContext(context: string, normalized: string): void {
     ...
     * @param normalized - 正規化されたショートカット
     */
    removeFromContext(context: string, normalized: string): void {
        ...
        try {
            ...
            if (saved) {
                ...
                for (const [shortcut, data] of Object.entries(customizations)) {
                    ...
                    } else if (data.replacement && data.callback) {
                        this.unregister(shortcut);
     ...
     * @returns ショートカット配列
     */
    getShortcutsByGroup(group: string): ShortcutInfo[] {
        const groupSet = this.shortcutGroups.get(group);
        if (!groupSet) return [];
     ...
     * @returns ショートカット配列
     */
    getShortcutsByContext(context: string): ShortcutInfo[] {
        const contextSet = this.contexts.get(context);
     ...
     * @returns 競合マップ
     */
    getConflicts(): Map<string, ShortcutData[]> {

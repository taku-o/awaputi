...
export class EnhancedAutocompleteEngine {
     ...
     * 前方一致を取得
     */
    private getPrefixMatches(input: string): Suggestion[] {
     ...
     * 人気のコマンドを取得
     */
    private getPopularCommands(): Suggestion[] {
        const popularCommands = Array.from(this.usageStats.entries())
            .sort((a, b) => b[1] - a[1])
            .slice(0, 5)
    ...
    private getUsageCount(command: string): number {
        return this.usageStats.get(command) || 0;
    }

    /**
     * 実行結果から学習
     */
    public learnFromExecution(command: string, args: any[], success: boolean): void {
     ...
     * 設定を更新
     */
    public updateSettings(settings: Partial<AutocompleteSettings>): void {

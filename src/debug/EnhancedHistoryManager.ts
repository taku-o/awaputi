...
interface ExtendedStatistics extends HistoryStatistics {
    successRate: string;
    totalEntries: number;
}
...
export class EnhancedHistoryManager {
    ...
    public navigate(direction: 'up' | 'down', filter: string | null = null): { command: string } | null {
        ...
        if (direction === 'up') {
            ...
            newIndex = Math.min(this.history.length, this.currentIndex + 1);
        }
     ...
     * 統計情報を更新
     */
    private updateStatistics(entry: HistoryEntry): void {
     ...
     * 統計情報を取得
     */
    public getStatistics(): ExtendedStatistics {
        return {
            ...this.statistics,
    ...
    public exportHistory(format: ExportFormat = 'json', options: ExportOptions = {}): string {
        ...
        if (successOnly) {
            data = data.filter(entry => entry.success);
        }
    ...
    public importHistory(data: string, format: ImportFormat = 'json', options: ImportOptions = {}): ImportResult {
        ...
        for (const entry of importedEntries) {
            ...
            this.history.push({
                ...
                commandName: entry.commandName || entry.command?.split(' ')[0] || '',
                args: entry.args || []
            });

...
export class PerformanceVisualizer {
     ...
     * 統計情報レンダリング
     */
    private renderStatistics(ctx: CanvasRenderingContext2D): void {
        if (!this.canvas) return;
     ...
     * ホバー情報表示
     */
    private renderHoverInfo(ctx: CanvasRenderingContext2D): void {
        if (!this.canvas) return;
     ...
     * アラート表示
     */
    private renderAlerts(ctx: CanvasRenderingContext2D): void {
...
class PerformanceChart {
    ...
    public render(ctx: CanvasRenderingContext2D, globalSettings: PerformanceSettings): void {
    ...
    private renderGrid(ctx: CanvasRenderingContext2D, globalSettings: PerformanceSettings): void {
        const pos = this.config.position;
        const gridLines = 5;
    ...
    private renderThresholds(ctx: CanvasRenderingContext2D, globalSettings: PerformanceSettings): void {
        const pos = this.config.position;
    ...
    private renderDataLine(ctx: CanvasRenderingContext2D): void {
        const pos = this.config.position;
    ...
    public isPointInChart(x: number, y: number): boolean {
        const pos = this.config.position;
...
class PerformanceHeatmap {
    ...
    public updateHeatmap(data: HeatmapData): void {
        // パフォーマンス指標を0-1に正規化
    ...
    private renderHeatmapCells(ctx: CanvasRenderingContext2D): void {
        const pos = this.config.position;
    ...
    private renderColorScale(ctx: CanvasRenderingContext2D, globalSettings: PerformanceSettings): void {
        const pos = this.config.position;

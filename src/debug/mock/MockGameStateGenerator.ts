...
interface StateGenerationOptions {
    level?: number;
    minScore?: number;
    maxScore?: number;
    bubbleCount?: number;
...
export class MockGameStateGenerator {
    ...
    public generateProgressiveStates(count: number): GameState[] {
    ...
    public generateEndGameState(): GameState {
        return {
    ...
    public generateWinState(level: number): GameState {
        return {
            ...this.generateGameState({ level }),
            bubbles: [],
            score: level * 5000 + Math.random() * 2000
        };
    }

    private generateMockBubbles(count: number): any[] {
    ...
    private generateRandomPowerUps(): string[] {

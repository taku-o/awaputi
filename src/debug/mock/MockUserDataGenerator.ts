...
export class MockUserDataGenerator {
    private usernameTemplates = [
        'Player', 'Gamer', 'BubbleMaster', 'PopKing', 'ArcadeHero',
    ...
    public generateUser(options: UserGenerationOptions = {}): UserData {
        ...
        return {
            ...
            statistics: {
                totalPlayTime: gamesPlayed * (300 + Math.random() * 600), // 5-15 min per game
    ...
    public generateUsers(count: number): UserData[] {
        return Array.from({ length: count }, () => this.generateUser());
    }

    public generateLeaderboard(count: number): UserData[] {
        const users = this.generateUsers(count);
        return users.sort((a, b) => b.totalScore - a.totalScore);
    }

    public generateNewUser(): UserData {
    ...
    public generateExpertUser(): UserData {
        return this.generateUser({
    ...
    private generateUsername(): string {
        const template = this.usernameTemplates[Math.floor(Math.random() * this.usernameTemplates.length)];
        const number = Math.floor(Math.random() * 9999) + 1;
        return `${template}${number}`;
    }

    private generateAchievements(level: number): string[] {
        const maxAchievements = Math.min(level, this.achievements.length);

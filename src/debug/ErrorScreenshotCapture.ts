...
export class ErrorScreenshotCapture {
     ...
     */
    private async captureDOMScreenshot(): Promise<string> {
        // html2canvasライブラリが必要
     ...
     * スクリーンショットをストレージに保存
     */
    private storeScreenshot(screenshot: Screenshot): void {
        try {
            // サイズ制限チェック
     ...
     * ストレージサイズ制限の適用
     */
    private enforceStorageSizeLimit(): void {
     ...
     * 保存されたスクリーンショットの読み込み
     */
    private loadStoredScreenshots(): void {
        try {
     ...
     * 古いスクリーンショットのクリア
     */
    public clearOldScreenshots(): void {
     ...
     * 特定のスクリーンショットの取得
     */
    public getScreenshot(id: string): Screenshot | undefined {
     ...
     * キャプチャ設定の更新
     */
    public updateSettings(newSettings: Partial<CaptureSettings>): void {
        this.captureSettings = {
            ...this.captureSettings,
            ...newSettings
        };
     ...
     * スクリーンショットの手動削除
     */
    public deleteScreenshot(id: string): Screenshot | null {
     ...
     * 全スクリーンショットのクリア
     */
    public clearAllScreenshots(): void {
        this.storedScreenshots = [];
        this.saveToLocalStorage();

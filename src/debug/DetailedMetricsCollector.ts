...
interface ParticleEntityMetrics {
    total: number;
...
export class DetailedMetricsCollector {
     ...
     * 全詳細メトリクスの収集
     */
    public collectDetailedMetrics(): void {
     ...
     * レンダリング詳細メトリクス収集
     */
    private collectRenderingDetails(): void {
     ...
     * メモリ詳細メトリクス収集
     */
    private collectMemoryDetails(): void {
     ...
     * ガベージコレクション分析
     */
    private analyzeGCPatterns(memoryMetrics: MemoryMetrics): void {
     ...
     * ゲーム詳細メトリクス収集
     */
    private collectGameDetails(): void {
     ...
     * エンティティメトリクス収集
     */
    private collectEntityMetrics(gameMetrics: GameMetrics): void {
     ...
     * 物理シミュレーション統計
     */
    private collectPhysicsMetrics(gameMetrics: GameMetrics): void {
        gameMetrics.physics = {
     ...
     * オーディオ詳細メトリクス収集
     */
    private collectAudioDetails(): void {
     ...
     * ネットワーク詳細メトリクス収集
     */
    private collectNetworkDetails(): void {
     ...
     * システム詳細メトリクス収集
     */
    private collectSystemDetails(): void {
    ...
    // WebGL プロファイラー設定
    private setupWebGLProfiler(): void {
        const canvas = this.gameEngine?.canvas;
        if (!canvas) return;
    ...
    // メモリ追跡設定
    private setupMemoryTracker(): void {
        if (process.env.NODE_ENV === 'development') {
    ...
    // ゲームループプロファイラー設定
    private setupGameLoopProfiler(): void {
    ...
    // オーディオプロファイラー設定
    private setupAudioProfiler(): void {
        const audioManager = this.gameEngine?.audioManager;
        if (!audioManager) return;
    ...
    private estimateVertexCount(): number {
    ...
    // パブリックAPI
    public getExtendedMetrics(): Record<string, any> {
        return {
 ...
 * 拡張レンダリングメトリクス
 */
class RenderingMetrics implements MetricTimestamp {

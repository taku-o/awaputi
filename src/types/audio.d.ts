...
export interface PlaySoundOptions { volume?: number,
  pitch?: number;
    pan?: number;
    loop?: boolean;
    fadeIn?: number;
...
export interface BGMSystem extends Manager { audioManager: AudioManager,
  ...
  volume?: number;
    fadeIn?: number;
    startTime?: number;
    export interface BGMMetadata { title?: string,
  artist?: string;
    album?: string;
    genre?: string;
    bpm?: number;
...
export interface AudioConfigurationManager { configManager: ConfigurationManager | null,
  ...
  sfxGainNode: GainNode,
    ...
    export interface AudioSettings { volumes: { maste,r: number,
    sfx: number,
    bgm: number,
            muted: boolean,
  effects: { compression: boolean,
...
export interface AudioAccessibilitySupport { // Audio accessibility interface (placeholder),
  initialize(): Promise<void>;
  dispose(): void;

export interface AudioEffectManager { // Audio effect manager interface (placeholder),
  initialize(): Promise<void>;
  dispose(): void;

export interface SoundPoolManager { // Sound pool manager interface (placeholder),
  initialize(): Promise<void>;
  dispose(): void;

export interface AudioEffectContextManager { // Audio effect context manager interface (placeholder),
  initialize(): Promise<void>;
  dispose(): void;

export interface SoundEffectRenderer { // Sound effect renderer interface (placeholder),
  initialize(): Promise<void>;
  dispose(): void;

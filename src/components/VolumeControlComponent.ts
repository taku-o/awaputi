...
export class VolumeControlComponent {
    ...
    private createVolumeControlUI(parentContainer: HTMLElement): void {
        ...
        label.innerHTML = '🔊 ' + (this.localizationManager.getText?.('settings.audio.masterVolume') || 'Master Volume');
        label.style.cssText = `
            color: #ffffff;
            font-size: 16px;
            min-width: 100px;
    ...
    private createVolumeButton(type: 'up' | 'down', icon: string, clickHandler: () => void): HTMLButtonElement {
        ...
        button.setAttribute('aria-label', 
            type === 'up' ? 
            this.localizationManager.getText?.('settings.audio.volumeUp') || 'Volume Up' :
            this.localizationManager.getText?.('settings.audio.volumeDown') || 'Volume Down'
        );
        ...
        try {
            ...
            if (this.progressBar) {
                this.progressBar.setAttribute('aria-label', 
                    `${this.localizationManager.getText?.('settings.audio.masterVolume') || 'Master Volume'}: ${Math.round(this.currentVolume * 100)}%`);

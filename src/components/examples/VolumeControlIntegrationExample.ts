...
export class VolumeControlIntegrationExample {
    ...
    getVolumeControlStatus(): any {
        ...
        return {
            isEnabled: this.volumeControl.isEnabled(),
     ...
     * 音量制御の表示/非表示切り替え
     */
    setVolumeControlVisible(visible: boolean): void {
        if (this.volumeControl) {
     ...
     */
    onExternalVolumeChange(volume: number): void {
        if (this.volumeControl) {

...
export class SettingsImportExportComponent { private gameEngine: GameEngine;
    ...
    language: navigator.language,
        ...
        if (this.accessibilityManager) {
            try {
                ...
                // 統計情報
                exportData.accessibility.stats = this.accessibilityManager.getStats();'} catch (error) { : undefined''
     ...
     * @returns 検証結果
     */
    private async validateImportFile(file: File): Promise<ValidationResult> { // ファイルサイズチェック
     ...
     * @returns 適用結果
     */
    private async applyImportedSettings(data: any): Promise<ApplyResult> { let appliedCount = 0,

...
interface RuleDefinitionOptions { 
    category?: string;
    description?: string;
    check: (oldValue: any, newValue: any, context: ValidationContext) => boolean | RuleResult;
    severity?: 'low' | 'warning' | 'medium' | 'high' | 'critical';
...
export class ValidationRuleDefinitions {
     ...
     */
    private addBubbleHealthRules(): void {
     ...
     */
    private addSpecialEffectRules(): void {
     ...
     * Get rule by name
     */
    getRule(name: string): RuleDefinition | null { 
     ...
     */
    getBubbleHealthLimits(bubbleType: string): BubbleHealthLimits { 
        const limits: Record<string, BubbleHealthLimits> = {
            normal: { min: 1, max: 5 },
            stone: { min: 1, max: 8 },
            iron: { min: 2, max: 12 },
     ...
     * Get score limits
     */
    getScoreLimits(bubbleType: string): ScoreLimits { 
        const limits: Record<string, ScoreLimits> = {
            normal: { max: 50 },
            stone: { max: 100 },
            iron: { max: 150 },
            diamond: { max: 200 },
     ...
     */
    getChangeThreshold(bubbleType: string, propertyType: string): number { 

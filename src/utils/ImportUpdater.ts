...
export class ImportUpdater {
            ...
            try {
                ...
                for (const importInfo of imports) {
                    ...
                    if (className && this.matchesClassName(importInfo, className)) {
                        ...
                        results.byClassName.get(className)!.push({
                            ...importInfo,
                    ...
                    if (fileName && this.matchesFileName(importInfo, fileName)) {
                        ...
                        results.byFileName.get(fileName)!.push({
                            ...importInfo,
                            containingFile: filePath
                        });
                        results.total++;
    ...
    extractImportsFromContent(content: string, filePath: string): ImportInfo[] {
        ...
        // 名前付きインポート
        const namedImportMatches = Array.from(content.matchAll(this.importPatterns.namedImport));
        ...
        // デフォルトインポート
        const defaultImportMatches = Array.from(content.matchAll(this.importPatterns.defaultImport));
        ...
        // 名前空間インポート
        const namespaceImportMatches = Array.from(content.matchAll(this.importPatterns.namespaceImport));
        ...
        // サイドエフェクトインポート
        const sideEffectImportMatches = Array.from(content.matchAll(this.importPatterns.sideEffectImport));
        ...
        // 動的インポート
        const dynamicImportMatches = Array.from(content.matchAll(this.importPatterns.dynamicImport));

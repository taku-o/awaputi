...
interface DirtyRegionResult extends DirtyRegionBounds {
    full?: boolean;
}

interface DirtyRegionManagerConfig {
    enabled?: boolean;
    minRegionSize?: number;
    maxRegionCount?: number;
    mergeThreshold?: number;
...
export class BasicDirtyRegionManager {
     ...
     */
    calculateDirtyRegions(objects: TrackedObject[]): DirtyRegionResult[] {
     ...
     * Calculate dirty regions from object changes
     */
    calculateObjectDirtyRegions(objects: TrackedObject[]): void {
     ...
     */
    hasObjectChanged(obj: TrackedObject): boolean {
     ...
     */
    calculateObjectBounds(obj: TrackedObject): DirtyRegionBounds {
        const bounds: DirtyRegionBounds = {
            x: obj.x || 0,
            y: obj.y || 0,
            width: obj.width || 0,
            height: obj.height || 0
        ...
        if (obj.rotation || (obj.scale !== undefined && obj.scale !== 1)) {
            const _centerX = bounds.x + bounds.width / 2;
            const _centerY = bounds.y + bounds.height / 2;
     ...
     * Add a dirty region
     */
    addDirtyRegion(bounds: DirtyRegionBounds): void {
        // Expand region slightly to avoid edge effects
        ...
        if (expanded.width >= this.config.minRegionSize || 
            expanded.height >= this.config.minRegionSize) {
            this.config.regions.add(expanded);
        }
    }
     ...
     * Add user-defined dirty regions
     */
    addUserDirtyRegions(): void {
        const regions = Array.from(this.userDirtyRegions);
     ...
     */
    mergeOverlappingRegions(regions: DirtyRegionBounds[]): void {
        ...
        for (const region of regions) {
            ...
            if (!wasMerged) {
                this.config.mergedRegions.push({ ...region });
            }
        }
     ...
     */
    shouldMergeRegions(region1: DirtyRegionBounds, region2: DirtyRegionBounds, threshold: number): boolean {
     ...
     * Merge two regions
     */
    mergeRegions(target: DirtyRegionBounds, source: DirtyRegionBounds): void {
     ...
     */
    optimizeDirtyRegionSizes(regions: DirtyRegionBounds[]): void {
        const maxRegions = this.config.maxRegionCount;
     ...
     */
    updateObjectTracking(obj: TrackedObject): void {
        this.objectTracking.set(obj.id, {
            x: obj.x || 0,
            y: obj.y || 0,
            width: obj.width || 0,
            height: obj.height || 0,
     ...
     */
    updateDirtyRegionStats(regions: DirtyRegionBounds[]): void {
        this.config.stats.totalRegions = regions.length;
        ...
        if (regions.length === 0) {
            this.config.stats.skippedRedraws++;
        }
     ...
     */
    markDirtyRegion(bounds: DirtyRegionBounds): void {
        this.userDirtyRegions.add(bounds);
    }
     ...
     */
    configure(config: DirtyRegionManagerConfig): void {

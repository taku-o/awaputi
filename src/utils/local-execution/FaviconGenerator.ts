...
class FaviconGenerator { /**
    ...
    static async generateMissingFavicons(config: FaviconConfig = { ): Promise<GenerationResult> { }
        ...
        try { const result: GenerationResult = {
            ...
            // 生成リクエスト準備
            const requests: GenerationRequest[] = finalConfig.sizes.map(size => ({ size)
                config: finalConfig,);
                configHash)));
    ...
    static clearCache(size: number | null = null): void { FaviconCacheManager.clear(size);
        ...
        } else {  const keysToDelete = Array.from(this.generationCache.keys() }
    ...
    private static async _renderFaviconWithCache(canvasInfo: CanvasInfo | null, request: GenerationRequest): Promise<FaviconDetail> {
        ...
        // キャッシュ保存
        if (config.cacheEnabled) { FaviconCacheManager.set(size, configHash, dataURL); }
        }
...
'';
export default FaviconGenerator; : undefined
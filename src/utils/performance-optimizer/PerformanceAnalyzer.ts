...
export class PerformanceAnalyzer {
    ...
    constructor(config: AnalyzerConfig = {}) {
        ...
        this.maxHistorySize = config.maxHistorySize || 300;
    ...
    updateBasicStats(): void {
        try {
            ...
            // 平均フレーム時間
            this.stats.avgFrameTime = this.frameTimeHistory.reduce((sum, time) => sum + time, 0) / this.frameTimeHistory.length;
    ...
    reset(): void {
        try {
            ...
            this.stabilityAnalysis.frameTimeBuffer = [];
            this.stabilityAnalysis.performanceSnapshots = [];
            this.stabilityAnalysis.issueHistory = [];
        } catch (error) {

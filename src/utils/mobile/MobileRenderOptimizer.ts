...
';
    ...
    applyDeviceSpecificOptimizations(): void { ''
        ...
        } else if (userAgent.includes('android') { this.applyAndroidOptimizations(); }
        } else { this.applyWebOptimizations(); }
        }
    }
    ...
    updateFrameTime(frameTime: number): void { const stats = this.renderMonitoring.frameStats;
        ...
        const variance = this.frameTimeBuffer.reduce((acc, time) => { return acc + Math.pow(time - avgFrameTime, 2); }

...
interface MemoryPressureDetection {
    enabled: boolean;
...
interface CompletedLoadItem extends LoadItem {
    loadTime: number;
    status: 'success';
}

interface FailedLoadItem extends LoadItem {
    error: string;
    status: 'failed';
}
...
export class MobileResourceManager {
     ...
     */
    getTextureFromPool(key: string, creator: () => any): any {
        const pool = this.resourceConfig.textures.texturePool;
     ...
     * Clear texture pool
     */
    clearTexturePool(ratio: number = 1.0): void {
        const pool = this.resourceConfig.textures.texturePool;
     ...
     */
    streamAsset(assetId: string, position: any, priority: number = 0): LoadItem | null {
        const streaming = this.resourceConfig.streaming;
     ...
     */
    async loadAsset(loadItem: LoadItem): Promise<void> {
     ...
     */
    updateLoadStatistics(loadTime: number, success: boolean): void {
     ...
     */
    getResourceStatistics(): ResourceUsageStats {
        return {
     ...
     */
    setMemoryPressureCallbacks(warningCallback?: (pressure: string) => void, criticalCallback?: (pressure: string) => void): void {

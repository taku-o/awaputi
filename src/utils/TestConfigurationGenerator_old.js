...
export class TestConfigurationGenerator {
    ...
    extractCanonicalExpectations() {
        ...
        // フォールバック実装
        return this._legacyExtractCanonicalExpectations();
    }
     ...
     * @private
     */
    _legacyExtractCanonicalExpectations() {
        try {
            const expectations = {
                bubbleTypes: {},
                gameBalance: {},
    ...
    generateTestFile(expectations, testType) {
        ...
        // フォールバック実装
        return this._legacyGenerateTestFile(expectations, testType);
    }

    /**
     * レガシーテストファイル生成（フォールバック用）
     * @param {Object} expectations - 期待値
     * @param {string} testType - テストタイプ
     * @returns {string|null} テストコード
     * @private
     */
    _legacyGenerateTestFile(expectations, testType) {
        try {
            switch (testType) {
                case 'bubble':
    ...
    // === レガシーメソッド（プライベート・フォールバック用のみ保持） ===

    /**
     * Bubble.test.jsを生成（フォールバック用）
     * @param {Object} expectations - 期待値
...
`;
    ...
    updateTestFiles(expectations, options = {}) {
        ...
        // フォールバック実装
        return this._legacyUpdateTestFiles(expectations, options);
    }

    /**
     * レガシーテストファイル更新（フォールバック用）
     * @param {Object} expectations - 期待値
     * @param {Object} options - オプション
     * @returns {Object} 更新結果
     * @private
     */
    _legacyUpdateTestFiles(expectations, options = {}) {
        try {
            const results = {
                updated: [],
                failed: [],
    ...
    validateConfigurationSync() {
        ...
        // フォールバック実装
        return this._legacyValidateConfigurationSync();
    }
     ...
     * @private
     */
    _legacyValidateConfigurationSync() {
        try {
            const canonical = this.extractCanonicalExpectations();
            if (!canonical) {

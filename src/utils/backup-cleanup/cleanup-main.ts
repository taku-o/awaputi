...
function displayResults(result: CleanupResult): void {
    ...
    if (result.executionState.results.deletion?.sizeReduction) {
        const sizeReduction = result.executionState.results.deletion.sizeReduction;
        console.log('\n💾 SIZE REDUCTION:');
    ...
    // レポート情報
    if (result.executionState.results.reports?.reportFileName) {

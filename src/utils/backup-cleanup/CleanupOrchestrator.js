...
export class CleanupOrchestrator {
    ...
    async executeCleanup() {
        ...
        try {
            ...
            this.log(`Cleanup process failed: ${error.message}`, 'error');
            this.executionState.errors.push(error);
            this.executionState.phase = 'failed';
            this.executionState.endTime = new Date().toISOString();
            
            return this.generateFinalResult('error', error);
        }
    }
    ...
    /**
     * 安全性検証フェーズの実行
     * @param {Array} candidateFiles - 候補ファイル配列
     * @returns {Promise<Object>} 安全性検証結果
     */
    async runSafetyVerificationPhase(candidateFiles) {
    ...
    /**
     * 削除実行フェーズの実行
     * @param {Array} verifiedFiles - 検証済みファイル配列
     * @returns {Promise<Object>} 削除結果
     */
    async runDeletionPhase(verifiedFiles) {
    ...
    /**
     * ユーザー確認の要求
     * @param {Array} filesToDelete - 削除予定ファイル配列
     * @returns {Promise<boolean>} 確認結果
     */
    async requestUserConfirmation(filesToDelete) {
     ...
     * @param {string} phase - エラー発生フェーズ
     * @returns {Promise<Object>} 回復結果
     */
    async handleErrorAndRecover(error, phase) {
     ...
     * @param {Error} error - エラー（オプション）
     * @returns {Object} 最終結果
     */
    generateFinalResult(status, error = null) {
        const result = {
            status,
            executionState: this.executionState,
            summary: {
                totalExecutionTime: this.executionState.endTime ? 
                    new Date(this.executionState.endTime) - new Date(this.executionState.startTime) : 
                    null,

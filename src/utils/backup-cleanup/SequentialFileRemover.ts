            ...
            try {
                ...
                if(deletionResult.success) {
                    ...
                    if (verificationResult.deleted) {'
                        ...
                        '';
                        if (testResult.passed') { results.summary.succeeded++; }
...
';
        ...
        } catch (error) { // ファイルが存在しない場合はエラーが発生する（期待される動作）
...
';
        try { ' }'
            ...
            } catch (error) { // ブランチ取得エラーは無視 }
            } catch (error) { gitInfo.error = (error as Error).message; }
        }

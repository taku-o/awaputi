...
interface TestResults {
    basicImportTest: boolean;
    buildTest: boolean;
    syntaxTest: boolean;
}

interface PostDeletionTestResult {
    passed: boolean;
    tests: TestResults;
    errors: string[];
    executedAt: string;
...
export class SequentialFileRemover {
     ...
     * 検証済みファイルの安全削除実行
     */
    async removeFilesSafely(verifiedFiles: FileInfo[]): Promise<DeletionResults> {
     ...
     * 削除前バックアップ記録作成
     */
    async createDeletionBackup(filePath: string): Promise<BackupRecord> {
     ...
     * 削除確認
     */
    async verifyDeletion(filePath: string): Promise<VerificationResult> {
        ...
        try {
            ...
            verification.error = 'File still exists';
        } catch (error) {
            // ファイルが存在しない場合はエラーが発生する（期待される動作）
     ...
     * 削除後テスト実行
     */
    async runPostDeletionTests(): Promise<PostDeletionTestResult> {
        const testResult: PostDeletionTestResult = {
            passed: false,
            tests: {
     ...
     * 基本インポートテストの実行
     */
    async runBasicImportTest(): Promise<boolean> {
        try {
            // 主要なコアファイルが正常にインポートできるかテスト
            const coreFiles = [
     ...
     * 基本ビルドテストの実行
     */
    async runBasicBuildTest(): Promise<boolean> {
        try {
            // package.jsonの存在確認
     ...
     * 基本構文テストの実行
     */
    async runBasicSyntaxTest(): Promise<boolean> {
        try {
        ...
        async function scanDir(dir: string) {
            try {
                ...
                for (const entry of entries) {
                    ...
                    } else if (entry.name.endsWith('.js') && !entry.name.includes('_old') && !entry.name.includes('_original') && !entry.name.includes('_backup')) {
     ...
     * Gitファイル情報取得
     */
    async getGitFileInfo(filePath: string): Promise<GitInfo> {
        ...
        try {
            ...
            gitInfo.error = (error as Error).message;
        }
     ...
     * 復旧手順の生成
     */
    generateRecoveryInstructions(filePath: string, gitInfo: GitInfo): RecoveryInstructions {
     ...
     * バックアップ記録の取得
     */
    getBackupRecord(): Record<string, BackupRecord> {
     ...
     * 削除実行レポートの生成
     */
    generateDeletionReport(deletionResults: DeletionResults): DeletionReport {

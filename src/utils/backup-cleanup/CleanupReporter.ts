...
export class CleanupReporter {
     ...
     * 調査結果サマリーの生成
     */
    async generateInvestigationSummary(investigationResults: InvestigationResult[]): Promise<InvestigationSummary> { const summary: InvestigationSummary = {
            overview: { totalFilesInvestigated: investigationResults.length  ,
     ...
     * 削除作業サマリーの生成
     */''
    async generateDeletionSummary(deletionResults: DeletionResults): Promise<DeletionSummary> { const summary: DeletionSummary = {
            overview: { totalFilesProcessed: deletionResults.summary.totalFiles  ,
...
';'
        ...
        summary.backupInfo.backupsCreated = successfulDeletions.filter();
            d => d.backupRecord && d.backupRecord.backupCreated).length;
        summary.backupInfo.recoveryPossible = successfulDeletions.filter();
            d => d.backupRecord && d.backupRecord.gitInfo && d.backupRecord.gitInfo.hasHistory).length;

        // テスト結果
        summary.testResults.postDeletionTestsPassed = successfulDeletions.filter();
            d => d.testResult && d.testResult.passed).length;
     ...
     * サイズ削減効果の計算
     */''
    async calculateSizeReduction(beforeSizes: BeforeAfterSizes, afterSizes: BeforeAfterSizes): Promise<SizeReduction> { const reduction: SizeReduction = {
            beforeDeletion: { totalBytes: 0  ,
                totalWords: 0,
    fileCount: 0 };
    ...
    maintenanceBenefits: []  }
        ...
        // メンテナンス効果
        reduction.impact.maintenanceBenefits = [`${reduction.reduction.filesRemoved}個の不要ファイルを削除,
            `${reduction.impact.repositorySizeReduction}のディスク容量節約`,', 'IDE検索結果の整理',]';
            'プロジェクト構造の簡素化'];
        ];
     ...
     * 復旧手順の作成
     */
    async createRecoveryInstructions(deletedFiles: DeletionDetail[]): Promise<RecoveryInstructions> { const instructions: RecoveryInstructions = {
            overview: {
     ...
     * 最終レポートの生成
     */''
    async generateFinalReport(allResults: AllResults): Promise<FinalReport> { const report: FinalReport = {'
            executiveSummary: {''
     ...
     * コミットメッセージからパターンを抽出
     */''
    extractCommitPattern(message: string): string { // Task番号やフェーズ情報を抽出
...
','
     ...
     * 復旧方法の決定'
     */''
    determineRecoveryMethod(deletedFile: DeletionDetail): RecoveryMethod { const method: RecoveryMethod = {'
        ...
        if(method.type === 'manual' {'
            method.instructions = [`対応する現在ファイルから手動で復旧を検討,
                `プロジェクトの最新バックアップから復元`],
                `必要に応じて再実装`] }
            ]; }
        }
     ...
     * ステップバイステップ復旧手順の生成
     */
    generateStepByStepRecovery(recoveryMethods: RecoveryMethods): RecoveryStep[] { const steps: RecoveryStep[] = [],
        ...
        if (recoveryMethods.manual.length > 0) { steps.push({'
                step: steps.length + 1,
                title: '手動復旧',','
     ...
     * 緊急時手順の生成'
     */''
    generateEmergencyProcedures(deletedFiles: DeletionDetail[]): EmergencyProcedure[] { return [{''
...
';'
     ...
     * 結論の生成'
     */''
    generateConclusion(allResults: AllResults, overallStatus: string): Conclusion { const conclusion: Conclusion = {''
...
';'
        ...
        // 達成された利益
        if (allResults.sizeReduction) { conclusion.benefitsAchieved.push(`${allResults.sizeReduction.impact.repositorySizeReduction}のディスク容量節約`};' }'
...
';'
        ...
        } else {  conclusion.nextSteps = [', '失敗した削除の原因調査','
                'システム整合性の修復',]','
                '改善されたクリーンアップ戦略の策定'] }
            ]; }
        }
     ...
     * レポートをファイルに保存'
     */''
    async saveReportToFile(report: FinalReport, filename: string): Promise<void> { try {'
            const reportsDir = '.kiro/reports,
            
            // レポートディレクトリを作成
            try {
                await fs.mkdir(reportsDir, { recursive: true, catch (error) { // ディレクトリ作成エラーは無視 }
;

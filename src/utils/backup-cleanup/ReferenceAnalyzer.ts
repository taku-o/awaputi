...
interface ReferencesByType {
    import: ImportReference[];
    string: StringReference[];
}

interface ReferencesByLocation {
    [directory: string]: (ImportReference | StringReference)[];
}

interface ContextAnalysis {
    byType: ReferencesByType;
...
export class ReferenceAnalyzer {
     ...
     * 文字列参照の検索
     */
    async searchStringReferences(filePath: string): Promise<StringSearchResult> {
        const fileName = path.basename(filePath);
        const fileNameWithoutExt = path.basename(filePath, path.extname(filePath));
     ...
     * レポートファイルを除外した参照の検索
     */
    async excludeReportFiles(references: (ImportReference | StringReference)[]): Promise<(ImportReference | StringReference)[]> {
     ...
     * 参照コンテキストの分析
     */
    async analyzeReferenceContext(references: (ImportReference | StringReference)[]): Promise<ContextAnalysis> {
    ...
    async generateReferenceReport(filePath: string, analysis: AnalysisInput = {}): Promise<ReferenceReport> {
        ...
        const allReferences: (ImportReference | StringReference)[] = [
     ...
     */
    private containsStringReference(line: string, filePath: string, fileName: string, fileNameWithoutExt: string): boolean {
     ...
     * レポートファイルかどうかの判定
     */
    private isReportFile(filePath: string): boolean {
     ...
     * 行の前後コンテキストを取得
     */
    private getContext(lines: string[], lineIndex: number, contextLines: number = 2): ReferenceContext {
     ...
     * 安全性に関する警告を生成
     */
    private generateSafetyWarnings(contextAnalysis: ContextAnalysis): SafetyWarning[] {
     ...
     * 安全性に関する推奨事項を生成
     */
    private generateSafetyRecommendations(contextAnalysis: ContextAnalysis): SafetyRecommendation[] {
...
export class ReferenceAnalyzer {
     ...
     * 指定されたファイルへの参照を検索
     */
    async searchImportReferences(filePath: string): Promise<ImportAnalysisResult> {
        const fileName = path.basename(filePath);
        const fileNameWithoutExt = path.basename(filePath, path.extname(filePath));
        const relativePath = filePath.replace(/^\.\//, '');
     ...
     * 文字列参照の検索
     */
    async searchStringReferences(filePath: string): Promise<StringAnalysisResult> {
        const fileName = path.basename(filePath);
        const fileNameWithoutExt = path.basename(filePath, path.extname(filePath));
     ...
     * レポートファイルを除外した参照の検索
     */
    async excludeReportFiles(references: Reference[]): Promise<Reference[]> {
        return references.filter(ref => !this.isReportFile(ref.file));
    }
     ...
     * 参照コンテキストの分析
     */
    async analyzeReferenceContext(references: Reference[]): Promise<ContextAnalysis> {
        const contextAnalysis: ContextAnalysis = {
            byType: {
     ...
     * 文字列に対象ファイルへの参照が含まれているかチェック
     */
    containsStringReference(line: string, filePath: string, fileName: string, fileNameWithoutExt: string): boolean {
        return line.includes(fileName) ||
               line.includes(fileNameWithoutExt) ||
               line.includes(filePath);
    }
     ...
     * レポートファイルかどうかの判定
     */
    isReportFile(filePath: string): boolean {
        return /file-size-report\.json$/.test(filePath) ||
               /\.log$/.test(filePath) ||
               /report/.test(filePath.toLowerCase());
    }
     ...
     * 行の前後コンテキストを取得
     */
    getContext(lines: string[], lineIndex: number, contextLines: number = 2): ContextInfo {
        const start = Math.max(0, lineIndex - contextLines);
        const end = Math.min(lines.length - 1, lineIndex + contextLines);
     ...
     * 安全性に関する警告を生成
     */
    generateSafetyWarnings(contextAnalysis: ContextAnalysis): SafetyWarning[] {
        const warnings: SafetyWarning[] = [];
     ...
     * 安全性に関する推奨事項を生成
     */
    generateSafetyRecommendations(contextAnalysis: ContextAnalysis): SafetyRecommendation[] {
        const recommendations: SafetyRecommendation[] = [];

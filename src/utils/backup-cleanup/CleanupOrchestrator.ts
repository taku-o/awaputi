...
export class CleanupOrchestrator {
     ...
     * 調査フェーズの実行
     */
    async runInvestigationPhase(): Promise<InvestigationPhaseResult> {
     ...
     * 整合性確認フェーズの実行
     */
    async runIntegrityValidationPhase(): Promise<IntegrityPhaseResult> {
     ...
     * レポート生成フェーズの実行
     */
    async runReportingPhase(): Promise<ReportingPhaseResult> {
     ...
     * 最終結果の生成
     */
    generateFinalResult(status: string, error: Error | null = null): FinalResult {
        const result: FinalResult = {
            status,
            executionState: this.executionState,
            summary: {
                totalExecutionTime: this.executionState.endTime ? 
     ...
     * ステータス別推奨事項の生成
     */
    generateRecommendations(status: string): string[] {
     ...
     * 設定オプションの取得
     */
    getOptions(): CleanupOptions {
        return { ...this.options };
    }
}

export default CleanupOrchestrator;
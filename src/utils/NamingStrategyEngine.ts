...
export class NamingStrategyEngine {
    ...
    constructor() {
        // ドメインベースのプレフィックス定義
        this.domainPrefixes = new Map<string, string>([
            ['src/core', 'Core'],
            ['src/debug', 'Debug'],
            ['src/ui', 'UI'],
            ['src/scenes', 'Scene'],
        ...
        this.specialCases = new Map<string, SpecialCaseMapping>([
            ...
            ['BaseDialog', {
                'src/scenes/components': 'SceneBaseDialog',
                'src/ui': 'UIBaseDialog'
            }],
            ['PerformanceMonitor', {
                'src/analytics': 'AnalyticsPerformanceMonitor',
    ...
    validateNamingConventions(newNames: NameInfo[]): NameValidation[] {
        ...
        for (const nameInfo of newNames) {
            ...
            // 予約語の検証
            if (this.isReservedWord(nameInfo.new)) {
                result.isValid = false;
    ...
    generateDirectoryPrefix(dirPath: string): string {
        ...
        this.domainPrefixes.forEach((prefix, path) => {
    ...
    evaluateReadability(resolutions: (FileNameResolution | ClassNameResolution)[]): number {
        ...
        for (const resolution of resolutions) {
            ...
            // 略語の過度な使用を避ける
            if (!this.hasExcessiveAbbreviations(name)) {
                score += 5;
            }
        }

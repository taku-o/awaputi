...
interface ExtendedStatistics extends CleanupStatistics {
...
export class ProactiveCleanupManager {
     ...
     */
    registerTimer(timerId: number, type: 'timeout' | 'interval' = 'timeout'): void {
     ...
     */
    registerCanvasContext(context: CanvasRenderingContext2D, canvas: HTMLCanvasElement): void {
     ...
     */
    registerCustomCleanup(name: string, cleanupFn: () => void, priority: number = 5): void {
     ...
     */
    unregisterCustomCleanup(name: string): void {
    ...
    private _cleanupEventListeners(maxAge: number): number {
        ...
        for (const [key, listeners] of Array.from(this.cleanupTargets.eventListeners)) {
    ...
    private _evictExpiredCacheItems(): number {
        ...
        for (const [key, item] of Array.from(this.cleanupTargets.imageCache)) {
        ...
        for (const [key, item] of Array.from(this.cleanupTargets.audioCache)) {
            if (now - item.accessed > this.limits.maxCacheAge) {

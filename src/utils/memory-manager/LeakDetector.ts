...
interface ObjectCreationPattern {
    count: number;
    totalSize: number;
    averageSize: number;
    creationRate: number;
...
export class LeakDetector {
    ...
    performLeakDetection(): DetectionResults {
        if (!this.enabled) {
            return { detected: false, reason: 'Detection disabled' } as DetectionResults;
        }
        
        const now = Date.now();
        if (now - this.lastAnalysis < this.analysisWindow) {
            return { detected: false, reason: 'Analysis too frequent' } as DetectionResults;
        }

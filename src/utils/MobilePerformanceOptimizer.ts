...
declare global {
    interface Navigator {
        deviceMemory?: number;
        // hardwareConcurrency is already defined in lib.dom.d.ts and is readonly
    }
}
...
export class MobilePerformanceOptimizer {
    ...
    private detectGPUCapabilities(): void {
        try {
            ...
            if (gl) {
                ...
                hardware.gpu.performance = this.estimateGPUPerformance(hardware.gpu.renderer);
            }
        } catch (error) {
            console.warn('[MobilePerformanceOptimizer] Failed to detect GPU capabilities:', error);
        }
    }
        ...
        return new Promise((resolve) => {
            ...
            const benchmark = () => {
                ...
                if (elapsed < maxTime) {
                    ...
                    const score = iterations / maxTime; // iterations per ms
    ...
    private setupComponentIntegration(): void {
        ...
        if (this.memoryManager && this.resourceManager) {
            this.memoryManager.addMemoryPressureCallback((pressure) => {
                this.resourceManager!.handleMemoryPressureChange('system', pressure);
            });
        }
    ...
    private updatePerformanceMetrics(): void {
        ...
        // Limit history size
        if (this.monitoring.performanceHistory.length > 100) {
            this.monitoring.performanceHistory.shift();
        }
    }
    ...
    private evaluatePerformance(): void {
        ...
        if (performanceIssues.length > 0) {
            console.warn(`[MobilePerformanceOptimizer] Performance issues detected:`, performanceIssues);
    ...
    private applyAdaptiveOptimizations(): void {
        ...
        if (metrics.fps < this.config.targets.targetFPS && 
            ...
            if (this.memoryManager) {
                this.memoryManager.performStandardCleanup();
            }
    ...
    getPerformanceStatistics(): PerformanceStatistics {
        return {
            ...
            components: {
                resource: this.resourceManager?.getResourceStatistics(),
                render: this.renderOptimizer?.getRenderStatistics(),
                battery: this.batteryOptimizer?.getBatteryStatistics(),

...
export class FrameStabilizer {
    ...
    constructor(targetFPS: number = 60) {
        ...
        this.adaptiveTargeting = {
            baseTargetFPS: targetFPS,
        ...
        this.stabilizationThresholds = {
            excellentStability: 2.0, // variance < 2ms
    ...
    private updateStatisticalAnalysis(): void {
        ...
        const data = this.frameTimingAnalysis.bufferFull ? 
        ...
        this.frameTimingAnalysis.median = dataSize % 2 === 0 ? 
    ...
    private predictNextFrameTime(): void {
        ...
        for (let i = 0; i < Math.min(5, recentFrames.length); i++) {
            const frame = recentFrames[recentFrames.length - 1 - i];
        ...
        if (this.framePacing.predictionHistory.length > 20) {
            this.framePacing.predictionHistory.shift();
        }
     ...
     */
    private detectDisplaySynchronization(): void {
     ...
     * @param frameTimes - Array of frame times
     * @returns Common intervals
     */
    private detectCommonIntervals(frameTimes: number[]): number[] {
        const intervals: Record<string, number> = {};
        
        for (const time of frameTimes) {
            const rounded = Math.round(time * 2) / 2; // Round to 0.5ms
            intervals[rounded.toString()] = (intervals[rounded.toString()] || 0) + 1;
        }
     ...
     */
    private calculateTearingRisk(): void {
     ...
     */
    private updateAdaptiveTargeting(): void {
        const now = Date.now();
        ...
        if (Math.abs(newTargetFPS - this.adaptiveTargeting.currentTargetFPS) > 2) {
     ...
     */
    private updatePerformanceZone(stabilityScore: number, currentFPS: number): void {
     ...
     * @returns New target FPS
     */
    private calculateAdaptiveTarget(): number {
    ...
    /**
     * Adjust target FPS
     * @param newTargetFPS - New target FPS
     */
    private adjustTargetFPS(newTargetFPS: number): void {
        ...
        if (this.adaptiveTargeting.adjustmentHistory.length > 20) {
            this.adaptiveTargeting.adjustmentHistory.shift();
        }
     ...
     */
    private evaluateQualityAdjustments(): void {
        const zone = this.adaptiveTargeting.performanceZone;
    ...
    getStabilizationRecommendations(): Recommendations {
        const zone = this.adaptiveTargeting.performanceZone;
        const analysis = this.frameTimingAnalysis;
    ...
    /**
     * Force stabilization to a specific target FPS

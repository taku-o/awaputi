...
interface RenderMetricsData { statistics?: { totalRenders: number;
        paintEvents: number;
...
interface NetworkMetricsData { summary?: { totalRequests: number;
...
interface InteractionMetricsData { summary?: { totalInteractions: number;
...
interface CustomMetricsData { [metricName: string]: { current: number;
        count: number;
        recent: CustomMetric[]; }
    }
...
';
    ...
    private analyzeMemoryMetrics(memoryMetrics?: MemoryMetricsData): MemoryAnalysis { ' }'
        ...
        return { currentUsage: current.used || 0,
...
';
    ...
    private analyzeCustomMetrics(customMetrics?: CustomMetricsData): CustomAnalysis { ''
        ...
        for(const [name, metric] of Object.entries(customMetrics) {
            analysis[name] = { current: metric.current;
...
';
    ...
    // Helper methods
    private calculateFrameScore(frameMetrics?: FrameMetricsData): number { if (!frameMetrics? .average?.fps) return 0;
        ...
        return 0.2; }
 : undefined
    private calculateMemoryScore(memoryMetrics?: MemoryMetricsData): number { if (!memoryMetrics? .current?.pressure) return 1.0;
        const pressure = memoryMetrics.current.pressure;
        return Math.max(0, 1 - pressure); }
 : undefined
    private calculateRenderScore(renderMetrics?: RenderMetricsData): number { if (!renderMetrics? .statistics?.averageDuration) return 1.0;
        ...
        return 0.4; }
 : undefined
    private calculateNetworkScore(networkMetrics?: NetworkMetricsData): number { if (!networkMetrics? .summary?.averageDuration) return 1.0;
        const avgDuration = networkMetrics.summary.averageDuration;
        return Math.max(0, 1 - (avgDuration / 2000); // Normalize to 2s max }
 : undefined
    private calculateInteractionScore(interactionMetrics?: InteractionMetricsData): number { if (!interactionMetrics? .summary?.averageResponseTime) return 1.0;
...
';
    ...
    private assessResourceHealth(current: any, trends: any): ResourceHealthAssessment { const issues: string[] = [],
...
';
    ...
    getAnalysisHistory(): PerformanceAnalysis[] { return [...this.analysisHistory];

    getLatestAnalysis(): PerformanceAnalysis | null { return this.analysisHistory[this.analysisHistory.length - 1] || null;
...
')';
    ...
    private generateTextReport(data: ReportData, sections: string[]): string { let report = `Performance Analysis Report\n`; }
        ...
        if(sections.includes('memory) { report += this.generateMemorySection(data.memory); }'
    ...
    private generateHTMLReport(data: ReportData, sections: string[]): string { let html = `<!DOCTYPE html>
<html>;
<head>;
...
`,
    ...
    private generateBottlenecksSection(bottlenecks?: PerformanceBottleneck[]): string { if (!bottlenecks || bottlenecks.length === 0) {
            return `=== BOTTLENECKS ===;
...
`;
    ...
    private generateRecommendationsSection(recommendations?: PerformanceRecommendation[]): string { if (!recommendations || recommendations.length === 0) {
            return `=== RECOMMENDATIONS ===;
...
";"
    ...
    addCustomTemplate(name: string, template: ReportTemplate): void { this.reportTemplates.set(name, template); }
}

...
export class ImportValidator {
     ...
     * ファイルのインポート構文を包括的に検証
     */''
    async validateFile(filePath: string): Promise<ValidationResult> { try {'
     ...
     * ファイル内容のインポート構文を検証'
     */''
    validateContent(content: string, filePath: string | null = null): ValidationResult { const result: ValidationResult = {
            filePath,
            isValid: true,
            errors: [],
            warnings: [],
...
";"
        // 相対パスの検証""
        if (importPath.startsWith('./') || importPath.startsWith('../) { if (currentFilePath) {'
...
';'
     ...
     * 括弧の整合性をチェック
     */ : undefined
    checkBraces(line: string): BraceIssue[] { const issues: BraceIssue[] = [],
        let braceCount = 0,""
     ...
     * 引用符の一貫性をチェック'
     */''
    checkQuoteConsistency(line: string): QuoteIssue[] { const issues: QuoteIssue[] = [],
     ...
     * インポートの順序を分析'
     */''
    analyzeImportOrder(content: string): ImportOrderAnalysis { const imports: ImportInfo[] = [],''
        const lines = content.split('\n),'
        ...
        for (const imp of imports) {
            ...
            if (imp.isRelative) {
                ...
                foundRelative = true; }
            } else if (imp.isNodeModule && foundRelative) { hasNodeModuleAfterRelative = true,
                break }
        }
     ...
     * 未使用インポートを検出（簡易版）"
     */""
    findUnusedImports(content: string): UnusedImport[] { const unused: UnusedImport[] = [],""
        const lines = content.split('\n),'
        ...
        for(let, i = 0, i < lines.length, i++) {
            ...
            if (this.isImportLine(line) {
                ...
                if (defaultMatch) { const name = defaultMatch[1],
    ...
    isNameUsedInContent(name: string, content: string, importLineIndex: number): boolean {,
        ...
        for(let, i = 0, i < lines.length, i++) {
            ...
            if (line.includes(name) {
                ...
                if (wordBoundaryRegex.test(line)} { return true,
     ...
     * 相対パスを解決
     */
    resolveRelativePath(relativePath: string, currentFilePath: string): string { const currentDir = path.dirname(currentFilePath);
     ...
     * 検証結果のサマリーを生成
     */
    generateValidationSummary(results: ValidationResult[]): ValidationSummary { const summary = {

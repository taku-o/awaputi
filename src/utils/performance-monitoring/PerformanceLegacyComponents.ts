...
interface PerformanceGathererConfig {
    collectors?: string[];
    interval?: number;
    enabledMetrics?: string[];
}

interface HistoryTrackerConfig {
...
export class PerformanceHistoryTracker {
    ...
    addDataPoint(timestamp: number, metrics: Map<string, number>): void {
    ...
    getHistory(metricId: string, timeRange: number): MetricValue[] {
        const dataPoints = this.history.get(metricId) || [];
        const now = Date.now();
...
export class PerformanceAlertManager {
    ...
    addAlert(metricId: string, threshold: number, condition: 'above' | 'below' | 'equal', callback?: (event: AlertEvent) => void): string {
        const alert: Alert = {
            metricId,
            threshold,
            condition,
    ...
    triggerAlert(alert: Alert, value: number): void {
    ...
    getAlertsInRange(startTime: number, endTime: number): AlertEvent[] {
...
export class MetricsRegistry {
    ...
    register(metricConfig: MetricConfig): string {
...
export class RealtimeMetricsStream {
    ...
    send(timestamp: number, metrics: Map<string, any>): void {
    ...
    subscribe(callback: (dataPoint: DataPoint) => void): () => void {

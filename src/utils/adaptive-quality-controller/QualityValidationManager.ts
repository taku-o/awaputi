...
export class QualityValidationManager {
    ...
    evaluateValidationResults(): EvaluationResult {
        ...
        if (samples.length === 0) {
            return {
                ...
                reason: 'no_samples',
                metrics: {
                    average: { fps: 0, frameTime: 0, memoryUsage: 0, renderTime: 0, updateTime: 0, droppedFrames: 0 },
                    baseline: { fps: 0, frameTime: 0, memoryUsage: 0, renderTime: 0, updateTime: 0, droppedFrames: 0 },
                    comparison: { fpsChange: 0, frameTimeChange: 0, memoryChange: 0, renderTimeChange: 0, updateTimeChange: 0, droppedFramesIncrease: 0 },
                    stability: { fpsVariance: 0, frameTimeVariance: 0, fpsStability: 0, frameTimeStability: 0, overallStability: 0 }
                },
        ...
        return {
            ...
            metrics: {
                average: avgMetrics,
                baseline: this.baselineMetrics || { fps: 0, frameTime: 0, memoryUsage: 0, renderTime: 0, updateTime: 0, droppedFrames: 0 },
    ...
    calculateAverageMetrics(samples: ValidationSample[]): AverageMetrics {
        ...
        return {
            fps: this.calculateAverage(metrics.map(m => m.fps)),
    ...
    compareWithBaseline(avgMetrics: AverageMetrics): ComparisonResult {
        ...
        return {
            fpsChange: baseline.fps !== 0 ? (avgMetrics.fps - baseline.fps) / baseline.fps : 0,
    ...
    evaluateStability(samples: ValidationSample[]): StabilityEvaluation {
        ...
        return {
            fpsVariance: this.calculateVariance(fpsValues),
    ...
    makeOverallJudgment(comparison: ComparisonResult, stability: StabilityEvaluation): boolean {
        ...
        // メモリ使用量の増加をチェック
        if (comparison.memoryChange > this.validationConfig.maxMemoryIncrease) {
            return false;
        }
        ...
        // ドロップフレームをチェック
        if (comparison.droppedFramesIncrease > 10) {
            return false;
        }
    ...
    mockGetCurrentFPS(): number {
        return 60 + (Math.random() - 0.5) * 10;
    }
    
    mockGetCurrentFrameTime(): number {
        return 16.67 + (Math.random() - 0.5) * 5;
    }
    
    mockGetCurrentMemoryUsage(): number {
        return 50 + Math.random() * 20;
    }
    
    mockGetCurrentRenderTime(): number {
        return 8 + Math.random() * 4;
    }
    
    mockGetCurrentUpdateTime(): number {
        return 4 + Math.random() * 2;
    }
    
    mockGetDroppedFrames(): number {

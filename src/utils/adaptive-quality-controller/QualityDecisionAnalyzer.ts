...
export class QualityDecisionAnalyzer {
    ...
    evaluateQualityAdjustment(performanceMetrics: PerformanceMetrics, currentQuality: CurrentQuality): EvaluationResult {
        try {
            ...
            return {
                needsAdjustment: decision.needsAdjustment,
            ...
            console.error('[QualityDecisionAnalyzer] Error in evaluation:', error);
            return {
    ...
    analyzeQualityDecision(performanceScore: number, trendAnalysis: TrendAnalysis, currentQuality: CurrentQuality): QualityDecision {
        ...
        } else if (performanceScore < this.analysisConfig.performanceThresholds.fair) {
            // 公正以下のパフォーマンス：品質低下を検討
            if (currentIndex > 0) {
                recommendedLevel = this.getQualityLevelByIndex(currentIndex - 1);
                needsAdjustment = true;
        ...
        } else if (trendAnalysis.trend === 'improving' && trendAnalysis.stability >= 5) {
            if (currentIndex < 4 && performanceScore >= this.analysisConfig.performanceThresholds.good) {
                recommendedLevel = this.getQualityLevelByIndex(currentIndex + 1);
                needsAdjustment = true;
    ...
    updatePerformanceAverages(metrics: PerformanceMetrics): void {
        ...
        if (this.performanceAverages.frameTime.length > sampleSize) {
            this.performanceAverages.frameTime.shift();
        }
        ...
        if (metrics.droppedFrames) {
            this.performanceAverages.dropFrameCount += metrics.droppedFrames;
        }
    }
    ...
    updateTrendHistory(trendCategory: string, stability: number): void {
        ...
        // 履歴サイズの制限
        if (this.trendHistory.length > 50) {

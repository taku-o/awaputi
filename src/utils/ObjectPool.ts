...
export class ObjectPool<T = any> {
    private createFunction: CreateFunction<T>;
 ...
 * 泡用オブジェクトプール
 */
export class BubblePool extends ObjectPool<BubblePoolObject> {
 ...
 */
export class FloatingTextPool extends ObjectPool<FloatingText> {
...
export class PoolManager {
    ...
    constructor() {
        this.pools = new Map<string, ObjectPool<any>>();
     ...
     */
    getPool(poolName: string): ObjectPool<any> | undefined {
     ...
     */
    addPool(poolName: string, pool: ObjectPool<any>): void {
     ...
     */
    return(poolName: string, obj: any): void {
        const pool = this.pools.get(poolName);
        if (pool) {
            pool.return(obj);
        }

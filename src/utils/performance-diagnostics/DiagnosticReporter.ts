...
export class DiagnosticReporter {
     ...
     */
    async generateTechnicalReport(diagnosticSession: DiagnosticSession): Promise<Report> {
     ...
     */
    formatReportAsText(report: Report): string {
        const lines: string[] = [];
     ...
     */
    getReportingCapabilities(): ReportingCapabilities {
        return {
     ...
     */
    configure(config: ReportingConfig): void {
...
class RecommendationEngine {
    ...
    async generate(analysisResults: DiagnosticResults): Promise<Recommendation[]> {
    ...
    async generateBottleneckRecommendations(bottleneck: Bottleneck): Promise<Recommendation[]> {
        const recommendations: Recommendation[] = [];
    ...
    async generateAnomalyRecommendations(anomaly: Anomaly): Promise<Recommendation[]> {
        return [{
            type: 'investigation',
            priority: 'medium',
    ...
    prioritizeAndDeduplicateRecommendations(recommendations: Recommendation[]): Recommendation[] {
        // 重複除去
...
class DiagnosticReportGenerator {
    ...
    setTemplate(templateName: string, template?: ReportTemplate): void {
        if (template) {

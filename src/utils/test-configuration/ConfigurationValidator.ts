...
interface GameBalance { baseScores?: Record<string, number>,
    bubbles?: Record<string, any> }
interface Metadata { extractedAt?: number,
    sourceFiles?: string[];
...
export class ConfigurationValidator extends BaseComponent { private validationRules: Map<string, ValidationRule>,
    ...
    private validationCategories: { critical: string[]  ,
        important: string[],
    optional: string[],
    optional: string[];
        };
...
';'
     ...
     * @returns 検証結果
     */)
    validateConfigurationSync(expectations: Expectations): ConfigurationValidation { try {
...
' }'
            ...
            if (missingInGameBalance.length === 0 && missingInBaseScores.length === 0) { validation.categories.important.passed++ }
        }
     ...
     * @returns カテゴリ名
     */
    private getCategoryForProperty(property: string): string { for(const [category, properties] of Object.entries(this.validationCategories) {''
     ...
     * @param source - マージ元
     */
    private mergeValidationResults(target: BaseValidation, source: BaseValidation): void { if (!source.valid) {
            target.valid = false }
     ...
     * @returns 検証統計
     */
    getValidationStatistics(): ValidationStatistics { const recent = this.validationHistory.slice(-10);

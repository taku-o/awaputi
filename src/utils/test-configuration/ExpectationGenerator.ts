...
${Object.entries(expectations.bubbleTypes || {}).map(([bubbleType, config]) => `        test('should create ${bubbleType} bubble with expected configuration', () => {
    ...
    describe('Expected Configuration Values', () => {
        ...
        test('should have all expected bubble types', () => {
            const bubbleTypes = Object.keys(expectedConfigurations);
        ...
        Object.entries(expectedConfigurations).forEach(([bubbleType, expectedConfig]) => {
            test(\`should have correct configuration for \${bubbleType} bubble\`, () => {
                const bubble = new Bubble(mockGameEngine, 0, 0, bubbleType);
                ...
                if (expectedConfig.score !== undefined) {
                    expect(actualConfig.score).toBe(expectedConfig.score);
                }
                if (expectedConfig.size !== undefined) {
                    expect(actualConfig.size).toBe(expectedConfig.size);
                }
...
describe('GameBalance Configuration Tests', () => {
    describe('Base Scores Configuration', () => {
        test('should have all expected base scores', () => {
            ...
            for (const [bubbleType, expectedScore] of Object.entries(expectedScores)) {
                expect(GameBalance.baseScores[bubbleType]).toBe(expectedScore);
            }
        });
        
        test('should have valid score values', () => {
            for (const [bubbleType, score] of Object.entries(GameBalance.baseScores)) {
    ...
    describe('Configuration Integrity', () => {
        ...
        test('should not have null or undefined critical values', () => {

...
interface GameBalanceExpectations {
...
interface AllConfigurations {
    bubbleTypes: Record<string, BubbleConfig>;
 ...
 */
export class ConfigurationParser extends BaseComponent {
    ...
    setupParsingStrategies(): void {
        this.parsingStrategies = {
            ...
            configurationManager: {
                parser: this.parseConfigurationManager.bind(this)
            }
        };
    }
     ...
     */
    parseGameBalanceFile(filePath: string): GameBalanceExpectations | null {
        try {
            ...
            // baseScores の抽出
            const baseScoresMatch = content.match(/baseScores:\s*\{([^}]+)\}/);
            if (baseScoresMatch) {
                const scoresContent = baseScoresMatch[1];
                expectations.baseScores = {};
                const scoreRegex = /(\w+):\s*(\d+)/g;
                let scoreMatch;
                while ((scoreMatch = scoreRegex.exec(scoresContent)) !== null) {
                    const [, bubbleType, score] = scoreMatch;
            ...
            if (bubblesMatch) {
                ...
                // 各バブルタイプの設定を抽出
                const bubbleTypeRegex = /(\w+):\s*\{([^}]*)\}/g;
                let bubbleTypeMatch;
                
                while ((bubbleTypeMatch = bubbleTypeRegex.exec(bubblesContent)) !== null) {
                    const [, bubbleType, configContent] = bubbleTypeMatch;
                    ...
                    if (effectsMatch) {
                        ...
                        const effectProperties: Record<string, RegExp> = {
     ...
     * Bubble.jsファイルを解析
     */
    parseBubbleFile(filePath: string): Record<string, BubbleConfig> | null {
        try {
            if (!fs.existsSync(filePath)) {
            ...
            if (hardcodedConfigMatch) {
                ...
                while ((caseMatch = caseRegex.exec(configContent)) !== null) {
                    const [, bubbleType, configStr] = caseMatch;
     ...
     */
    parseConfigurationManager(): Record<string, BubbleConfig> | null {
        try {
            const expectations: Record<string, BubbleConfig> = {};
            const bubbleTypes = [
     ...
     * 設定を統合解析
     */
    parseAllConfigurations(): AllConfigurations | null {
        try {

...
interface ClassConflictInfo {
    name: string;
    occurrences: ClassInfo[];
    count: number;
}

interface FunctionConflictInfo {
    name: string;
...
export class NamingConflictDetector {
    ...
    constructor(config: DetectorConfig = {}) {
        ...
        this.config = {
            ...
            detectFileNames: config.detectFileNames ?? true,
            detectClassNames: config.detectClassNames ?? true,
            detectFunctionNames: config.detectFunctionNames ?? true,
            ...
            // 警告レベルの設定
            warningLevel: config.warningLevel || 'strict'
        };
    ...
    async scanProject(): Promise<ScanResult> {
        ...
        const scanResult: ScanResult = {
            files: new Map<string, string[]>(),      // ファイル名 -> [パス配列]
     ...
     * ディレクトリを再帰的にスキャン
     */
    private async scanDirectory(dirPath: string, scanResult: ScanResult): Promise<void> {
        try {
     ...
     * 単一ファイルをスキャン
     */
    private async scanFile(filePath: string, scanResult: ScanResult): Promise<void> {
        try {
     ...
     * 新しい名前が競合するかどうかをチェック
     */
    async checkNamingConflict(name: string, type: 'file' | 'class' | 'function', context: string = ''): Promise<ConflictCheckResult> {
        // キャッシュが古い場合は再スキャン
     ...
     * ファイル名の競合をチェック
     */
    private async checkFileNameConflict(fileName: string, filePath: string, result: ConflictCheckResult): Promise<ConflictCheckResult> {
     ...
     * クラス名の競合をチェック
     */
    private async checkClassNameConflict(className: string, filePath: string, result: ConflictCheckResult): Promise<ConflictCheckResult> {
     ...
     * 関数名の競合をチェック
     */
    private async checkFunctionNameConflict(functionName: string, filePath: string, result: ConflictCheckResult): Promise<ConflictCheckResult> {
     ...
     * ファイル名の代替案を生成
     */
    private generateFileNameSuggestions(originalName: string, filePath: string): string[] {
     ...
     * クラス名の代替案を生成
     */
    private generateClassNameSuggestions(originalName: string, filePath: string): string[] {
        const suggestions: string[] = [];
     ...
     * 関数名の代替案を生成
     */
    private generateFunctionNameSuggestions(originalName: string, filePath: string): string[] {
        const suggestions: string[] = [];
        const dirName = path.basename(path.dirname(filePath));
     ...
     * 競合を分析
     */
    private analyzeConflicts(scanResult: ScanResult): void {
        // ファイルの競合
     ...
     * クラスからクラス名を抽出
     */
    private extractClasses(content: string): ExtractedClass[] {
     ...
     * 改善提案を生成
     */
    private generateRecommendations(scanResult: ScanResult): Recommendation[] {

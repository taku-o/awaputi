...
export interface SafetyResults {
    results: SafetyReportResult[];
    totalFiles: number;
    safeToDelete: number;
    unsafeToDelete: number;
    totalWarnings: number;
    totalErrors: number;
    safeFiles: SafetyReportResult[];
    unsafeFiles: SafetyReportResult[];
}
...
export class SafetyValidator {
    ...
    async validateCurrentFileExists(oldFilePath: string): Promise<CurrentFileCheck> {
        const currentFilePath = this.getCurrentFilePath(oldFilePath);
        try {
    ...
    private isActiveStringReference(context: string): boolean {
        const inactivePatterns = [
            /\/\/.*/, // Comments
            /\/\*[\s\S]*?\*\//, // Block comments
            /console\.(log|debug|info|warn|error)/, // Console logs
    ...
    async validateFileSize(filePath: string): Promise<FileSizeValidation> {
        try {
    ...
    async generateSafetyReport(filePath: string, referenceResult: ReferenceResult): Promise<SafetyReportResult> {
        const currentFileCheck = await this.validateCurrentFileExists(filePath);
    ...
    async validateBatch(files: string[], referenceResults: ReferenceResult[]): Promise<SafetyResults> {
        const results: SafetyReportResult[] = [];
        ...
        return {
            ...
            safeFiles: results.filter(r => r.isSafeToDelete),
            unsafeFiles: results.filter(r => !r.isSafeToDelete)
        };
    }
}
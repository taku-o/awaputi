...
describe('GameEndSharing', () => {
    ...
    beforeEach(async () => {
        ...
        mockSocialSharingManager = {
            ...
            onHighScore: jest.fn<() => Promise<{ success: boolean; method: string }>>().mockResolvedValue({ 
                success: true, 
                method: 'web-share'
            }),
            onGameEnd: jest.fn<() => Promise<{ success: boolean; method: string }>>().mockResolvedValue({ 
                success: true, 
                method: 'web-share'
            })
        };
        ...
        mockGameEngine = {
            ...
            stageManager: {
                getCurrentStage: jest.fn().mockReturnValue({
                    id: 'normal',
                    type: 'normal'
                })
            },
            statisticsManager: {
                getAccuracy: jest.fn().mockReturnValue(85.5),
    ...
    describe('prepareGameEndData', () => {
        ...
        test('StatisticsManagerが無い場合のフォールバック', () => {
            mockGameEngine.statisticsManager = null;
    ...
    describe('showHighScoreSharePrompt', () => {
        test('ハイスコア共有プロンプトが正しく表示される', async () => {
            ...
            expect(mockSocialSharingManager.onHighScore).toHaveBeenCalledWith({
                ...gameEndData,
                title: '新記録達成！ BubblePop',
                text: '🎉 新記録達成！5,000点を記録しました！ステージ: boss',
                hashtags: ['BubblePop', 'NewRecord', 'HighScore', 'Gaming'],
    ...
    describe('showGameEndSharePrompt', () => {
        test('ゲーム終了時共有プロンプトが正しく表示される', async () => {
            ...
            expect(mockSocialSharingManager.onGameEnd).toHaveBeenCalledWith({
                ...gameEndData,
                title: 'BubblePop - 2,500点獲得',
                text: '🎮 BubblePopで2,500点を獲得！ 精度85%の素晴らしいプレイ！ 最大コンボ12連鎖達成！',
                hashtags: ['BubblePop', 'Gaming', 'Score'],
    ...
    describe('getGameTime', () => {
        ...
        test('GameEngineにgetGameTimeがない場合は残り時間から逆算', () => {
            delete mockGameEngine.getGameTime;

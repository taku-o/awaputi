...
export class MobileTestSuite {
     ...
     */
    async stopDeviceSimulation(): Promise<void> {
    ...
    setDevice(deviceName: string): void {
        return this.deviceSimulator.switchDevice(deviceName);
    }
     ...
     * テストエラー記録
     */
    recordTestError(suiteName: string, testName: string, error: Error | string): void {
     ...
     * テスト失敗記録
     */
    recordTestFailure(suiteName: string, testName: string, error: Error | string): void {
     ...
     */
    getDeviceUserAgent(device: string): string {
        return (this.deviceSimulator as any).getDeviceInfo(device).userAgent;
    }
    
    /**
     * デバイス画面サイズ取得（MobileDeviceSimulatorに委譲）
     */
    getDeviceScreen(device: string): any {
        return (this.deviceSimulator as any).getDeviceInfo(device).screen;
    }
    
    /**
     * デバイスピクセル比取得（MobileDeviceSimulatorに委譲）
     */
    getDevicePixelRatio(device: string): number {
        return (this.deviceSimulator as any).getDeviceInfo(device).pixelRatio;
    }
    ...
    getExecutionState(): any {
        return (this.testRunner as any).getExecutionState();
    }
     ...
     */
    updateExecutionConfig(newConfig: Record<string, any>): void {
        return (this.testRunner as any).updateConfig(newConfig);
    }
    
    /**
     * レポート設定更新（MobileTestReporterに委譲）
     */
    updateReportConfig(newConfig: Record<string, any>): void {
    ...
    getDebugInfo(): DebugInfo {
        return {
            mainController: {
                ...
                testRunner: (this.testRunner as any).getDebugInfo(),
 ...
 */
export function getMobileTestSuite(): MobileTestSuite {

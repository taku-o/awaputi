...
export class MobileTestRunner {
        ...
        while (retries > 0) {
            try {
                ...
                if (result.passed) {
                    ...
                    this.mobileTestSuite.recordTestFailure(suiteName, test.name, result.error || 'Test failed');
                }
    ...
    private async preTestSetup(test: TestFunction, context: TestContext): Promise<void> {
        ...
        if (this.mobileTestSuite.deviceSimulator) {
            // resetDeviceState method might not exist, so we'll skip this for now
    ...
    private async errorTestCleanup(test: TestFunction, context: TestContext, error: Error): Promise<void> {
        try {
            ...
            // デバイス状態の強制リセット
            if (this.mobileTestSuite.deviceSimulator) {
                // forceResetDevice method might not exist, so we'll skip this for now

...
export class BubblePhysicsEngine implements IBubblePhysicsEngine { public gameEngine: any;
     ...
     * 境界との衝突判定
     */
    handleBoundaryCollision(bubble: Bubble): void { const canvas = this.gameEngine.canvas,
     ...
     * 泡に力を適用
     */
    applyForceToBubble(bubble: Bubble | any, direction: Vector2, strength: number): void { if (!bubble || !bubble.isAlive) return,
     ...
     */
    isBubbleVisible(bubble: Bubble): boolean { const margin = 100,
        return (
     ...
     * 画面外の泡の処理
     */
    handleOffscreenBubble(bubble: Bubble, deltaTime: number, offscreenBubbles: Set<Bubble>, offscreenTimer: Map<Bubble, number>): boolean { const canvas = this.gameEngine.canvas,
     ...
     * 点と線分の最短距離を計算
     */
    pointToLineDistance(point: Position, lineStart: Position, lineEnd: Position): number { const A = point.x - lineStart.x,

...
';
    ...
    showError(message: string, duration: number = 5000): void { this.errorMessage = message;''
        ...
        this.errorTimeout = setTimeout(() => { this.clearError(); }, duration);
    }
';
...
')';
    startOperation(type: string, message: string = '', timeout: number = 300000): void { // 5 minutes default
        ...
        this.notifyOperationChange('start', { type)
            message,)';
...
';
    ...
    endOperation(result: string = 'success', message: string = '', data: any = { ): void {
        ...
        if(this.operationTimeout) {
            clearTimeout(this.operationTimeout, this.operationTimeout = null; }
        }

        // Add, to history, if(this.operationType && this.operationStartTime) {
    ...
    getOperationType(): string | null { return this.operationType; }

    getOperationProgress(): number { return this.operationProgress; }
    ...
    getOperationHistory(): OperationHistoryEntry[] { return [...this.operationHistory];
...
export class BackupStatusManager {
    ...
    async loadBackupStatus(): Promise<void> { try {
...
;
    ...
    updateBackupStatus(updates: Partial<BackupStatus>): void { const previous = { ...this.backupStatus;
    ...
    getBackupCount(): number { return this.backupStatus.backupCount; }

    getTotalBackupSize(): number { return this.backupStatus.totalSize; }

    isAutoBackupEnabled(): boolean { return this.backupStatus.autoBackupEnabled; }

    getNextBackupTime(): string | number | Date | null { return this.backupStatus.nextBackup; }

    getBackupHistory(): BackupHistoryEntry[] { return [...(this.backupStatus.backupHistory || [])]; }

    addBackupToHistory(backupInfo: Partial<BackupHistoryEntry>): void { if (!this.backupStatus.backupHistory) {
...
';
    ...
    private notifyUpdate(action: string, data: any): void { this.updateCallbacks.forEach(callback => { )
            ...
            } catch (error) { console.error('Backup status update callback error:', error }
        });
    }
';

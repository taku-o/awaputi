    ...
    renderDialog(bounds, data) {
        ...
        formats.forEach((format, index) => {
            ...
            this.renderer.drawText(`• ${format}`, 
                contentBounds.contentX + 10, itemY, {
                    fontSize: 12,
        ...
        this.renderer.drawText('Include:', 
            contentBounds.contentX, contentBounds.contentY + 160, {
                fontSize: 14,
                bold: true
            });
        ...
        options.forEach((option, index) => {
            const itemY = contentBounds.contentY + 190 + index * 25;
    ...
    renderDialog(bounds, data) {
        ...
        if (data.selectedFile) {
            this.renderer.drawText(`Selected: ${data.selectedFile.name}`, 
                contentBounds.contentX, contentBounds.contentY + 100, {
                    fontSize: 12,
                    color: colors.text
                });
    ...
    renderDialog(bounds, data) {
        ...
        dataTypes.forEach((type, index) => {
            this.renderer.drawText(`• ${type}`, 
                contentBounds.contentX + 10, contentBounds.contentY + 150 + index * 20, {
                    fontSize: 11,
                    color: colors.textSecondary
                });
        });
        ...
        this.renderer.drawText(`Type "${requiredText}" to confirm:`, 
            contentBounds.contentX, contentBounds.contentY + 240, {
                fontSize: 12,
                color: colors.text
            });
    ...
    renderDialog(bounds, data) {
        ...
        if (data.startTime) {
            ...
            this.renderer.drawText(elapsedText, 
                contentBounds.contentX, contentBounds.contentY + 110, {
                    fontSize: 11,
                    color: colors.textSecondary
                });
        }
    ...
    renderDialog(bounds, data) {
        ...
        lines.forEach((line, index) => {
            this.renderer.drawText(line, 
                contentBounds.contentX, contentBounds.contentY + 20 + index * 20, {
                    fontSize: 14,
                    color: colors.text
                });
        });

...
interface Themes { light: ThemeColors,
    ...
    async showShareDialog(shareData: ShareData): Promise<ShareResult> { try {
            ...
            if (isMobile) { return await this.showMobileBottomSheet(shareData) } else { return await this.showTabletDialog(shareData),' }'
...
","
    ...
    private generatePreviewMessage(shareData: ShareData): string { ""
        if (shareData.type === 'score'', ' }
            ...
            return `BubblePopで実績「${shareData.achievement?.name}」を解除！ #BubblePop`;} else if(shareData.type === 'challenge' {'
...
','
                ...
                container.dispatchEvent(new CustomEvent('share-complete', { detail: result ) } catch (error) {,

...
export class LeaderboardDataManager {
    ...
    private refreshInterval: number = 300000; // 5分
    ...
    async getData(viewType: string, options: FetchOptions = {}): Promise<RankingData> {
        try {
            ...
            return {
                error: 'データの取得に失敗しました',
                rankings: [],
                metadata: { total: 0, lastUpdate: Date.now() }
            };
        }
    }

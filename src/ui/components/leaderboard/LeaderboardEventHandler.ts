...
interface EntrySelectEventData {
    entry?: any;
    index: number;
}

interface DetailsToggleEventData {
    entry?: any;
    show: boolean;
}
...
export class LeaderboardEventHandler {
     ...
     * @param {Object} options - オプション
     */
    handleSingleClick(x: number, y: number, target: EventTarget | null, options: any): void {
        if (!target) {
            // 背景クリック - 詳細を閉じる
     ...
     * @param {Object} options - オプション
     */
    startHover(target: EventTarget, options: any): void {
        // ホバー遅延
        if (this.eventState.hoverTimer) {
            clearTimeout(this.eventState.hoverTimer);
        }
        
        this.eventState.hoverTimer = window.setTimeout(() => {
            // コールバック実行
     ...
     * @param {Object} target - ターゲット
     */
    handleHoverEnd(target: EventTarget): void {
        if (this.eventState.hoverTimer) {
            clearTimeout(this.eventState.hoverTimer);
     ...
     */
    detectGesture(deltaX: number, deltaY: number, duration: number): void {
     ...
     * @param {Object} target - ターゲット
     */
    handleEntryClick(target: EventTarget): void {
     ...
     */
    getTargetAt(x: number, y: number): EventTarget | null {
        // タブチェック
     ...
     */
    updateUIElements(elements: Partial<UIElements>): void {
    ...
    addCallback<K extends keyof typeof this.callbacks>(
        ...
        const callbackSet = this.callbacks[eventType];
        if (callbackSet) {
     ...
     */
    setSelectionState(newState: Partial<SelectionState>): void {

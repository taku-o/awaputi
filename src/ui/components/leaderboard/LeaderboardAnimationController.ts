 ...
 */
interface SelectAnimationOptions {
    duration?: number;
    intensity?: number;
    type?: string;
}
 ...
 */
interface PerformanceInfo extends PerformanceMetrics {
    activeAnimations: {
        entries: number;
        hovers: number;
        selects: number;
...
export class LeaderboardAnimationController {
    ...
    updateScrollAnimation(): void {
        const current = this.animationState.scrollOffset;
        ...
        if (Math.abs(current - target) > 1) {
            ...
            this.animationState.scrollOffset = target;
     ...
     * フェードアニメーションを更新
     */
    updateFadeAnimation(): void {
        const current = this.animationState.fadeOpacity;
        const target = this.animationState.targetFadeOpacity;
        
        if (Math.abs(current - target) > 0.01) {
            const diff = target - current;
            this.animationState.fadeOpacity += diff * this.animationConfig.fadeSpeed;
        } else {
            this.animationState.fadeOpacity = target;
        }
    }
     ...
     * @param {number} currentTime - 現在時刻
     */
    updateEntryAnimations(currentTime: number): void {
        for (const [entryId, animation] of this.animationState.entryAnimations.entries()) {
     ...
     * @param {number} currentTime - 現在時刻
     */
    updateHoverAnimations(currentTime: number): void {
        for (const [elementId, animation] of this.animationState.hoverAnimations.entries()) {
            const elapsed = currentTime - animation.startTime;
            const progress = Math.min(elapsed / animation.duration, 1);
     ...
     * @param {number} currentTime - 現在時刻
     */
    updateSelectAnimations(currentTime: number): void {
        for (const [elementId, animation] of this.animationState.selectAnimations.entries()) {
            const elapsed = currentTime - animation.startTime;
            const progress = Math.min(elapsed / animation.duration, 1);
    ...
    scroll(deltaY: number, options: ScrollOptions = {}): void {
        try {
            const sensitivity = options.isTouchEvent ? 
                this.scrollConfig.touchSensitivity : 
                this.scrollConfig.wheelSensitivity;
            ...
            if (this.scrollConfig.smoothScrolling) {
                ...
                this.animationState.scrollOffset += scrollAmount;
            ...
            // スナップ処理
            if (this.scrollConfig.snapToEntry) {
                this.snapToNearestEntry();
            }
     ...
     * 最寄りのエントリーにスナップ
     */
    snapToNearestEntry(): void {
        const entryHeight = this.animationConfig.entryHeight;
    ...
    startSelectAnimation(target: string | { id: string }, options: SelectAnimationOptions = {}): void {
        try {
            ...
            this.interactionState.selectedEntry = target;
            this.interactionState.lastClickTime = currentTime;
            
            // 選択アニメーション開始
            this.animationState.selectAnimations.set(targetId, {
                startTime: currentTime,
                duration: options.duration || this.animationConfig.transitionDuration * 2,
                maxIntensity: options.intensity || 1.0,
                currentIntensity: 0,
     ...
     */
    startEntryAnimation(entryId: string, animationConfig: EntryAnimationConfig): void {
        try {
            const currentTime = Date.now();
     ...
     */
    setScrollBounds(maxScroll: number): void {
        this.scrollConfig.scrollBounds.max = Math.max(0, maxScroll);
    }
     ...
     */
    setScrollPosition(position: number, animated: boolean = true): void {
        ...
        if (animated) {
            ...
            this.animationState.scrollOffset = clampedPosition;
            this.animationState.targetScrollOffset = clampedPosition;
        }
    }
    ...
    getScrollPosition(): number {
        return this.animationState.scrollOffset;
    }
     ...
     */
    getHoverState(targetId: string): HoverAnimation | null {
        return this.animationState.hoverAnimations.get(targetId) || null;
    }
     ...
     */
    getSelectState(targetId: string): SelectAnimation | null {
        return this.animationState.selectAnimations.get(targetId) || null;
    }
    ...
    hasActiveAnimations(): boolean {
        return this.animationState.isScrolling ||
     ...
     * @param {number} currentTime - 現在時刻
     */
    updatePerformanceMetrics(currentTime: number): void {
     ...
     */
    updateConfig(newConfig: UpdateConfig): void {
        if (newConfig.animationConfig) {
    ...
    clearAllAnimations(): void {
        ...
        this.animationState.fadeOpacity = 1.0;
        this.animationState.targetFadeOpacity = 1.0;

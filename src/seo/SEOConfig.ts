...
export function getSocialImageUrl(platform: SocialPlatform = 'openGraph', variant: ImageVariant = 'default'): string {
    const baseUrl = getBaseUrl();
    const config = SEOConfig.socialImages[platform];

    if (config && variant in config && config[variant as keyof typeof config]) {
        return `${baseUrl}${config[variant as keyof typeof config]}`;
    }

    return `${baseUrl}${SEOConfig.socialImages.fallback}`;
}
...
export interface DailyChallengeWithProgress extends DailyChallengeData {
...
export class DailyChallengeManager {
     ...
     * 今日のチャレンジをチェック・生成
     */
    async checkAndGenerateTodaysChallenges(): Promise<void> {
     ...
     * 今日のチャレンジを生成
     */
    async generateTodaysChallenges(): Promise<void> {
        try {
     ...
     * 最適な難易度を計算
     */
    calculateOptimalDifficulty(): DifficultyLevel {
     ...
     * 目標値を計算
     */
    calculateTargetValue(template: ChallengeTemplate, difficulty: DifficultyLevel): number {
     ...
     * 報酬を選択
     */
    selectReward(difficulty: DifficultyLevel): ChallengeReward {
     ...
     * ランダムテンプレート選択
     */
    selectRandomTemplates(templates: string[], count: number): string[] {
     ...
     * 今日のチャレンジ取得
     */
    getTodaysChallenges(): DailyChallengeWithProgress[] {
     ...
     * プレイヤー実績データ読み込み
     */
    async loadPlayerPerformance(): Promise<void> {
        try {
     ...
     * 統計から実績データを更新
     */
    updatePerformanceFromStats(stats: DetailedStatistics): void {
        if (stats.gameStats) {
     ...
     * 生成データ保存
     */
    async saveGenerationData(): Promise<void> {
        try {
            const data = {
     ...
     * 手動チャレンジ再生成（デバッグ用）
     */
    async regenerateTodaysChallenges(): Promise<void> {
        await this.cleanupExpiredChallenges();

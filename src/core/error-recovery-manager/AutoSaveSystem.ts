...
export interface PlayerSaveStatistics {
...
export interface TransitionSaveData {
    from: string;
    to: string;
    progress: number;
    startTime: number;
...
export interface BubbleSaveEntry {
    id: string;
    type: BubbleType;
...
export interface PhysicsSaveData {
    gravity: number;
    friction: number;
    bounce: number;
    timeStep: number;
...
export interface BubbleSaveData {
    bubbles: BubbleSaveEntry[];
    grid: GridSaveData;
...
export interface AutoSaveSystemState {
    isRunning: boolean;
    lastSaveTime: number;
    saveCount: number;
    restoreCount: number;
...
export const SAVE_TYPE_PRIORITIES: Record<SaveType, number> = {
    emergency: 10,
    beforeCritical: 9,
    gameEnd: 8,
    beforeUnload: 7,
...
export const ERROR_RECOVERY_THRESHOLDS = {
...
export function calculateSavePointSize(savePoint: SavePoint): number {
    try {
...
export function validateSaveData(data: GameStateSaveData): ValidationResult {
...
export function compressSaveData(data: SavePoint): SavePoint {
    try {
...
export function decompressSaveData(data: SavePoint): SavePoint {
...
export function calculateChecksum(data: any): string {
    // 簡単なチェックサム計算（実際のプロジェクトではより堅牢なハッシュを使用）
...
export function formatSaveType(saveType: SaveType): string {
    const typeNames: Record<SaveType, string> = {
        initial: '初期保存',
        periodic: '定期保存',
        manual: '手動保存',
        final: '最終保存',
...
export function sortSavePointsByPriority(savePoints: SavePoint[]): SavePoint[] {
    return [...savePoints].sort((a, b) => {
...
export class AutoSaveSystem {
    ...
    performSave(saveType: SaveType = 'manual', options: SaveOptions = {}): boolean {
        ...
        try {
            ...
            if (this.config.compressionEnabled || options.compress) {
                finalSavePoint = compressSaveData(savePoint);
            }
    ...
    private cleanupOldSavePoints(): void {
        if (this.savePoints.length <= this.config.maxSavePoints) return;
     ...
     * ゲーム状態をキャプチャ
     */
    private captureGameState(): GameStateSaveData {
        const gameState: GameStateSaveData = {
            timestamp: Date.now(),
     ...
     * ゲーム状態が有効かチェック
     */
    private isValidGameState(gameState: GameStateSaveData): boolean {
        const validation = validateSaveData(gameState);
        return validation.valid;
    }
    ...
    private handleCriticalAction(action: any): void {
        // 重要なアクション前の保存
     ...
     * 保存点を削除
     */
    deleteSavePoint(savePointId: string): boolean {
     ...
     * 保存点の整合性をチェック
     */
    validateAllSavePoints(): { valid: number; invalid: number; repaired: number } {
        let validCount = 0;
        let invalidCount = 0;
        let repairedCount = 0;
     ...
     * ストレージ使用量を取得
     */
    getStorageUsage(): StorageQuotaInfo {
        try {
            let used = 0;
     ...
     * 統計情報を取得
     */
    getStatistics(): AutoSaveStatistics {
        const storageUsage = this.getStorageUsage();
        const averageSaveSize = this.savePoints.length > 0 ?
     ...
     * 設定を更新
     */
    updateSettings(newSettings: Partial<AutoSaveConfig>): void {

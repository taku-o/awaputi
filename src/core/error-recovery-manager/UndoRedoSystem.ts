...
export interface UndoRedoSystemState {
    isRecording: boolean;
...
export interface BubbleManagerInterface {
    exportState(): BubbleState;
    importState(state: BubbleState): void;
}

export interface ScoreManagerInterface {
    getState(): ScoreState;
...
export const DEFAULT_UNDO_REDO_CONFIG: UndoRedoConfig = {
    ...
    criticalActions: new Set(['reset', 'newGame', 'delete', 'clear', 'restart', 'quit'])
} as const;
...
export function isActionIgnored(actionType: string, ignoredActions: Set<string>): boolean {
    return ignoredActions.has(actionType);
}

export function isActionCritical(actionType: string, criticalActions: Set<string>): boolean {
    return criticalActions.has(actionType);
}

export function calculateStateSize(state: GameStateSnapshot): number {
    try {
...
export function formatActionDescription(action: ActionRecord): string {
    const timestamp = new Date(action.timestamp).toLocaleTimeString();
    return `${action.type} (${action.subtype}) at ${timestamp}`;
}

export function validateGameState(state: GameStateSnapshot): boolean {
    return state !== null && 
           typeof state === 'object' && 
...
export class UndoRedoSystem {
     ...
     */
    private applyUndoRedoStyles(): void {
        if (document.getElementById('undo-redo-styles')) return;
        
        const styles = `
            .${CSS_CLASSES.UNDO_REDO_BTN} {
                position: fixed;
                width: 45px;
                height: 45px;
                border-radius: 50%;
            ...
            .${CSS_CLASSES.UNDO_BTN} {
                bottom: 80px;
                right: 20px;
            }
            
            .${CSS_CLASSES.REDO_BTN} {
                bottom: 135px;
                right: 20px;
            }
            
            .${CSS_CLASSES.UNDO_REDO_BTN}:not(:disabled):hover {
                background: #6c757d;
                color: white;
                transform: scale(1.1);
            }
            
            .${CSS_CLASSES.UNDO_REDO_BTN}:disabled {
                opacity: 0.3;
                cursor: not-allowed;
            }
            
            .${CSS_CLASSES.UNDO_REDO_BTN}:focus {
     ...
     * ゲームアクションを処理
     */
    private handleGameAction(action: ActionData): void {
        if (!this.config.enabled || !this.state.isRecording) return;
     ...
     * 状態変更を処理
     */
    private handleStateChange(stateChange: StateChangeData): void {
        // 状態変更に基づくアクション記録
     ...
     * アクションを記録
     */
    recordAction(action: ActionData): void {
        try {
            ...
            console.error('[UndoRedoSystem] Action recording error:', error);
        }
    }
    ...
    private captureGameState(): GameStateSnapshot {
        ...
        try {
            ...
            console.warn('[UndoRedoSystem] State capture error:', error);
        }
     ...
     */
    undo(): boolean {
        if (!this.canUndo() || this.state.operationInProgress) {
        ...
        try {
            ...
            console.error('[UndoRedoSystem] Undo error:', error);
            this.state.isRecording = true;
            this.state.operationInProgress = false;
            this.showUndoFeedback({ 
     ...
     */
    redo(): boolean {
        if (!this.canRedo() || this.state.operationInProgress) {
        ...
        try {
            ...
            this.showUndoFeedback({ 
                message: FEEDBACK_MESSAGES.REDO_SUCCESS,
                type: 'success'
            });
            ...
            console.error('[UndoRedoSystem] Redo error:', error);
            this.state.isRecording = true;
            this.state.operationInProgress = false;
            this.showUndoFeedback({ 
    ...
    private updateButtons(): void {
        if (this.ui.undoButton) {
            ...
            this.ui.undoButton.title = this.canUndo() ? 
                KEYBOARD_SHORTCUTS.UNDO.description : 
                '元に戻せません';
        }
        ...
        if (this.ui.redoButton) {
            ...
            this.ui.redoButton.title = this.canRedo() ? 
                KEYBOARD_SHORTCUTS.REDO.description : 
                'やり直せません';
        }
    }
     ...
     * フィードバック表示
     */
    private showUndoFeedback(options: FeedbackOptions): void {
        const feedback = createFeedbackElement(options);
    ...
    getStatistics(): UndoRedoStatistics {
        const totalStateSize = this.actionHistory.reduce((sum, action) => {
            return sum + calculateStateSize(action.stateBefore) + 
                   (action.stateAfter ? calculateStateSize(action.stateAfter) : 0);
        }, 0);
        ...
        return {
            ...
            averageStateSize: this.actionHistory.length > 0 ? 
     ...
     * アクション履歴を取得
     */
    getActionHistory(): ReadonlyArray<ActionRecord> {
     ...
     * 特定のアクションを検索
     */
    findActionById(actionId: string): ActionRecord | null {
        return this.actionHistory.find(action => action.id === actionId) || null;
    }

    /**
     * アクションタイプでフィルター
     */
    getActionsByType(actionType: string): ActionRecord[] {
        return this.actionHistory.filter(action => action.type === actionType);
    }
    ...
    setRecording(enabled: boolean): void {
        this.state.isRecording = enabled;

...
export class GameEngineRenderer {
    ...
    render(): void {
        ...
        // レンダリング最適化開始
        if (this.gameEngine.renderOptimizer?.optimize) {
        ...
        // パフォーマンスモニタリング
        if (this.gameEngine.performanceMonitor?.recordRenderTime) {
            this.gameEngine.performanceMonitor.recordRenderTime();
        }
        // デバッグ情報の描画
        if (this.gameEngine.debugMode && this.gameEngine.debugInterface?.render) {
            this.gameEngine.debugInterface.render(this.gameEngine.context);
        }
    }
     ...
     */
    startScreenShake(duration: number, intensity: number): void {
        this.screenShake = {
            active: true,
            duration,
            intensity,
     ...
     * 画面揺れがアクティブかチェック
     */
    private isScreenShakeActive(): boolean {

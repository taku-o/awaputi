...
'';
     ...
     */''
    async testSupportedMimeTypes(capabilities') { const mimeTypesToTest = [''
            'text/plain','';
            'image/png','';
            'image/jpeg','';
            'image/gif','';
...
'';
        for(const mimeType of mimeTypesToTest') {'
            try {'
                ...
                } catch (error) { // MIMEタイプがサポートされていない }
            }
...
'';
    ...
    convertToWebShareData(shareData) {
        ...
        // ファイル共有（スクリーンショット等）
        if (shareData.files && this.shareCapabilities.fileShare) { webShareData.files = shareData.files; }
        } else if (shareData.screenshot && this.shareCapabilities.fileShare) { // スクリーンショットをファイルに変換

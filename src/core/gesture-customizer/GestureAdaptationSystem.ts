...
// 型定義
export interface GestureAdaptationConfig {
    oneHandedMode: boolean;
    touchSensitivity: number;
    alternativeGestures: AlternativeGesturesConfig;
}

export interface AlternativeGesturesConfig {
...
export interface UnrecognizedGestureRecord {
    timestamp: number;
...
export interface PreferencesSerialized {
    oneHandedMode: boolean;
    preferredHand: PreferredHand;
    gestureComplexity: GestureComplexity;
    touchSensitivity: number;
...
export function isValidGestureData(data: any): data is GestureData {
    return data && (
...
export class GestureAdaptationSystem {
    ...
    private saveUserPreferences(): void {
        try {
            ...
            localStorage.setItem('gestureCustomizer_preferences', JSON.stringify(preferences));
        } catch (error) {
     ...
     * 適応的閾値の設定
     */
    private setAdaptiveThresholds(): void {
     ...
     * 適応学習の更新
     */
    updateAdaptiveLearning(gestureName: string, gestureData: GestureData, success: boolean): void {
        if (!this.adaptationSystem.learningEnabled) return;
        
        // 成功/失敗パターンの学習
        this.learnGesturePattern(gestureName, gestureData, success);
        // ユーザープロファイルの更新
        this.updateUserProfileFromData(gestureData, success);
        // 適応的閾値の調整
     ...
     * 適応的閾値の調整
     */
    private adjustAdaptiveThresholds(success: boolean): void {
    ...
    private getSuccessRate(): number {
        const total = this.stats.successfulGestures + this.stats.failedGestures;
     ...
     * 簡単なジェスチャーの有効化
     */
    private enableSimpleGestures(): void {
     ...
     * 通常ジェスチャーの有効化
     */
    private enableNormalGestures(): void {
     ...
     * 高度なジェスチャーの有効化
     */
    private enableAdvancedGestures(): void {
        // すべてのジェスチャーを有効化
     ...
     * 高度なジェスチャーの提案
     */
    private suggestAdvancedGestures(): void {
     ...
     * 片手操作モードの有効化
     */
    enableOneHandedMode(preferredHand: PreferredHand = 'right'): void {
     ...
     * UIの片手操作調整
     */
    private adjustUIForOneHanded(preferredHand: PreferredHand): void {
        // UI要素を操作しやすい位置に移動
     ...
     * ジェスチャーフィードバックの提供
     */
    provideGestureFeedback(gestureName: string, gestureData: GestureData): void {
        // 視覚フィードバック
     ...
     * 未認識ジェスチャーの記録
     */
    recordUnrecognizedGesture(gestureData: GestureData, context?: GestureContext): void {
        try {
            // 学習データとして保存
            ...
            console.warn('Failed to record unrecognized gesture:', error);
        }
    }
     ...
     * ジェスチャー代替案の提案
     */
    suggestGestureAlternatives(gestureData: GestureData): string[] {
     ...
     * カスタムジェスチャーの追加
     */
    addCustomGesture(name: string, pattern: GesturePattern, action: string): void {
        const customGesture: CustomGesture = {
            name,
            pattern,
            action,
            createdAt: Date.now(),
    ...
    removeCustomGesture(name: string): boolean {
        const removed = this.userPreferences.customGestures.delete(name);
        if (removed) {
            this.stats.customizationChanges++;
            this.saveUserPreferences();
     ...
     * ジェスチャー統計の更新
     */
    private updateGestureStats(gestureName: string, success: boolean): void {
    ...
    updateUserProfile(profileUpdates: Partial<UserProfile>): void {
        Object.assign(this.adaptationSystem.userProfile, profileUpdates);
        this.setAdaptiveThresholds();
    ...
    setLearningEnabled(enabled: boolean): void {
        this.adaptationSystem.learningEnabled = enabled;
     ...
     * 詳細な統計情報の取得
     */
    getDetailedStats(): GestureStats & { sessionDuration: number; successRate: number } {
     ...
     * 適応システムの状態取得
     */
    getAdaptationStatus(): AdaptationStatus {
        return {
    ...
    getSuggestions(): ImprovementSuggestion[] {
        return this.adaptationSystem.suggestions;
    }
    
    /**
     * 提案の実行
     */
    applySuggestion(suggestion: ImprovementSuggestion): boolean {
        try {
    ...
    importConfiguration(configJson: string): boolean {
        try {
            ...
            if (config.userPreferences) {
                ...
                if (prefs.disabledGestures) {
                    this.userPreferences.disabledGestures = new Set(prefs.disabledGestures);
                }
                if (prefs.alternativeBindings) {
                    this.userPreferences.alternativeBindings = new Map(prefs.alternativeBindings);
                }
            }

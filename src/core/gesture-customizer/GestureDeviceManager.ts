...
export interface TouchGestureData extends GestureEventData {
    type: 'touch';
    points?: TouchPoint[];
    startPosition?: Position;
    currentPosition?: Position;
...
export class GestureDeviceManager {
     ...
     * キーボード入力の最適化
     */
    private optimizeForKeyboard(): void {
     ...
     * ゲームパッド入力の最適化
     */
    private optimizeForGamepad(): void {
    ...
    private handleWheel(event: WheelEvent): void {
        ...
        const wheelGesture: WheelGestureData = {
            type: 'wheel',
            deltaX: event.deltaX,
            deltaY: event.deltaY,
            deltaZ: event.deltaZ,
     ...
     * キーコンボの作成
     */
    private createKeyCombo(key: string, modifiers: KeyModifiers): string {
        const parts: string[] = [];
     ...
     * ゲームパッド設定
     */
    private configureGamepad(gamepad: Gamepad): void {
        // ゲームパッド固有の設定
     ...
     * StickyKeys有効状態のチェック
     */
    private checkStickyKeysEnabled(): boolean {
        // Windowsの場合のみ実装可能
        return false; // 簡略化
    }
     ...
     * FilterKeys有効状態のチェック
     */
    private checkFilterKeysEnabled(): boolean {
        // Windowsの場合のみ実装可能
        return false; // 簡略化
    }
     ...
     * 方向計算（度）
     */
    private calculateDirection(start: Position, end: Position): number {
     ...
     * 速度計算
     */
    private calculateVelocity(currentTouches: TouchPoint[]): void {
     ...
     * マルチタッチ処理
     */
    private handleMultiTouch(touches: TouchPoint[]): void {
        if (touches.length === 2) {
            // ピンチジェスチャーの検出
     ...
     * エッジタッチの検出
     */
    private detectEdgeTouch(position: Position): void {
    ...
    private handleOrientationChange(): void {
        ...
        this.notifyGestureEvent('orientationChange', {
            type: 'touch', // ダミー型
     ...
     * ジェスチャーイベントの通知
     */
    private notifyGestureEvent(eventType: GestureEventType, data: GestureEventData): void {
     ...
     * イベントハンドラーの設定
     */
    setGestureEventHandler(handler: GestureEventHandler): void {
     ...
     * 設定の更新
     */
    updateSettings(newSettings: DeviceSettingsUpdate): void {

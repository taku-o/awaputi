...
export interface AccessibilityTranslationData {
    [key: string]: string;
}

export interface TranslationCategoryStats {
    keyCount: number;
    arrayCount: number;
}
...
export class TranslationDataManager {
     ...
     * アクセシビリティ専用翻訳データの初期化
     */
    private initializeAccessibilityTranslations(): void {
        // 日本語アクセシビリティ翻訳
     ...
     * @returns 翻訳テキスト
     */
    getTranslation(language: string, key: string, fallbackLanguage: string = DEFAULT_FALLBACK_LANGUAGE): string | string[] {
        // メイン翻訳から取得
     ...
     */
    setAccessibilityData(language: string, accessibilityData: AccessibilityTranslationData): void {
        ...
        for (const [language, translations] of this.translations.entries()) {
            for (const [key, value] of Object.entries(translations)) {
                ...
                const matches = exactMatch 
                    ? searchValue === searchText || key === searchTerm
                    : searchValue.includes(searchText) || key.includes(searchTerm);
     ...
     * @returns 検証結果
     */
    validateTranslations(language: string, referenceLanguage: string = DEFAULT_FALLBACK_LANGUAGE): TranslationValidationResult {
    ...
    exportTranslations(options: TranslationExportOptions = {}): string {
        ...
        for (const language of targetLanguages) {
            ...
            // メイン翻訳
            if (this.translations.has(language)) {
                exportData[language].main = this.translations.get(language);
            }
            
            // アクセシビリティ翻訳
            if (includeAccessibility && this.accessibilityTranslations.has(language)) {
                exportData[language].accessibility = this.accessibilityTranslations.get(language);
            }
        }
     ...
     * @returns インポート結果
     */
    importTranslations(data: string, format: TranslationExportFormat = 'json'): TranslationImportResult {
        ...
        try {
            ...
            for (const [language, languageData] of Object.entries(parsedData)) {
                try {
                    ...
                    if (languageData.accessibility) {
                        this.setAccessibilityData(language, languageData.accessibility);
                    }
     ...
     * @returns 実行された操作数
     */
    executeBulkOperations(operations: BulkTranslationOperation[]): number {
        let executedCount = 0;
     ...
     * @returns マージ結果
     */
    mergeTranslationData(language: string, newData: TranslationData, overwrite: boolean = false): TranslationMergeResult {
        const result: TranslationMergeResult = {
            mergedKeys: 0,
            conflictKeys: [],
            newKeys: 0
        };
    ...
    private convertToYAML(data: Record<string, any>): string {
        // 簡単なYAML変換（実際のプロジェクトではライブラリ使用推奨）
        let yaml = '';
        for (const [language, languageData] of Object.entries(data)) {
            yaml += `${language}:\n`;
            for (const [category, categoryData] of Object.entries(languageData)) {
    ...
    private parseYAML(yaml: string): Record<string, any> {
        // 簡単なYAML解析（実際のプロジェクトではライブラリ使用推奨）
        const data: Record<string, any> = {};
        ...
        for (const line of lines) {
            ...
            if (line.match(/^[a-z]+:/)) {
                ...
                data[currentLang][currentCategory] = {};
            } else if (line.match(/^    /)) {
     ...
     */
    clearTranslations(language: string | null = null): void {
        if (language) {
     ...
     */
    createBackup(): string {
        return this.exportTranslations({ includeAccessibility: true });
    }
     ...
     * @returns 復元結果
     */
    restoreFromBackup(backupData: string): TranslationImportResult {
        // 現在のデータをクリア

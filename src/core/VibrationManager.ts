...
export class VibrationManager {
    ...
    private fallbackStrategies: any;
    private simplifiedPatterns: Map<string, number[]> | null;
    private gamepadCheckInterval: NodeJS.Timeout | null;
    ...
    initialize(): void {
        try {
            ...
            getErrorHandler().handleError(error, 'VIBRATION_MANAGER_ERROR', {
     ...
     * 振動機能の検出
     */
    detectVibrationCapabilities(): void {
     ...
     * デバイス性能の推定
     */
    estimateDevicePerformance(): void {
        ...
        try {
            ...
            if (saved) {
                ...
                if (preferences.intensityByEvent) {
                    this.userPreferences.intensityByEvent = new Map(preferences.intensityByEvent);
                }
            ...
            console.warn('Failed to load vibration manager preferences:', error);
        }
    }
    ...
    saveUserPreferences(): void {
        try {
            ...
            localStorage.setItem('vibrationManager_preferences', JSON.stringify(preferences));
        } catch (error) {
            console.warn('Failed to save vibration manager preferences:', error);
        }
    }
     ...
     * ゲームパッド接続処理
     */
    handleGamepadConnected(gamepad: any): void {
     ...
     * ゲームパッド切断処理
     */
    handleGamepadDisconnected(gamepad: any): void {
     ...
     * バッテリーレベル変更の処理
     */
    handleBatteryLevelChange(level: number): void {
     ...
     * 振動パターンの簡略化
     */
    simplifyVibrationPatterns(): void {
        // 複雑なパターンを簡単なものに置き換え
    ...
    setupEventListeners(): void {
        ...
        if (this.gameEngine) {
            // バブル関連
            this.gameEngine.addEventListener?.('bubblePopped', (event: any) => {
                this.triggerGameEventVibration('bubblePop', event);
            });
            
            this.gameEngine.addEventListener?.('bubbleBurst', (event: any) => {
                this.triggerGameEventVibration('bubbleBurst', event);
            });
            
            // ゲーム進行
            this.gameEngine.addEventListener?.('comboAchieved', (event: any) => {
                this.triggerGameEventVibration('combo', event);
            });
            
            this.gameEngine.addEventListener?.('bonusTriggered', (event: any) => {
                this.triggerGameEventVibration('bonus', event);
            });
            
            this.gameEngine.addEventListener?.('powerUpCollected', (event: any) => {
                this.triggerGameEventVibration('powerUp', event);
            });
            
            this.gameEngine.addEventListener?.('levelUp', (event: any) => {
                this.triggerGameEventVibration('levelUp', event);
            });
            
            this.gameEngine.addEventListener?.('gameOver', (event: any) => {
                this.triggerGameEventVibration('gameOver', event);
            });
            
            this.gameEngine.addEventListener?.('victory', (event: any) => {
                this.triggerGameEventVibration('victory', event);
            });
            
            // ダメージと警告
            this.gameEngine.addEventListener?.('playerDamaged', (event: any) => {
                this.triggerGameEventVibration('damage', event);
            });
            
            this.gameEngine.addEventListener?.('warning', (event: any) => {
                this.triggerGameEventVibration('warning', event);
            });
            
            // 特殊効果
            this.gameEngine.addEventListener?.('electricEffect', (event: any) => {
                this.triggerGameEventVibration('electric', event);
            });
            
            this.gameEngine.addEventListener?.('explosionEffect', (event: any) => {
                this.triggerGameEventVibration('explosion', event);
            });
            
            this.gameEngine.addEventListener?.('freezeEffect', (event: any) => {
                this.triggerGameEventVibration('freeze', event);
            });
            
            this.gameEngine.addEventListener?.('magneticEffect', (event: any) => {
        ...
        document.addEventListener('click', (event) => {
            if (this.shouldTriggerUIVibration('click', event.target as HTMLElement)) {
     ...
     * ゲームイベント振動のトリガー
     */
    triggerGameEventVibration(eventType: string, eventData: any): void {
        ...
        this.triggerVibration(eventType, {
            intensity,
            category: 'game',
            eventData
        });
     ...
     * UI振動の判定
     */
    shouldTriggerUIVibration(action: string, target: HTMLElement): boolean {
        ...
        const hasClickHandler = (target as any).onclick || target.addEventListener;
     ...
     * 振動機能の有無確認
     */
    hasVibrationCapability(): boolean {
     ...
     * 振動のトリガー
     */
    triggerVibration(patternName: string, options: any = {}): void {
     ...
     * 振動パターンの取得
     */
    getVibrationPattern(patternName: string): number[] | undefined {
        // カスタムパターンを優先
        ...
        // 簡略化パターンの確認
        if (this.simplifiedPatterns && this.simplifiedPatterns.has(patternName)) {
            return this.simplifiedPatterns.get(patternName);
        }
     ...
     * パターン強度の調整
     */
    adjustPatternIntensity(pattern: number[], intensity: number): number[] {
        if (intensity >= 1.0) {
            return pattern;
        }
     ...
     * 振動の実行
     */
    executeVibration(pattern: number[], options: any): void {
        const vibrationPromises: Promise<void>[] = [];
        ...
        if (this.userPreferences.gamepadVibration) {
            vibrationPromises.push(this.executeGamepadVibration(pattern, options));
        }
        ...
        // 振動完了時の処理
        Promise.all(vibrationPromises).then(() => {
            ...
            try {
                ...
                if (success) {
                    ...
                    const totalDuration = pattern.reduce((sum, duration, index) => {
                        return sum + duration + (index < pattern.length - 1 ? 0 : 0);
                    }, 0);
     ...
     * ゲームパッド振動の実行
     */
    executeGamepadVibration(pattern: number[], options: any): Promise<void> {
     ...
     * 単一ゲームパッド振動の実行
     */
    executeSingleGamepadVibration(gamepad: any, pattern: number[], options: any): Promise<void> {
        return new Promise((resolve, reject) => {
            try {
     ...
     * 振動完了の処理
     */
    handleVibrationComplete(vibrationId: string): void {
     ...
     * フォールバック処理
     */
    handleVibrationFallback(patternName: string, options: any): void {
        ...
        // 音響フィードバック
        if (alternatives.includes('audio')) {
            this.triggerAudioFallback(patternName, options);
        }
     ...
     * 統計更新
     */
    updateVibrationStats(patternName: string, pattern: number[]): void {
     ...
     * バッテリー影響の推定
     */
    estimateBatteryImpact(pattern: number[]): number {
     ...
     * カスタムパターンの追加
     */
    addCustomPattern(patternName: string, pattern: number[]): void {
     ...
     * イベント別強度の設定
     */
    setEventIntensity(eventType: string, intensity: number): void {
     ...
     * 手動振動のトリガー
     */
    triggerManualVibration(pattern: number[] | string, options: any = {}): void {

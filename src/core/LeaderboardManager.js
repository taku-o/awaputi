...
export class LeaderboardManager {
     ...
     */
    async addScore(leaderboardType, scoreData) {
    ...
    getLeaderboard(leaderboardType = 'global', limit = 10) {
        try {
            ...
            const result = leaderboard.entries.slice(0, limit).map((entry, index) => ({
                ...entry,
                rank: index + 1
            }));
     ...
     * @returns {Promise<boolean>} 記録成功可否
     */
    async recordPlayerScore(playerName, scoreData) {
        const enhancedScoreData = {
            ...scoreData,

...
export class AlternativeInputManager {
     ...
     * @param {Object} config - 設定オブジェクト
     */
    async initializeSwitchInput(config) {
     ...
     * @param {Object} config - 設定オブジェクト
     */
    async initializeVoiceControl(config) {
    ...
    createFeedbackElements() {
        // サブコントローラーに委譲（基本的なフィードバック要素のみ作成）
    ...
    setupFocusManagement() {
        document.addEventListener('focusin', (event) => {
            this.announceElementToScreenReader(event.target);
        });
    }
    ...
    applyConfig(config) { if (config.motor?.alternativeInput) Object.assign(this.config, config.motor.alternativeInput); }

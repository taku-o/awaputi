...
export class GameContentDescriber {
    ...
    setupGameEventListeners(): void {
        ...
        if (this.gameEngine.addEventListener) {
            this.gameEngine.addEventListener('gameStateChanged', (event: any) => {
                this.handleGameStateChange(event.newState, event.oldState);
            });

            this.gameEngine.addEventListener('scoreChanged', (event: any) => {
            ...
            this.gameEngine.addEventListener('comboChanged', (event: any) => {
                this.handleComboChange(event.combo, event.isNewCombo);
            });

            this.gameEngine.addEventListener('specialEffectTriggered', (event: any) => {
        ...
        if (this.gameEngine.bubbleManager) {
            this.gameEngine.bubbleManager.addEventListener?.('bubbleSpawned', (event: any) => {
                this.handleBubbleSpawned(event.bubble);
            });

            this.gameEngine.bubbleManager.addEventListener?.('bubbleBurst', (event: any) => {
        ...
        if (this.gameEngine.sceneManager) {
            this.gameEngine.sceneManager.addEventListener?.('sceneChanged', (event: any) => {
     ...
     * ゲーム状態変更の処理
     */
    handleGameStateChange(newState: any, oldState: any): void {
     ...
     * スコア変更の処理
     */
    handleScoreChange(newScore: number, oldScore: number): void {
     ...
     * バブルポップの処理
     */
    handleBubblePopped(bubble: any, points: number): void {
     ...
     * コンボ変更の処理
     */
    handleComboChange(combo: number, isNewCombo: boolean): void {
        if (combo <= 1) {
            // コンボ終了
     ...
     * 特殊効果の処理
     */
    handleSpecialEffect(effect: string, description: string): void {
     ...
     * バブル出現の処理
     */
    handleBubbleSpawned(bubble: any): void {
        // 重要な特殊バブルの場合のみ通知
     ...
     * バブル破裂の処理
     */
    handleBubbleBurst(bubble: any): void {
        // 自動破裂（時間切れ）の場合
     ...
     * シーン変更の処理
     */
    handleSceneChange(newScene: any, oldScene: any): void {
        const sceneDescriptions: any = {
     ...
     * 現在のゲーム状態説明
     */
    describeCurrentGameState(gameState: any): void {
     ...
     * バブルタイプ説明の取得
     */
    getBubbleTypeDescription(bubble: any): string {
     ...
     * 特殊バブル判定
     */
    isSpecialBubble(bubble: any): boolean {
        const specialTypes = [
     ...
     * 重要なバブル判定
     */
    isImportantBubble(bubble: any): boolean {
     ...
     * 特殊バブル説明の取得
     */
    getSpecialBubbleDescription(bubble: any): string {
     ...
     * バブル位置の取得
     */
    getBubblePosition(bubble: any): string {
        ...
        const canvasWidth = (canvas as any).width || (canvas as any).offsetWidth;
        const canvasHeight = (canvas as any).height || (canvas as any).offsetHeight;
        ...
        for(const [regionName, region] of Object.entries(this.spatialInfo.screenRegions)) {
            ...
            if (relativeX >= r.x && relativeX < r.x + r.width &&
                relativeY >= r.y && relativeY < r.y + r.height) {
     ...
     * 空間情報の更新
     */
    updateSpatialInfo(bubble: any): void {
        if (bubble.id) {
        ...
        for(const [id, info] of this.spatialInfo.bubblePositions) {
            if ((info as any).timestamp < cutoff) {
     ...
     * シーン導入説明の提供
     */
    provideSceneIntroduction(scene: any): void {
     ...
     * 効果表示名の取得
     */
    getEffectDisplayName(effect: string): string {
        const displayNames: any = {
            'bonusTime': 'ボーナスタイム',
            'timeStop': '時間停止',
            'doubleScore': 'ダブルスコア',
     ...
     * テンプレート埋め込み
     */
    fillTemplate(template: string, variables: any): string {
        let result = template;
     ...
     * 説明のアナウンス
     */
    announceDescription(description: string, priority = 'polite'): void {
     ...
     * コンテキストヘルプの提供
     */
    provideContextualHelp(context: string): string | null {
        let helpText = '';
     ...
     * 詳細ゲーム状況の説明
     */
    describeDetailedGameSituation(): string {
        const gameState = this.getCurrentGameState();
     ...
     * 画面上の特殊バブル取得
     */
    getSpecialBubblesOnScreen(): string[] {
        const specialBubbles: string[] = [];
        
        try {
            const bubbleManager = this.gameEngine?.bubbleManager;
            if (bubbleManager && bubbleManager.bubbles) {
                bubbleManager.bubbles.forEach((bubble: any) => {
                    if (this.isSpecialBubble(bubble)) {
                        const position = this.getBubblePosition(bubble);

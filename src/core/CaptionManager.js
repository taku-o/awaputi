...
export class CaptionManager {
    constructor(audioAccessibilityManager) {
        ...
        // 音響効果の説明マッピング
        this.soundDescriptions = new Map([
            // ゲーム音響効果
        ...
        // 音楽説明マッピング
        this.musicDescriptions = new Map([
    ...
    setupEventListeners() {
        ...
        if (this.gameEngine) {
            // 音響効果イベント
            this.gameEngine.addEventListener?.('audioPlayed', (event) => {
        ...
        // 設定変更の監視
        document.addEventListener('accessibilitySettingsChanged', (event) => {
    ...
    setupAudioManagerIntegration() {
        ...
        if (audioManager.playMusic) {
            ...
            audioManager.playMusic = (musicId, options = {}) => {
        ...
        switch (level) {
            ...
            case 'detailed':
                ...
                if (description.category && description.category !== 'game') {
                    const language = this.languageSupport.get(this.userPreferences.languagePreference);
                    const categoryDesc = language.categories[description.category];
    ...
    getDurationForSound(soundId, category) {
        ...
        if (description && description.duration) {
            return this.config.captionDuration[description.duration] || this.config.captionDuration.medium;
        }
    ...
    showManualCaption(text, options = {}) {
        ...
        const captionId = this.generateCaptionId();
        const duration = this.config.captionDuration[description.duration] || 

...
export class AchievementEventIntegrator {
     ...
     * BubbleManagerとの統合
     */
    private integrateBubbleManager(): void {
        ...
        // popBubbleメソッドを拡張
        bubbleManager.popBubble = (bubble: any, ...args: any[]) => {
     ...
     * ScoreManagerとの統合
     */
    private integrateScoreManager(): void {
        ...
        // addScoreメソッドを拡張
        scoreManager.addScore = (points: number, ...args: any[]) => {
     ...
     * GameSceneとの統合
     */
    private integrateGameScene(): void {
     ...
     */
    private integrateStatisticsManager(): void {
     ...
     * ゲーム終了時のデータ収集
     */
    private collectGameEndData(): GameEndData {
        const scoreManager = this.gameEngine.scoreManager;
        const bubbleManager = this.gameEngine.bubbleManager;
     ...
     * アイテム使用状況をチェック
     */
    private checkItemsUsed(): boolean {
     ...
     * 精度実績をチェック
     */
    private checkAccuracyAchievements(gameData: GameEndData): void {
        // 95%と99%の精度実績をチェック
     ...
     * 夜間プレイをチェック
     */
    private checkNightTimePlay(gameData: GameEndData): void {
     ...
     * スピードチャレンジをチェック
     */
    private checkSpeedChallenge(): void {
        const scoreManager = this.gameEngine.scoreManager;
     ...
     * 低HPサバイバルチェック
     */
    private checkLowHpSurvival(): void {
        const playerData = this.gameEngine.playerData;

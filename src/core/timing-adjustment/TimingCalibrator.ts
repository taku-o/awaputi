...
// 型定義
export interface TimingAdjustmentManager {
    gameEngine: any;
...
export interface TimingConfiguration {
    profiles: Record<ProfileType, TimingProfile>;
...
// 型ガード
export function isValidResponseRecord(record: any): record is ResponseRecord {
    return record && 
...
export function isValidTimingProfile(profile: any): profile is TimingProfile {
    return profile && 
...
export function isAccessibilitySettings(settings: any): settings is AccessibilitySettings {
    return settings && 
...
export function isMediaQueryList(query: any): query is MediaQueryList {
    return query && 
           typeof query.matches === 'boolean' &&
           typeof query.addEventListener === 'function';
}

export function hasAccessibilityManager(engine: any): engine is GameEngine {
    return engine && 
...
export class TimingCalibrator {
    ...
    detectSystemPreferences(): void {
        ...
        prefersReducedMotion.addEventListener('change', (e: MediaQueryListEvent) => {
            this.state.systemPreferences.reducedMotion = e.matches;
            if (e.matches) {
                this.adjustForReducedMotion();
            }
        });

        prefersHighContrast.addEventListener('change', (e: MediaQueryListEvent) => {
            this.state.systemPreferences.highContrast = e.matches;
            if (e.matches) {
                this.config.profiles.standard.preferences.showTimeWarnings = true;
            }
        });
    ...
    trackUserInteraction(interaction: UserInteraction): void {
        ...
        // 最新50件のみ保持
        if (this.state.userInteractionData.recentResponses.length > MAX_RECENT_RESPONSES) {
    ...
    private performAdaptiveLearning(): void {
        ...
        if (adaptationNeeded && recommendedAdjustment) {
     ...
     * 推奨プロファイルを取得
     */
    getRecommendedProfile(accessibilitySettings: AccessibilitySettings): ProfileType {
    ...
    private adjustForReducedMotion(): void {
        ...
        currentProfile.customTimeouts.animation = REDUCED_MOTION_ANIMATION_TIMEOUT;
        currentProfile.customTimeouts.transition = REDUCED_MOTION_TRANSITION_TIMEOUT;
    ...
    integrateWithAccessibilityManager(): void {
        ...
        // 設定の同期
        accessibilityManager.on('settingsChanged', (settings: AccessibilitySettings) => {
     ...
     * キャリブレーションデータの検証
     */
    validateCalibrationData(): ValidationResult {
        const data = this.adaptiveLearning.data;
        const issues: string[] = [];
     ...
     * キャリブレーション記録を保存
     */
    saveCalibrationRecord(adjustments: ProfileAdjustment[], result: CalibrationResult): void {
        ...
        // 履歴サイズを制限（最新100件）
        if (this.state.calibrationHistory.length > 100) {
            this.state.calibrationHistory = this.state.calibrationHistory.slice(-100);
        }
     ...
     * プロファイル調整を記録
     */
    recordProfileAdjustment(property: string, oldValue: any, newValue: any, reason: AdjustmentReason): void {
    ...
    resetAdaptiveLearning(): void {
        ...
        this.state.userInteractionData.recentResponses = [];
        this.state.userInteractionData.averageResponseTime = 0;
     ...
     * キャリブレーション精度を測定
     */
    measureCalibrationAccuracy(): number {
        const recentResponses = this.state.userInteractionData.recentResponses;
     ...
     * 設定を取得
     */
    getConfiguration(): Readonly<TimingConfiguration> {
        return {
     ...
     * 現在の状態を取得
     */
    getCurrentState(): Readonly<TimingState> {
        return {
            currentProfile: this.state.currentProfile,
            userInteractionData: {
                ...this.state.userInteractionData,
     ...
     */
    destroy(): void {
        // メディアクエリリスナーのクリーンアップは自動的に処理される
        // 必要に応じて明示的なクリーンアップを追加

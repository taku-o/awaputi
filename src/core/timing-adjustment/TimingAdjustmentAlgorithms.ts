...
import type { TimerManager,
    ...
    PausedTimer,
    TimerType,
    TimerPriority } from './TimingFeedbackSystem.js';

// 型定義
export interface AlgorithmTimingAdjustmentManager extends TimingAdjustmentManager {
    timers: ExtendedTimerManager;
    emitEvent: (eventName: string, data: any) => void;
    showTimeWarning: (timerId: string) => void;
    showExtensionFeedback: (timerId: string, remainingTime: number) => void;
}

export interface ExtendedTimerManager extends TimerManager {
...
export interface RegisteredTimer extends ActiveTimer {
    registeredAt: number;
...
export interface TimerExtensionEvent {
    timerId: string;
...
// 型ガード
export function isValidTimerConfig(config: any): config is TimerConfig {
    return config &&
...
export function isRegisteredTimer(timer: any): timer is RegisteredTimer {
    return timer &&
...
export function isValidExtensionInfo(info: any): info is ExtensionInfo {
    return info &&
...
export function hasTimerExtensions(manager: ExtendedTimerManager): boolean {
    return manager.extensions instanceof Map;
}
export class TimingAdjustmentAlgorithms {
    ...
    applyProfile(profileName: string): boolean {
        ...
        // イベントを発火
        const eventData: ProfileChangeEvent = {
            profile: profileName as ProfileType,
            settings: profile,
            previousProfile
        };
     ...
     * 調整倍率を取得
     */
    getAdjustmentMultiplier(): number {
     ...
     * タイマーを削除
     */
    unregisterTimer(timerId: string): boolean {
        const wasActive = this.timers.active.has(timerId);
     ...
     * 警告タイマーを設定
     */
    private setupWarningTimer(timerId: string, config: TimerConfig): void {
        const profile = this.getCurrentProfile();
     ...
     * タイマーを延長
     */
    extendTimer(timerId: string, requestedBy: ExtensionRequestor = 'user'): boolean {
        const timer = this.timers.active.get(timerId) as RegisteredTimer;
        if (!timer) {
        ...
        // イベントを発火
        const eventData: TimerExtensionEvent = {
            timerId,
            extensionAmount,
     ...
     * 時間延長をリクエスト
     */
    requestTimeExtension(): string | null {
     ...
     * すべてのタイマーを一時停止
     */
    pauseAllTimers(reason: PauseReason = 'manual'): number {
     ...
     * すべてのタイマーを再開
     */
    resumeAllTimers(reason: PauseReason | 'user' = 'manual'): number {
     ...
     * アクティブなタイマーに調整を適用
     */
    private applyAdjustmentsToActiveTimers(): void {
     ...
     * パフォーマンス向上度を計算
     */
    private calculatePerformanceGain(optimizedCount: number): number {
        // 簡単な計算式：削除されたタイマー数に基づく
     ...
     * アルゴリズム性能統計を取得
     */
    getAlgorithmStatistics(): AlgorithmStatistics {
        return {
     ...
     * 平均延長回数を計算
     */
    private calculateAverageExtensions(): number {
        if (!hasTimerExtensions(this.timers) || this.timers.extensions.size === 0) {
            return 0;
        }
     ...
     * 特定のタイマータイプの統計を取得
     */
    getTimerTypeStatistics(type: TimerType): { count: number, averageRemaining: number, extensions: number } {
        let count = 0;
    ...
    getUrgentTimers(threshold: number = 5000): Array<{ id: string, remaining: number, priority: TimerPriority }> {
        ...
        return urgentTimers.sort((a, b) => {
            ...
            if (aPriority !== bPriority) {
                return bPriority - aPriority; // 高い優先度から
            }

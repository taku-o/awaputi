...
export class AsyncOperationQueue {
     ...
     */
    async enqueue(operation: () => Promise<unknown>, options: any = {}) {
     ...
     * 優先度順にキューに挿入
     */
    insertByPriority(item: QueueOperation) {
     ...
     * 個別操作の実行
     */
    async executeOperation(item: QueueOperation) {
        ...
        try {
            ...
            this.completedOperations.set(item.id, {
                ...item,
                result,
                executionTime,
            ...
            await this.handleOperationError(item, error as Error, startTime);
        }
    }
     ...
     * 操作エラーの処理
     */
    async handleOperationError(item: QueueOperation, error: Error, startTime: number) {
        const executionTime = Date.now() - startTime;
        ...
        // 最終的な失敗処理
        this.failedOperations.set(item.id, {
            ...item,
            error,
            executionTime,
            failedAt: Date.now()
     ...
     * 実行統計の更新
     */
    updateExecutionStats(executionTime: number) {
    ...
    cleanupCompletedOperations() {
        ...
        if (this.completedOperations.size > maxHistorySize) {
            ...
            entries.sort((a, b) => (a[1] as any).completedAt - (b[1] as any).completedAt);
        ...
        for (const [id, operation] of this.failedOperations.entries()) {
            if (now - (operation as any).failedAt > maxAge) {
    ...
    destroy() {
        ...
        const activePromises = Array.from(this.activeOperations.values())
            .map(item => new Promise<void>(resolve => {
                const queueItem = item as QueueOperation;
...
// シングルトンインスタンス
let queueInstance: AsyncOperationQueue | null = null;

/**
 * AsyncOperationQueueシングルトンインスタンスの取得
 */
export function getAsyncOperationQueue() {
    if (!queueInstance) {

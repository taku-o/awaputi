...
export class CalculationEngine {
    ...
    private _evictLeastValuableCache(): void {
        ...
        for (const [key, cached] of this.cache.entries()) {
            // 価値スコアを計算（低いほど削除対象）
            const timeSinceLastAccess = now - (cached.lastAccess || cached.timestamp);
    ...
    /**
     * 拡張されたキャッシュ統計を取得
     * @returns 詳細なキャッシュ統計情報
     */
    getExtendedCacheStats(): any {
 ...
 * @returns CalculationEngineインスタンス
 */

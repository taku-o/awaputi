...
// 型定義
export interface SimplificationLevels {
    none: SimplificationLevelConfig;
...
export interface SimplificationLevelConfig {
    name: string;
    description: string;
    complexity: number;
...
export interface AutoModeCriteria {
...
export interface AutoModeEvaluation {
...
export interface CurrentModeConfig {
    mode: ModeConfig & { current: boolean };
...
export interface AvailableMode extends ModeConfig {
    key: string;
    isCurrent: boolean;
}

export interface AvailableLevel extends SimplificationLevelConfig {
    key: string;
    isCurrent: boolean;
}

export interface RecommendedMode extends ModeConfig {
    key: string;
    reason: string;
    priority?: number;
    confidence?: number;
...
// 定数
export const DEFAULT_SIMPLIFICATION_LEVELS: SimplificationLevels = {
    none: {
        name: '標準',
        description: '通常の複雑さ',
        complexity: 1.0,
...
export const DEFAULT_AUTO_CRITERIA: AutoModeCriteria = {
    errorThreshold: 3,
...
// ユーティリティ関数
export function validateModeTransition(fromMode: string, toMode: string, transitionRules: Record<string, string[]>): TransitionValidation {
    const allowedTransitions = transitionRules[fromMode];
...
export function calculateComplexityDifference(level1: SimplificationLevel, level2: SimplificationLevel): number {
    const complexity1 = COMPLEXITY_MAPPING.levels[level1];
...
export function findModeByLevel(level: SimplificationLevel): string {
    for(const [modeKey, config] of Object.entries(DEFAULT_MODES)) {
...
export class SimplificationModeController {
    ...
    constructor() {
        ...
        this.transitionRules = { ...MODE_TRANSITION_RULES };
     ...
     * モード遷移を設定
     */
    private setupModeTransitions(): void {
        // Initial setup - transition rules are already loaded from constants
    }

    /**
     * モードを変更
     */
    changeMode(newMode: string, reason: ModeTransitionType = 'manual'): CurrentModeConfig {
        ...
        if (this.complexityTrend.length > 100) {
            this.complexityTrend = this.complexityTrend.slice(-50);
        }
    ...
    changeLevel(newLevel: SimplificationLevel, reason: ModeTransitionType = 'manual'): ModeConfig {
        ...
        this.complexityTrend.push({
            timestamp: Date.now(),
            complexity: this.simplificationLevels[newLevel].complexity,
            mode: 'custom',
            level: newLevel
        });
     ...
     * 利用可能なモードを取得
     */
    getAvailableModes(): AvailableMode[] {
     ...
     * モード遷移の推奨を取得
     */
    getRecommendedModes(): RecommendedMode[] {
     ...
     * 遷移優先度を計算
     */
    private calculateTransitionPriority(fromMode: string, toMode: string): number {
        const fromComplexity = this.simplificationLevels[this.modes[fromMode].level].complexity;
     ...
     * 遷移信頼度を計算
     */
    private calculateTransitionConfidence(fromMode: string, toMode: string): number {
        // 履歴から成功した遷移の回数を調べる
        const successfulTransitions = this.modeHistory.filter(
            h => h.from === fromMode && h.to === toMode
        ).length;
     ...
     * 複雑さスコアに対応するレベルを検索
     */
    findLevelByComplexity(targetComplexity: number): SimplificationLevel {
        return findBestComplexityMatch(targetComplexity);
    }
     ...
     * 複雑度トレンドを取得
     */
    getComplexityTrend(): ComplexityTrend[] {
        return this.complexityTrend.slice(-20); // 最新20件
    }
     ...
     * カスタムモードを削除
     */
    removeCustomMode(modeKey: string): boolean {
     ...
     * モードが存在するかチェック
     */
    hasModeKey(modeKey: string): boolean {
        return modeKey in this.modes;
    }
    ...
    hasLevel(level: string): boolean {
        return isValidSimplificationLevel(level);
    }

    /**
     * カスタムレベルを追加
     */
    addCustomLevel(key: string, config: SimplificationLevelConfig): void {
        if (key in this.simplificationLevels) {
            throw new Error(`Level ${key} already exists`);
        }
        
        this.simplificationLevels = {
            ...this.simplificationLevels,
            [key]: config
        } as SimplificationLevels;
    }

    /**
     * 遷移ルールを更新
     */
    updateTransitionRules(rules: Partial<Record<string, string[]>>): void {
        Object.assign(this.transitionRules, rules);
    }
     ...
     * 現在のモードとレベルを取得
     */
    getCurrentState(): { mode: string; level: SimplificationLevel } {
        return {
            mode: this.currentMode,
            level: this.currentLevel
        };
     ...
     * 自動モード設定を取得
     */
    getAutoModeSettings(): { enabled: boolean; criteria: AutoModeCriteria } {
        return {
            enabled: this.autoMode,
            criteria: this.autoModeCriteria
        };
    }
}
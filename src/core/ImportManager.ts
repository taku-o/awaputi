...
export class ImportManager {
     ...
     * 競合解決ストラテジーの登録
     */
    registerConflictResolvers(): void {
        // マージ解決
     ...
     * データのインポート
     */
    async importData(importData: any, conflictResolution = 'merge', options = {}): Promise<any> {
        try {
            ...
            this.updateStatistics(false, 0, false);
     ...
     * 競合の検出
     */
    async detectConflicts(importData: any): Promise<any[]> { 
        try {
            const conflicts: any[] = [];
     ...
     * データの比較
     */
    compareData(dataType: string, existingData: any, newData: any): any[] {
        const conflicts: any[] = [];
     ...
     * PlayerDataの比較
     */
    comparePlayerData(existing: any, imported: any): any[] { 
        const conflicts: any[] = [];
        ...
        for (const field of criticalFields) {
            if (field in existing && field in imported) {
                ...
                } else if (existing[field] !== imported[field]) { 
                    conflicts.push({
                        type: 'VALUE_CONFLICT',
                        field,
                        existing: existing[field],
     ...
     * 設定の比較
     */
    compareSettings(existing: any, imported: any): any[] { 
        const conflicts: any[] = [];
     ...
     * 統計の比較
     */
    compareStatistics(existing: any, imported: any): any[] { 
        const conflicts: any[] = [];
     ...
     * 汎用データの比較
     */
    compareGeneric(existing: any, imported: any): any[] {
        const conflicts: any[] = [];
     ...
     * 競合の解決
     */
    async resolveConflicts(importData: any, conflicts: any[], resolutionStrategy: string, options = {}): Promise<any> {
        try {
     ...
     * インポートデータのプレビュー
     */
    async previewImport(importData: any, conflictResolution = 'merge'): Promise<any> { 
        try {
            // 検証の実行
            const validationResult = await this.validateImportData(importData);
            ...
            const resolutionPreview = resolver ? 
     ...
     * インポート警告の生成
     */
    generateImportWarnings(importData: any, conflicts: any[]): string[] {
        const warnings: string[] = [];
        ...
        if (importData.header && importData.header.gameVersion) {
            const currentVersion = (window as any).GAME_VERSION || '1.0.0';
     ...
     * 統計の更新
     */
    updateStatistics(success: boolean, duration: number, partial: boolean): void {
        try {
            if (success) {
...
class MergeResolver { 
    ...
    async resolve(importData: any, conflicts: any[], options = {}): Promise<any> {
        ...
        for (const conflict of conflicts) {
            ...
            if (resolvedData.userData) {
                ...
                resolvedData[conflict.dataType] = mergedData;
            }
        }
    ...
    async mergeData(dataType: string, existing: any, imported: any, conflicts: any[]): Promise<any> {
    ...
    mergePlayerData(existing: any, imported: any, conflicts: any[]): any {
        ...
        if (imported.unlockedStages) { 
            merged.unlockedStages = [...new Set([...existing.unlockedStages, ...imported.unlockedStages])];
        }
        if (imported.ownedItems) { 
            merged.ownedItems = [...new Set([...existing.ownedItems, ...imported.ownedItems])];
        }
    ...
    getMergeAction(conflict: any): string {
    ...
    getMergeResult(conflict: any): any {
        switch(conflict.type) {
            case 'VALUE_CONFLICT':
    ...
    async resolve(importData: any, conflicts: any[], options = {}): Promise<any> {
        ...
        for (const conflict of conflicts) {
            if (resolvedData.userData) {
                ...
                resolvedData[conflict.dataType] = conflict.existingData;
            }
        }
        ...
        for (const conflict of conflicts) {
            ...
            if (selection === 'import') {
                ...
                if (resolvedData.userData) {
                    ...
                    resolvedData[conflict.dataType] = conflict.newData;
                }
                ...
                if (resolvedData.userData) {
                    ...
                    resolvedData[conflict.dataType] = merged;
                }
            }
        }

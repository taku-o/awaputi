...
export class TextScalingManager {
    ...
    private layoutContainers: Set<HTMLElement>;
    private gameElements: Set<HTMLElement>;
    private dynamicStyleSheet: HTMLStyleElement | null;
    private cssVariables: Map<string, string>;
    private currentBreakpoint: string;
    private viewportObserver: ResizeObserver | null;
    private stats: any;
    ...
    saveUserPreferences(): void {
        try {
            localStorage.setItem('textScaling_preferences', JSON.stringify(this.userPreferences));
        } catch (error) {
    ...
    createDynamicStyleSheet(): void {
        ...
        this.dynamicStyleSheet.textContent = `
            ...
            /* スムーズトランジション */
            .text-scaling-enabled.smooth-transitions * {
            ...
            /* スケーリング対象要素 */
            .text-scalable {
                font-size: calc(var(--original-font-size, 1rem) * var(--text-scale-factor, 1));
            ...
            /* 読みやすさ向上フォント */
            .dyslexia-friendly {
                font-family: var(--dyslexia-font-family) !important;
                font-weight: 400 !important;
            }
            
            .high-readability {
                font-family: var(--readability-font-family) !important;
            ...
            @media (max-width: 768px) {
                ...
                .text-scalable {
                    font-size: calc(var(--original-font-size, 1rem) * var(--mobile-scale-factor, 1));
                }
            }
     ...
     * ブレークポイント用の調整
     */
    adjustForBreakpoint(breakpoint: string): void {
        let scaleFactor = 1.0;
     ...
     * 初期要素の分析
     */
    analyzeInitialElements(): void {
        const elements = document.querySelectorAll('*');
        
        elements.forEach(element => {
            this.categorizeElement(element as HTMLElement);
        });
    ...
    categorizeElement(element: HTMLElement): void {
        ...
        if (this.isLayoutContainer(element)) {
            this.layoutContainers.add(element);
        }
     ...
     * テキストコンテンツの有無をチェック
     */
    hasTextContent(element: HTMLElement): boolean {
     ...
     * ゲーム要素の判定
     */
    isGameElement(element: HTMLElement): boolean {
        const gameSelectors = [
            'canvas',
            '.game-canvas',
            '.bubble',
     ...
     * レイアウトコンテナの判定
     */
    isLayoutContainer(element: HTMLElement): boolean {
     ...
     * スケーラブル要素の登録
     */
    registerScalableElement(element: HTMLElement): void {
        ...
        this.scaledElements.set(element, {
            originalStyle,
            isScaled: false,
            category: 'text'
        });
     ...
     * 新しい要素の処理
     */
    processNewElement(element: HTMLElement): void {
     ...
     * 要素へのスケーリング適用
     */
    applyScalingToElement(element: HTMLElement): void {
        ...
        try {
            const originalStyle = elementData.originalStyle;
     ...
     * レイアウト保持の判定
     */
    shouldPreserveLayout(element: HTMLElement): boolean {
        // 固定サイズのコンテナ内にある要素
     ...
     * レイアウト保持の適用
     */
    applyLayoutPreservation(element: HTMLElement): void {
        const styles = window.getComputedStyle(element);
     ...
     * フォントファミリーの適用
     */
    applyFontFamily(element: HTMLElement): void {
     ...
     * パフォーマンス統計の更新
     */
    updatePerformanceStats(operationTime: number): void {
     ...
     * 文字間隔の調整
     */
    adjustLetterSpacing(adjustment: number): void {
     ...
     * 単語間隔の調整
     */
    adjustWordSpacing(adjustment: number): void {
     ...
     * スムーズトランジションの切り替え
     */
    toggleSmoothTransitions(enabled: boolean): void {

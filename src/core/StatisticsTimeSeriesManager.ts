/**
 * StatisticsTimeSeriesManager.ts
 * 時系列統計データの管理クラス
...
export class StatisticsTimeSeriesManager {
    ...
    private maxDataPoints: number = 1000; // 最大データポイント数
    private aggregationIntervals: { [key: string]: number } = {
        hourly: 60 * 60 * 1000,
        daily: 24 * 60 * 60 * 1000,
     ...
     */
    recordTimeSeriesData(score: number, playTime: number, completed: boolean, additionalData: { [key: string]: any } = {}): void {
        try {
            ...
            console.error('StatisticsTimeSeriesManager.recordTimeSeriesData error:', error);
        }
    }
     ...
     */
    getTimeSeriesData(options: GetTimeSeriesOptions = {}): TimeSeriesDataPoint[] | AggregatedData[] {
        try {
            const {
                startDate,
                endDate,
            ...
            let filteredData: TimeSeriesDataPoint[] = [...this.timeSeriesData];

            // 日付フィルタリング
            if (startDate) {
                filteredData = filteredData.filter(point => point.timestamp >= startDate);
            }
            if (endDate) {
            ...
            // 集計処理
            if (aggregation !== 'none') {
                return this.aggregateData(filteredData, aggregation, metric);
            }
            ...
            console.error('StatisticsTimeSeriesManager.getTimeSeriesData error:', error);
            return [];
        }
    }
     ...
     */
    aggregateData(data: TimeSeriesDataPoint[], interval: string, metric: string): AggregatedData[] {
        try {
            ...
            const aggregated: { [key: number]: AggregationBucket } = {};
            ...
            data.forEach(point => {
                ...
                aggregated[bucketTime].sum += (point as any)[metric] || 0;
                aggregated[bucketTime].values.push((point as any)[metric] || 0);
            });
            ...
            console.error('StatisticsTimeSeriesManager.aggregateData error:', error);
            return [];
        }
    }
     ...
     */
    getAggregatedTimeSeriesData(period: string = 'daily', days: number = 30): any {
        try {
            const now = Date.now();
            ...
            return {
                ...
                period,
                days,
                summary: this.generateSummary(scoreData as AggregatedData[], efficiencyData as AggregatedData[], playTimeData as AggregatedData[])
            };

        } catch (error) {
            console.error('StatisticsTimeSeriesManager.getAggregatedTimeSeriesData error:', error);
            return {
                score: [],
                efficiency: [],
                playTime: [],
    ...
    getTimeSeriesStatisticsSummary(): any {
        try {
            ...
            console.error('StatisticsTimeSeriesManager.getTimeSeriesStatisticsSummary error:', error);
            return {
                totalDataPoints: 0,
                dateRange: null,
                trends: {},
     ...
     */
    getRecentPerformance(days: number = 7): any {
        try {
            ...
            console.error('StatisticsTimeSeriesManager.getRecentPerformance error:', error);
            return {
                period: days,
                gamesPlayed: 0,
                averageScore: 0,
    ...
    getPeakPlayingTimes(): any {
        try {
            ...
            console.error('StatisticsTimeSeriesManager.getPeakPlayingTimes error:', error);
            return {
    ...
    getPlayTimeDistribution(): any {
        try {
            ...
            console.error('StatisticsTimeSeriesManager.getPlayTimeDistribution error:', error);
            return {
                distribution: {},
                average: 0,
                median: 0,
     ...
     * @returns {Object} 比較結果
     */
    compareTimePeriods(period1: string = '7d', period2: string = '14d'): any {
        try {
            const now = Date.now();
            ...
            return {
                ...
                comparison: {
                    scoreChange: this.calculatePercentageChange(stats2.averageScore, stats1.averageScore),
            ...
            console.error('StatisticsTimeSeriesManager.compareTimePeriods error:', error);
     ...
     * @returns {Object} 期間統計
     */
    calculatePeriodStats(data: TimeSeriesDataPoint[]): any {
        if (data.length === 0) {
            return {
                totalGames: 0,
                averageScore: 0,
     ...
     */
    calculateTrendFromData(data: TimeSeriesDataPoint[], metric: string): string {
        if (data.length < 3) return 'stable';

        const values = data.map(point => (point as any)[metric] || 0);
     ...
     */
    calculateAverages(): any {
        if (this.timeSeriesData.length === 0) return {};

        const totals = this.timeSeriesData.reduce((acc, point) => {
     ...
     * @returns {Object} サマリー
     */
    generateSummary(scoreData: AggregatedData[], efficiencyData: AggregatedData[], playTimeData: AggregatedData[]): any {
        return {
            totalDataPoints: scoreData.length,
...
// シングルトンインスタンス管理
let statisticsTimeSeriesManagerInstance: StatisticsTimeSeriesManager | null = null;

/**
 * StatisticsTimeSeriesManagerのシングルトンインスタンスを取得
 * @returns {StatisticsTimeSeriesManager} シングルトンインスタンス
 */
export function getStatisticsTimeSeriesManager(): StatisticsTimeSeriesManager {
    if (!statisticsTimeSeriesManagerInstance) {
 ...
 */
export function reinitializeStatisticsTimeSeriesManager(): StatisticsTimeSeriesManager {
    statisticsTimeSeriesManagerInstance = new StatisticsTimeSeriesManager();
    return statisticsTimeSeriesManagerInstance;
}
...
export interface LargeDataMetadata extends ChunkMetadata {
    dataType: string;
    version: string;
    originalSize: number;
...
export interface DataManagerReference {
...
export interface AsyncQueue {
    enqueue<T>(operation: () => Promise<T>, options: QueueOptions): Promise<T>;
}
...
export interface PlayerDataManager {
    saveData(data: any): Promise<SaveResult>;
    getAllData(): any;
}
...
// ユーティリティ関数
export function generateStorageKey(dataType: string, prefix?: string): string {
    const actualPrefix = prefix || STORAGE_KEYS.PREFIX;
    return `${actualPrefix}${dataType}`;
}

export function parseStorageKey(key: string): { prefix: string; dataType: string; isChunk: boolean; chunkIndex?: number } {
    const prefixes = [STORAGE_KEYS.PREFIX, STORAGE_KEYS.DIRECT_PREFIX, STORAGE_KEYS.CACHE_PREFIX];
...
export function shouldUseChunking(dataSize: number, threshold: number = STORAGE_LIMITS.CHUNK_SIZE): boolean {
    return dataSize > threshold;
}

export function validateDataType(dataType: string): boolean {
    return /^[a-zA-Z0-9_-]+$/.test(dataType) && dataType.length <= 50;
}
...
export class DataStorageManager {
                ...
                switch(dataType) {
                    ...
                    default:
                        if (shouldUseChunking(dataSize, mergedOptions.chunkSize)) {
                            ...
                            result = await this.saveGenericData(dataType, data, mergedOptions);
                        }
                        break;
                }
    ...
    async loadWithQueue(dataType: string, options: LoadOptions = {}): Promise<any> {
        ...
        return await this.dataManager.asyncQueue.enqueue(async () => {
            try {
                ...
                switch(dataType) {
                    ...
                    default:
                        ...
                        if (metadata) {
                            ...
                            result = await this.loadGenericData(dataType, mergedOptions);
                        }
                        break;
                }
                ...
                // キャッシュに保存
                if (mergedOptions.useCache && result !== null) {
                    const cacheKey = this.generateCacheKey(dataType, mergedOptions);
                    const tags = [`dataType:${dataType}`, ...(mergedOptions.tags || [])];
                    this.dataManager.cache.set(cacheKey, result, { 
                ...
                this.errorHandler.handleError(error as Error, 'DATA_LOAD_ERROR', {
                    operation: 'load',
                    dataType,
                    options: mergedOptions
                });
                ...
                if (mergedOptions.fallbackToDefault) {
                    try {
                        return await this.getDefaultData(dataType);
                    } catch (fallbackError) {
     ...
     * 汎用データの保存
     */
    private async saveGenericData(dataType: string, data: any, options: SaveOptions): Promise<SaveResult> {
     ...
     * 汎用データの読み込み
     */
    private async loadGenericData(dataType: string, options: LoadOptions): Promise<any> {
     ...
     * チャンク分割による大容量データ保存
     */
    private async saveLargeDataInChunks(dataType: string, serializedData: string, options: SaveOptions): Promise<SaveResult> {
        const chunkSize = options.chunkSize || STORAGE_LIMITS.CHUNK_SIZE;
        const chunks: string[] = [];
    ...
    private hashOptions(options: Record<string, any>): string {
        ...
        for (let i = 0; i < optionsStr.length; i++) {
            hash = ((hash << 5) + hash) + optionsStr.charCodeAt(i);
        }
     ...
     * キャッシュエントリの有効性チェック
     */
    private isCacheValid(cacheEntry: CacheEntry, maxAge?: number): boolean {
     ...
     * データ検証
     */
    private validateData(data: any, dataType: string): DataValidationResult {
    ...
    async getStorageQuota(): Promise<StorageQuota> {
        try {
            ...
            console.warn('[DataStorageManager] Failed to get storage estimate:', error);
        }
        ...
        try {
            ...
            console.warn('[DataStorageManager] Failed to estimate storage usage:', error);
        }
     ...
     * データの削除
     */
    async deleteData(dataType: string): Promise<boolean> {
        try {
            if (!validateDataType(dataType)) {
                throw new Error(`Invalid dataType: ${dataType}`);
            }

...
export class StatisticsFilterManager {
    ...
    private filterPeriods: { [key: string]: FilterPeriod };
     ...
     * フィルター期間の設定
     */
    setPeriod(period: string, customStart: Date | null = null, customEnd: Date | null = null): void {
        ...
        if (period === 'custom') {
            ...
            this.currentFilter.customStart = null;
     ...
     * フィルターされた統計データの取得
     */
    async getFilteredStatistics(): Promise<FilteredResult> {
        ...
        try {
            ...
            const result: FilteredResult = {
                ...
                metadata: {
                    totalRecords: sortedStats.sessions?.length || 0,
                    filteredAt: new Date(),
    ...
    calculateDateRange(): DateRange {
        ...
        if (this.currentFilter.period === 'custom') {
            return {
                start: this.currentFilter.customStart!,
        ...
        // デフォルト: 指定日数前から現在まで
        const period = this.filterPeriods[this.currentFilter.period];
        const start = new Date(now);
     ...
     * フィルターの適用
     */
    applyFilters(rawStats: any, timeSeriesData: TimeSeriesData, dateRange: DateRange): FilteredStatistics {
        ...
        // カテゴリフィルター
        if (!this.currentFilter.categories.includes('all')) {
            filteredStats = this.filterByCategories(filteredStats, this.currentFilter.categories);
        }
     ...
     * 日付範囲によるフィルタリング
     */
    filterByDateRange(stats: FilteredStatistics, dateRange: DateRange, timeSeriesData: TimeSeriesData): FilteredStatistics {
        const filteredStats = { ...stats };
        
        // 各統計項目を期間でフィルタリング
        if (stats.sessions) {
            filteredStats.sessions = stats.sessions.filter((session: Session) => {
        ...
        // 時系列データから集計値を再計算
        if (timeSeriesData.daily.length > 0) {
            filteredStats.periodSummary = this.calculatePeriodSummary(timeSeriesData.daily);
        }
     ...
     * ソートの適用
     */
    applySorting(stats: FilteredStatistics): FilteredStatistics {
        const sortedStats = { ...stats };
        
        if (stats.sessions && Array.isArray(stats.sessions)) {
            sortedStats.sessions = [...stats.sessions].sort((a: Session, b: Session) => {
     ...
     * 期間サマリーの計算
     */
    calculatePeriodSummary(dailyData: any[]): any {
     ...
     * サマリーの生成
     */
    generateSummary(stats: FilteredStatistics): any {
        ...
        const totalScore = sessions.reduce((sum: number, s: Session) => sum + (s.score || 0), 0);
        const totalPlayTime = sessions.reduce((sum: number, s: Session) => sum + (s.playTime || 0), 0);
        const avgAccuracy = sessions.reduce((sum: number, s: Session) => sum + (s.accuracy || 0), 0) / sessions.length;
        const maxCombo = Math.max(...sessions.map((s: Session) => s.maxCombo || 0));
    ...
    /**
     * 期間別フィルタリング
     * @param statisticsData - 統計データ
     * @param period - フィルター期間
     * @returns フィルタリングされた統計データ
     */
    filterByPeriod(statisticsData: any, period: string): any {
        try {
            ...
            const filteredSessions = statisticsData.sessions ?
                statisticsData.sessions.filter((session: Session) => {
    ...
    generateCacheKey(): string {
        ...
        const customDates = customStart && customEnd ?
     ...
     */
    clearCache(): void {
        this.filterState.cachedResults.clear();
    }
     ...
     * 利用可能な期間フィルターの取得
     */
    getAvailablePeriods(): Array<{ key: string; label: string; days: number | null }> {

...
export interface PeriodLeaderboards {
    daily?: Record<string, PeriodLeaderboard>;
...
export interface DateKeyComponents {
    year: number;
    month: number;
    day?: number;
    week?: number;
...
// ユーティリティ関数
export function isValidPeriodType(period: string): period is PeriodType {
    return PERIOD_TYPES.includes(period as PeriodType);
}

export function isValidScoreEntry(entry: any): entry is ScoreEntry {
    return entry &&
...
export function isValidLeaderboard(leaderboard: any): leaderboard is Leaderboard {
    return leaderboard &&
           Array.isArray(leaderboard.entries) &&
           typeof leaderboard.lastUpdated === 'number';
}

export function clampEntryCount(count: number, maxEntries: number = DEFAULT_MAX_ENTRIES): number {
    return Math.max(0, Math.min(count, maxEntries));
}
...
export function getUniquePlayersCount(entries: ScoreEntry[]): number {
    return new Set(entries.map(entry => entry.playerName)).size;
}

export function formatDateKey(date: Date, period: PeriodType, weekStartDay: WeekStartDay = DEFAULT_WEEK_START_DAY): string {
...
export function getWeekStart(date: Date, weekStartDay: WeekStartDay = DEFAULT_WEEK_START_DAY): Date {
...
export function parseDateKey(key: string, period: PeriodType): Date | null {
    try {
...
export class LeaderboardRankingManager {
     ...
     * リーダーボードの更新
     */
    updateLeaderboards(leaderboardKey: string, scoreEntry: ScoreEntry): RankingUpdateResult {
        try {
     ...
     * 期間別ボードの更新
     */
    private updatePeriodBoard(data: LeaderboardData, period: PeriodType, key: string, scoreEntry: ScoreEntry): void {
     ...
     * 期間統計の取得
     */
    getPeriodStats(period: PeriodType): LeaderboardStatistics {
        if (!isValidPeriodType(period)) {
     ...
     * 期間別ランキングの再計算
     */
    recalculatePeriodRankings(): void {
        const data = this.leaderboardManager.data;
     ...
     * リーダーボードのソート
     */
    sortLeaderboard(leaderboard: Leaderboard | PeriodLeaderboard): void {
     ...
     * 日付キーのフォーマット
     */
    formatDateKey(date: Date, period: PeriodType): string {
     ...
     * 現在の期間キーの取得
     */
    getCurrentPeriodKey(period: PeriodType): string {
        const now = new Date();
     ...
     * 期間の開始日を取得
     */
    getPeriodStartDate(key: string, period: PeriodType): Date | null {
        return parseDateKey(key, period);
    }
     ...
     * 効率的なランキング更新
     */
    updateRankingEfficiently(leaderboardKey: string, newEntry: ScoreEntry): boolean {
        if (!isValidScoreEntry(newEntry)) {
            return false;
        }
     ...
     * 挿入位置の検索（バイナリサーチ）
     */
    private findInsertionIndex(entries: ScoreEntry[], newEntry: ScoreEntry): number {
        let left = 0;
     ...
     * プレイヤーのランクを検索
     */
    private findPlayerRank(leaderboard: Leaderboard, playerName: string): number | undefined {
        const index = leaderboard.entries.findIndex(entry => entry.playerName === playerName);
     ...
     * リーダーボードメタデータの更新
     */
    private updateLeaderboardMetadata(leaderboard: Leaderboard): void {
     ...
     * 空の統計情報を作成
     */
    private createEmptyStats(period: PeriodType): LeaderboardStatistics {
        return {
            totalPlayers: 0,
            totalScores: 0,
            averageScore: 0,
     ...
     * ランキングデータの検証
     */
    validateLeaderboard(leaderboard: Leaderboard): boolean {
        return isValidLeaderboard(leaderboard) &&
     ...
     * 期間範囲の取得
     */
    getPeriodRange(period: PeriodType, date: Date = new Date()): PeriodRange {
     ...
     * 統計情報の集計
     */
    aggregateStatistics(entries: ScoreEntry[]): LeaderboardStatistics {
        const scores = entries.map(entry => entry.score);
        const uniquePlayers = getUniquePlayersCount(entries);
        return {
            totalPlayers: uniquePlayers,

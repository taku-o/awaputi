...
export class CacheSystem {
     ...
     * キャッシュをクリア
     */
    clear(prefix: string | null = null): number {
        try {
            let count = 0;
     ...
     * キャッシュの有効期限を更新
     */
    updateExpiry(key: string, ttl: number | null = null): boolean {
        try {
            if (!this.cache.has(key)) {
                return false;
            }
 ...
 */
export function getCacheSystem(options: CacheOptions = {}): CacheSystem {
    if (!instance) {

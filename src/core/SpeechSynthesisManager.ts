...
export class SpeechSynthesisManager {
    ...
    private selectedVoices: Map<string, SpeechSynthesisVoice>;
    private voicePreferences: Map<string, RegExp>;
    private languagePatterns: Map<string, RegExp>;
    private eventListeners: Map<string, Set<Function>>;
    private stats: any;
    private userPreferences: any;
    private voicesByLanguage: Map<string, any[]>;
     ...
     * 音声品質の推定
     */
    private estimateQuality(voice: SpeechSynthesisVoice): string {
        // ローカル音声は一般的に高品質
    ...
    private detectLanguage(text: string): string {
        ...
        for (const lang of this.languagePatterns.keys()) {
    ...
    private preprocessText(text: string, language: string): string {
        ...
        // カスタム辞書による置換
        for (const original of this.config.pronunciation.customDictionary.keys()) {
            const replacement = this.config.pronunciation.customDictionary.get(original);
            if (replacement) {
    ...
    private expandAbbreviations(text: string, language: string): string {
        ...
        for (const abbr of abbreviations.keys()) {
     ...
     * 発話の作成
     */
    private createUtterance(text: string, options: any = {}): SpeechSynthesisUtterance {

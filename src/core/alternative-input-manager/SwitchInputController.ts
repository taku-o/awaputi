...
export interface DetailedSwitchStats extends SwitchStats {
    scanningActive: boolean;
...
export interface SwitchInputControllerConfig {
    switchInput?: Partial<SwitchInputConfig>;
...
export interface GameEngine {
    handleBubbleClick: (bubbleData: BubbleData) => void;
}
...
export interface ScanHighlightElement extends HTMLElement {
    className: string;
...
// 型ガード
export function isHIDDevice(device: any): device is HIDDevice {
    return device &&
...
export function hasGameEngine(controller: SwitchInputController): controller is SwitchInputController & { gameEngine: GameEngine } {
    return controller.gameEngine !== undefined &&
...
export class SwitchInputController {
    ...
    private setupSwitchListeners(): void {
        ...
        // 外部スイッチデバイスサポート
        if ('navigator' in window && 'hid' in (navigator as any)) {
     ...
     * HIDスイッチサポートを設定
     */
    private async setupHIDSwitchSupport(): Promise<void> {
        try {
     ...
     * スイッチデバイス判定
     */
    private isSwitchDevice(device: HIDDevice): boolean {
     ...
     * スイッチデバイスに接続
     */
    private async connectSwitchDevice(device: HIDDevice): Promise<void> {
        try {
            await device.open();
     ...
     * HIDスイッチ入力を処理
     */
    private handleHIDSwitchInput(data: DataView): void {
        // HIDレポートからスイッチ状態を解析
     ...
     * スイッチ入力を処理
     */
    private handleSwitchInput(keyCode: string, action: 'down' | 'up'): void {
     ...
     * スイッチ活性化を処理
     */
    private handleSwitchActivation(switchAction: string): void {
     ...
     * セカンダリアクションを実行
     */
    private performSecondaryAction(): void {
        if (this.scanningState.isScanning) {
     ...
     * キャンセルアクションを実行
     */
    private performCancelAction(): void {
        if (this.scanningState.isScanning) {
            this.stopScanning();
        } else {
     ...
     * スキャニンググループを作成
     */
    private createScanningGroups(): void {
     ...
     * 位置によって要素をグループ化
     */
    private groupElementsByPosition(): HTMLElement[][] {
     ...
     * タイプによって要素をグループ化
     */
    private groupElementsByType(): HTMLElement[][] {
        const buttonGroup: HTMLElement[] = [];
     ...
     * スキャンハイライトを作成
     */
    private createScanHighlight(): void {
     ...
     * スキャン進行をスケジュール
     */
    private scheduleScanAdvance(): void {
        if (!this.scanningState.isScanning) return;
     ...
     * ゲーム要素を活性化
     */
    private activateGameElement(element: HTMLElement): void {
        // ゲーム固有の活性化ロジック
     ...
     * 代替メニューを表示
     */
    private showAlternativeMenu(): void {
     ...
     * 前のレベルに戻る
     */
    private returnToPreviousLevel(): void {
     ...
     * 視覚フィードバックを提供
     */
    private provideVisualFeedback(type: FeedbackType, data: FeedbackData): void {
        // 実装は省略（視覚エフェクト）
     ...
     * オーディオフィードバックを再生
     */
    private playAudioFeedback(type: FeedbackType): void {
        // 実装は省略（オーディオ再生）
     ...
     * ハプティックフィードバックを提供
     */
    private provideHapticFeedback(type: FeedbackType): void {
    ...
    getStats(): DetailedSwitchStats {
        return {
            ...
            successRate: this.stats.switchActivations > 0
                ? (this.stats.successfulSelections / this.stats.switchActivations) * 100
                : 0
        };
    }
     ...
     * 設定を更新
     */
    updateConfig(newConfig: SwitchInputControllerConfig): void {
     ...
     * インタラクティブ要素の数を取得
     */
    getInteractiveElementsCount(): number {
     ...
     * スキャニンググループ情報を取得
     */
    getScanningGroupsInfo(): { groupCount: number; elementsInGroups: number[] } {
        return {
            groupCount: this.scanningGroups.length,

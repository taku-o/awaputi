...
export interface DetailedVoiceStats extends VoiceStats {
    isListening: boolean;
    isInitialized: boolean;
...
export interface VoiceControllerConfig {
    voiceControl?: Partial<VoiceConfig>;
}
...
export interface VoiceUIElement extends HTMLElement {
    className: string;
...
export interface ExtendedSpeechRecognition {
    continuous: boolean;
...
export function isValidSpeechRecognitionEvent(event: any): event is SpeechRecognitionEvent {
    return event &&
        event.results &&
...
export class VoiceInputController {
     ...
     * コンテキスト固有コマンドをセットアップ
     */
    private setupContextualCommands(): void {
        Object.entries(CONTEXTUAL_COMMANDS).forEach(([context, commands]) => {
     ...
     * マイクインジケーターを作成
     */
    private createMicrophoneIndicator(): void {
     ...
     * コマンド履歴を作成
     */
    private createCommandHistory(): void {
     ...
     * 音声制御パネルを作成
     */
    private createVoiceControlPanel(): void {
    ...
    private handleSpeechResult(event: SpeechRecognitionEvent): void {
        if (!isValidSpeechRecognitionEvent(event)) return;
     ...
     * テキストをコマンドにマッピング
     */
    private mapTranscriptToCommand(transcript: string): CommandAction | null {
        const normalizedTranscript = transcript.toLowerCase().trim();

        // 基本コマンドをチェック
        let foundCommand: CommandAction | null = null;
        this.voiceConfig.commands.forEach((command, phrase) => {
            if (!foundCommand && normalizedTranscript.includes(phrase.toLowerCase())) {
                foundCommand = command as CommandAction;
            }
        });
     ...
     * 部分マッチングでコマンドを検索
     */
    private findPartialMatch(transcript: string): CommandAction | null {
        const allCommands = new Map<string, CommandAction>();
        ...
        let matchedCommand: CommandAction | null = null;
        allCommands.forEach((command, phrase) => {
            if (!matchedCommand && this.calculateSimilarity(transcript, phrase.toLowerCase()) > DEFAULT_SIMILARITY_THRESHOLD) {
     ...
     * マイクインジケーターを更新
     */
    private updateMicrophoneIndicator(isListening: boolean): void {
     ...
     * 現在の音声状態を取得
     */
    getVoiceState(): Readonly<VoiceState> {
        return {
            ...this.voiceState,
     ...
     * 音声を選択
     */
    selectVoice(voiceName: string): boolean {
     ...
     * カスタムコマンドハンドラーを追加
     */
    addCommandHandler(command: CommandAction, handler: CommandHandler): void {
     ...
     * コマンドハンドラーを削除
     */
    removeCommandHandler(command: CommandAction): boolean {

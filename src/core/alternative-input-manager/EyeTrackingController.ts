...
export interface GazeData extends GazePoint {
    timestamp: number;
}

export interface CalibrationPoint extends GazePoint {
    id?: string;
}
...
export interface DetailedEyeTrackingStats extends EyeTrackingStats {
    isCalibrated: boolean;
...
export interface ElementActivationData {
    element: HTMLElement;
    position: GazePoint;
}
...
// WebGazer関連型定義
export interface WebGazerPrediction extends GazePoint {
    eyeFeatures?: {
        blink?: boolean;
        leftEye?: EyeFeature;
        rightEye?: EyeFeature;
...
export interface WebGazerInstance {
    setGazeListener: (callback: GazeListenerCallback) => WebGazerInstance;
...
// 型ガード
export function isValidGazePoint(point: any): point is GazePoint {
    return point &&
...
export function hasWebGazer(): boolean {
...
export class EyeTrackingController {
    ...
    constructor() {
        ...
        this.eyeTrackingConfig = {
            ...
            gazeTolerance: DEFAULT_GAZE_TOLERANCE,
     ...
     * 視線インターフェースを作成
     */
    private createGazeInterface(): void {
     ...
     * 滞留インジケーターを作成
     */
    private createDwellIndicator(): void {
     ...
     * 視線データを処理
     */
    private handleGazeData(data: WebGazerPrediction, clock: number): void {
        if (!isWebGazerPrediction(data) || !this.eyeTrackingConfig.enabled) return;
     ...
     * 視線データを平滑化
     */
    private smoothGazeData(gazeData: GazePoint): GazePoint {
     ...
     * 視線履歴を更新
     */
    private updateGazeHistory(gazeData: GazePoint, timestamp: number): void {
        const gazeEntry: GazeData = {
            x: gazeData.x,
            y: gazeData.y,
            timestamp: timestamp
        };
        ...
        // 履歴サイズを制限
        if (this.eyeState.gazeHistory.length > GAZE_HISTORY_LIMIT) {
     ...
     * 滞留を検出
     */
    private detectDwelling(gazeData: GazePoint, timestamp: number): void {
     ...
     * 視線要素を活性化
     */
    private activateGazeElement(element: HTMLElement, gazeData: GazePoint): void {
        // クリック可能要素の判定
        if (isClickableElement(element)) {
            ...
            this.provideFeedback('element_activated', {
                element: element,
                position: gazeData
            });
        }
     ...
     * まばたき検出
     */
    private detectBlinking(data: WebGazerPrediction, timestamp: number): void {
     ...
     * ダブルまばたきアクションを処理
     */
    private handleDoubleBlinkAction(): void {
    ...
    async startCalibration(): Promise<CalibrationResult> {
        ...
        try {
            ...
            return { success: this.eyeState.isCalibrated, accuracy };
        } catch (error) {
            this.hideCalibrationInterface();
     ...
     * キャリブレーションシーケンスを実行
     */
    private async performCalibrationSequence(): Promise<void> {
     ...
     * キャリブレーション精度をテスト
     */
    private async testCalibrationAccuracy(): Promise<number> {
        ...
        return accuracyResults.length > 0
            ? accuracyResults.reduce((sum, acc) => sum + acc, 0) / accuracyResults.length
            : 0;
    }
     ...
     * 統計情報を取得
     */
    getStats(): DetailedEyeTrackingStats {
        return {
            ...this.stats,
     ...
     * 現在の視線状態を取得
     */
    getEyeState(): Readonly<EyeState> {
        return {
            ...this.eyeState,
            currentGaze: { ...this.eyeState.currentGaze },
            gazeHistory: [...this.eyeState.gazeHistory],
            calibrationData: {
                ...this.eyeState.calibrationData,
    ...
    resetCalibration(): void {
        this.eyeState.isCalibrated = false;
        this.eyeState.calibrationData = {
            points: [],
            accuracy: 0,
            completed: false
     ...
     * 視線履歴をクリア
     */
    clearGazeHistory(): void {
        this.eyeState.gazeHistory = [];

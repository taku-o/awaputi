...
export interface GestureBufferEntry {
    position: Position3D;
    timestamp: number;
}

export interface GesturePattern {
    pattern: GesturePatternType;
    threshold: number;
    duration: number;
}
...
export interface HeadTrackingStats {
...
export interface DetailedHeadTrackingStats extends HeadTrackingStats {
    isCalibrated: boolean;
    isTracking: boolean;
    currentPosition: Position3D;
    neutralPosition: Position3D | null;
...
export interface NavigationFeedbackData extends FeedbackData {
...
export interface TrackingUIElement extends HTMLElement {
    className: string;
...
// 型ガード
export function isValidHeadPosition(position: any): position is Position3D {
    return position &&
...
export function hasMediaPipeGlobals(): boolean {
    return typeof window !== 'undefined' &&
        'FaceMesh' in window &&
        'Camera' in window;
}

export function isValidGestureEvent(event: any): event is GestureEvent {
    return event &&
...
declare global {
    interface Window {
        ...
        Camera: {
            new (videoElement: HTMLVideoElement, options: CameraOptions): MediaPipeCamera;
        };
    }
}
...
export class HeadTrackingController {
     ...
     * ヘッドトラッキングをセットアップ
     */
    private setupHeadTracking(): void {
        if (!this.videoElement || !this.faceMeshLoaded || !hasMediaPipeGlobals()) return;
     ...
     * Face Mesh結果を処理
     */
    private processFaceMeshResults(results: MediaPipeFaceMeshResults): void {
     ...
     * ランドマークから頭部位置を計算
     */
    private calculateHeadPosition(landmarks: FaceLandmark[]): HeadPosition {
     ...
     * 頭部位置を平滑化
     */
    private smoothHeadPosition(position: HeadPosition): Position3D {
     ...
     * ジェスチャー認識を処理
     */
    private processGestureRecognition(position: Position3D): void {
        const now = Date.now();
        ...
        // 各ジェスチャーパターンをチェック
        this.gestureRecognition.gestures.forEach((gestureConfig, gestureName) => {
     ...
     * 振動パターンを検出
     */
    private detectOscillation(data: GestureBufferEntry[], axis: keyof Position3D, threshold: number): boolean {
        if (data.length < MIN_OSCILLATION_DATA_POINTS) return false;
     ...
     * 傾きパターンを検出
     */
    private detectTilt(data: GestureBufferEntry[], axis: keyof Position3D, threshold: number): boolean {
        if (data.length < MIN_GESTURE_DATA_POINTS) return false;
     ...
     * ジェスチャー認識を処理
     */
    private handleGestureRecognition(gestureName: string): void {
        // 同じジェスチャーの連続認識を防ぐ
        ...
        // 履歴サイズを制限
        if (this.gestureRecognition.recognizedGestures.length > MAX_GESTURE_HISTORY) {
     ...
     * トラッキングインターフェースを作成
     */
    private createTrackingInterface(): void {
     ...
     * 頭部ポインターを作成
     */
    private createHeadPointer(): void {
        if (this.headPointer) {
     ...
     * トラッキング情報表示を作成
     */
    private createTrackingInfo(): void {
     ...
     */
    private createCalibrationInterface(): void {
     ...
     * 頭部ポインターを更新
     */
    private updateHeadPointer(position: Position3D): void {
     ...
     * トラッキング情報を更新
     */
    private updateTrackingInfo(): void {
     ...
     * ナビゲーションをシミュレート
     */
    private simulateNavigation(direction: NavigationDirection): void {
        const key = NAVIGATION_KEY_MAP[direction];
        if (key) {
     ...
     * ヘッドキャリブレーションを実行
     */
    private async performHeadCalibration(): Promise<void> {
     ...
     * 統計を更新
     */
    private updateStats(position: Position3D): void {
        // 平均位置を更新
     ...
     * 統計情報を取得
     */
    getStats(): DetailedHeadTrackingStats {
        return {
            ...this.stats,
     ...
     * 現在の頭部状態を取得
     */
    getHeadState(): Readonly<HeadState> {
        return {
            ...this.headState,
            currentPosition: { ...this.headState.currentPosition },
            neutralPosition: this.headState.neutralPosition 
     ...
     * ジェスチャーコールバックを追加
     */
    addGestureCallback(gestureName: string, callback: GestureCallback): void {
        this.gestureRecognition.gestureCallbacks.set(gestureName, callback);
    }

    /**
     * ジェスチャーコールバックを削除
     */
    removeGestureCallback(gestureName: string): boolean {
        return this.gestureRecognition.gestureCallbacks.delete(gestureName);
    }

    /**
     * カスタムジェスチャーパターンを追加
     */
    addGesturePattern(gestureName: string, pattern: GesturePattern): void {
        this.gestureRecognition.gestures.set(gestureName, pattern);
    }

    /**
     * ジェスチャーパターンを削除
     */
    removeGesturePattern(gestureName: string): boolean {
        return this.gestureRecognition.gestures.delete(gestureName);
    }

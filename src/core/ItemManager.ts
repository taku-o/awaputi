...
export class ItemManager {
     ...
     */
    useRevival(): boolean {
        if (!this.config.enableRevivalItems) {
            return false;
        }
        ...
        // 復活エフェクトを再生
        if (this.gameEngine && this.gameEngine.effectManager) {
            this.gameEngine.effectManager.createRevivalEffect();
        }
    ...
    private applyItemEffect(effect: ItemEffect): void {
        switch (effect.type) {
            ...
            case 'time_extend':
                ...
                // if (this.gameEngine && this.gameEngine.timeManager) {
                //     this.gameEngine.timeManager.addTime(effect.value);
                // }
     ...
     * アイテム効果を削除
     */
    private removeItemEffect(effectType: string): void {

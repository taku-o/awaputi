...
export class OptimizedTranslationLoader {
    ...
    async loadLanguage(language, options = {}) {
        ...
        try {
            ...
            this.loadingPromises.delete(language);
            getErrorHandler().handleError(error, 'TRANSLATION_LOAD_ERROR', {
     ...
     * 最適化された言語読み込み（内部メソッド）
     */
    async _loadLanguageOptimized(language, priority, options) {
     ...
     * 並列読み込み
     */
    async _loadParallel(language, files, options) {
        const translations = {};
     ...
     * 順次読み込み
     */
    async _loadSequential(language, files, options) {
        const translations = {};
     ...
     * チャンク読み込み
     */
    async _loadChunked(language, files, options) {
    ...
    async preloadLanguages(languages, options = {}) {
        ...
        console.log('Preload completed', {
            ...
            successful: preloadResults.filter(r => r.value?.success).length,
            failed: preloadResults.filter(r => !r.value?.success).length
        });

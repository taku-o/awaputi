...
export class FontErrorIntegration {
    ...
    private _registerFontErrorCategories(): void {
    ...
    private _setupCustomErrorHandling(): void {
        // 元のhandleErrorメソッドをラップ
        const originalHandleError = this.errorHandler.handleError.bind(this.errorHandler);
        
        this.errorHandler.handleError = (error: Error, context: string = 'UNKNOWN', metadata: FontErrorMetadata = {}) => {
            // フォント関連エラーかチェック
    ...
    private _isFontRelatedError(error: Error, context: string, metadata: FontErrorMetadata): boolean {

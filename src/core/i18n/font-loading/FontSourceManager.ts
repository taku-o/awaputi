...
export interface IFontSource {
    load(fontFamily: string, options?: FontLoadOptions): Promise<any>;
    isAvailable(): boolean;
}
...
export class FontSourceManager {
    ...
    getLoadAttemptHistory(sourceName: string, fontFamily: string | null = null): LoadAttempt | Record<string, LoadAttempt> | null {
        if (fontFamily) {
            const attemptKey = `${sourceName}:${fontFamily}`;
    ...
    clearLoadHistory(sourceName: string | null = null): void {
        if (sourceName) {

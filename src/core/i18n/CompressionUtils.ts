...
export interface BalancedCompressionResult {
    data: string;
    compressed: boolean;
    level: CompressionLevel;
    patterns: Map<string, string>;
...
export interface MaximumCompressionResult {
    data: string;
    compressed: boolean;
    level: CompressionLevel;
    patterns: Map<string, string>;
...
export interface ByteOptimizationResult {
    data: string;
...
export interface DetailedCompressionStats extends CompressionStats {
...
export class CompressionUtils {
    ...
    compress(data: any, options: CompressionOptions = {}): CompressionResult {
        ...
        try {
            const {
                ...
                customPatterns = new Map<string, string>()
            } = options;
     ...
     * データを展開
     */
    decompress(compressedData: CompressionResult): any {
        const startTime = performance.now();
        try {
     ...
     * 統計情報を取得
     */
    getStats(): DetailedCompressionStats {
        return {
            ...this.stats,
     ...
     * カスタムパターンを追加
     */
    addCustomPattern(pattern: string, replacement: string): void {
     ...
     * カスタムパターンを削除
     */
    removeCustomPattern(pattern: string): boolean {

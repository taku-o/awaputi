...
export class OptimizedTranslationLoader {
    ...
    async preloadLanguages(languages: string[], options: LoadOptions = {}): Promise<Map<string, any>> {
        ...
        console.log('Preload completed', {
            ...
            successful: preloadResults.filter(r => r.status === 'fulfilled' && (r.value as PreloadResult)?.success).length,
            failed: preloadResults.filter(r => r.status === 'rejected' || !(r.value as PreloadResult)?.success).length
        });
        ...
        const stats: PerformanceStats = {
            ...
            network: {
                ...
                averageBytesPerRequest: this.networkRequests > 0
                    ? Math.round(this.totalBytesLoaded / this.networkRequests)
                    : 0
            },
            loadTimes: {},
            memory: {
    ...
    cleanup(): void {
        ...
        console.log('OptimizedTranslationLoader cleaned up');
    }
}

// シングルトンインスタンス
let optimizedTranslationLoaderInstance: OptimizedTranslationLoader | null = null;

/**
 * OptimizedTranslationLoaderのシングルトンインスタンスを取得
 */
export function getOptimizedTranslationLoader(): OptimizedTranslationLoader {
    if (!optimizedTranslationLoaderInstance) {
        optimizedTranslationLoaderInstance = new OptimizedTranslationLoader();
    }
    return optimizedTranslationLoaderInstance;
}
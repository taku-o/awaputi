...
export interface FormatterRegistry { [name: string]: FormatterInterface;
     ...
     * フォーマッターを削除
     */
    removeFormatter(name: string): boolean { return this.formatters.delete(name);
    
    /**
     * 利用可能なフォーマッターを取得
     */
    getAvailableFormatters(): string[] { return Array.from(this.formatters.keys()));
     ...
     * 地域設定情報を取得
     */ : undefined
    getRegionalSettings(language: string, region: string | null = null): RegionalSettings { return this.regionalSettingsManager.getCompleteSettings(language, region);
...
export class NumberFormatter implements NumberFormatterInterface { format(value: any, language: string, region?: string | null): string {
    ...
    formatWithSettings(value: any, numberFormatSettings: NumberFormatSettings | null, format: string = 'default': string { try {'
            ...
            // 小数点記号の置換
            if (parts.length > 1) { formatted = parts[0] + decimalSeparator + parts[1] } else { formatted = parts[0] }
 ...
 * 日付フォーマッター
 */
export class DateFormatter implements DateFormatterInterface { format(value: any, language: string, region?: string | null, options: DateTimeFormatOptions = {): string {
        try {
            ...
            return new Intl.DateTimeFormat(locale, formatOptions).format(date) } catch (error) {
     ...
     * 地域設定を使用した日付フォーマット'
     */''
    formatWithSettings(value: any, dateFormatSettings: DateFormatSettings | null, format: string = 'medium', regionInfo: RegionInfo = {}: string { try {
            const date = new Date(value);
            if(isNaN(date.getTime()) {''
                return String(value);
            ';'
        ...
        } catch (error) { console.warn(`Date formatting with settings failed:`, error);
            return String(value);
    /**
     * 地域設定を使用した時刻フォーマット'
     */''
    formatTimeWithSettings(value: any, timeFormatSettings: TimeFormatSettings | null, format: string = 'medium', regionInfo: RegionInfo = { }: string { try {
            const date = new Date(value);
            if(isNaN(date.getTime()) {''
                return String(value);
            ';'
            ...
            return formatted;
        } catch (error) { console.warn(`Time formatting with settings failed:`, error);
            return String(value);
    /**
     * 月名を取得'
     */''
...
export class CurrencyFormatter implements CurrencyFormatterInterface { format(value: any, language: string, region?: string | null, currency: string | null = null): string {
        try {
            ...
            return new Intl.NumberFormat(locale, { ')'
                style: 'currency'),
                currency: currencyCode)).format(number  } catch (error) {
 ...
 * 相対時間フォーマッター
 */
export class RelativeTimeFormatter implements RelativeTimeFormatterInterface { format(value: any, language: string, region?: string | null): string {
        try {
            const date = new Date(value);
            const now = new Date();
            ...
            ,
                ...
                return rtf.format(seconds, 'second); else if (absDiff < 3600000) { // 1時間未満'
...
export class ListFormatter implements ListFormatterInterface { format(value: any, language: string, region?: string | null, options: ListFormatOptions = {): string {
        try {
            ...
            return new Intl.ListFormat(locale, formatOptions).format(value) } catch (error) {
...
export class PluralFormatter implements PluralFormatterInterface { format(text: any, count: any, language: string): string {
    ...
    getPluralRules(language: string): Intl.PluralRules | PluralRuleFallback { try {
            return new Intl.PluralRules(language) } catch (error) {

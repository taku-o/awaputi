...
export interface NumberTestCase {
    value: number;
    language: string;
    expected: string;
}
...
export interface RegionalTestCase {
    language: string;
    region: string;
}

export interface FormatStringTestCase {
    text: string;
...
export interface MultiLanguageResults {
    [language: string]: {
        number: string;
        currency: string;
        date: string;
...
export interface TestResultWithReport {
    results: TestResult[];
    report: TestReport;
}
...
export class RegionalFormattingTest {
    ...
    async runAllTests(): Promise<TestResult[]> {
        ...
        try {
            ...
            await this.testAdvancedFormatStrings();
     ...
     * 数値フォーマットテスト
     */
    async testNumberFormatting(): Promise<void> {
     ...
     * 日付フォーマットテスト
     */
    async testDateFormatting(): Promise<void> {
     ...
     * 時刻フォーマットテスト
     */
    async testTimeFormatting(): Promise<void> {
     ...
     * 複数言語同時テスト
     */
    async testMultipleLanguages(): Promise<void> {
        ...
        for (const language of languages) {
            try {
                ...
                results[language] = {
                    number: numberFormatter.formatWithSettings(testValue, settings.numberFormat),
     ...
     * 地域設定統合テスト
     */
    async testRegionalSettingsIntegration(): Promise<void> {
     ...
     * 高度なフォーマット文字列テスト
     */
    async testAdvancedFormatStrings(): Promise<void> {
     ...
     * テスト結果を追加
     */
    private addTestResult(category: string, testName: string, passed: boolean, actual: string, expected: string): void {
    ...
    cleanup(): void {
        this.testResults = [];
        console.log('RegionalFormattingTest cleaned up');
    }
}
 ...
 * テストを実行するヘルパー関数
 */
export async function runRegionalFormattingTests(): Promise<TestResultWithReport> {
...
// シングルトンインスタンス
let regionalFormattingTestInstance: RegionalFormattingTest | null = null;

/**
 * RegionalFormattingTestのシングルトンインスタンスを取得
 */
export function getRegionalFormattingTest(): RegionalFormattingTest {
    if (!regionalFormattingTestInstance) {
        regionalFormattingTestInstance = new RegionalFormattingTest();
    }
    return regionalFormattingTestInstance;
}
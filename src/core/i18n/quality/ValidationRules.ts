...
// 型定義
export interface ValidationContext { targetLanguage?: string;
...
export interface LengthDetails { sourceLength: number,
...
export interface ConsistencyIssue { type: string,
    message: string;
    examples?: string[];
    suggestion?: string; ,}

export interface CapitalizationResult { passed: boolean,
    type?: string;
    message?: string;
    examples?: string[];
    suggestion?: string; }
...
export interface PolitenessResult { passed: boolean;
    type?: string;
    message?: string;
    suggestion?: string; }
';
 ...
 * パラメータ整合性検証ルール
 */
export class ParameterConsistencyRule implements IValidationRule { public name: string,
     ...
     * パラメータを抽出
     */)
    extractParameters(text: string): string[] { const patterns = [ }]
            /\{([^}]+)\}/g,     // {param}
            /\{\{([^}]+)\}\}/g, // {{param}
            /%s/g,              // %s;
            /%d/g,              // %d;
 ...
 * 翻訳長制限検証ルール
 */
export class LengthValidationRule implements IValidationRule { public name: string,
    ...
    constructor(''';
        ...
        // 言語別の長さ許容率
        this.lengthTolerances = {' }', 'en': { min: 0.7, max: 1.5, name: '英語' ,},'', 'zh-CN': { min: 0.5, max: 1.2, name: '中国語簡体字' ,},'', 'zh-TW': { min: 0.5, max: 1.2, name: '中国語繁体字' ,},'', 'ko': { min: 0.8, max: 1.4, name: '韓国語' ,},'', 'default': { min: 0.6, max: 1.8, name: 'その他' ,}
    ...
    validate(sourceText: string, translationText: string, context: ValidationContext = { ): ValidationResult {''
        ...
        if(translationLength < minLength) { return {  };
                ...
                details: { sourceLength: sourceLength;
 ...
 * フォーマット検証ルール
 */
export class FormatValidationRule implements IValidationRule { public name: string,
    ...
    extractHtmlTags(text: string): string[] { const htmlTagPattern = /<\/? [a-zA-Z][^>]*>/g;
        ...
        return matches.map(tag => { );''
            ...
            return normalized.toLowerCase();).sort();
    }
    
    /**
     * マークダウン要素を抽出'
     */ : undefined''
    extractMarkdownElements(text: string): MarkdownElement[] { const patterns = [' }]'
            { type: 'bold', pattern: /\*\*[^*\n]+\*\*/g ,},''
            { type: 'italic', pattern: /\*[^*\n]+\*/g ,},''
            { type: 'code', pattern: /`[^`\n]+`/g ,},''
            { type: 'code_block', pattern: /```[\s\S]*? ```/g ,}, : undefined''
            { type: 'link', pattern: /\[[^\]]+\]\([^)]+\')/g ,},''
    ...
    validate(sourceText: string, translationText: string, context: ValidationContext = { ): ValidationResult {''
        ...
        } else {  const missingTags = sourceHtmlTags.filter(tag => !translationHtmlTags.includes(tag);
 ...
 * 文化的適切性検証ルール
 */
export class CulturalAppropriatenessRule implements IValidationRule { public name: string,
     ...
     * 新しい文化的ルールを追加'
     */''
    addCulturalRule(language: string, type: 'inappropriate' | 'sensitive', rule: CulturalRule): void { if (!this.culturalRules[language]) { }
            this.culturalRules[language] = { inappropriate: [], sensitive: [] ,}
        
        if (!this.culturalRules[language][type]) { this.culturalRules[language][type] = []; }
 ...
 * 翻訳完成度検証ルール
 */
export class CompletenessValidationRule implements IValidationRule { public name: string,
     ...
     * 未翻訳マーカーを追加
     */'
    addUntranslatedMarker(marker: string): void { if(!this.untranslatedMarkers.includes(marker) {''
 ...
 * 翻訳一貫性検証ルール
 */
export class ConsistencyValidationRule implements IValidationRule { public name: string,
    ...
    validate(sourceText: string, translationText: string, context: ValidationContext = { ): ValidationResult {''
        ...
        if (!capitalizationResult.passed) { issues.push(capitalizationResult); }
        
        // 句読点の一貫性チェック
        const punctuationResult = this.checkPunctuationConsistency(sourceText, translationText, targetLanguage);
        if (!punctuationResult.passed && punctuationResult.issues) { issues.push(...punctuationResult.issues);
...
export class ValidationRuleFactory {
     ...
     * 検証ルールを作成
     */
    createRule(ruleType: string): IValidationRule { const RuleClass = this.ruleClasses.get(ruleType);
        if (!RuleClass) { }
     ...
     * すべての利用可能なルールタイプを取得
     */
    getAvailableRuleTypes(): string[] { return Array.from(this.ruleClasses.keys(); }
    
    /**
     * 新しいルールクラスを登録
     */
    registerRule(ruleType: string, RuleClass: ValidationRuleConstructor): void { this.ruleClasses.set(ruleType, RuleClass); }
}
';
// デフォルトファクトリーインスタンス

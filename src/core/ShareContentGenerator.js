...
export class ShareContentGenerator {
    constructor(localizationManager, socialI18nManager = null) {
        ...
        this.platformLimits = {
            twitter: {
                maxLength: 280,
                hashtagLimit: 2,
                urlLength: 23 // Twitter URL短縮の固定長
            },
            facebook: {
                maxLength: 63206,
                titleMaxLength: 100,
    ...
    generateAchievementMessage(achievementData, platform = 'generic', options = {}) {
        try {
            ...
            // レア実績の特別処理
            if (achievementData.rarity && achievementData.rarity === 'legendary') {
                message = this.addLegendaryEmojis(message, language);
            }
    ...
    generateFallbackMessage(type, data, platform) {
        ...
        const fallbackMessages = {
            score: {
                ja: `スコア: ${data.score || 0}点`,
                en: `Score: ${data.score || 0} points`,
    ...
    getStats() {
        return {
            ...
            truncationRate: this.stats.generated > 0 ? 

...
export function isElementInViewport(bounds: ElementBounds, viewport: ViewportBounds): boolean {
...
export function createCustomEffectStyles(
    ...
    const baseStyle: CustomEffectStyle = {
        position: 'absolute',
        left: `${bounds.left}px`,
        top: `${bounds.top}px`,
...
export function getEffectCSS(effectType: EffectType, config: FocusRingConfig): string {
    const animation = CSS_ANIMATIONS[effectType] || CSS_ANIMATIONS.pulse;
...
export function clearTimerIfExists(timers: Map<string, number>, timerType: TimerType): void {
    const timer = timers.get(timerType);
    if (timer) {
        clearTimeout(timer);
...
export class FocusEffectRenderer {
    ...
    positionFocusRing(element: HTMLElement): void {
        ...
        const timer = window.setTimeout(() => {
            ring.classList.remove(CSS_CLASSES.ACTIVE);
    ...
    updateFocusOverlay(element: HTMLElement): void {
        ...
        setTimeout(() => {
            overlay.classList.remove(CSS_CLASSES.ACTIVE);
        }, fadeDelay);
    }
     ...
     * パフォーマンス最適化のための描画制御
     */
    optimizeRendering(): RenderingOptimizationResult {
        // 要素が画面外にある場合はスキップ
        if (this.state.currentFocusElement) {
    ...
    private isLandmarkElement(element: HTMLElement): boolean {
        const tagName = element.tagName.toLowerCase() as LandmarkType;
    ...
    private isGroupElement(element: HTMLElement): boolean {
        const tagName = element.tagName.toLowerCase() as GroupType;
        const role = element.getAttribute('role');
     ...
     * カスタムエフェクトのクリア
     */
    private clearCustomEffects(): void {
        document.querySelectorAll(`.${CSS_CLASSES.CUSTOM_EFFECT}`).forEach(el => {
            if (el.parentNode) {
    ...
    private clearAllTimers(): void {
        this.state.animationTimers.forEach((timer) => {
            clearTimeout(timer);
        });

...
export class FeedbackEffectRenderer {
    ...
    createScaleEffect(options: ScaleEffectOptions): EffectResult | null {
        try {
            ...
            if (options.origin) {
                options.target.style.transformOrigin = options.origin;
            }
     ...
     * 音量ベースフィードバックのトリガー
     */
    private triggerVolumeBasedFeedback(volume: number): void {
        try {
     ...
     * エッジフィードバックのトリガー
     */
    private triggerEdgeFeedback(color: string, intensity: number): void {
        try {
     ...
     * エフェクトのクリーンアップ
     */
    cleanupEffect(effect: EffectResult): void {
        try {
            if (!effect) return;
    ...
    generateReport(): RenderReport {
        try {
            const effectTypes: Record<EffectType, EffectTypeStatus> = {
            ...
            return {
                ...
                performance: {
                    activeEffects: 0, // 実装では実際のアクティブエフェクト数を使用
    ...
    private calculateAverageFrameTime(): number {
        return this.frameCount > 0 ? this.lastFrameTime / this.frameCount : 0;
    }

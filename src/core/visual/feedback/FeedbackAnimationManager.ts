...
export interface FeedbackManagerConfig {
    animations: AnimationGlobalConfig;
...
export interface FeedbackManagerState {
    activeAnimations: Map<string, AnimationEffect>;
    animationQueue: AnimationQueueItem[];
    performance: PerformanceMetrics;
}

export interface AnimationConfig {
    flash: FlashAnimationConfig;
...
export interface FlashEffectOptions extends EffectOptions {
    backgroundOverride?: string;
    fadeOutDelay?: number;
}

export interface GlowEffectOptions extends EffectOptions {
    glowSize?: number;
    spread?: number;
    blur?: number;
}

export interface PulseEffectOptions extends EffectOptions {
    iterations?: number;
    scaleMin?: number;
    scaleMax?: number;
}

export interface RippleEffectOptions extends EffectOptions {
    startSize?: number;
    endSize?: number;
    borderStyle?: string;
    originX?: number;
...
export interface ShakeEffectOptions extends EffectOptions {
    maxDistance?: number;
    steps?: number;
...
export interface AnimationTypeStats {
    count: number;
...
export const ANIMATION_PRIORITIES: Record<Priority, number> = {
    low: 1,
    normal: 2,
    high: 3,
    critical: 4
...
export function createAnimationKeyframes(type: EffectType, options: EffectOptions): Keyframe[] {
    switch(type) {
        ...
        case 'glow':
            ...
            return [
                { boxShadow: `0 0 ${glowSize}px ${options.color}`, opacity: 0.8 },
...
export function generateShakeKeyframes(distance: number, steps: number): Keyframe[] {
...
export function calculateAnimationTiming(duration: number, easing: string): KeyframeAnimationOptions {
    return {
...
export class FeedbackAnimationManager {
     ...
     * フラッシュエフェクトの作成
     */
    createFlashEffect(options: FlashEffectOptions): AnimationEffect | null {
        try {
     ...
     * グローエフェクトの作成
     */
    createGlowEffect(options: GlowEffectOptions): AnimationEffect | null {
        try {
            const validation = validateEffectOptions(options);
            if (!validation.isValid) {
     ...
     * パルスエフェクトの作成
     */
    createPulseEffect(options: PulseEffectOptions): AnimationEffect | null {
        try {
            const validation = validateEffectOptions(options);
            if (!validation.isValid) {
            ...
            const _pulseIntensity = 0.3 + (normalizeIntensity(options.intensity) * 0.7);
     ...
     * リップルエフェクトの作成
     */
    createRippleEffect(options: RippleEffectOptions): AnimationEffect | null {
        try {
            const validation = validateEffectOptions(options);
            if (!validation.isValid) {
     ...
     * シェイクエフェクトの作成
     */
    createShakeEffect(options: ShakeEffectOptions): AnimationEffect | null {
        try {
            const validation = validateEffectOptions(options);
            if (!validation.isValid) {
     ...
     * 全アニメーションの停止とクリーンアップ
     */
    stopAllAnimations(): CleanupResult {
     ...
     * アニメーション設定の更新
     */
    updateAnimationConfig(type: EffectType, config: Partial<AnimationConfig[EffectType]>): void {
    ...
    private updatePerformanceMetrics(): void {
        this.performanceMetrics.activeCount = this.activeAnimations.size;
    ...
    getStatistics(): AnimationStatistics {
        const byType: Record<EffectType, AnimationTypeStats> = {
        ...
        // 平均値の計算
        for (const type of Object.keys(byType) as EffectType[]) {

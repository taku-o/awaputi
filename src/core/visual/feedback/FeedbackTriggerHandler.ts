...
// 定数
export const DEFAULT_GAME_EVENT_MAPPINGS: Record<GameEventType, GameEventMapping> = {
    bubblePop: {
        pattern: 'flash',
        color: '#4CAF50',
        intensity: 0.8,
...
export const VOLUME_LEVEL_MAPPINGS: Record<VolumeLevel, VolumeLevelMapping> = {
    quiet: {
        range: [0, 0.2],
        color: '#E0E0E0',
        effects: [],
...
export const TRIGGER_PRIORITIES: Record<TriggerPriority, number> = {
    low: 1,
    normal: 2,
    high: 3,
    critical: 4
...
export function getVolumeLevel(volume: number): VolumeLevel {
    for (const [level, config] of Object.entries(VOLUME_LEVEL_MAPPINGS)) {
...
export function calculateTriggerProbability(baseProb: number, intensity: number, volume: number): number {
    const intensityFactor = Math.min(2.0, Math.max(0.1, intensity));
    const volumeFactor = Math.min(1.0, Math.max(0.1, volume));
    return Math.min(1.0, baseProb * intensityFactor * volumeFactor);
}

export function selectRandomEdge(): string {
    return EDGE_TYPES[Math.floor(Math.random() * EDGE_TYPES.length)];
}

export function normalizeIntensity(intensity: number, globalIntensity: number): number {
    return Math.min(2.0, Math.max(0, intensity * globalIntensity));
}
...
export class FeedbackTriggerHandler {
    ...
    constructor(mainController: VisualFeedbackManager) {
        ...
        // イベントタイプごとの統計初期化
        Object.values(DEFAULT_GAME_EVENT_MAPPINGS).forEach((_, key) => {
    ...
    setupEventListeners(): void {
        try {
            ...
            getErrorHandler().handleError(error as Error, 'TRIGGER_HANDLER_ERROR', {
     ...
     * ゲームイベントフィードバックのトリガー
     */
    private triggerGameEventFeedback(eventType: GameEventType, eventData: GameEventData): void {
        try {
            ...
            // イベントフィルタリング
            if (!this.shouldTriggerEvent(eventType, eventData)) {
                return;
            }
            ...
            const feedbackOptions: VisualFeedbackOptions = {
                ...
                intensity,
                duration: finalMapping.duration || this.config.feedbackTypes[finalMapping.pattern]?.duration || 500,
     ...
     * イベントトリガーの可否判定
     */
    private shouldTriggerEvent(eventType: GameEventType, eventData: GameEventData): boolean {
        // ユーザー設定のフィルタリング
        ...
        if (this.userPreferences.filterSettings.eventWhitelist.length > 0 &&
            !this.userPreferences.filterSettings.eventWhitelist.includes(eventType)) {
            return false;
        }
     ...
     * フィードバックターゲットの選択
     */
    private selectFeedbackTarget(eventType: GameEventType, eventData: GameEventData): TargetSelectionResult {
        try {
            ...
            switch (eventType) {
                case 'bubblePop':
                    element = eventData?.bubble || this.feedbackElements.get('game-area');
    ...
    triggerVolumeBasedFeedback(volume: number): void {
        try {
            ...
            if (!volumeConfig.effects.length) {
                return;
            }
     ...
     * 統計の更新
     */
    private updateStatistics(eventType: GameEventType, options: VisualFeedbackOptions, startTime: number): void {
    ...
    removeEventListeners(): void {
        try {
            ...
            for (const [eventType, handler] of this.eventListenerState.handlers) {
            ...
            this.eventListenerState.active = false;
    ...
    destroy(): void {
        ...
        this.feedbackElements = null as any;
        this.feedbackContainer = null as any;

...
export const DEFAULT_MOTION_CONFIG: Partial<MotionConfig> = {
...
export function isValidMotionLevel(level: string): level is MotionLevel {
    return MOTION_LEVEL_HIERARCHY.includes(level as MotionLevel);
}

export function isValidAnimationCategory(category: string): category is AnimationCategory {
    return [
        'general', 'particles', 'camera', 'background', 'ui',
        'parallax', 'transitions', 'effects', 'gameplay'
    ].includes(category);
...
export function isAnimationData(data: any): data is AnimationData {
    return data &&
...
export function calculateRotationSpeed(rotation: number, duration: number): number {
    return Math.abs(rotation / duration);
}
...
export function clampDuration(duration: number): number {
    return Math.max(0.1, Math.min(60, duration));
}
...
export function createMotionWarningStyle(): string {
    return `
        .motion-warning {
...
export class AnimationController {
     ...
     * カテゴリ設定の適用
     */
    private applyCategorySettings(element: HTMLElement, category: string): void {
     ...
     * 段階的制御の適用
     */
    private applyGranularControls(element: HTMLElement, category: string): void {
     ...
     * 選択的軽減の適用
     */
    private applySelectiveReduction(element: HTMLElement, category: string): void {
     ...
     * アニメーションの検証と調整
     */
    private validateAndAdjustAnimation(animationData: AnimationData): void {
     ...
     * 回転速度の検証
     */
    private validateRotationSpeed(element: HTMLElement, computedStyle: CSSStyleDeclaration, animationData: AnimationData): void {
     ...
     * スケール変更の検証
     */
    private validateScaleChange(element: HTMLElement, computedStyle: CSSStyleDeclaration, animationData: AnimationData): void {
        const transform = computedStyle.transform;
     ...
     * アニメーションオブザーバーの設定
     */
    private setupAnimationObserver(animationData: AnimationData): void {
     ...
     * カテゴリ別アニメーションの一時停止
     */
    pauseAnimationsByCategory(category: string): number {
        let pausedCount = 0;
     ...
     * カテゴリ別アニメーションの再開
     */
    resumeAnimationsByCategory(category: string): number {
        let resumedCount = 0;
     ...
     * アニメーションの検索
     */
    findAnimations(predicate: (animation: AnimationData) => boolean): AnimationData[] {
     ...
     * カテゴリ別アニメーション一覧取得
     */
    getAnimationsByCategory(category: string): AnimationData[] {
        return this.findAnimations(animation => animation.category === category);
    }
    ...
    getActiveAnimations(): AnimationData[] {
        return Array.from(this.activeAnimations.values()).filter(animation => !animation.isPaused);
    }
    
    /**
     * 一時停止中のアニメーション一覧取得
     */
    getPausedAnimations(): AnimationData[] {
        return Array.from(this.activeAnimations.values()).filter(animation => animation.isPaused);
    }
    ...
    resetPerformanceStats(): void {
        ...
        this.stats.animationsPaused = 0;
        this.stats.animationsReduced = 0;
        this.stats.performanceAdjustments = 0;

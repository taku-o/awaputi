...
export interface AnimationController { pauseAnimation: (animationI;d: string) => boolean,
    adjustAnimationIntensity: (animationI;d: string, multiplier: number) => boolean,
    pauseAllAnimations: () => number ,}
}

export interface ConfigManager { setMotionLevel: (leve;l: MotionLevel) => boolean }
}
...
export const SAFETY_THRESHOLDS: SafetyThresholds = { maxRotationSpeed: 45, // degrees per second
    maxScaleChange: 1.5;
...
export function isSafetyViolation(violation: any): violation is SafetyViolation { return violation &&
...
export function clampThreshold(value: number, min: number, max: number): number { return Math.max(min, Math.min(max, value); }

export function calculateRotationSpeed(rotation: number, duration: number): number { return duration > 0 ? Math.abs(rotation / duration) : 0; }
...
export class VestibularSafetyManager {
     ...
     * 安全性監視の開始
     */
    startSafetyMonitoring(): void { if (!this.safetyMonitor.enabled || this.monitoringActive) {
            return; }
        
        this.monitoringActive = true;
        
        const monitor = (currentTime: number) => {  if (!this.monitoringActive) { }
                return; }
            }
     ...
     * 前庭安全性のチェック
     */
    checkVestibularSafety(): SafetyViolation[] { if (!this.config.vestibularSafety) {
            return []; }
        ...
        try { // アクティブアニメーションの検証
            ...
            } catch (error) { console.error('[VestibularSafetyManager] Error during safety check:', error }
     ...
     * DOM要素の安全性検証
     */
    private checkDOMElementsSafety(violations: SafetyViolation[]): void { try {
            // 点滅要素の検出
        ...
        } catch (error) { console.warn('[VestibularSafetyManager] Error checking DOM elements:', error }
    }
    ...
    private handleSafetyViolations(violations: SafetyViolation[]): void { this.safetyMonitor.violationCount += violations.length;
        ...
        violations.forEach(violation => { )
            try {);''
                ...
                } catch (error) { console.error('[VestibularSafetyManager] Error handling violation:', error }
        });
        
        // 連続違反の場合、緊急停止
        if (this.safetyMonitor.violationCount > this.safetyThresholds.emergencyStopTrigger) { this.emergencyStop(); }
    }
    ...
    private immediateStop(element: HTMLElement): void { try {'
        ...
        } catch (error) { console.error('[VestibularSafetyManager] Error in immediate stop:', error }
    }
    ...
    private reduceIntensity(element: HTMLElement, multiplier: number): void { try {'
            ...
            this.safetyMonitor.interventionCount++;''
        } catch (error) { console.error('[VestibularSafetyManager] Error reducing intensity:', error }
    }
    ...
    private addWarningOverlay(element: HTMLElement, severity: ViolationSeverity): void { ''
        ...
        } catch (error) { console.error('[VestibularSafetyManager] Error adding warning overlay:', error }
    }
    ...
    private showSafetyAlert(message: string): void { try {'
            ...
            // 自動削除
            setTimeout(() => {  if (alert.parentNode) { }
    ...
    emergencyStop()';
        ...
        } catch (error) { console.error('[VestibularSafetyManager] Error during emergency stop:', error }
    }
    ...
    private logIntervention(type: InterventionType, violation: Partial<SafetyViolation>): void { const log: SafetyInterventionLog = {''
        ...
        // ログサイズの制限
        if (this.interventionLog.length > 100) { this.interventionLog.shift(); }
    }
     ...
     * 安全性統計の取得
     */
    getSafetyStats(): SafetyStats { return { monitoring: this.safetyMonitor.enabled,
     ...
     * 介入ログの取得
     */
    getInterventionLog(): SafetyInterventionLog[] { return [...this.interventionLog];
     ...
     * アクティブアラートの取得
     */
    getActiveAlerts(): SafetyAlert[] { return Array.from(this.activeAlerts.values(); }

...
export interface AccessibilityRenderConfig {
    enabled: boolean;
...
export interface ChallengeItemElements {
    container: HTMLElement;
    header: HTMLElement;
    title: HTMLElement;
...
export const DEFAULT_ACCESSIBILITY_CONFIG: AccessibilityRenderConfig = {
    enabled: true,
    highContrast: false,
    reducedMotion: false,
...
export const ACCESSIBILITY_STYLES = `
    ...
    .challenge-item:focus {
        outline: 3px solid var(--accent-color, #4A90E2) !important;
...
// ユーティリティ関数
export function formatDeadlineTime(deadline: Date): string {
    const now = new Date();
...
export function formatRewardText(reward: ChallengeReward): string {
    const parts: string[] = [];
    ...
    if (reward.currency && reward.currency.length > 0) {
        const currencyTexts = reward.currency.map(curr => 
...
export function calculateProgressPercentage(progress: number, target: number): number {
    return Math.min(Math.round((progress / target) * 100), 100);
}

export function getDifficultyInfo(difficulty: ChallengeDifficulty): DifficultyConfig {
...
export function detectMediaQueryState(): MediaQueryState {
    return {
...
export class ChallengeUIRenderer {
     ...
     * ソートコントロールの作成
     */
    createSortControls(): HTMLElement {
        const container = document.createElement('div');
     ...
     * チャレンジリストの作成
     */
    createChallengeList(): HTMLElement {
     ...
     * 進捗セクションの作成
     */
    createProgressSection(): HTMLElement {
    ...
    private assembleElements(): void {
        const { container, header, filterControls, sortControls, loadingIndicator, 
    ...
    private createChallengeItemElements(
        ...
        difficulty.textContent = difficultyInfo.icon ? 
    ...
    applyAccessibilityStyles(): void {
        let accessibilityCSS = ACCESSIBILITY_STYLES;
    ...
    applyResponsiveStyles(): void {
        const { mobile, tablet } = this.mediaQueryState;
    ...
    private applyThemeStyles(): void {
        const { theme } = this.config.styles;
        let themeCSS = '';
    ...
    private injectStyle(id: string, css: string, scope: StyleScope): void {
        ...
        // 記録
        this.injectedStyles.set(fullId, {
            id: fullId,
            css,
            element: style,
            scope
     ...
     * 注入されたスタイルの取得
     */
    getInjectedStyles(): StyleInjection[] {
     ...
     * レンダリング設定の更新
     */
    updateRenderConfig(newConfig: Partial<UIRenderConfig>): void {

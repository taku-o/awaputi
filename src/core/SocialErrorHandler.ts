...
export class SocialErrorHandler {
    ...
    private errorNotificationCallbacks: Set<Function>;
    ...
    ): Promise<{recovered: boolean, errorInfo?: ErrorInfo, error?: any}> {
        try {
     ...
     * エラーモーダルの表示
     */
    showErrorModal(message: string, errorInfo: ErrorInfo): void {
        // 既存のモーダルがあれば削除
     ...
     * ログレベルの取得
     */
    getLogLevel(severity: string): string {
        switch(severity) {
            case 'critical':
            case 'high':
                return 'error';

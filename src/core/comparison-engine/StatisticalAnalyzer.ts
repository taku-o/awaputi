...
export interface SignificanceTestOptions {
...
export const NORMALITY_THRESHOLDS = {
    skewness: 2.0,
    kurtosis: 7.0,
    shapiroWilk: 0.05,
    normalityScore: 0.5
...
export function isValidArray(data: any): data is number[] {
    return Array.isArray(data) && data.every(isValidNumber);
}

export function validateStatisticalInput(data: any[]): number[] {
    return data.filter(isValidNumber);
}

export function calculatePooledVariance(stats1: BasicStatistics, stats2: BasicStatistics): number {
...
export function calculateStandardError(standardDeviation: number, sampleSize: number): number {
...
export function calculateCoefficientOfVariation(mean: number, standardDeviation: number): number {
    return mean !== 0 ? Math.abs(standardDeviation / mean) : 0;
}

export function createEmptyStatistics(): BasicStatistics {
    return {
        count: 0,
        sum: 0,
        mean: 0,
...
export class StatisticalAnalyzer {
     ...
     * 基本統計を計算
     */
    calculateBasicStatistics(data: any[]): BasicStatistics {
     ...
     * 歪度を計算
     */
    calculateSkewness(data: number[], mean: number, stdDev: number): number {
     ...
     * 尖度を計算
     */
    calculateKurtosis(data: number[], mean: number, stdDev: number): number {
    ...
    performSignificanceTest(
        data1: any[],
        data2: any[],
        options: SignificanceTestOptions = {}
        ...
        if (testType === 't-test' || testType === 'welch-t-test') {
            const tResult = this.performTTest(validData1, validData2, options);
     ...
     * Mann-Whitney U検定を実行
     */
    performMannWhitneyU(data1: number[], data2: number[]): MannWhitneyResult {
        const n1 = data1.length;
        const n2 = data2.length;

        const combined: RankData[] = [
            ...data1.map(value => ({ value, group: 1, rank: 0 })),
    ...
    calculateEffectSize(stats1: BasicStatistics, stats2: BasicStatistics): EffectSize {
        ...
        const glassD = stats2.standardDeviation !== 0 
            ? (stats1.mean - stats2.mean) / stats2.standardDeviation 
            : 0;
     ...
     * Hedge's gを計算
     */
    private calculateHedgesG(cohensD: number, n1: number, n2: number): number {
        const df = n1 + n2 - 2;
    ...
    testNormality(data: number[]): NormalityTestResult {
        ...
        return {
            isNormal: normalityScore > NORMALITY_THRESHOLDS.normalityScore,
     ...
     * t検定のp値を近似計算
     */
    approximateTTestPValue(t: number, df: number): number {
        if (df > 30) {
            // 大きな自由度では標準正規分布で近似
     ...
     * 標準正規分布の臨界値を取得
     */
    getZCriticalValue(confidenceLevel: number): number {
        // よく使用される信頼水準の臨界値
     ...
     * 標準正規分布の累積分布関数
     */
    standardNormalCDF(x: number): number {
        return 0.5 * (1 + this.erf(x / MATHEMATICAL_CONSTANTS.SQRT_2));
    }
     ...
     * 統計分析レポートの生成
     */
    generateAnalysisReport(data1: any[], data2: any[]): {
        data1Stats: BasicStatistics;

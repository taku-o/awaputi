...
export interface DifficultyAdjustedMetrics {
    [stageId: string]: {
...
export const DEFAULT_DIFFICULTY_ADJUSTMENTS: Record<DifficultyLevel, number> = {
    easy: 0.7,
    normal: 1.0,
    hard: 1.3,
    expert: 1.6,
...
export function extractValidPlays(plays: any[]): PlayData[] {
    return plays.filter(isValidPlayData);
}

export function getPlayScore(play: PlayData): number {
    return play.score || 0;
}

export function getPlayTime(play: PlayData): number {
    return play.completionTime || 0;
}

export function getPlayAccuracy(play: PlayData): number {
    return play.accuracy || 0;
}
...
export function calculateEfficiency(play: PlayData): number {
...
export function createEmptyStageStatistics(stageId: string): StageStatistics {
    return {
        stageId,
        playCount: 0,
...
export class StageComparisonAnalyzer {
     ...
     * ステージ推奨事項を生成
     */
    generateStageRecommendations(results: StageComparisonResult): StageRecommendation[] {
    ...
    private getDifficultyLevel(stageId: string): DifficultyLevel {
        return normalizeDifficultyLevel(stageId);
    }
    ...
    private calculateConsistency(plays: PlayData[]): number {
    ...
    private calculatePerformanceRating(plays: PlayData[]): number {
    ...
    private calculateMasteryLevel(plays: PlayData[]): number {
        if (plays.length === 0) return 0;
    ...
    private calculateScorePerSecond(plays: PlayData[]): BasicStatistics {
        const validPlays = plays.filter(p => getPlayScore(p) > 0 && getPlayTime(p) > 0);
        if (validPlays.length === 0) {
    ...
    private calculateAccuracyConsistency(plays: PlayData[]): number {
        const accuracyData = plays.map(getPlayAccuracy);
        const stats = this.statisticalAnalyzer.calculateBasicStatistics(accuracyData);
        return 1 - (stats.standardDeviation / 100); // 正規化された一貫性
    }

    private calculateTimeConsistency(plays: PlayData[]): number {
        const timeData = plays.map(getPlayTime).filter(t => t > 0);
    ...
    private calculateLearningRate(plays: PlayData[]): number {
    ...
    private identifyStrengthAreas(stats: StageStatistics): string[] {
    ...
    private identifyWeaknessAreas(stats: StageStatistics): string[] {
    ...
    private analyzeRecentTrend(plays: PlayData[]): ImprovementTrend {
    ...
    private calculateNormalizedPerformance(stats: StageStatistics, adjustmentFactor: number): NormalizedPerformance {
        const normalizedScore = stats.scores.mean / adjustmentFactor;
    ...
    private calculateSkillLevel(stats: StageStatistics, adjustmentFactor: number): SkillLevel {
    ...
    private generateComparisonSummary(stage1Id: string, stage2Id: string, comparisons: { 
        score: SignificanceTestResult; 
        time: SignificanceTestResult; 
        accuracy: SignificanceTestResult; 
    }): string[] {
        ...
        if (comparisons.time.significant && comparisons.time.testStatistic !== null) {
            const faster = comparisons.time.testStatistic < 0 ? stage1Id : stage2Id;
            summary.push(`時間: ${faster}が有意に高速`);
        }

        if (comparisons.accuracy.significant && comparisons.accuracy.testStatistic !== null) {
            const accurate = comparisons.accuracy.testStatistic > 0 ? stage1Id : stage2Id;
    ...
    private analyzeMasteryProgression(stageStats: { [stageId: string]: StageStatistics }): MasteryProgression {
        ...
        const overallMastery = masteryLevels.length > 0 
            ? masteryLevels.reduce((sum, m) => sum + m.mastery, 0) / masteryLevels.length 
            : 0;
    ...
    private groupMasteryByDifficulty(masteryLevels: Array<{ stage: string; mastery: number; difficulty: DifficultyLevel }>): { [difficulty: string]: number } {
        ...
        for (const [difficulty, masteries] of Object.entries(grouped)) {
            result[difficulty] = masteries.reduce((sum, m) => sum + m, 0) / masteries.length;
        }
        return result;
    }
     ...
     * 難易度調整係数の取得
     */
    getDifficultyAdjustments(): Record<DifficultyLevel, number> {
        return { ...this.difficultyAdjustments };
    }
}
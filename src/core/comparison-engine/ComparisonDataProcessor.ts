...
export interface NormalizedComparisonData {
    dataset1: number[];
    dataset2: number[];
...
export interface OutlierRemovalMetadata {
    threshold: number;
    timestamp: string;
...
// 定数
export const DEFAULT_CONFIG: DataProcessorConfig = {
...
export function isValidDataset(dataset: any): dataset is number[] {
    return Array.isArray(dataset) && dataset.length > 0;
}

export function isValidComparisonData(data: any): data is ComparisonInputData {
    return data &&
           typeof data === 'object' &&
...
export function filterValidNumbers(data: any[]): number[] {
    return data.filter(isValidNumber);
}

export function calculateBasicStats(data: number[]): StatisticalSummary {
    if (data.length === 0) {
...
export function calculateMedianFromSorted(sortedData: number[]): number {
...
export function calculateQuantileFromSorted(sortedData: number[], quantile: number): number {
...
export function createProcessingError(message: string, processingTime: number = 0): ProcessedComparisonData {
    return {
...
export class ComparisonDataProcessor {
     ...
     * 比較データを検証
     */
    validateComparisonData(data: any): ValidationResult {
        const errors: string[] = [];
     ...
     * 個別データセットを検証
     */
    private validateDataset(dataset: any, datasetName: string): DatasetValidationResult {
        const errors: string[] = [];
        const warnings: string[] = [];
     ...
     * Z-score正規化
     */
    private zScoreNormalization(data: number[]): number[] {
     ...
     * Min-Max正規化
     */
    private minMaxNormalization(data: number[], targetMin: number = 0, targetMax: number = 1): number[] {
     ...
     * ロバスト正規化（中央値とIQRを使用）
     */
    private robustNormalization(data: number[]): number[] {
        const sorted = [...data].sort((a, b) => a - b);
     ...
     * 個別データセットから異常値を除去
     */
    private removeDatasetOutliers(dataset: number[], threshold: number): number[] {
        if (dataset.length === 0) {
            return dataset;
        }
     ...
     * 個別データセットをサンプリング
     */
    private sampleDataset(dataset: number[], method: SamplingMethod, rate: number): number[] {
        if (dataset.length === 0) {
            return dataset;
        }
     ...
     * ランダムサンプリング
     */
    private randomSample(dataset: number[], sampleSize: number): number[] {
     ...
     * 系統サンプリング
     */
    private systematicSample(dataset: number[], sampleSize: number): number[] {
     ...
     * 層化サンプリング
     */
    private stratifiedSample(dataset: number[], sampleSize: number): number[] {
        // 簡易的な層化：値の範囲を3つに分割
     ...
     * サンプリングが必要かどうかを判定
     */
    private shouldSample(data: NormalizedComparisonData): boolean {
        return data.dataset1.length > this.config.maximumSampleSize || 
     ...
     * データ品質スコアを計算
     */
    calculateDataQuality(data: NormalizedComparisonData): number {
        try {
     ...
     * 個別データセットの品質を計算
     */
    private calculateDatasetQuality(dataset: number[]): number {
        if (dataset.length === 0) {
            return 0;
        }
     ...
     * 検証品質を計算
     */
    private calculateValidationQuality(dataset1: any[], dataset2: any[]): number {
        const quality1 = this.calculateDatasetQuality(filterValidNumbers(dataset1));
     ...
     * データ警告を生成
     */
    generateDataWarnings(data: NormalizedComparisonData): string[] {
        const warnings: string[] = [];
    ...
    calculateMedian(sortedData: number[]): number {
        return calculateMedianFromSorted(sortedData);
    }

    /**
     * 分位数を計算
     */
    calculateQuantile(sortedData: number[], quantile: number): number {
        return calculateQuantileFromSorted(sortedData, quantile);
    }

    /**
     * メトリクス計算ヘルパー
     */
    calculateMetric(data: number[], metricType: MetricType): number {
        if (data.length === 0) {
            return 0;
        }
     ...
     * データ品質レポートを生成
     */
    generateDataQualityReport(data: NormalizedComparisonData): DataQualityReport {
        const dataset1Quality = this.calculateDatasetQuality(data.dataset1);
        const dataset2Quality = this.calculateDatasetQuality(data.dataset2);
        ...
        const balanceScore = size1 > 0 && size2 > 0 
            ? Math.min(size1, size2) / Math.max(size1, size2) 
            : 0;
     ...
     * 統計サマリーを生成
     */
    generateStatisticalSummary(dataset: number[]): StatisticalSummary {
        return calculateBasicStats(dataset);
    }
    ...
    generateNormalizationResult(
        data: number[],
        method: NormalizationType,
        parameters: Partial<NormalizationParameters> = {}
    ): NormalizationResult {

...
// 型定義
export interface LineChartContext extends CanvasRenderingContext2D {
...
export interface ProcessedLineData extends LineChartData {
    x: number;
    index: number;
...
export interface LineThemeColors {
    primary: string;
    secondary: string;
    accent: string;
    dark: string;
...
export interface LineFontTheme {
    family: string;
    size: number;
    weight: string;
    color: string;
...
export interface LineAxesOptions {
    x: LineAxisOptions;
    y: LineAxisOptions;
}
...
export interface LineTickOptions {
    show?: boolean;
    count?: number;
    length?: number;
    color?: string;
...
export interface LineAxisTitleOptions {
    text?: string;
    font?: LineFontConfiguration;
    color?: string;
...
export interface LineFontConfiguration {
    family?: string;
    size?: number;
    weight?: string;
    style?: string;
...
export interface LineGridOptions {
    show?: boolean;
    color?: string;
    width?: number;
    dashArray?: number[];
...
export interface LineGridAxisOptions {
    show?: boolean;
    count?: number;
    color?: string;
    width?: number;
...
export interface SeriesLegendOptions {
    show?: boolean;
...
export interface LineInteractionOptions {
    hover?: LineHoverOptions;
    click?: LineClickOptions;
    tooltip?: LineTooltipOptions;
    crosshair?: CrosshairOptions;
}

export interface LineHoverOptions {
    enabled?: boolean;
...
export interface LineClickOptions {
    enabled?: boolean;
    selectPoint?: boolean;
    selectLine?: boolean;
...
export interface LineTooltipOptions {
    enabled?: boolean;
...
export interface LineTooltipStyle {
    background?: string;
    color?: string;
    border?: string;
    borderRadius?: number;
...
export interface LineChartArea {
    x: number;
    y: number;
    width: number;
    height: number;
...
export interface LineRenderPerformance {
    renderTime: number;
...
// 定数
export const DEFAULT_LINE_OPTIONS: Partial<LineChartOptions> = {
    padding: 20,
    showAxes: true,
    showGrid: true,
    showPoints: true,
...
export const DEFAULT_LINE_THEME: LineChartTheme = {
    colors: {
        primary: '#3B82F6',
        secondary: '#10B981',
        accent: '#F59E0B',
...
// ユーティリティ関数
export function validateLineData(data: any[]): LineValidationResult {
...
export function calculateLineLength(points: Point2D[]): number {
    let totalLength = 0;
...
export function interpolatePoint(p1: Point2D, p2: Point2D, t: number): Point2D {
    return {
        x: p1.x + (p2.x - p1.x) * t,
        y: p1.y + (p2.y - p1.y) * t
    };
...
export function calculateBezierPoint(p0: Point2D, p1: Point2D, p2: Point2D, p3: Point2D, t: number): Point2D {
    const mt = 1 - t;
    const mt2 = mt * mt;
    const mt3 = mt2 * mt;
    const t2 = t * t;
...
export function calculateTrendLine(data: ProcessedLineData[]): TrendAnalysis {
    const n = data.length;
    if (n < 2) {
        return {
            slope: 0,
...
export function formatLineValue(value: number, decimals: number = 2): string {
...
export class LineChartRenderer {
     ...
     * メインレンダリング関数
     */
    render(context: LineChartContext, data: (number | LineChartData)[], options: LineChartOptions): LineRenderResult {
        try {
     ...
     * チャートエリアの計算
     */
    private calculateChartArea(canvas: HTMLCanvasElement, options: LineChartOptions): LineChartArea {
        const padding = options.padding || DEFAULT_LINE_OPTIONS.padding!;
        return {
            x: padding,
            y: padding,
     ...
     * スケールの計算
     */
    private calculateScales(data: ProcessedLineData[], chartArea: LineChartArea): LineChartScales {
    ...
    private renderLines(context: LineChartContext, data: ProcessedLineData[], chartArea: LineChartArea, scales: LineChartScales, options: LineChartOptions): LineData[] {
        ...
        // ラインスタイルの設定
        if (lineOptions.dashArray && lineOptions.dashArray.length > 0) {
            context.setLineDash(lineOptions.dashArray);
        } else {
            context.setLineDash([]);
        }
     ...
     * チャート設定のバリデーション
     */
    validateOptions(options: Partial<LineChartOptions>): LineValidationResult {
        const errors: LineValidationError[] = [];
        const warnings: LineValidationWarning[] = [];
        ...
        if (options.lineWidth && (options.lineWidth < 0 || options.lineWidth > LINE_CONFIG.MAX_LINE_WIDTH)) {
            errors.push({
                field: 'lineWidth',
        ...
        if (options.pointRadius && (options.pointRadius < POINT_CONFIG.MIN_RADIUS || options.pointRadius > POINT_CONFIG.MAX_RADIUS)) {
            warnings.push({
                field: 'pointRadius',

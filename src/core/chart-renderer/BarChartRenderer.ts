...
export function normalizeDataValue(value: number, min: number, max: number): number {
    if (max === min) return 0.5;
    return (value - min) / (max - min);
}

export function calculateOptimalBarWidth(chartWidth: number, dataCount: number, maxWidth: number = 50): number {
    const availableWidth = chartWidth * BAR_CONFIG.DEFAULT_WIDTH_RATIO;
    const calculatedWidth = availableWidth / dataCount;
    return Math.min(calculatedWidth, maxWidth);
}

export function generateColorPalette(count: number, baseColors: string[] = DEFAULT_THEME.palette): string[] {
    const colors: string[] = [];
...
export function calculateTextWidth(text: string, font: string, context: CanvasRenderingContext2D): number {
...
export class BarChartRenderer {
     ...
     * メインレンダリング関数
     */
    render(context: ChartContext, data: (number | RawChartData)[], options: ChartOptions): ChartRenderResult {
        try {
    ...
    private calculateChartArea(canvas: HTMLCanvasElement, options: ChartOptions): ChartArea {
        const padding = options.padding || DEFAULT_CHART_OPTIONS.padding!;
        return {
            x: padding,
            y: padding,
     ...
     * スケールの計算
     */
    private calculateScales(data: ProcessedChartData[], chartArea: ChartArea): ChartScales {
     ...
     * 軸ラベルの描画
     */
    private renderAxisLabels(context: ChartContext, chartArea: ChartArea, scales: ChartScales, options: ChartOptions): void {
    ...
    private renderBars(context: ChartContext, data: ProcessedChartData[], chartArea: ChartArea, scales: ChartScales, options: ChartOptions): BarData[] {
        ...
        data.forEach((item, index) => {
            const barHeight = Math.max(BAR_CONFIG.MIN_BAR_HEIGHT, Math.abs(item.value) * scales.yScale);
            ...
            // ラベルの描画
            if (item.label && data.length <= BAR_CONFIG.MAX_BARS_FOR_LABELS) {
                context.fillStyle = options.theme.colors.text;
                context.font = `${fontSize}px ${fontFamily}`;
                context.textAlign = 'center';
     ...
     * チャート設定のバリデーション
     */
    validateOptions(options: Partial<ChartOptions>): ValidationResult {
        const errors: ValidationError[] = [];
        const warnings: ValidationWarning[] = [];

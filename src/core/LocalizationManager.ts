...
export class LocalizationManager {
    ...
    private translationDataManager: TranslationDataManager;
    private culturalAdaptationHandler: CulturalAdaptationHandler;
    private integrationController: I18nIntegrationController;
    private languageChangeListeners: Set<Function>;
     ...
     */
    getAccessibilityTranslation(key: string): string {
    ...
    getSupportedLanguages(): string[] {
        return this.translationDataManager.getAvailableLanguages();
    }
     ...
     */
    isRTL(language: string | null = null): boolean {
     ...
     */
    getTextDirection(language: string | null = null): string {
        const lang = language || this.currentLanguage;
     ...
     */
    formatDate(date: Date, language: string | null = null): string {
        const lang = language || this.currentLanguage;
     ...
     */
    formatTime(date: Date, language: string | null = null): string {
        const lang = language || this.currentLanguage;
     ...
     * @param {string} language - 言語コード（省略時は現在の言語）
     */
    applyCSSDirection(element: HTMLElement, language: string | null = null): void {
        const lang = language || this.currentLanguage;
     ...
     * @param {string} language - 言語コード（省略時は現在の言語）
     */
    applyRegionalFont(element: HTMLElement, language: string | null = null): void {
        const lang = language || this.currentLanguage;
     ...
     * @param {Function} listener - イベントリスナー
     */
    removeLanguageChangeListener(listener: Function): void {
     ...
     */
    notifyLanguageChange(oldLanguage: string, newLanguage: string): void {
    ...
    updateSettings(settings: any): void {
        ...
        if (settings.fallbackLanguage) {
            this.fallbackLanguage = settings.fallbackLanguage;
        }
    ...
    getSystemHealth(): any {
        ...
        return {
            ...
            metrics: {
                supportedLanguages: translationStats.languageCount,
                currentLanguage: this.currentLanguage,
...
// シングルトンインスタンス
let localizationManagerInstance: LocalizationManager | null = null;

...
export class CaptionManager {
     ...
     * キャプションコンテナの作成
     */
    private createCaptionContainer(): void {
    ...
    private updateStyleSheet(): void {
        ...
        this.dynamicStyleSheet.textContent = `
            ...
            .caption-item.slide-in {
                animation: slideInFromBottom 0.3s ease-out;
            }
            
            .caption-item.bounce-in {
            ...
            .reduced-motion .caption-item {
                transition: opacity 0.1s ease;
                animation: none !important;
            }
            
            .reduced-motion .caption-item.visible {
                transform: none;
            }
        `;
    }
     ...
     * 設定からスタイルを更新
     */
    private updateStyleFromPreferences(): void {
    ...
    private setupAudioManagerIntegration(): void {
        ...
        if (audioManager.playSound) {
            ...
            audioManager.playSound = (soundId: string, options: PlaySoundOptions = {}) => {
                const result = originalPlaySound(soundId, options);
                // キャプション表示
        ...
        if (audioManager.playMusic) {
            ...
            audioManager.playMusic = (musicId: string, options: PlayMusicOptions = {}) => {
                const result = originalPlayMusic(musicId, options);
    ...
    private handleAudioEvent(event: AudioEvent): void {
        ...
        // カテゴリフィルタリング
        if (!this.shouldShowCaption(category)) {
            return;
        }
     ...
     * 音響効果説明の取得
     */
    private getAudioDescription(soundId: string, options: any): SoundDescription | null {
     ...
     * 冗長性レベルの適用
     */
    private applyVerbosityLevel(description: SoundDescription, options: any): SoundDescription {
        const level = this.userPreferences.verbosityLevel;
     ...
     * キャプション継続時間の取得
     */
    private getDurationForSound(soundId: string, category: string): number {
        const description = this.soundDescriptions.get(soundId);
        ...
        switch (category) {
            ...
            default:
                return this.config.captionDuration.medium;
        }
    }
     ...
     * 最大キャプション数の制限
     */
    private enforceMaxCaptions(): void {
        const maxCaptions = this.config.maxCaptionLines;
     ...
     * キャプションの非表示
     */
    private hideCaption(captionId: number): void {
        const captionData = this.activeCaptions.get(captionId);
     ...
     * キャプションIDの生成
     */
    private generateCaptionId(): number {
        return ++this.maxCaptionId;
    }
    
    /**
     * 統計更新
     */
    private updateCaptionStats(category: string): void {
        this.stats.captionsDisplayed++;
        
        const count = this.stats.captionsByCategory.get(category) || 0;
        this.stats.captionsByCategory.set(category, count + 1);
    }
    
    private updateDisplayTimeStats(displayTime: number): void {
        const currentTotal = this.stats.totalDisplayTime;
    ...
    clearAllCaptions(): void {
        for (const captionId of this.activeCaptions.keys()) {
            this.hideCaption(captionId);
        }
     ...
     * カスタム説明の追加
     */
    addCustomDescription(soundId: string, description: string): void {
        this.userPreferences.customDescriptions.set(soundId, description);
        this.saveUserPreferences();
    ...
    setPosition(position: string): void {
        if (!this.config.positioning.alternatives.includes(position)) {
    ...
    setFontSize(fontSize: number): void {
        const size = Math.max(12, Math.min(32, fontSize));
     ...
     * 色の設定
     */
    setColors(textColor: string, backgroundColor: string): void {
        this.userPreferences.textColor = textColor;
    ...
    generateReport(): CaptionReport {
        ...
        return {
            ...
            performance: {
                averageDisplayTime: this.stats.averageDisplayTime,

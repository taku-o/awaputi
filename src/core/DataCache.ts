...
export interface CacheStatsReport extends CacheStatistics {
    hitRate: string;
    size: number;
    maxSize: number;
    memoryUsage: number;
...
export class DataCache {
     ...
     * タグに基づくキャッシュ無効化
     */
    invalidateByTag(tags: string | string[]): number {
     ...
     * メモリ容量確保のための削除
     */
    evictToMakeSpace(requiredSpace: number): void {
     ...
     * イベントリスナーの削除
     */
    off(event: CacheEventType, callback: CacheEventCallback): void {
        if (this.listeners.has(event)) {
     ...
     * イベントの発火
     */
    private emit(event: CacheEventType, data: CacheEventData): void {
        if (this.listeners.has(event)) {

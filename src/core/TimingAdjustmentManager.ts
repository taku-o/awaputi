...
export class TimingAdjustmentManager {
    ...
    private calibrator: TimingCalibrator;
    private algorithms: TimingAdjustmentAlgorithms;
    private feedback: TimingFeedbackSystem;
     ...
     * 設定をマージ
     */
    mergeConfig(newConfig: any): void {
        // 深い結合を実行
    ...
    applyProfile(profileName: string): void {
        return this.algorithms.applyProfile(profileName);
    }
    ...
    getCurrentProfile(): any {
        return this.algorithms.getCurrentProfile();
    }
    ...
    getAdjustmentMultiplier(): number {
        return this.algorithms.getAdjustmentMultiplier();
    }
    
    /**
     * タイマーを登録（アルゴリズムに委譲）
     */
    registerTimer(timerId: string, config: any): void {
        return this.algorithms.registerTimer(timerId, config);
    }
    ...
    unregisterTimer(timerId: string): void {
        return this.algorithms.unregisterTimer(timerId);
    }
    ...
    extendTimer(timerId: string): void {
        return this.algorithms.extendTimer(timerId);
    }
    
    /**
     * 時間延長をリクエスト（アルゴリズムに委譲）
     */
    requestTimeExtension(): void {
        return this.algorithms.requestTimeExtension();
    }
    ...
    showTimeWarning(timerId: string): void {
        return this.feedback.showTimeWarning(timerId);
    }
    ...
    showExtensionFeedback(timerId: string, remainingTime: number): void {
        return this.feedback.showExtensionFeedback(timerId, remainingTime);
    }
     ...
     */
    suggestProfileChange(recommendedProfile: string): void {
        return this.feedback.suggestProfileChange(recommendedProfile);
    }
    
    /**
     * カスタムタイマーを作成（アルゴリズムに委譲）
     */
    createCustomTimer(id: string, duration: number, options: any = {}): any {
    ...
    getRemainingTime(timerId: string): number {
        return this.algorithms.getRemainingTime(timerId);
    }
     ...
     * ゲーム状態変更を処理
     */
    handleGameStateChange(state: any): void {
        if (state.paused) {
    ...
    trackUserInteraction(interaction: any): void {
        return this.calibrator.trackUserInteraction(interaction);
    }
    
    /**
     * バブル作成時の処理（アルゴリズムに委譲）
     */
    handleBubbleCreated(bubble: any): void {
        return this.algorithms.handleBubbleCreated(bubble);
    }

...
export class ColorBlindnessSupport {
    ...
    private saveUserPreferences(): void { try {
        ...
        } catch (error) { console.warn('Failed to save color blindness support preferences:', error }
    }
    ...
    private createDynamicStyleSheet()';
        ...
        this.dynamicStyleSheet.textContent = `;
            /* 色覚サポートの基本スタイル */
            .color-blind-support { --pattern-opacity: 0.7,
            ...
            /* 特定のゲーム要素用スタイル */
            .bubble.cb-enhanced { stroke-width: 3px,
                stroke-dasharray: 4,2 }
            
            .game-ui.cb-enhanced { border: 2px solid currentColor,
                box-shadow: inset 0 0 0 1px rgba(255,255,255,0.5 }
            
            /* デバッグ用シミュレーション */
            .cb-simulation-active { filter: var(--colorblind-filter }
            
            /* 高コントラストモードとの統合 */
            .high-contrast .cb-pattern::before { opacity: 1 }
            
            .high-contrast .cb-label { background: #000,
                color: #fff;
     ...
     * パターンの生成
     */'
    private generatePatterns(): void { for (const [patternName, pattern] of this.visualPatterns) {''
     ...
     * 自動検出の実行
     */
    private performAutoDetection(): void { const startTime = performance.now();
        ...
        } catch (error) { console.warn('Auto-detection failed:', error } finally { this.stats.detectionTime = performance.now() - startTime; }
    }
    ...
    private analyzeAndEnhanceElement(element: HTMLElement): void { if(this.enhancedElements.has(element) {
        ...
        if (elementType) { this.enhanceElement(element, elementType); }
    }
    
    /**
     * 要素タイプの判定
     */
    private determineElementType(element: HTMLElement): string | null { // クラス名による判定
    ...
    private enhanceElement(element: HTMLElement, elementType: string): void { const mapping = this.gameElementMapping.get(elementType) || 
        ...
        const enhancement: ElementEnhancement = { element,
            type: elementType;
            originalStyle: {
     ...
     * ラベルテキストの取得'
     */''
    private getLabelText(elementType: string): string { const labelMap: Record<string, string> = {''
            'bubble-normal': '通常',
            'bubble-stone': '石',
            'bubble-iron': '鉄',
            'bubble-diamond': 'ダイヤ',
     ...
     * ゲーム要素の強化
     */
    private enhanceGameElement(element: HTMLElement): void { if (!this.config.enabled) return;
     ...
     * カスタムマッピングの追加
     */
    addCustomMapping(elementType: string, mapping: GameElementMapping): void { this.userPreferences.customMappings.set(elementType, mapping);
        this.saveUserPreferences();
         }
     ...
     * 利用可能な形状の取得
     */
    getAvailableShapes(): string[] { return Array.from(this.shapeEnhancements.keys(); }
     ...
     * デバッグ情報の取得
     */ : undefined
    getDebugInfo(): any { return { enabled: this.config.enabled,
     ...
     * レポートの生成
     */
    generateReport(): any { const sessionDuration = Date.now() - this.stats.sessionStart;
        ...
        return { timestamp: new Date().toISOString('),
            ...
            userPreferences: { ...this.userPreferences;
                customMappings: Array.from(this.userPreferences.customMappings.entries( };
            elements: { enhanced: this.enhancedElements.size;

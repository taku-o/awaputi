...
export class InsightGenerator {
    ...
    generate(statisticsData: StatisticsData, options: Partial<InsightConfig> = {}): InsightResult {
        ...
        if (statisticsData.timeSeries && statisticsData.timeSeries.available) {
            insights.push(...this.analyzeTimeSeriesInsights(statisticsData.timeSeries));
        }
        ...
        return {
            generationId: this.generateInsightId(),
            timestamp: Date.now(),
     ...
     * 基本統計の分析
     */
    private analyzeBasicStatistics(basicStats: any): Insight[] {
        ...
        if (basicStats.totalGamesPlayed > 0) {
            ...
            } else if (basicStats.completionRate < 40) {
                insights.push(this.createInsight(
     ...
     * バブル関連パフォーマンスの分析
     */
    private analyzeBubblePerformance(bubbleStats: any): Insight[] {
        const insights: Insight[] = [];
     ...
     * コンボパフォーマンスの分析
     */
    private analyzeComboPerformance(comboStats: any): Insight[] {
        const insights: Insight[] = [];
        ...
        } else if (comboStats.highestCombo < 5) {
            insights.push(this.createInsight(
                this.insightTypes.OPPORTUNITY,
     ...
     * 健康管理の分析
     */
    private analyzeHealthManagement(healthStats: any): Insight[] {
        const insights: Insight[] = [];
     ...
     * プレイスタイルの分析
     */
    private analyzePlayStyle(playstyleStats: any): Insight[] {
        const insights: Insight[] = [];
    ...
    private analyzeTimePatterns(timeStats: any): Insight[] {
        ...
        } else if (peakHour.hour >= 22 || peakHour.hour <= 2) {
            insights.push(this.createInsight(
                this.insightTypes.PATTERN,
                'Night Owl Player',
        ...
        // 曜日パターンの分析
        if (peakDay.day === 0 || peakDay.day === 6) { // 日曜日または土曜日
            insights.push(this.createInsight(
                this.insightTypes.PATTERN,
                'Weekend Player',
     ...
     * 進歩の分析
     */
    private analyzeProgress(progressStats: any): Insight[] {
        const insights: Insight[] = [];
     ...
     * 時系列洞察の分析
     */
    private analyzeTimeSeriesInsights(timeSeriesData: any): Insight[] {
        const insights: Insight[] = [];
        ...
        } else if (scoreTrend && scoreTrend.overall.trend === 'declining') {
            insights.push(this.createInsight(
                this.insightTypes.WARNING,
                'Performance Decline',
     ...
     * クロス分析（複数指標の関連性）
     */
    private performCrossAnalysis(statisticsData: StatisticsData): Insight[] {
        const insights: Insight[] = [];
     ...
     * 推奨事項の生成
     */
    private generateRecommendations(insights: Insight[], statisticsData: StatisticsData): Recommendation[] {
        ...
        // 一般的な推奨事項
        if (statisticsData.bubbles && parseFloat(statisticsData.bubbles.accuracy) < 70) {
    ...
    private createRecommendationFromInsight(insight: Insight, statisticsData: StatisticsData): Recommendation | null {
        // 簡略化された実装
    ...
    private filterAndPrioritizeInsights(insights: Insight[], config: InsightConfig): Insight[] {
        // 重複除去と優先順位付け
    ...
    private calculateAnalysisConfidence(statisticsData: StatisticsData, insights: Insight[]): number {
        return 0.85;
    }
    
    private createEmptyInsights(): InsightResult {
        return {
            insights: [],
            recommendations: [],

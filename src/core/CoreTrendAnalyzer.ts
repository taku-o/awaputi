...
export class CoreTrendAnalyzer {
    ...
    addDataPoint(seriesId: string, data: TrendData): void {
        if (!this.dataHistory.has(seriesId)) {
        ...
        // データ量を制限
        if (series.length > this.config.windowSize * 2) {
            series.splice(0, series.length - this.config.windowSize * 2);
        }
    ...
    analyzeTrend(seriesId: string): TrendAnalysisResult | null {
        const cachedResult = this.analysisCache.get(seriesId);
        if (cachedResult) {
            return cachedResult;
        }
    ...
    private performAnalysis(data: TrendData[]): TrendAnalysisResult {
    ...
    private detectSeasonality(values: number[]): SeasonalityInfo {
        // 簡略版の季節性検出
        if (values.length < 12) {
    ...
    private generatePredictions(values: number[], slope: number): number[] {
    ...
    getSeries(seriesId: string): TrendData[] {
        return this.dataHistory.get(seriesId) || [];
    }

    clearSeries(seriesId: string): void {
        this.dataHistory.delete(seriesId);
        this.analysisCache.delete(seriesId);
    }

    getAllSeriesIds(): string[] {
        return Array.from(this.dataHistory.keys());
    }
...
export function getCoreTrendAnalyzer(): CoreTrendAnalyzer {
    if (!instance) {

...
export class StatisticsDataManager {
     ...
     */
    validateStatistics(statistics: any): any {
        const issues: string[] = [];
     ...
     */
    repairStatistics(statistics: any): any {
     ...
     * @param {Object} statistics - 修復対象の統計データ
     */
    repairNumericFields(statistics: any): void {
        const numericFields = [
            'totalGamesPlayed', 'totalPlayTime', 'totalScore',
        ...
        // Infinity値の修復
        if (statistics.shortestSession === Infinity) {
            statistics.shortestSession = 0;
        }
    }
     ...
     * @param {Object} statistics - 修復対象の統計データ
     */
    repairArrayFields(statistics: any): void {
        if (!Array.isArray(statistics.timeStats?.playTimeByHour)) {
            statistics.timeStats.playTimeByHour = new Array(24).fill(0);
        }
        
        if (!Array.isArray(statistics.timeStats?.playTimeByDay)) {
            statistics.timeStats.playTimeByDay = new Array(7).fill(0);
        }
        
        if (!Array.isArray(statistics.reactionTimeStats?.recentTimes)) {
            statistics.reactionTimeStats.recentTimes = [];
        }
    }
     ...
     * @param {Object} statistics - 修復対象の統計データ
     */
    repairObjectFields(statistics: any): void {
        if (typeof statistics.bubbleTypeStats !== 'object') {
     ...
     */
    deepMergeStatistics(target: any, source: any): any {
        const result = { ...target };
     ...
     */
    calculateDataIntegrity(statistics: any): any {
        const checks = {
     ...
     */
    checkBubbleAccuracy(statistics: any): any {
     ...
     * @returns {Object} チェック結果
     */
    checkAverageCalculations(statistics: any): any {
        const checks: any[] = [];
     ...
     * @returns {Object} チェック結果
     */
    checkTimeConsistency(statistics: any): any {
        const checks: any[] = [];
     ...
     * @returns {Object} チェック結果
     */
    checkStageConsistency(statistics: any): any {
        ...
        return {
            passed: difference <= statistics.totalGamesPlayed * 0.05, // 5%の差は許容
            totalStageGames,
...
// シングルトンインスタンス管理
let statisticsDataManagerInstance: StatisticsDataManager | null = null;

/**
 * StatisticsDataManagerのシングルトンインスタンスを取得
 * @returns {StatisticsDataManager} シングルトンインスタンス
 */
export function getStatisticsDataManager(): StatisticsDataManager {
    if (!statisticsDataManagerInstance) {
 ...
 */
export function reinitializeStatisticsDataManager(): StatisticsDataManager {
    statisticsDataManagerInstance = new StatisticsDataManager();
    return statisticsDataManagerInstance;
}
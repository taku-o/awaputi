...
interface EffectsConfigProvider {
...
export class EffectsCalculator {
     ...
     * パーティクル数を計算
     */
    calculateParticleCount(effectType: string, intensity: number = 1.0, modifiers: EffectModifiers = {}): number {
        const config = this.getEffectsConfig();
        
        // エフェクトタイプ別の基本パーティクル数
        const baseParticleCounts: { [key: string]: number } = {
            bubble_pop: 8,
            bubble_chain: 15,
            bubble_explosion: 25,
            screen_shake: 0,
     ...
     * 効果強度を計算
     */
    calculateEffectIntensity(baseIntensity: number, modifiers: EffectModifier[] = []): number {
        ...
        for (const modifier of modifiers) {
            switch(modifier.type) {
                case 'multiply':
                    intensity *= modifier.value || 1;
                    break;
                case 'add':
                    intensity += modifier.value || 0;
                    break;
                case 'power':
                    intensity = Math.pow(intensity, modifier.value || 1);
                    break;
                case 'clamp':
    ...
    calculateScreenShake(shakeType: string, intensity: number = 1.0, options: { easing?: string } = {}): ShakeSettings {
        ...
        // シェイクタイプ別の設定
        const shakeSettings: { [key: string]: { amplitude: number; frequency: number; duration: number } } = {
    ...
    calculateParticleTrajectory(
        ...
        for (let i = 0; i <= steps; i++) {
            ...
            switch(trajectoryType) {
                case 'linear':
                    x = startPos.x + (finalEndPos.x - startPos.x) * t;
                    y = startPos.y + (finalEndPos.y - startPos.y) * t;
                    break;

                case 'arc':
                    x = startPos.x + (finalEndPos.x - startPos.x) * t;
                    y = startPos.y + (finalEndPos.y - startPos.y) * t - Math.sin(t * Math.PI) * 50;
                    break;

                case 'spiral':
                    const angle = t * Math.PI * 4;
                    const radius = 30 * (1 - t);
                    x = startPos.x + (finalEndPos.x - startPos.x) * t + Math.cos(angle) * radius;
                    y = startPos.y + (finalEndPos.y - startPos.y) * t + Math.sin(angle) * radius;
                    break;
     ...
     * 色の遷移を計算
     */
    calculateColorTransition(startColor: Color, endColor: Color, progress: number, blendMode: string = 'linear'): Color {
    ...
    calculateEffectPriority(effectType: string, context: EffectContext = {}): number {
        ...
        // コンテキストによる調整
        if (context.isCombo && context.comboCount && context.comboCount > 5) {
        ...
        if (context.remainingTime && context.remainingTime < 10000) {
            // 10秒未満
     ...
     * デバッグ情報を取得
     */
    getDebugInfo(): { hasEffectsConfig: boolean; effectsConfig: EffectsConfig; version: string } {
        return {
 ...
 */
export function getEffectsCalculator(): EffectsCalculator {

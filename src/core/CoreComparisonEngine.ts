...
export class CoreComparisonEngine {
    ...
    compare(source: any, target: any, path: string = ''): ComparisonResult {
    ...
    private compareValues(source: any, target: any, path: string, differences: DifferenceItem[]): void {
        // null/undefined の比較
        ...
        if (Array.isArray(source) && Array.isArray(target)) {
            this.compareArrays(source, target, path, differences);
            return;
        }
    ...
    private compareArrays(source: any[], target: any[], path: string, differences: DifferenceItem[]): void {
        ...
        if (this.config.ignoreOrder) {
            ...
            for (let i = 0; i < maxLength; i++) {
                ...
                } else if (this.config.deepComparison) {
                    this.compareValues(source[i], target[i], currentPath, differences);
    ...
    private compareObjects(source: Record<string, any>, target: Record<string, any>, path: string, differences: DifferenceItem[]): void {
        ...
        for (const key of allKeys) {
            ...
            if (!(key in source)) {
                ...
                this.compareValues(source[key], target[key], currentPath, differences);
    ...
    private calculateStatistics(differences: DifferenceItem[], source: any, target: any): ComparisonStatistics {
    ...
    private determineComplexity(source: any, target: any): 'low' | 'medium' | 'high' {
        const sourceSize = this.getObjectSize(source);
    ...
    private getObjectSize(obj: any): number {
        ...
        try {
            ...
            return 1000; // エラーの場合は大きなサイズとして扱う
        }
    }
...
export function getCoreComparisonEngine(): CoreComparisonEngine {
    if (!instance) {

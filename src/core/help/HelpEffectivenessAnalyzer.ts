...
export class HelpEffectivenessAnalyzer {
    ...
    public helpAnalytics?: any;
    public helpFeedbackSystem?: any;
    ...
    constructor(gameEngine: GameEngine) {
        ...
        // 分析対象システムの参照
        this.helpAnalytics = undefined;
        this.helpFeedbackSystem = undefined;
        ...
        // サブコンポーネントの初期化（依存注入）
        this.metricsCollector = new HelpMetricsCollector(this as any);
        this.dataAnalyzer = new HelpDataAnalyzer(this as any);
        this.reportGenerator = new HelpReportGenerator(this as any);
    ...
    initialize(): void {
        try {
            ...
            this.loggingSystem.error('HelpEffectivenessAnalyzer', 'Failed to initialize analyzer', error as any);
    ...
    async initializeDependentSystems(): Promise<void> {
        try {
            ...
            this.loggingSystem.error('HelpEffectivenessAnalyzer', 'Failed to initialize dependent systems', error as any);
        }
    }
    ...
    async analyzeEffectiveness(options: AnalysisOptions = {}): Promise<AnalysisResult> {
        try {
            ...
            if (!this.metricsCollector.validateDataQuality(rawData as any)) {
            ...
            // 5. 効果性スコア計算（データ分析に委譲）
            const effectivenessScore = this.dataAnalyzer.calculateEffectivenessScore({
                usage: usageAnalysis,
                engagement: engagementAnalysis,
                satisfaction: satisfactionAnalysis
            });
            ...
            if (analysisOptions.includeTrends) {
                trendAnalysis = await this.dataAnalyzer.analyzeTrends(rawData as any, analysisOptions.period || 'all');
            }
            ...
            const analysisResult: AnalysisResult = {
                ...
                dataQuality: this.metricsCollector.assessDataQuality(rawData) as unknown as DataQuality,
                // 主要指標
            ...
            this.loggingSystem.error('HelpEffectivenessAnalyzer', 'Failed to analyze effectiveness', error as any);
            throw error;
        }
    }
    ...
    async generateEffectivenessReport(reportType: string = 'comprehensive', options: Record<string, any> = {}): Promise<any> {
        try {
            // Map reportType to available methods
            switch (reportType) {
                case 'executive':
                    return await this.reportGenerator.generateExecutiveReport(options);
                case 'technical':
                    return await this.reportGenerator.generateTechnicalReport(options);
                case 'comprehensive':
                default:
                    return await this.reportGenerator.generateComprehensiveReport(options);
            }
        } catch (error) {
            this.loggingSystem.error('HelpEffectivenessAnalyzer', `Failed to generate ${reportType} report`, error as any);
            throw error;
        }
    }
    ...
    startPeriodicAnalysis(): void {
        ...
        setInterval(async () => {
            try {
                ...
                this.loggingSystem.error('HelpEffectivenessAnalyzer', 'Periodic analysis failed', error as any);
            }
        }, 3600000); // 1時間
    }
    ...
    getConfig(): ConfigResult {
        return {
            main: { ...this.config },
            metricsCollector: this.metricsCollector.getCollectionStats ? this.metricsCollector.getCollectionStats() : {},
            dataAnalyzer: this.dataAnalyzer.getAnalysisStats ? this.dataAnalyzer.getAnalysisStats() : {},
            reportGenerator: {} // HelpReportGenerator doesn't have getGenerationStats method
        };
    }
    ...
    getPerformanceStats(): PerformanceStats {
        return {
            metricsCollection: this.metricsCollector.getCollectionStats ? this.metricsCollector.getCollectionStats() : {},
            dataAnalysis: this.dataAnalyzer.getAnalysisStats ? this.dataAnalyzer.getAnalysisStats() : {},
            reportGeneration: {} // HelpReportGenerator doesn't have getGenerationStats method
        };
    }
    ...
    validateDataQuality(rawData: RawData): boolean {
        return this.metricsCollector.validateDataQuality(rawData as any);
    }
    ...
    async analyzeTrends(rawData: RawData, period: string): Promise<TrendAnalysis> {
        const result = await this.dataAnalyzer.analyzeTrends(rawData as any, period);
    ...
    generateRecommendations(analysisResults: any): Recommendations {
        return this.dataAnalyzer.generateRecommendations(analysisResults) as Recommendations;
    }
    ...
    cleanup(): void {
        try {
            ...
            this.loggingSystem.error('HelpEffectivenessAnalyzer', 'Failed to cleanup analyzer', error as any);
        }
    }
}

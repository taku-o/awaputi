...
export interface OverallErrorStatistics {
    categories: Record<string, ErrorStatistics>;
    totalErrors: number;
...
export interface DefaultHelpContent {
    version: string;
    category: string;
    language: string;
    sections: Array<{
...
export class HelpErrorHandler {
     ...
     * @returns 復旧成功フラグ
     */
    async attemptRecovery(errorType: string, context: ErrorContext): Promise<boolean> {
        try {
     ...
     * @returns 復旧成功フラグ
     */
    private async executeRecoveryStrategy(errorType: string, context: ErrorContext): Promise<boolean> {
        switch (errorType) {
     ...
     * @returns 復旧成功フラグ
     */
    private async recoverContentLoad(context: ErrorContext): Promise<boolean> {
        try {
            // キャッシュクリア後再試行
     ...
     * @returns 復旧成功フラグ
     */
    private async recoverTutorialExecution(context: ErrorContext): Promise<boolean> {
        try {
            // DOM要素の再確認
     ...
     * @returns 復旧成功フラグ
     */
    private async recoverSearchOperation(context: ErrorContext): Promise<boolean> {
        try {
            // インデックス再構築
     ...
     * @returns ユーザーメッセージ
     */
    private translateErrorToUserMessage(error: Error): string {
     ...
     * @returns デフォルトコンテンツ
     */
    private getDefaultHelpContent(): DefaultHelpContent {
        return {
            version: '1.0.0',
     ...
     * @returns デフォルト結果
     */
    private getDefaultErrorResult(): ErrorResult {
        return {
            success: false,
            strategy: 'none',
     ...
     * @param category - カテゴリ
     */
    private incrementResolvedCount(category: string): void {
        const stats = this.errorStats.get(category);
        if (stats) {
            stats.resolved++;
        }
     ...
     * @param category - カテゴリ
     */
    private incrementUnresolvedCount(category: string): void {
        const stats = this.errorStats.get(category);
        if (stats) {
            stats.unresolved++;
        }
     ...
     * @returns 解決率（パーセンテージ）
     */
    private calculateOverallResolutionRate(): number {
        let totalErrors = 0;
        let totalResolved = 0;
 ...
 */
export function getHelpErrorHandler(gameEngine: GameEngine): HelpErrorHandler {

...
export class ContextManager {
     ...
     * @returns 現在のコンテキスト情報
     */
    detectCurrentContext(): Context | null {
        try {
     ...
     * @returns 関連ヘルプ一覧
     */
    getRelevantHelp(context: Context): HelpItem[] {
        try {
     ...
     * @returns 提案アクション一覧
     */
    suggestNextActions(currentState: Context): ActionSuggestion[] {
        try {
     ...
     * @returns スマートヘルプ
     */
    getSmartHelp(userBehavior: any): SmartHelp | null {
        try {
            // 行動パターンの分析
     ...
     * ツールチップレジストリの設定
     */
    private setupTooltipRegistry(): void {
     ...
     * @returns 最後のアクション
     */
    private getLastUserAction(): UserAction | null {
     ...
     * @returns 類似フラグ
     */
    private isContextSimilar(context1: Context, context2: Context | null): boolean {
     ...
     * @returns シーンヘルプ
     */
    private getSceneHelp(sceneName: string): HelpItem[] {
     ...
     * @returns アクションヘルプ
     */
    private getActionHelp(action: UserAction): HelpItem[] {
     ...
     * @returns 状況ヘルプ
     */
    private getSituationalHelp(context: Context): HelpItem[] {
        const help: HelpItem[] = [];
     ...
     */
    private attachTooltipListeners(element: HTMLElement, tooltipId: string): void {
     ...
     * @param tooltipId - ツールチップID
     */
    private showRegisteredTooltip(tooltipId: string): void {
    ...
    private createTooltipElement(content: string | TooltipContent, options: TooltipOptions): HTMLElement {
        ...
        if (typeof content === 'string') {
            ...
            tooltip.innerHTML = (content as TooltipContent).html || content.toString();
        }
     ...
     * @returns 調整後の位置
     */
    private calculateTooltipPosition(x: number, y: number, tooltip: HTMLElement): Position {
     ...
     * @returns 困っている兆候
     */
    private detectStruggles(patterns: BehaviorPatterns): Struggle[] {
     ...
     * @returns ヘルプコンテンツ
     */
    private generateHelpForStruggles(struggles: Struggle[]): HelpContent {
     ...
     * @returns 信頼度（0-1）
     */
    private calculateHelpConfidence(patterns: BehaviorPatterns): number {
 ...
 */
export function getContextManager(gameEngine: GameEngine): ContextManager {

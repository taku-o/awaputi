...
export interface AvailableTutorial {
    id: string;
    title: string;
    description: string;
    difficulty: 'beginner' | 'intermediate' | 'advanced';
...
export interface TutorialAccessibilityManager {
    applyToTutorial(overlay: any, config: TutorialConfig): TutorialConfig;
...
export interface TutorialStatsManager {
...
export interface TutorialProgressManager {
...
export interface TutorialValidationEngine {
    determineValidationFunction(step: TutorialStep): ((...args: any[]) => boolean) | null;
...
export class TutorialManager {
     ...
     * @returns アクション結果
     */
    async waitForUserAction(step: TutorialStep): Promise<ActionResult> {
        return new Promise<ActionResult>((resolve) => {
            ...
            const handleAction = (actionResult: ActionResult) => {
     ...
     * @returns 前提条件満足フラグ
     */
    checkPrerequisites(tutorial: Tutorial): boolean {
     ...
     * チュートリアルデータを読み込み
     */
    async loadTutorialData(): Promise<void> {
        try {
     ...
     * @returns チュートリアルデータ
     */
    convertTourToTutorial(tourData: any): Tutorial {
        return {
            ...tourData,
     ...
     */
    handleOverlayNavigation(data: NavigationData): void {
        try {
     ...
     * @returns チュートリアル一覧
     */
    getAvailableTutorials(): AvailableTutorial[] {
        try {
     ...
     * @returns 統計情報
     */
    getTutorialStatistics(): Record<string, any> {
        try {
            const progress = this.progressManager.getProgress();
     ...
     */
    updateAccessibilityConfig(accessibilityConfig: any): void {
        try {
 ...
 */
export function getTutorialManager(gameEngine: GameEngine): TutorialManager {

...
export class ConfigurationDebugger {
     ...
     * 未使用キーを登録
     */
    public registerUnusedKey(category: string, key: string): void {
        if (!this.debugConfig.enabled) {
            return;
        }
     ...
     * デバッグレポートを生成
     */
    public generateReport(options: GenerateReportOptions = {}): DebugReport {
        const {
            includeUsage = true,
     ...
     * デバッグ情報を表示
     */
    public displayDebugInfo(options: GenerateReportOptions = {}): void {
        if (!this.debugConfig.enabled) {
     ...
     * 設定値の詳細情報を取得
     */
    public getKeyDetails(category: string, key: string): KeyDetails {
        const fullKey = `${category}.${key}`;
        ...
        return {
            ...
            averageAccessTime: this.performanceTracking.accessTimes.get(fullKey)?.[0] || 0,
            cacheHitRate: this.performanceTracking.cacheHitRates.get(fullKey)?.hits || 0
        };
    }
     ...
     * @private
     */
    private _generateStatistics(): StatisticsReport {
     ...
     * @private
     */
    private _generateUsageReport(topN: number): UsageReport {
     ...
     * @private
     */
    private _generatePerformanceReport(topN: number): PerformanceReport {
     ...
     * @private
     */
    private _generateErrorReport(topN: number): ErrorReport {

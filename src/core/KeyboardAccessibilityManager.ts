...
export class KeyboardAccessibilityManager {
     ...
     * ショートカットのカテゴリ分類
     */
    private categorizeShortcut(name: string): string {
        const categories = {
     ...
     * ショートカットの優先度取得
     */
    private getShortcutPriority(name: string): string {
        const priorities = {
     ...
     * カスタマイズ用ショートカットの追加
     */
    private addCustomizationShortcuts(): void {
        const customizationShortcuts = new Map([
            ['showShortcutHelp', {
                keys: ['Ctrl+Alt+?'],
        ...
        customizationShortcuts.forEach((shortcut, name) => {
            this.keyboardManager.addShortcut(name, shortcut.keys, shortcut.callback, {
                description: shortcut.description,
                context: 'global'
            });
            
            // 内部管理にも追加
            this.customizations.shortcuts.set(name, {
                ...shortcut,
                originalKeys: [...shortcut.keys],
                priority: 'high'
            });
            ...
            if (savedConfig) {
                ...
                if (config.customShortcuts) {
                    Object.entries(config.customShortcuts).forEach(([name, keys]) => {
                        this.setCustomShortcut(name, keys as string[], false); // 保存しない
                    });
                }
                // プロファイルの復元
                if (config.profiles) {
    ...
    setupEventListeners() {
        ...
        if (this.keyboardManager.addEventListener) {
            this.keyboardManager.addEventListener('shortcutTriggered', (name: string, keyCombo: string) => {
     ...
     * ショートカット実行の処理
     */
    handleShortcutTriggered(name: string, keyCombo: string) {
        // 使用統計の更新
     ...
     * 使用統計の更新
     */
    updateUsageStatistics(name: string) {
     ...
     * カスタムショートカットの設定
     */
    setCustomShortcut(actionName: string, newKeys: string[], save: boolean = true): boolean {
        const shortcut = this.customizations.shortcuts.get(actionName);
        if (!shortcut) {
    ...
    private checkForConflicts(keys: string[], excludeAction: string | null = null): ConflictInfo[] {
        ...
        this.customizations.shortcuts.forEach((shortcut, name) => {
            ...
            shortcut.keys.forEach((existingKeyCombo: string) => {
     ...
     * 競合の処理
     */
    handleConflicts(conflicts: ConflictInfo[], actionName: string, newKeys: string[]) {
    ...
    checkShortcutConflicts() {
        ...
        this.customizations.shortcuts.forEach((shortcut, name) => {
            shortcut.keys.forEach((keyCombo: string) => {
     ...
     * ショートカット記録の開始/停止
     */
    toggleShortcutRecording(actionName: string | null = null): void {
        if (this.state.isRecording) {
            this.stopShortcutRecording();
        } else {
            this.startShortcutRecording(actionName);
        }
    }
     ...
     * ショートカット記録の開始
     */
    startShortcutRecording(actionName: string | null = null) {
     ...
     * 記録中のキーダウン処理
     */
    handleRecordingKeyDown(event: KeyboardEvent) {
     ...
     * 記録中のキーアップ処理
     */
    handleRecordingKeyUp(event: KeyboardEvent) {
        // Enter キーで記録完了
     ...
     * プロファイル管理
     */
    createProfile(profileName: string, description?: string): any {
        if (this.customizations.profiles.has(profileName)) {
    ...
    applyProfile(profileName: string): boolean {
        const profile = this.customizations.profiles.get(profileName);
        if (!profile) {
        ...
        try {
            // プロファイルのショートカットを適用
            profile.shortcuts.forEach((keys: string[], actionName: string) => {
    ...
    generateShortcutHelp() {
        ...
        this.customizations.shortcuts.forEach((shortcut, name) => {
            ...
            if (categories[categoryName]) {
                categories[categoryName].push(helpEntry);
            }
        });
     ...
     * ARIA Live領域での通知
     */
    announceViaAriaLive(message: string) {
     ...
     * ショートカットの削除
     */
    removeShortcut(action: string, keys: string): void {
        this.keyboardManager.removeShortcut(action);
        console.log(`Removed shortcut: ${action} (${keys})`);
    }
    
    /**
     * レポート生成
     */
    generateReport(): KeyboardReport {
        const customizedShortcuts = Array.from(this.customizations.shortcuts.values()).filter(s => s.modified);
        return {
            totalShortcuts: this.customizations.shortcuts.size,
     ...
     * 使用統計の取得
     */
    private getUsageStatistics(): UsageStatistics {
        const stats = {
            mostUsed: [],
            leastUsed: [],
            recentlyModified: [],

...
export class StatisticsEventHandler {
    ...
    private lastComboTime: number | null;
    private comboStartTime: number | null;

    constructor(statistics: any, sessionStats: any) {
     ...
     */
    onGameStart(stageId: string): void {
        ...
        // 最初のプレイ日記録
        if (!this.statistics.timeStats.firstPlayDate) {
            this.statistics.timeStats.firstPlayDate = Date.now();
        }
    ...
    onGameEnd(data: any): void {
        const playTime = Date.now() - (this.gameStartTime || 0);
    ...
    onBubblePopped(bubbleData: any): void {
        ...
        if (this.statistics.bubbleTypeStats[type] !== undefined) {
            this.statistics.bubbleTypeStats[type]++;
        }
        ...
        // 特殊泡効果統計
        if (isSpecial) {
            this.statistics.specialBubblesPopped = (this.statistics.specialBubblesPopped || 0) + 1;
        }
    }
     ...
     */
    onComboUpdate(comboData: any): void {
        ...
        if (isBreak) {
            ...
            if (!this.comboStartTime && comboCount === 1) {
     ...
     */
    onDamageTaken(damageData: any): void {
        ...
        // クリティカルHP状況の記録
        if (currentHp <= 5) {
            this.statistics.hpDetailStats.criticalHpEvents++;
        }
    ...
    onHpHealed(healData: any): void {
        ...
        // 死の淵からの回復記録
        if (previousHp === 1 && currentHp > 1) {
            this.statistics.hpDetailStats.nearDeathRecoveries++;
        }
    }
    ...
    onAchievementUnlocked(achievementData: any): void {
        ...
        if (ap) {
            this.statistics.progressStats.totalAP += ap;
        }
    }
    ...
    onItemPurchased(itemData: any): void {
        ...
        // 通貨別支出統計（将来の機能）
        if (currency && cost) {
            this.statistics.spendingByCurrency = this.statistics.spendingByCurrency || {};
            this.statistics.spendingByCurrency[currency] = 
                (this.statistics.spendingByCurrency[currency] || 0) + cost;
        }
    }
     ...
     */
    updateScoreDistribution(score: number): void {
     ...
     */
    updateSessionLengthStats(playTime: number): void {
     ...
     */
    updateReactionTimeStats(reactionTime: number): void {
        ...
        reactionStats.average = reactionStats.recentTimes.reduce((sum: number, time: number) => sum + time, 0) / totalCount;
     ...
     */
    updateComboDetailStats(maxCombo: number): void {
        const comboStats = this.statistics.comboDetailStats;
     ...
     */
    updateHpDetailStats(gameDamage: number, perfectHealth: boolean): void {
    ...
    updateStageDetailStats(stageId: string, playTime: number, score: number, completed: boolean): void {
        ...
        // リトライ回数更新
        if (!stageDetailStats.stageRetryCount[stageId]) {
            stageDetailStats.stageRetryCount[stageId] = 0;
        }
        if (!completed) {
            stageDetailStats.stageRetryCount[stageId]++;
        }
    ...
    updateTimeDetailStats(): void {
        ...
        if (today === lastPlay) {
            ...
            if (lastPlay === yesterday.toDateString()) {
                ...
                timeStats.regularPlayStreak = 1; // ストリークリセット
            }
        }
    }
...
// シングルトンインスタンス管理
let statisticsEventHandlerInstance: StatisticsEventHandler | null = null;
 ...
 */
export function getStatisticsEventHandler(statistics: any, sessionStats: any): StatisticsEventHandler {
    if (!statisticsEventHandlerInstance) {
 ...
 */
export function reinitializeStatisticsEventHandler(statistics: any, sessionStats: any): StatisticsEventHandler {
    statisticsEventHandlerInstance = new StatisticsEventHandler(statistics, sessionStats);
    return statisticsEventHandlerInstance;
}

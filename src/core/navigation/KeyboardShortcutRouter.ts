...
export interface ShortcutDebugInfo {
    shortcuts: Array<[string, ShortcutDefinition]>;
...
export interface SceneManager {
    switchScene: (sceneName: string, options?: any) => boolean;
    currentScene?: {
...
export interface ExtendedGameEngine extends GameEngine {
...
// 型ガード
export function isValidShortcutAction(action: string): action is ShortcutAction {
    return ['contextualHelp', 'documentationHelp', 'quickHelp',
        'goBack', 'fullscreen', 'toggleSettings',
        'openUserInfo', 'showMenu', 'pauseGame',
        'screenshot', 'debugMode', 'customAction'
    ].includes(action);
...
export function isShortcutDefinition(shortcut: any): shortcut is ShortcutDefinition {
    return shortcut &&
...
export function hasSceneManager(engine: any): engine is ExtendedGameEngine & { sceneManager: SceneManager } {
    return engine &&
           engine.sceneManager &&
...
export function isValidKeyCode(code: string): code is KeyCode {
    return /^(Key[A-Z]|Digit[0-9]|F[0-9]+|Arrow(Up|Down|Left|Right)|Enter|Escape|Space|Tab|Slash|Backquote)$/.test(code);
}
...
export class KeyboardShortcutRouter {
     ...
     * 修飾キーの状態を更新
     */
    private updateModifierKeys(event: KeyboardEvent, isPressed = true): void {
     ...
     * キーの組み合わせ文字列を生成
     */
    private generateKeyCombo(event: KeyboardEvent): string {
        const parts: string[] = [];
     ...
     * ショートカットの処理
     */
    private processShortcut(keyCombo: string, event: KeyboardEvent): boolean {
        try {
     ...
     * ショートカットアクションの実行
     */
    private executeShortcut(shortcut: ShortcutDefinition, event: KeyboardEvent): boolean {
        try {
     ...
     * コンテキストヘルプの処理
     */
    private handleContextualHelp(context: ShortcutExecutionContext): boolean {
        // F1キーでのコンテキストヘルプ
     ...
     * フルスクリーン切り替えの処理
     */
    private handleFullscreen(): boolean {
        try {
    ...
    private navigateToScene(
        ...
        additionalData: NavigationOptions = {}
    ): boolean {
        try {
            // ナビゲーションコンテキストをプッシュ
     ...
     * ショートカットの追加
     */
    addShortcut(key: string, shortcut: ShortcutDefinition): boolean {
        if (!isShortcutDefinition(shortcut)) {
            if (this.config.enableLogging) {
     ...
     * ショートカットの削除
     */
    removeShortcut(key: string): boolean {
     ...
     * ショートカットの有効/無効化
     */
    setShortcutEnabled(key: string, enabled: boolean): boolean {
        const shortcut = this.shortcuts.get(key) || this.modifierShortcuts.get(key);
        if (shortcut) {
    ...
    hasShortcut(key: string): boolean {
        return this.shortcuts.has(key) || this.modifierShortcuts.has(key);
    }

    /**
     * 特定のアクションのショートカットを取得
     */
    getShortcutsByAction(action: ShortcutAction): { key: string; shortcut: ShortcutDefinition }[] {
        return this.getAllShortcuts().filter(item => item.shortcut.action === action);
    }
     ...
     * アクティブな修飾キーを取得
     */
    getActiveModifiers(): ModifierKey[] {
     ...
     */
    getNavigationContext(): NavigationContextManager {
     ...
     * デバッグ情報の取得
     */
    getDebugInfo(): ShortcutDebugInfo {
        return {

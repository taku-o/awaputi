...
// 型ガード
export function isValidAccessMethod(method: string): method is AccessMethod {
    return ['menu_click', 'keyboard_h', 'keyboard_s', 'keyboard_i',
        'button_click', 'explicit_return', 'shortcut', 'navigation',
        'breadcrumb', 'back_button', 'escape_key', 'system'
    ].includes(method);
}

export function isValidSceneName(scene: string): scene is SceneName {
    return ['menu', 'help', 'settings', 'game', 'stage_select',
        'user_info', 'leaderboard', 'achievements', 'shop',
        'tutorial', 'credits', 'options'
    ].includes(scene);
}

export function isNavigationContext(context: any): context is NavigationContext {
    return context &&
...
export function isReturnContext(returnContext: any): returnContext is ReturnContext {
...
export function hasSceneManager(engine: any): engine is GameEngine & { sceneManager: SceneManager } {
    return engine &&
           engine.sceneManager &&
...
export class NavigationContextManager {
     ...
     * 戻り先コンテキストを設定
     */
    setReturnContext(returnContext: ReturnContext): boolean {
        try {
            ...
            const context: NavigationContext = {
                ...
                timestamp: Date.now(),
                data: { ...returnContext.contextData },
     ...
     */
    hasContextByMethod(accessMethod: string): boolean {
     ...
     */
    hasContextFromScene(scene: string): boolean {
     ...
     * コンテキストの履歴を取得
     */
    getHistory(limit?: number | null): NavigationContext[] {
     ...
     * 循環ナビゲーションの検出
     */
    private isCircularNavigation(scene: string): boolean {
     ...
     * 最後のコンテキストのアクセス方法を取得
     */
    getLastAccessMethod(): AccessMethod | null {
     ...
     * スタック内の特定コンテキストまで戻る
     */
    popToContext(contextId: string): NavigationContext[] {
        const poppedContexts: NavigationContext[] = [];
     ...
     * コンテキストの検索
     */
    findContext(predicate: (context: NavigationContext) => boolean): NavigationContext | null {
        return this.navigationStack.find(predicate) || null;
    }

    /**
     * コンテキストフィルタリング
     */
    filterContexts(predicate: (context: NavigationContext) => boolean): NavigationContext[] {
        return this.navigationStack.filter(predicate);
    }
    ...
    getStatistics(): {
        ...
        return {
            totalContexts: this.navigationStack.length,

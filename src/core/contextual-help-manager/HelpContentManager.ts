...
// ユーティリティ関数
export function isValidHelpCategory(category: string): category is HelpCategory {
    return HELP_CATEGORIES.includes(category as HelpCategory);
}
...
export function isValidHelpPriority(priority: string): priority is HelpPriority {
    return HELP_PRIORITIES.includes(priority as HelpPriority);
}

export function calculateReadingTime(content: HelpContent): number {
...
export class HelpContentManager {
     ...
     * ヘルプデータベースを初期化
     */
    initializeHelpDatabase(): HelpDatabase {
        return {
            // ゲーム基本操作
            gameBasics: {
                category: 'basic',
     ...
     * カテゴリインデックスの更新
     */
    private updateCategoryIndex(key: string, item: HelpItem): void {
     ...
     * 難易度インデックスの更新
     */
    private updateDifficultyIndex(key: string, item: HelpItem): void {
     ...
     * 検索インデックスの更新
     */
    private updateSearchIndex(key: string, item: HelpItem): void {
     ...
     * コンテンツの複雑度を計算
     */
    private calculateContentComplexity(content: HelpContent): number {
        let complexity = 0;
     ...
     * コンテンツの重みを計算
     */
    private calculateContentWeight(item: HelpItem): number {
        let weight = 1;
     ...
     * ヘルプコンテンツからキーワードを抽出
     */
    extractKeywords(item: HelpItem): string[] {
     ...
     * 結果から除外すべきかチェック
     */
    private shouldExcludeFromResults(item: HelpItem, options: SearchOptions): boolean {
        if (options.category && item.category !== options.category) return true;
     ...
     * 検索スコアを計算
     */
    private calculateSearchScore(keyword: string, key: string, originalQuery: string): number {
        ...
        // 完全一致ボーナス
        if (keyword === originalQuery.toLowerCase()) {
            score *= 1.5;
        }
     ...
     * マッチングを検出
     */
    private findMatches(keyword: string, item: HelpItem): SearchMatch[] {
     ...
     * カテゴリ別ヘルプを取得
     */
    getByCategory(category: HelpCategory): string[] {
        const index = this.categoryIndex.get(category);
     ...
     * 難易度別ヘルプを取得
     */
    getByDifficulty(difficulty: DifficultyLevel): string[] {
        const index = this.difficultyIndex.get(difficulty);
        return index ? [...index.keys] : [];
    }
     ...
     * ヘルプコンテンツを取得
     */
    getContent(key: string): HelpItem | null {
     ...
     * 優先度順でヘルプを取得
     */
    getByPriority(priority: HelpPriority): string[] {
    ...
    getContentStatistics(): ContentStatistics {
        const totalItems = Object.keys(this.helpDatabase).length;
        
        const categoryCounts = {} as Record<HelpCategory, number>;
        this.categoryIndex.forEach((index, category) => {
            categoryCounts[category] = index.count;
        });
        
        const difficultyCounts = {} as Record<DifficultyLevel, number>;
        this.difficultyIndex.forEach((index, difficulty) => {
            difficultyCounts[difficulty] = index.count;
        });
        ...
        const totalReadingTime = Object.values(this.helpDatabase)
            .reduce((sum, item) => sum + (item.content.metadata?.readingTime || 0), 0);
     ...
     * コンテンツの妥当性を検証
     */
    validateContent(key: string): ContentValidation {
        const item = this.getContent(key);
        if (!item) {
            return {
                isValid: false,
    ...
    shouldRebuildIndex(): boolean {
        const hoursSinceRebuild = (Date.now() - this.lastRebuild.getTime()) / (1000 * 60 * 60);
     ...
     * コンテンツの追加
     */
    addContent(key: string, item: HelpItem): boolean {
        if (this.helpDatabase[key]) {
            return false; // 既に存在
        }
     ...
     * コンテンツの更新
     */
    updateContent(key: string, item: HelpItem): boolean {
        if (!this.helpDatabase[key]) {
            return false; // 存在しない
        }

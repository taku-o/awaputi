...
export interface BubbleTypeInfo {
    name: string;
    color: string;
    effect: string;
    points: number;
...
export interface DisplayElementsConfig {
...
export interface MediaQueryHandler {
...
export interface AutoHideTimer {
    id: number | null;
    duration: number;
    paused: boolean;
}

export interface HelpDimensions {
    width: number;
    height: number;
    maxWidth: number;
    maxHeight: number;
...
export const POSITION_CONSTRAINTS: PositionConstraints = {
    minDistance: 10,
    maxDistance: 20,
    preferredSide: 'bottom',
    fallbackSides: ['top', 'right', 'left']
} as const;
...
export function calculateOptimalPosition(
    ...
    const positions = {
        ...
        top: {
            top: targetRect.top - helpDimensions.height - POSITION_CONSTRAINTS.minDistance,
            left: targetRect.left,
    ...
    if (isPositionValid(preferredPosition, helpDimensions, viewport)) {
...
export function adjustPositionForViewport(
    ...
    // 右端チェック
    if (adjusted.left + dimensions.width > viewport.width) {
        adjusted.left = viewport.width - dimensions.width - POSITION_CONSTRAINTS.minDistance;
    }
    
    // 左端チェック
    if (adjusted.left < POSITION_CONSTRAINTS.minDistance) {
        adjusted.left = POSITION_CONSTRAINTS.minDistance;
    }

    // 下端チェック
    if (adjusted.top + dimensions.height > viewport.height) {
        adjusted.top = viewport.height - dimensions.height - POSITION_CONSTRAINTS.minDistance;
    }

    // 上端チェック
    if (adjusted.top < POSITION_CONSTRAINTS.minDistance) {
        adjusted.top = POSITION_CONSTRAINTS.minDistance;
    }
...
export class HelpDisplayController {
     ...
     * 表示要素を初期化
     */
    private initializeDisplayElements(): void {
     ...
     * ヘルプコンテナを作成
     */
    private createHelpContainer(): void {
    ...
    private setupResponsiveHandling(): void {
        const mobileQuery = window.matchMedia(`(max-width: ${RESPONSIVE_BREAKPOINTS.mobile}px)`);
        const tabletQuery = window.matchMedia(`(max-width: ${RESPONSIVE_BREAKPOINTS.tablet}px)`);

        const mobileHandler: MediaQueryHandler = {
            query: mobileQuery,
            handler: this.debouncedHandleResize
        };
        const tabletHandler: MediaQueryHandler = {
            query: tabletQuery,
            handler: this.debouncedHandleResize
        };
    ...
    private handleResponsiveChange(): void {
        const isMobile = window.innerWidth <= RESPONSIVE_BREAKPOINTS.mobile;
     ...
     * ヘルプをキューに追加
     */
    private addToQueue(content: HelpContent, options: HelpDisplayOptions): void {
        const queueItem: HelpQueue = {
            content,
            options,
    ...
    private calculatePosition(targetElement?: HTMLElement): void {
        ...
        } else if (this.displayConfig.position === 'overlay') {
     ...
     * コンテキスト位置を設定
     */
    private setContextualPosition(targetElement: HTMLElement): void {
        ...
        const viewport: ViewportConstraints = {
     ...
     * 表示アニメーション
     */
    private async showAnimation(): Promise<void> {
     ...
     * スライドイン アニメーション
     */
    private async slideInAnimation(): Promise<void> {
        const config = ANIMATION_CONFIGS.slide;
     ...
     * フェードイン アニメーション
     */
    private async fadeInAnimation(): Promise<void> {
        const config = ANIMATION_CONFIGS.fade;
     ...
     * スケールイン アニメーション
     */
    private async scaleInAnimation(): Promise<void> {
        const config = ANIMATION_CONFIGS.scale;
    ...
    private releaseFocusTrap(): void {
        this.tabNavigationState.trapped = false;
    ...
    private updateFocusableElements(): void {
        const elements = Array.from(
            this.helpContainer.querySelectorAll(FOCUSABLE_SELECTOR)
        ) as HTMLElement[];
    ...
    private updatePosition(): void {
        if (this.activeHelp?.options.targetElement) {
    ...
    private getFontSize(): string {
        return FONT_SIZES[this.displayConfig.fontSize] || FONT_SIZES.medium;
    }
    ...
    private handleTabNavigation(event: KeyboardEvent): void {
        if (!this.tabNavigationState.trapped || this.tabNavigationState.focusableElements.length === 0) {
            return;
        }
     ...
     * 設定を更新
     */
    updateConfig(newConfig: Partial<HelpDisplayConfig>): void {

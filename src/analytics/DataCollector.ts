...
export interface BubbleInteractionData {
    sessionId: string;
    timestamp: number;
    bubbleType: string;
    action: 'popped' | 'missed' | 'expired';
...
export interface ScoreContextInfo {
    comboCount: number;
    timeRemaining: number;
    activeItems: string[];
}
...
export interface ItemUsageEventData {
    sessionId: string;
    timestamp: number;
    itemType: string;
    action: 'purchased' | 'used' | 'expired';
...
export interface EventStatsExtended extends EventStats {
    queueSize: number;
...
export class DataCollector {
     ...
     * セッション終了
     */
    endSession(endInfo: SessionEndInfo): void {
    ...
    collectPerformanceData(performanceMetrics: PerformanceMetrics): void {
        ...
        const performanceData: PerformanceData = {
            ...
            loadTimes: performanceMetrics.loadTimes || null,
     ...
     * データ収集可否チェック
     */
    private shouldCollectData(feature: FeatureType): boolean {
     ...
     * バッチをストレージに保存
     */
    private async saveBatchToStorage(batch: AnalyticsEvent[]): Promise<void> {
        // イベントタイプ別にグループ化
     ...
     * イベントタイプに対応するストア名を取得
     */
    private getStoreNameForEventType(eventType: EventType): string | null {
        const storeMap: Record<EventType, string> = {
            'session': 'sessions',
     ...
     * イベント統計の取得
     */
    getEventStats(): EventStatsExtended {
        return {
            ...this.eventStats,

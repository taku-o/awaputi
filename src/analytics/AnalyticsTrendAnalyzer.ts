...
export class AnalyticsTrendAnalyzer {
    ...
    private ANOMALY_THRESHOLD: number;
    private MIN_DATA_POINTS: number;
    private SEASONAL_PERIOD: number;
     ...
     */
    async analyzeWeeklyTrend(dataType: string, startDate: Date | null = null) {
     ...
     * @returns {Promise<Object>} トレンド分析結果
     */
    async analyzeMonthlyTrend(dataType: string, startDate: Date | null = null) {
     ...
     * @returns {Object} 計算結果
     */
    calculateTrend(data: any[], dataType: string, period: string) {
        const metrics = this.extractMetrics(data, dataType);
        ...
        const trendAnalysis = {
            ...
            metrics: {
                scoreImprovement: this.calculateScoreImprovement(timeSeriesData),
    ...
    extractMetrics(data: any[], dataType: string) {
        return data.map(session => {
            ...
            const accuracy = session.bubblesPopped && session.bubblesMissed ?
     ...
     */
    groupByTimeUnit(metrics: any[], period: string) {
     ...
     * @returns {number} 改善率（％）
     */
    calculateScoreImprovement(timeSeriesData: any[]) {
     ...
     * @returns {number} 変化率（％）
     */
    calculatePlayTimeChange(timeSeriesData: any[]) {
        if (timeSeriesData.length < 2) return 0;
     ...
     * @returns {number} 変化率（％）
     */
    calculateAccuracyChange(timeSeriesData: any[]) {
        if (timeSeriesData.length < 2) return 0;
     ...
     */
    calculateTrendDirection(timeSeriesData: any[]) {
        ...
        const strength = maxPossibleSlope > 0 ?
     ...
     */
    generateTrendSummary(trendAnalysis: any) {
     ...
     */
    detectAnomalies(data: any[], threshold = this.ANOMALY_THRESHOLD) {
        if (data.length < this.MIN_DATA_POINTS) {
            return [];
        }
        ...
        data.forEach((dataPoint, index) => {
            const value = dataPoint.averageScore || dataPoint.score || 0;
     ...
     */
    seasonalAdjustment(data: any[], period = this.SEASONAL_PERIOD) {
        ...
        return data.map((dataPoint, index) => {
            const seasonalIndex = seasonalIndices[index % period];
            const adjustedValue = seasonalIndex > 0 ?
                (dataPoint.averageScore || dataPoint.score || 0) / seasonalIndex :
                (dataPoint.averageScore || dataPoint.score || 0);
     ...
     * @returns {Array} 季節指数
     */
    calculateSeasonalIndices(values: number[], period: number) {

...
export class RealtimeDashboard {
    ...
    constructor(container: HTMLElement, performanceDataCollector: any, realtimeMonitor: any, options: any = {}) {
    ...
    initializeCharts() {
        ...
        if (this.options.enableFPS) {
            ...
            if (fpsCanvas) {
                this.charts.set('fps', new (window as any).Chart(fpsCanvas, {
                    type: 'line',
                    data: {
                        datasets: [{
                            label: 'FPS',
        ...
        if (this.options.enableMemory) {
            ...
            if (memoryCanvas) {
                this.charts.set('memory', new (window as any).Chart(memoryCanvas, {
                    type: 'line',
                    data: {
                        datasets: [{
                            label: 'メモリ使用量',
        ...
        if (this.options.enableErrors) {
            ...
            if (errorsCanvas) {
                this.charts.set('errors', new (window as any).Chart(errorsCanvas, {
                    type: 'bar',
                    data: {
                        datasets: [{
                            label: 'エラー数',
        ...
        if (this.options.enableLatency) {
            ...
            if (latencyCanvas) {
                this.charts.set('latency', new (window as any).Chart(latencyCanvas, {
                    type: 'line',
                    data: {
                        datasets: [{
                            label: 'レイテンシ',
    ...
    setupEventListeners() {
        ...
        window.addEventListener('realtime-alert', (event) => {
            this.displayAlert((event as CustomEvent).detail);
        });
    }
    ...
    updateCharts() {
        ...
        if (fpsChart) {
            fpsChart.data.datasets[0].data = this.dataHistory.timestamps.map((time, i) => ({
                x: time,
    ...
    exportData() {
        const exportData = {
            ...
            statistics: {
                ...
                memory: {
                    average: this.calculateAverage(this.dataHistory.memory),
                    min: Math.min(...this.dataHistory.memory),
                    max: Math.max(...this.dataHistory.memory)
                },
                errors: {
                    total: this.dataHistory.errors[this.dataHistory.errors.length - 1] || 0,
                    rate: this.calculateErrorRate()
                },
                latency: {
                    average: this.calculateAverage(this.dataHistory.latency),

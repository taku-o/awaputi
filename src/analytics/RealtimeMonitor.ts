...
export class RealtimeMonitor {
    ...
    private alerts: any[];
    private warningHistory: Map<string, number>;
    private debugPanel: HTMLElement | null;
    private monitoringStats: any;
    private debugPanelTimer: any;

    constructor(performanceDataCollector: any, options: any = {}) {
    ...
    initialize() {
        ...
        window.addEventListener('performance-warning', (event) => {
            this.handlePerformanceWarning((event as CustomEvent).detail);
        });
     ...
     * アラートの生成
     */
    generateAlert(alertId: string, alertData: any) {
        // クールダウンチェック
     ...
     * ページ内通知の表示
     */
    showInPageNotification(alert: any) {
    ...
    createDebugPanel() {
        ...
        // パネルの折りたたみ機能
        const toggleButton = this.debugPanel.querySelector('#debug-panel-toggle') as HTMLButtonElement;
        const content = this.debugPanel.querySelector('.debug-panel-content') as HTMLElement;
    ...
    updateDebugPanel() {
        ...
        // 現在の統計
        const currentStatsElement = this.debugPanel.querySelector('#current-stats') as HTMLElement;
        ...
        // 最近のアラート
        const recentAlertsElement = this.debugPanel.querySelector('#recent-alerts') as HTMLElement;
        ...
        // 監視統計
        const monitoringStatsElement = this.debugPanel.querySelector('#monitoring-stats') as HTMLElement;
    ...
    calculateErrorRate(): number {
        ...
        const recentErrors = errorData.filter((error: any) => error.timestamp > oneMinuteAgo);
     ...
     * 警告の重要度レベル取得
     */
    getSeverityLevel(warningType: string): string {
     ...
     * 警告メッセージのフォーマット
     */
    formatWarningMessage(warningData: any): string {
     ...
     * クールダウンチェック
     */
    isInCooldown(alertId: string): boolean {
     ...
     * 警告のクールダウン設定
     */
    setWarningCooldown(alertId: string) {

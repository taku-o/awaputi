...
export class PerformanceDataCollector {
    ...
    setupMemoryTracking() {
        ...
        const trackMemory = () => {
            ...
            if (this.isRunning) {
                this.memoryTimer = setTimeout(trackMemory, this.options.memoryInterval);
            }
        };
    ...
    setupResourceTimingTracking() {
        const trackResourceLoad = (event: Event) => {
        ...
        // リソースの読み込み開始時間を記録
        const recordLoadStart = (event: Event) => {
            if (event.target) {
                (event.target as any).loadStartTime = performance.now();
            }
        };
     ...
     * FPSデータの記録
     */
    recordFPSData(data: any) {
        this.performanceData.fps.push(data);
     ...
     * ナビゲーションタイミングの記録
     */
    recordNavigationTiming(entry: any) {
        const data = {
            timestamp: Date.now(),
            name: entry.name,
     ...
     * リソースタイミングの記録
     */
    recordResourceTiming(entry: any) {
        const data = {
            timestamp: Date.now(),
            name: entry.name,
            duration: entry.duration,
     ...
     * リソースロード時間の記録
     */
    recordResourceLoadTime(data: any) {
        this.performanceData.networkRequests.push(data);
        this.trimDataArray('networkRequests');
     ...
     * データ配列のトリミング
     */
    trimDataArray(arrayName: string) {
     ...
     * カスタムタイミング測定の開始
     */
    startMeasure(name: string, startMark: string | null = null): string | null {
     ...
     * カスタムタイミング測定の終了
     */
    endMeasure(name: string, startMark: string | null = null): any {
     ...
     * パフォーマンスデータの取得
     */
    getPerformanceData(category: string | null = null, startTime: number | null = null, endTime: number | null = null) {
    ...
    calculateFPSStatistics() {
        ...
        return {
            ...
            min: Math.min(...values),
            max: Math.max(...values),
            belowThreshold: values.filter((fps: number) => fps < 30).length,
    ...
    calculateMemoryStatistics() {
        ...
        return {
            ...
            averageUsage: Math.round(usageValues.reduce((sum: number, val: number) => sum + val, 0) / usageValues.length * 100) / 100,
    ...
    calculateLoadTimeStatistics() {
        ...
        return {
            averageLoadTime: Math.round(durations.reduce((sum: number, val: number) => sum + val, 0) / durations.length),
    ...
    calculateNetworkStatistics() {
        ...
        return {
            ...
            averageLoadTime: durations.length > 0 ? Math.round(durations.reduce((sum: number, val: number) => sum + val, 0) / durations.length) : 0,
            totalDataTransferred: sizes.reduce((sum: number, val: number) => sum + val, 0),
            averageResourceSize: sizes.length > 0 ? Math.round(sizes.reduce((sum: number, val: number) => sum + val, 0) / sizes.length) : 0
        };
    }
    ...
    convertToCSV(data: any): string {
        ...
        data.rawData.fps.forEach((item: any) => {
            csv += `FPS,${item.timestamp},${item.fps},frameTime:${item.frameTime}\n`;
        });

        data.rawData.memory.forEach((item: any) => {

...
export class AnalyticsPerformanceMonitor {
    ...
    private fpsRequestId: number | null;
    private memoryIntervalId: any;
     ...
     * @returns {Array} 警告リスト
     */
    checkPerformanceWarnings(metrics: any): any[] {
        const warnings = [];
     ...
     */
    getFPSStats(): any {
        const fpsHistory = this.performanceHistory
        ...
        return {
            current: this.performanceMetrics.fps,
    ...
    getMemoryStats(): any {
        ...
        return {
            ...
            peak: {
                used: Math.max(...usedMemoryHistory),
    ...
    generatePerformanceReport(): any {
        ...
        const recentWarnings = this.performanceHistory
     ...
     * @returns {Object}
     */
    summarizeWarnings(warnings: any[]): any {
        const summary: any = {
     ...
     * @returns {Object}
     */
    calculateHealthScore(fpsStats: any, memoryStats: any): any {
        let score = 100;
        const factors = [];
        ...
        } else if (this.performanceMetrics.errorCount > 5) {
            score -= 5;

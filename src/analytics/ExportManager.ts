...
export class ExportManager {
    ...
    private config: any;
    private supportedDataTypes: Set<string>;
    private exportStats: any;
    ...
    async exportData(options: any = {}) {
        ...
        try {
            ...
            if (exportOptions.anonymize && this.privacyManager) {
                processedData = await this.privacyManager.anonymizeData(rawData);
            }
     ...
     */
    async collectAnalyticsData(dataTypes: any, filters: any = {}) {
        try {
            ...
            for (const dataType of normalizedTypes) {
                if (!this.supportedDataTypes.has(dataType)) {
     ...
     */
    async convertDataFormat(data: any, format: string) {
        try {
     ...
     */
    createExportMetadata(options: any, data: any) {
        const dataSize = this.calculateDataSize(data);
        const recordCounts: any = {};
        
        // 各データタイプのレコード数を計算
        for (const [dataType, records] of Object.entries(data)) {
            recordCounts[dataType] = Array.isArray(records) ? records.length : 1;
        }
        ...
        return {
            ...
            dataSize,
            recordCounts,
            totalRecords: Object.values(recordCounts).reduce((sum: number, count: any) => sum + count, 0),
     ...
     */
    normalizeDataTypes(dataTypes: any) {
        if (dataTypes === 'all') {
     ...
     */
    validateDataTypes(dataTypes: any) {
        const normalizedTypes = this.normalizeDataTypes(dataTypes);
        for (const dataType of normalizedTypes) {
    ...
    buildQuery(filters: any, dataType: string) {
        ...
        if (filters.bubbleType) {
            query.bubbleType = filters.bubbleType;
        }
     ...
     */
    generateFilename(dataTypes: any, format: string) {
    ...
    getExportStats() {
        return {
            ...this.exportStats,
            successRate: this.exportStats.totalExports > 0 ?

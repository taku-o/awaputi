...
export class ComparisonResultRenderer {
    ...
    constructor() {
        ...
        this.renderConfig = {
            ...
            formats: {
                ...
                time: (value: number) => this.formatTime(value),
                score: (value: number) => Math.round(value).toLocaleString()
            }
        };
    }
    ...
    generateComparisonSummary(comparisons: any, metrics: any[]): any {
        ...
        availableComparisons.forEach((comparison: any) => {
            totalImprovements += comparison.improvements || 0;
    ...
    generateBenchmarkSummary(comparison: any, metrics: any[]): any {
        ...
        const performanceRatio = {
            above_average: total > 0 ? (above_average / total) * 100 : 0,
    ...
    generateStageComparisonSummary(stageComparisons: any, metrics: any[]): any {
        ...
        let totalImprovements = 0;
        let totalDeclines = 0;
        const stagesByCategory: any = {
            tutorial: [],
            normal: [],
            special: []
        };

        availableComparisons.forEach((stage: any) => {
            totalImprovements += stage.comparison.improvements || 0;
     ...
     * @returns {Object} 詳細分析
     */
    generateDetailedAnalysis(comparisons: any): any {
        const analysis: any = {
            strengths: [],
            weaknesses: [],
            opportunities: []
        ...
        Object.entries(comparisons).forEach(([period, comparison]: [string, any]) => {
            if (!comparison.available) return;

            Object.entries(comparison.metrics || {}).forEach(([metricName, metric]: [string, any]) => {
        ...
        if (analysis.weaknesses.length > 0) {
            analysis.opportunities = this.identifyOpportunities(analysis.weaknesses);
        }
     ...
     */
    generateBenchmarkAnalysis(comparison: any): any {
        const analysis: any = {
            strengths: [],
            improvements: [],
            insights: []
        ...
        if (analysis.improvements.length > 0) {
            const worstMetric = analysis.improvements
                .sort((a: any, b: any) => a.percentileRank - b.percentileRank)[0];
     ...
     */
    generateStageAnalysisDetails(stageComparisons: any): any {
        const details: any = {
            byCategory: {},
            excellentStages: [],
            improvingStages: [],
        ...
        Object.entries(stageComparisons).forEach(([stageId, stageData]: [string, any]) => {
            if (!stageData.comparison || !stageData.comparison.available) return;
            ...
            if (performance.grade === 'A' || performance.grade === 'B') {
                ...
                details.strugglingStages.push(stageId);
            }
        });
     ...
     * @returns {Array} 推奨事項
     */
    generateStageRecommendations(stageComparisons: any): any[] {
        const recommendations: any[] = [];
        const weakStages: any[] = [];
        const improvingStages: any[] = [];
     ...
     */
    categorizeStage(stageId: string): string {
     ...
     */
    getStageInfo(stageId: string): any {
     ...
     * @returns {Array} 強み
     */
    identifyStageStrengths(stageData: any): string[] {
     ...
     * @returns {Array} 弱点
     */
    identifyStageWeaknesses(stageData: any): string[] {
     ...
     */
    identifyStrongMetrics(comparison: any): any[] {
        return Object.entries(comparison.metrics || {})
            .filter(([_, metric]: [string, any]) => metric.performance === 'above_average')
     ...
     */
    identifyWeakMetrics(comparison: any): any[] {
        return Object.entries(comparison.metrics || {})
            .filter(([_, metric]: [string, any]) => metric.performance === 'below_average')
            .map(([name, metric]: [string, any]) => ({
                name,
                percentileRank: metric.percentileRank,
                value: metric.current,
     ...
     * @returns {Array} 改善の機会
     */
    identifyOpportunities(weaknesses: any[]): any[] {

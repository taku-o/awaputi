...
export class DataComparator {
     ...
     * @returns {Object} 比較結果
     */
    calculateComparison(current: any, past: any, metrics: string[]) {
     ...
     */
    calculateStageComparison(current: any, past: any, metrics: string[]) {
        const comparison: any = {
            available: true,
            improvements: 0,
            declines: 0,
     ...
     */
    calculateBenchmarkComparison(current: any, benchmark: any, metrics: string[]) {
        const comparison: any = {
            available: true,
            above_average: 0,
            average: 0,
        ...
        for (const metricName of metrics) {
            const metric = this.metrics[metricName];
            if (!metric || !benchmark[metric.key]) continue;
            ...
            const differencePercent = benchmarkStats.median !== 0 ?
     ...
     */
    calculatePercentileRank(value: number, stats: any) {
        ...
        if (value <= stats.median) {
            ...
            if (stats.percentile75 === stats.median) return 75;
            const ratio = (value - stats.median) / (stats.percentile75 - stats.median);
            return 50 + (ratio * 25);
        } else {
            if (stats.max === stats.percentile75) return 100;
     ...
     */
    calculateStageDifficulty(stageData: any) {
     ...
     */
    formatChange(change: number, changePercent: number, metric: any) {
     ...
     */
    formatBenchmarkDifference(difference: number, differencePercent: number, metric: any) {
     ...
     */
    determineOverallTrend(comparison: any) {
     ...
     */
    determineOverallPerformance(comparison: any) {
     ...
     */
    calculatePerformanceMetrics(sessionData: any[]) {

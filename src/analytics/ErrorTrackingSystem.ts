...
export class ErrorTrackingSystem {
    ...
    private setupGlobalErrorHandlers(): void {
        ...
        window.onerror = (message, source, line, column, error) => {
            ...
            if (this.originalErrorHandler) {
                return this.originalErrorHandler(message, source, line, column, error);
            }
            return false;
        };
     ...
     * エラーの捕捉と記録
     */
    async captureError(errorData: Partial<ErrorReport>): Promise<string> {
        try {
    ...
    private async captureContext(): Promise<ErrorContext> {
        ...
        try {
            ...
            if (this.options.enableContextCapture) {
                context.gameState = await this.captureGameState();
            }
     ...
     */
    private getGameStateFromGlobalObjects(): any {
        const gameState: any = {};
        ...
        for (const key in obj) {
            ...
            try {
                ...
                if (typeof value === 'object' && value !== null) {
                    result[key] = this.extractSafeProperties(value, depth - 1);
                } else {
                    result[key] = value;
                }
     ...
     * エラーグループIDの生成
     */
    private getErrorGroupId(error: ErrorReport): string {
        // エラーメッセージとソースファイルに基づいてグループ化
     ...
     * エラーグループの更新
     */
    private updateErrorGroup(groupId: string, error: ErrorReport): void {
        ...
        // 最も高い重要度を保持
        if (this.getSeverityLevel(error.severity) > this.getSeverityLevel(group.severity)) {
     ...
     * エラーをストレージに保存
     */
    private saveErrorToStorage(error: ErrorReport): void {
        try {
     ...
     * エラーレポート送信
     */
    private async reportError(error: ErrorReport): Promise<boolean> {
        try {
            // ここで実際のレポート送信ロジックを実装
     ...
     * エラー一覧の取得
     */
    getErrors(filter?: { severity?: string; type?: string; resolved?: boolean }): ErrorReport[] {
     ...
     * エラーグループの解決マーク
     */
    markGroupResolved(groupId: string): boolean {

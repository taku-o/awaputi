...
'';
    ...
    detectBubbleInteractionAnomaly(data) {
        ...
        const anomalies = [];
        sessionGroups.forEach((interactions, sessionId) => {  const reactionTimes = interactions
    ...
    detectUnusualQuitPattern(data') {'
        ...
        if (quitRatio > 0.4) { // 40%以上が途中終了
            ...
            const quitTimes = quitSessions.map(s => { )
                const duration = s.endTime ? ) : undefined;
    }
...
';'
    ...
    generateRecommendations(results) {
        ...
        // タイプ別の推奨事項
        typeCount.forEach((count, type) => {  const recommendation = this.getRecommendationForType(type, count);
            if (recommendation) { }

...
interface TouchData { position: Position;
    }
}
...
export class GameInputManager extends InputManager { private gameScene: any
     ...
     * クリック処理
     */
    notifyClick(position: Position): void { if (this.gameScene.isPaused || this.gameEngine.isGameOver || this.gameEngine.inputDisabled) {
            return; }
        }
        
        // 座標変換（InputCoordinateConverterが利用可能な場合）
        let convertedPosition = position;
        try { if (this.inputCoordinateConverter && position.originalEvent) {
     ...
     * ポインター移動処理
     */
    notifyPointerMove(position: Position): void { if (this.gameScene.isPaused || this.gameEngine.isGameOver) {
            return; }
        }
     ...
     * ドラッグ開始処理
     */
    notifyDragStart(startPosition: Position): void { if (this.gameScene.isPaused || this.gameEngine.isGameOver || this.gameEngine.inputDisabled) {
            return; }
        }
        
        // 座標変換（InputCoordinateConverterが利用可能な場合）
        let convertedPosition = startPosition;
        try { if (this.inputCoordinateConverter && startPosition.originalEvent) {
     ...
     * ドラッグ移動処理
     */
    notifyDragMove(currentPosition: Position): void { if (this.gameScene.isPaused || this.gameEngine.isGameOver || this.gameEngine.inputDisabled) {
            return; }
        }
        
        // 座標変換（InputCoordinateConverterが利用可能な場合）
        let convertedPosition = currentPosition;
        try { if (this.inputCoordinateConverter && currentPosition.originalEvent) {
     ...
     * ドラッグ終了処理
     */
    notifyDragEnd(startPosition: Position, endPosition: Position, dragVector: DragVector): void { if (this.gameScene.isPaused || this.gameEngine.isGameOver || this.gameEngine.inputDisabled) {
            return; }
        }
     ...
     * ピンチジェスチャー処理
     */
    private handlePinchGesture(gestureData: GestureData): void { if (this.gameScene.isPaused || this.gameEngine.isGameOver) return;
     ...
     * ダブルタップジェスチャー処理
     */
    private handleDoubleTapGesture(gestureData: GestureData): void { if (this.gameScene.isPaused || this.gameEngine.isGameOver) return;

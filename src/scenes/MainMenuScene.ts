...
import type { MenuItem, MainMenuScene as IMainMenuScene } from '../types/game.js';
...
interface GameEngine {
    ...
    sceneManager: {
        switchScene(sceneName: string, contextData?: any): boolean;
    };
    responsiveCanvas?: {
        onResizeCallbacks: Array<() => void>;
    };
}
...
export class MainMenuScene extends Scene implements IMainMenuScene {
    ...
    constructor(gameEngine: GameEngine) {
        ...
        // メニュー項目の設定
        this.menuItems = [
            { id: 'start', key: 'menu.start', action: () => this.startGame(), label: '' },
            { id: 'shop', key: 'menu.shop', action: () => this.openShop(), label: '' },
            { id: 'settings', key: 'menu.settings', action: () => this.openSettings(), label: '' },
            { id: 'userInfo', key: 'menu.userInfo', action: () => this.openUserInfo(), label: '' },
            { id: 'help', key: 'menu.help', action: () => this.openHelp(), label: '' }
        ];

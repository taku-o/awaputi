...
import { GameEngine } from '../../types/gameEngine.js';
...
export class AccessibilitySettingsManager {
    ...
    private setupValidators(): void {
        // 数値範囲検証
        this.validators.set('range', (value: any, min: number, max: number): ValidationResult => {
        ...
        // コントラスト比検証（アクセシビリティ）
        this.validators.set('contrastRatio', (foreground: string, background: string): ValidationResult => {
        ...
        // フォントサイズ検証
        this.validators.set('fontSize', (value: any): ValidationResult => {
            const numValue = parseFloat(value);
            if (isNaN(numValue)) return { valid: false, error: '数値である必要があります' };
     ...
     * デフォルトプロファイルの設定
     */
    private setupDefaultProfile(): void {
     ...
     * 拡張された設定項目の取得
     */
    public getExtendedAccessibilitySettings(): AccessibilitySetting[] {
        return [
            {
            ...
            {
                key: 'accessibility.colorBlindSupport',
                label: '色覚サポート',
                type: 'toggle',
            ...
            {
                key: 'accessibility.contrastLevel',
                label: 'コントラストレベル',
                type: 'select',
                options: [
                    { value: 'low', label: '低' },
            ...
            {
                key: 'accessibility.keyboardNavigation',
                label: 'キーボードナビゲーション',
                type: 'toggle',
            ...
            {
                key: 'accessibility.voiceGuidance',
                label: '音声ガイダンス',
                type: 'toggle',
    ...
    public switchProfile(profileName: string): boolean {
        ...
        // 履歴サイズ制限
        if (this.profileHistory.length > 10) {
     ...
     * 設定のエクスポート
     */
    public exportSettings(format: string = 'json', includeProfiles: boolean = false): boolean {
        try {
            ...
            if (includeProfiles) {
                ...
                this.profiles.forEach((profile, name) => {
                    exportData.profiles![name] = profile;
                });
            }
            ...
            reader.onload = (event: ProgressEvent<FileReader>) => {
                try {
                    ...
                    if (importData.profiles) {
                        Object.entries(importData.profiles).forEach(([name, profile]) => {
     ...
     * 利用可能なプロファイルの一覧取得
     */
    public getAvailableProfiles(): ProfileInfo[] {
     ...
     * 統計情報の取得
     */
    public getStats(): ExtendedStatistics {
        return {
            ...this.stats,

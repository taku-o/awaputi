...
export class GameControlButtons {
     ...
     * @returns 表示状態
     */
    public isButtonVisible(buttonType: 'giveUp' | 'restart'): boolean {
     ...
     */
    private navigateButtons(direction: number, visibleButtons: ('giveUp' | 'restart')[]): void {
     ...
     */
    private setKeyboardFocus(buttonType: 'giveUp' | 'restart'): void {
    ...
    /**
     * ボタンアクティベーション処理
     * @param buttonType - アクティベートするボタンタイプ
     */
    private handleButtonActivation(buttonType: 'giveUp' | 'restart'): void {
     ...
     * @returns 完了したボタンタイプ
     */
    public handleTouchEnd(x: number, y: number, event: TouchEvent | null = null): 'giveUp' | 'restart' | null {
     ...
     * @returns クリック判定結果
     */
    private isButtonClicked(x: number, y: number, buttonType: 'giveUp' | 'restart'): boolean {
     ...
     * @returns 境界情報
     */
    private getButtonBounds(buttonType: 'giveUp' | 'restart'): ButtonBounds {
        const config = this.buttonConfig[buttonType];
        if (!config || !config.position) {
     ...
     * @param buttonType - ボタンタイプ
     * @returns スケーリングされた境界情報
     */
    private getScaledButtonBounds(buttonType: 'giveUp' | 'restart'): ButtonBounds {
        const config = this.buttonConfig[buttonType];
        if (!config || !config.position) {
            return { x: 0, y: 0, width: 0, height: 0 };
        }
     ...
     * @param buttonType - ボタンタイプ
     */
    private renderButton(context: CanvasRenderingContext2D, buttonType: 'giveUp' | 'restart'): void {
        const config = this.buttonConfig[buttonType];
     ...
     */
    private renderTouchFeedback(context: CanvasRenderingContext2D, bounds: ButtonBounds): void {
        context.save();
     ...
     * @param bounds - ボタンの境界情報
     */
    private renderFocusIndicator(context: CanvasRenderingContext2D, bounds: ButtonBounds): void {
        const focusOffset = 3;
     ...
     * @returns ボタン状態
     */
    public getButtonState(): ButtonState & { visibility: ButtonVisibility } {
        return { 
            ...this.buttonState,

...
'';
    ...
    private renderButton(context: CanvasRenderingContext2D, buttonType: 'giveUp' | 'restart'): void { const config = this.buttonConfig[buttonType];
        ...
        if (isActive) { backgroundColor = config.style.activeColor || config.style.borderColor; }
        } else if (isHovered && !this.deviceInfo.isTouchDevice) { backgroundColor = config.style.hoverColor; }
        }

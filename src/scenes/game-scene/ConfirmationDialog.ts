...
export class ConfirmationDialog {
     ...
     */
    public show(type: keyof DialogConfigs, onConfirm?: () => void, onCancel?: () => void): void {
     ...
     * @returns ダイアログ内かどうか
     */
    private isClickInsideDialog(x: number, y: number): boolean {
        if (!this.dialogState.visible || this.dialogLayout.x === undefined || this.dialogLayout.y === undefined) {
            return false;
        }
     ...
     * @param context - 描画コンテキスト
     */
    private renderModalOverlay(context: CanvasRenderingContext2D): void {
        const canvas = this.gameEngine.canvas;
     ...
     * @returns ダイアログ状態
     */
    public getDialogState(): Readonly<DialogState> {

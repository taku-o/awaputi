...
export class GameVisualizationManager {
     ...
     */
    public startDrag(x: number, y: number, targetBubble: Bubble | null = null): void { this.dragVisualization.isActive = true; }
     ...
     * @param y - 現在のY座標
     */
    public updateDrag(x: number, y: number): void { if (!this.dragVisualization.isActive) return;
         }
        this.dragVisualization.currentPosition = { x, y };
     ...
     */
    public updateDragVisualization(deltaTime: number): void { if (!this.dragVisualization.isActive) return;
     ...
     * ドラッグパーティクルの生成
     */
    private createDragParticle(): void { const start = this.dragVisualization.startPosition;
     ...
     */
    public renderBackground(context: CanvasRenderingContext2D): void { const canvas = this.gameEngine.canvas;
     ...
     * @param context - 描画コンテキスト
     */
    private applyScreenShake(context: CanvasRenderingContext2D): void { if (this.gameEngine.screenShakeRemaining > 0) {
     ...
     * @param context - 描画コンテキスト
     */
    public renderDragVisualization(context: CanvasRenderingContext2D): void { if (!this.dragVisualization.isActive) return;
     ...
     * @param context - 描画コンテキスト
     */
    private renderDragParticles(context: CanvasRenderingContext2D): void { this.dragVisualization.particles.forEach(particle => { );
            context.save();

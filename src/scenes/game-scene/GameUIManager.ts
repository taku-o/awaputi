...
export class GameUIManager {
     ...
     */
    private getScaledRenderingContext(context: CanvasRenderingContext2D): ScaledRenderingContext {
     ...
     */
    private renderControlButtons(context: CanvasRenderingContext2D): void {
     ...
     */
    public handleControlButtonClick(x: number, y: number, event: Event | null = null): boolean {
        // まずダイアログのクリック処理を試行
     ...
     */
    public updateMousePosition(x: number, y: number, event: MouseEvent | null = null): void {
     ...
     */
    public updateUIState(deltaTime: number): void {
        // ゲーム状態の監視とボタン表示状態の更新
     ...
     * ゲーム状態の監視とボタン表示状態の更新
     */
    private updateGameStateAndButtons(): void {
        // 現在のゲーム状態を取得
     ...
     * @returns 変化があったかどうか
     */
    private hasGameStateChanged(currentState: GameState): boolean {
        return (
     ...
     * ゲーム開始メッセージの表示
     */
    public showGameStartMessage(): void {
     ...
     * @param heal - 回復量
     */
    public onPlayerHealed(heal: number): void {
        const canvas = this.gameEngine.canvas;
     ...
     * @param warnings - 警告情報
     */
    public showTimeWarnings(warnings: TimeWarnings): void {
        const canvas = this.gameEngine.canvas;
     ...
     * @param combo - コンボ数
     */
    public updateComboDisplay(combo: number): void {
        if (combo > 1) {
     ...
     */
    private renderAnimatedScore(context: CanvasRenderingContext2D, playerData: PlayerData): void {
        try {
     ...
     * @param context - 描画コンテキスト
     */
    private renderTimeDisplay(context: CanvasRenderingContext2D): void {
        try {
            const scaledContext = this.getScaledRenderingContext(context);
     ...
     * @param playerData - プレイヤーデータ
     */
    private renderHPDisplay(context: CanvasRenderingContext2D, playerData: PlayerData): void {
        try {
            const scaledContext = this.getScaledRenderingContext(context);
     ...
     * @param hpRatio - HP比率
     */
    private renderHPBar(context: CanvasRenderingContext2D, hpRatio: number): void {
        try {
            const scaledContext = this.getScaledRenderingContext(context);
            const hpPosition = this.uiPositionCalculator.getStatusPosition('hp');
     ...
     * @param context - 描画コンテキスト
     */
    private renderComboDisplay(context: CanvasRenderingContext2D): void {
        try {
            ...
            if (combo > 1) {
                ...
                const baseX = (canvasInfo.width - margins.right) / this.scaledCoordinateManager.getScaleFactor();
                const baseY = margins.top / this.scaledCoordinateManager.getScaleFactor();
     ...
     * @param context - 描画コンテキスト
     */
    private renderSpecialEffectsStatus(context: CanvasRenderingContext2D): void {
        try {
            const scaledContext = this.getScaledRenderingContext(context);
            const margins = this.uiPositionCalculator.getResponsiveMargins();
            const hpPosition = this.uiPositionCalculator.getStatusPosition('hp');
     ...
     * @param context - 描画コンテキスト
     */
    private renderDetailedInfo(context: CanvasRenderingContext2D): void {
        const canvas = this.gameEngine.canvas;
     ...
     * @param context - 描画コンテキスト
     */
    public renderFloatingText(context: CanvasRenderingContext2D): void {

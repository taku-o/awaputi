...
export class GamePerformanceMonitor {
     ...
     * パフォーマンス監視の開始
     */
    public startMonitoring(): void { this.performanceMetrics.lastFpsUpdate = Date.now();
     ...
     * FPS計算
     */
    private calculateFPS(): void { const currentFps = this.performanceMetrics.frameCount;
     ...
     */
    private updateAverageFPS(currentFps: number): void { const history = this.performanceMetrics.frameHistory;
     ...
     * フレーム履歴の更新
     */
    private updateFrameHistory(): void { const frameTime = 1000 / this.performanceMetrics.fps;
     ...
     */
    public updateAverageUpdateTime(updateTime: number): void { const alpha = 0.1; // 平滑化係数
     ...
     */
    public updateAverageRenderTime(renderTime: number): void { const alpha = 0.1; // 平滑化係数
     ...
     * パフォーマンス閾値のチェック
     */
    private checkPerformanceThresholds(): void { const fps = this.performanceMetrics.fps;
    ...
    private handleLowFPS(fps: number): void { this.statistics.performanceWarnings++;
        ...
        } else if (!this.optimizations.reducedParticles) { this.enableReducedParticles(); }
        } else if (!this.optimizations.lowQualityMode) { this.enableLowQualityMode(); }
        }
    }
     ...
     */
    private handleHighBubbleCount(bubbleCount: number): void { console.warn(`[GamePerformanceMonitor] High bubble count: ${bubbleCount)`),
     ...
     * パフォーマンス表示の切り替え
     */
    public toggleMetricsDisplay(): void { this.performanceMetrics.showMetrics = !this.performanceMetrics.showMetrics; }
    }
     ...
     * @returns 統計情報
     */
    public getPerformanceStats(): PerformanceStats { return { }
     ...
     */
    public updateSettings(settings: PerformanceSettings): void { if (settings.showMetrics !== undefined) {

...
export class StageSelectDataManager {
     ...
     * ステージクリック処理
     */
    public handleStageClick(_x: number, y: number): void {
        const startY = 340;
        const itemHeight = 60;
     ...
     * プレイヤー情報を描画
     */
    public renderPlayerInfo(context: CanvasRenderingContext2D): void {
     ...
     * ステージリストを描画
     */
    public renderStageList(context: CanvasRenderingContext2D): void {
     ...
     * 操作説明を描画
     */
    public renderControls(context: CanvasRenderingContext2D): void {
        const canvas = this.gameEngine.canvas;
     ...
     * ステージ状態の設定
     */
    public setStageState(state: Partial<StageSelectionState>): void {
     ...
     * ステージ選択状態のリセット
     */
    public resetStageSelection(): void {
        this.selectedStageIndex = 0;
        this.scrollOffset = 0;
    }
     ...
     * ステージリストの更新チェック
     */
    public checkForStageUpdates(): StageUpdateResult {
     ...
     * ステージ選択範囲の制約チェック
     */
    public validateStageSelection(): void {
        const totalStages = this.unlockedStages.length + this.lockedStages.length;

...
export class EventStageDataManager {
     ...
     * イベントデータ管理のクリーンアップ
     */
    public cleanup(): void { this.stopNotificationUpdates(); }
    ...
    private startNotificationUpdates(): void { // 既存のインターバルをクリア
        ...
        // 30秒ごとに通知をチェック
        this.notificationCheckInterval = window.setInterval(() => { this.updateEventNotifications(); }, 30000);
    }
     ...
     * イベント通知クリック処理
     */
    public handleEventNotificationClick(x: number, y: number): boolean { const canvas = this.gameEngine.canvas;
        const badgeSize = 24;
        const badgeX = canvas.width - 50;
        const badgeY = 130;
     ...
     * イベントステージのクリック処理
     */
    public handleEventStageClick(_x: number, y: number): void { const sectionStartY = 120;
        const itemHeight = 40;
        const itemSpacing = 5;
     ...
     * イベント参加回数を取得
     */ : undefined
    private getEventParticipationCount(eventId: string): number { if (!this.gameEngine.eventStageManager) {
            return 0; }
     ...
     * イベント専用セクションを描画
     */
    public renderEventSection(context: CanvasRenderingContext2D): void { const canvas = this.gameEngine.canvas;
        ...
        this.availableEvents.forEach((event, index) => {  if (index < this.eventScrollOffset) return;
            if (index >= this.eventScrollOffset + this.maxVisibleEvents) return;
     ...
     * イベントデータの取得
     */
    public getEventData() { return { availableEvents: this.availableEvents,

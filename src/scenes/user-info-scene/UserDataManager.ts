...
// Extended game engine interface
...
export class UserDataManager {
    ...
    constructor(gameEngine: ExtendedGameEngine, eventBus: EventBus, sceneState: SceneState) {
    ...
    public async getUserProfile(): Promise<UserProfile> {
        ...
        try {
            const profile: UserProfile = {
                username: this.playerData?.username || 'プレイヤー',
                level: this.calculatePlayerLevel(),
                experience: this.calculateTotalExperience(),
                joinDate: this.playerData?.joinDate || new Date(),
     ...
     * 実績情報を取得
     */
    public async getUserAchievements(): Promise<UserAchievements> {
     ...
     * 基本統計情報を取得
     */
    private getBasicStatistics(): BasicStatistics {
     ...
     * 泡統計情報を取得
     */
    private getBubbleStatistics(): BubbleStatistics {
        const stats = this.statisticsManager ?
        ...
        return {
            ...
            normalPopped: stats.normalPopped || 0,
            specialPopped: stats.specialPopped || 0,
     ...
     * コンボ統計情報を取得
     */
    private getComboStatistics(): ComboStatistics {
        const stats = this.statisticsManager ?
     ...
     * ステージ統計情報を取得
     */
    private getStageStatistics(): StageStatistics {
        const stats = this.statisticsManager ?
     ...
     * 実績進捗を取得
     */
    private getAchievementProgress(): AchievementProgress[] {
        if (!this.achievementManager) return [];
     ...
     * データをエクスポート
     */
    public async exportUserData(): Promise<ExportData> {
        try {
     ...
     * データをインポート
     */
    public async importUserData(importData: ImportData): Promise<boolean> {
        try {
            // データの検証
     ...
     * キャッシュの有効性をチェック
     */
    private isCacheValid(type: keyof DataCache): boolean {
    ...
    private invalidateAchievementsCache(): void {
     ...
     * デフォルト実績を取得
     */
    private getDefaultAchievements(): UserAchievements {
        return {
            unlocked: [],
            total: 0,
            progress: [],

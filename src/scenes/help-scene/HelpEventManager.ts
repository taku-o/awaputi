...
export class HelpEventManager {
     ...
     * 入力フィールドの位置を更新
     */
    private updateInputPosition(): void { if (!this.hiddenInput || !this.gameEngine.canvas) {
            return; }
        }
    ...
    private handleIMEInput(event: Event): void { if (!this.searchFocused) {
        ...
        try { const target = event.target as HTMLInputElement;
...
';
    ...
    private debounceSearch(query: string): void { if (this.searchTimeout) {
        ...
        this.searchTimeout = window.setTimeout(() => { this.handleSearchInput(query); }
        }, 300); // 300ms のデバウンス
    }
...
';
     ...
     * コールバック設定
     */
    public setCallback<K extends keyof HelpEventCallbacks>(type: K, callback: HelpEventCallbacks[K]): void { this.callbacks[type] = callback; }
    }

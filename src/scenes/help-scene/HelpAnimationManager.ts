...
interface ContentTransitionState extends AnimationState {
    fromContent: any;
    toContent: any;
    type: 'slide' | 'fade' | 'scale';
}

interface CategoryTransitionState extends AnimationState {
    fromIndex: number;
    toIndex: number;
}

interface SearchTransitionState extends AnimationState {
    isEntering: boolean;
}

interface FocusTransitionState extends AnimationState {
    fromIndex: number;
    toIndex: number;
}
...
interface AnimationCollection {
    contentTransition: ContentTransitionState;
...
export class HelpAnimationManager {
     ...
     * アニメーション更新
     */
    public updateAnimations(currentTime: number): boolean {
     ...
     * イージング関数の適用
     */
    public applyEasing(progress: number, easingType: EasingType = 'easeOut'): number {
     ...
     * アニメーション状態の取得
     */
    public getAnimationState<K extends keyof AnimationCollection>(animationType: K): AnimationCollection[K] | null {
    ...
    public isAnyAnimationActive(): boolean {
     ...
     * アニメーション停止
     */
    public stopAnimation<K extends keyof AnimationCollection>(animationType: K): void {
    ...
    public stopAllAnimations(): void {
        (Object.keys(this.animations) as Array<keyof AnimationCollection>).forEach(type => {

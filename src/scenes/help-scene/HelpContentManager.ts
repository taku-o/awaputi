...
export class HelpContentManager {
     ...
     */
    async indexHelpContentForSearch(): Promise<void> {
        if (!this.searchEngine) {
            return;
        }
     ...
     * ヘルプコンテンツの読み込み
     */
    async loadHelpContent(): Promise<void> {
        try {
            if (!this.helpManager) {
     ...
     * カテゴリコンテンツの読み込み
     */
    async loadCategoryContent(categoryId: string): Promise<void> {
        try {
     ...
     * カテゴリ選択
     */
    async selectCategory(categoryId: string): Promise<any> {
     ...
     */
    async selectTopic(index: number, fromSearchResult: boolean = false): Promise<any> {
     ...
     * フィードバック関連
     */
    recordTopicFeedback(__topicId: string, feedback: any): void {
     ...
     * キャッシュ管理
     */
    setContentCache(key: string, content: any): void {
    ...
    getHelpFeedbackSystem(): any {
        return this.helpFeedbackSystem;
    }

    getHelpEffectivenessAnalyzer(): any {

...
export class StatisticsTab extends TabComponent {
    ...
    private setupEventListeners(): void {
        ...
        // 表示モード変更イベント
        this.eventBus.on('statistics-view-mode-changed', (mode: StatisticsViewMode) => {
     ...
     * @param height - 描画高さ
     */
    private renderStatistics(context: CanvasRenderingContext2D, x: number, y: number, width: number, height: number): void {
     ...
     * @param height - 描画高さ
     */
    private renderStatisticsCharts(context: CanvasRenderingContext2D, x: number, y: number, width: number, height: number): void {
        // チャート描画の暫定実装

...
export class SceneState {
     ...
     * @param notify - 変更通知を行うかどうか
     */
    update(updates: Record<string, any>, notify: boolean = true): void {
     ...
     * @returns リスナー削除用の関数
     */
    onChange(key: string, listener: ChangeListener): UnsubscribeFunction {
     ...
     * @param oldValue - 古い値
     */
    private notifyChange(key: string, value: any, oldValue: any): void {
        const listeners = this.changeListeners.get(key);
        if (listeners) {
     ...
     * @param changes - 変更の配列
     */
    private notifyBatchChange(changes: StateChange[]): void {

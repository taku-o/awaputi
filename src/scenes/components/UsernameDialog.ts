...
export interface GameEngineWithPlayerData extends GameEngine {
...
export class UsernameDialog extends ScenesBaseDialog {
     ...
     * @param text - 表示テキスト
     */
    private renderCursor(context: CanvasRenderingContext2D, textX: number, textY: number, text: string): void {
     ...
     * @param y - Y座標
     */
    private renderCharacterCounter(context: CanvasRenderingContext2D, x: number, y: number): void {
        const currentLength = this.dialogData.newUsername ? this.dialogData.newUsername.length : 0;
     ...
     */
    protected handleContentClick(x: number, y: number, layout?: Record<string, number>): boolean {
        const contentY = (layout?.contentY || this.layout.y) + 50;
    ...
    private handleBackspace(): void {
        if (this.cursorPosition > 0 && this.dialogData.newUsername) {
    ...
    private handleDelete(): void {
        if (this.dialogData.newUsername && this.cursorPosition < this.dialogData.newUsername.length) {
     ...
     * @returns 処理成功の場合true
     */
    private handleUsernameChange(): boolean {
        const newUsername = this.dialogData.newUsername?.trim();

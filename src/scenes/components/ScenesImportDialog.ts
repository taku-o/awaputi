...
// GameEngine interface for import dialog
...
export interface ImportGameSettings {
...
export interface ImportData extends ImportPlayerData, ImportGameSettings, ImportAchievementData {
    [key: string]: unknown;
}

export interface GameEngineWithData extends GameEngine {
    playerData?: {
        username?: string;
        ap?: number;
        tap?: number;
 ...
 * @extends ScenesBaseDialog
 */
export class ScenesImportDialog extends ScenesBaseDialog {
    // インポート用の設定
    ...
    // インポートタイプ
    private importType: ImportType = 'playerData';
    
    // プレビューデータ
    private previewData: unknown | null = null;
     ...
     * @returns バリデーション結果
     */
    private validateImportData(data: ImportData): ImportValidationResult {
        const errors: string[] = [];
    ...
    private async executeImport(): Promise<void> {
        ...
        try {
            await this.performImport(this.importData!);
            if (this.onResult) {
                this.onResult({
                    action: 'import',
                    data: {
     ...
     * @param data - プレイヤーデータ
     */
    private async importPlayerData(data: ImportData): Promise<void> {
     ...
     * @param data - ゲーム設定
     */
    private async importGameSettings(data: ImportData): Promise<void> {
        const gameEngine = this.gameEngine as GameEngineWithData;
        ...
        try {
            ...
            if (data.audio) {
                Object.assign(currentSettings.audio || {}, data.audio);
            }
            if (data.gameplay) {
     ...
     * @param data - 実績データ
     */
    private async importAchievements(data: ImportData): Promise<void> {
        try {
            const gameEngine = this.gameEngine as GameEngineWithData;

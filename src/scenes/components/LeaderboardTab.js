...
export class LeaderboardTab {
    ...
    setupEventListeners() {
        // タブ変更イベント
        this.eventBus.on('tabChanged', (newTab) => {
     ...
     * 描画
     */
    render(context, contentY, contentHeight) {
        try {
     ...
     * ローディング状態描画
     */
    renderLoadingState(context, y, height) {
        const canvas = this.gameEngine.canvas;
     ...
     * エラー状態描画
     */
    renderErrorState(context, y, height) {
        const canvas = this.gameEngine.canvas;
     ...
     * フッター描画
     */
    renderFooter(context, y, width) {
        if (!this.leaderboardUI?.leaderboardManager) return;
        
        // 統計情報を取得
        const stats = this.leaderboardUI.leaderboardManager.getStats();
     ...
     * クリック処理
     */
    handleClick(mouseX, mouseY, contentY, contentHeight) {
        try {
            if (!this.isInitialized || !this.leaderboardUI) {
                return false;
            }
     ...
     * マウスホバー処理
     */
    handleHover(mouseX, mouseY, contentY, contentHeight) {
        try {
            if (!this.isInitialized || !this.leaderboardUI) {
                return;
            }

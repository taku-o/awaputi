...
export class StatisticsRenderer {
     ...
     * @param height - 描画高さ
     */
    private renderDetailedStatistics(context: CanvasRenderingContext2D, x: number, y: number, width: number, height: number): void { if (!this.statisticsData) {
     ...
     * @returns 次のY座標
     */
    private renderBasicStatsSection(context: CanvasRenderingContext2D, x: number, y: number, width: number, height: number): number { if (!this.statisticsData || !this.statisticsData.basic) {
            return y + height + 20; }
        }
     ...
     * @returns 次のY座標
     */
    private renderBubbleStatsSection(context: CanvasRenderingContext2D, x: number, y: number, width: number, height: number): number { if (!this.statisticsData || !this.statisticsData.bubbles) {
            return y + height + 20; }
        }
        ...
        '';
        for (let i = 0; i < Math.min(5, items.length); i++') { const item = items[i];
     ...
     * @returns 次のY座標
     */
    private renderComboStatsSection(context: CanvasRenderingContext2D, x: number, y: number, width: number, height: number): number { if (!this.statisticsData || !this.statisticsData.combos) {
            return y + height + 20; }
        }
     ...
     * @returns 次のY座標
     */
    private renderStageStatsSection(context: CanvasRenderingContext2D, x: number, y: number, width: number, height: number): number { if (!this.statisticsData || !this.statisticsData.stages) {
            return y + height + 20; }
        }
     ...
     */
    private renderStatsList(context: CanvasRenderingContext2D, items: StatItem[], x: number, y: number, width: number, lineHeight: number): void { const adjustedLineHeight = this.accessibilitySettings.largeText ? lineHeight + 2 : lineHeight;
     ...
     */
    handleClick(x: number, y: number): boolean { // 現在の実装では特別なクリック処理は不要
     ...
     */
    update(deltaTime: number): void { // アニメーション等が必要な場合はここで処理
        // 現在の実装では静的表示のため何もしない }
    }

...
export class LeaderboardTab {
    ...
    private setupEventListeners(): void {
        ...
        // データ更新イベント（スコア記録時など）
        this.eventBus.on('scoreRecorded', () => {
            if (this.isVisible && this.leaderboardUI) {
     ...
     * 描画
     */
    render(context: CanvasRenderingContext2D, contentY: number, contentHeight: number): void {
        try {
     ...
     * ローディング状態描画
     */
    private renderLoadingState(context: CanvasRenderingContext2D, y: number, height: number): void {
        const canvas = this.gameEngine.canvas;
     ...
     * エラー状態描画
     */
    private renderErrorState(context: CanvasRenderingContext2D, y: number, height: number): void {
        const canvas = this.gameEngine.canvas;
     ...
     * フッター描画
     */
    private renderFooter(context: CanvasRenderingContext2D, y: number, width: number): void {
        if (!this.leaderboardUI?.leaderboardManager) return;
     ...
     * クリック処理
     */
    handleClick(mouseX: number, mouseY: number, contentY: number, contentHeight: number): boolean {
        try {
            if (!this.isInitialized || !this.leaderboardUI) {
                return false;
            }
     ...
     * マウスホバー処理
     */
    handleHover(mouseX: number, mouseY: number, contentY: number, contentHeight: number): void {
        try {
            if (!this.isInitialized || !this.leaderboardUI) {
                return;
            }

...
export class StatisticsFilterUI {
    ...
    private setupEventListeners(): void {
        // 外部からのフィルター変更イベント
        this.eventBus.on('set-period-filter', (period: string) => {
            this.setPeriodFilter(period);
        });
        
        this.eventBus.on('set-view-mode', (mode: string) => {
     ...
     * @returns 描画後のY座標
     */
    private renderPeriodFilter(context: CanvasRenderingContext2D, x: number, y: number, width: number): number {
     ...
     * @returns 描画後のY座標
     */
    private renderViewModeSelector(context: CanvasRenderingContext2D, x: number, y: number, width: number): number {

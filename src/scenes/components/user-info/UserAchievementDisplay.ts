...
// タブコンポーネントのインターフェース
interface AchievementsTabComponent {
    isActive: boolean;
...
export class UserAchievementDisplay {
    ...
    private setupEventListeners(): void {
        if (this.eventBus) {
            ...
            this.eventBus.on('achievementCategoryChanged', (category: string) => {
                this.currentAchievementCategory = category;
            });
            
            this.eventBus.on('achievementUnlocked', (achievementId: string) => {
     ...
     * 実績システムの初期化
     */
    private initializeAchievementSystems(): void {
        try {
     ...
     * 実績カテゴリフィルターを描画
     */
    private renderAchievementCategoryFilter(context: CanvasRenderingContext2D, y: number): number {

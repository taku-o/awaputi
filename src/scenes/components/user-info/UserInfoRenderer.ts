...
interface TabManager {
    renderTabHeaders(context: CanvasRenderingContext2D, x: number, y: number, width: number, height: number): void;
...
export class UserInfoRenderer {
    ...
    public render(
        ...
        try {
            ...
            // メインコンテンツをレンダリング
            if (tabManager) {
                this.renderContent(context, canvas, tabManager);
            }
            
            // エラーメッセージがある場合は表示
            if (this.sceneState.get('errorMessage')) {
     ...
     * 背景をレンダリング
     */
    private renderBackground(context: CanvasRenderingContext2D, canvas: HTMLCanvasElement): void {
        // グラデーション背景
     ...
     * タイトルをレンダリング
     */
    private renderTitle(context: CanvasRenderingContext2D, canvas: HTMLCanvasElement): void {
        const titleY = 30;
     ...
     * タブヘッダーをレンダリング
     */
    private renderTabs(context: CanvasRenderingContext2D, canvas: HTMLCanvasElement, tabManager: TabManager): void {
        const headerX = this.contentPadding;
     ...
     * メインコンテンツをレンダリング
     */
    private renderContent(context: CanvasRenderingContext2D, canvas: HTMLCanvasElement, tabManager: TabManager): void {
     ...
     * 戻るボタンをレンダリング
     */
    private renderBackButton(context: CanvasRenderingContext2D, canvas: HTMLCanvasElement): void {
    ...
    private renderErrorMessage(context: CanvasRenderingContext2D, canvas: HTMLCanvasElement): void {
        ...
        context.fillRect(
            messageX - textWidth / 2 - padding,
            messageY - textHeight / 2 - padding / 2,
            textWidth + padding * 2,
            textHeight + padding
        );
     ...
     * エラー状態をレンダリング
     */
    private renderErrorState(context: CanvasRenderingContext2D): void {
        const canvas = context.canvas;

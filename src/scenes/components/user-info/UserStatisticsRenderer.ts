...
interface StatisticsDisplaySettings {
...
// タブコンポーネントのインターフェース
interface StatisticsTabComponent {
    isActive: boolean;
...
export class UserStatisticsRenderer {
    ...
    private setupEventListeners(): void {
        ...
        this.eventBus.on('statisticsViewModeChanged', (mode: StatisticsViewMode) => {
            this.statisticsViewMode = mode;
        });

        this.eventBus.on('statisticsPeriodChanged', (period: PeriodFilter) => {
     ...
     * 拡張統計システムの初期化
     */
    private initializeExtendedStatistics(): void {
        try {
            // ChartRendererの初期化
     ...
     * 統計データのフォールバック描画
     */
    private renderStatisticsFallback(context: CanvasRenderingContext2D, y: number, height: number): void {
        const canvas = this.gameEngine.canvas;
     ...
     * 詳細統計の描画
     */
    public renderDetailedStatistics(context: CanvasRenderingContext2D, y: number, height: number): void {
        const canvas = this.gameEngine.canvas;

...
export class UserInfoEventHandler {
    ...
    private handleClickInput(;
        ...
        // ヘルプシステムの処理
        if(helpSystem && canvas && helpSystem.handleClick(x, y, canvas) { return true; }
        }
        
        // 戻るボタンの処理
        if(this.handleBackButtonClick(x, y) { return true; }
        }
        
        // タブヘッダーの処理
        if(tabManager && renderer && canvas && this.handleTabHeaderClick(x, y, tabManager, renderer, canvas) { return true; }
        }
        
        // タブコンテンツの処理
        if(tabManager && renderer && canvas && this.handleTabContentClick(x, y, tabManager, renderer, canvas) { return true; }
        }

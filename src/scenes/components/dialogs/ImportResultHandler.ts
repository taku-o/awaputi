...
export class ImportResultHandler {
     ...
     * 完了ステップを描画
     */
    renderCompleteStep(context: CanvasRenderingContext2D, layout: Layout, y: number): void {
     ...
     * 成功結果を描画
     */
    renderSuccessResult(context: CanvasRenderingContext2D, layout: Layout, y: number): void {
     ...
     * エラー結果を描画
     */
    renderErrorResult(context: CanvasRenderingContext2D, layout: Layout, y: number): void {
     ...
     * 成功統計情報を描画
     */
    renderSuccessStats(context: CanvasRenderingContext2D, layout: Layout, y: number): void {
     ...
     * 結果アクションを描画
     */
    renderResultActions(context: CanvasRenderingContext2D, layout: Layout, y: number): void {
        if (this.mainController.data.success) {
            this.renderSuccessActions(context, layout, y);
        } else {
            this.renderErrorActions(context, layout, y);
        }
    }
    ...
    handleComplete(): boolean {
        const result: ImportResult = {
            ...
            data: {
                ...
                importStats: this.mainController.data.parsedData ? 
        ...
        // 成功時の追加処理
        if (this.mainController.data.success) {
            this.handleSuccessCompletion(result);
        } else {
            this.handleErrorCompletion(result);
        }

        // コールバック実行
        if (this.mainController.onResult) {
            this.mainController.onResult(result);
        }
     ...
     * 成功完了処理
     */
    private handleSuccessCompletion(result: ImportResult): void {
        // 成功ログの記録
     ...
     * エラー完了処理
     */
    private handleErrorCompletion(result: ImportResult): void {
        // エラーログの記録
     ...
     * インポート統計の生成
     */
    generateImportStats(parsedData: any): ImportStats {
        return {
            playerDataImported: !!parsedData.playerData,
     ...
     * エラーメッセージの切り捨て
     */
    truncateErrorMessage(error: string, maxLength: number = 50): string {
     ...
     * トラブルシューティングヒントの取得
     */
    getTroubleshootingHints(error?: string): string[] {
        const hints: string[] = [];
     ...
     * インポートアイテム数のカウント
     */
    countImportedItems(data: any): number {
        let count = 0;
     ...
     * インポート期間の計算
     */
    calculateImportDuration(): string {
        // 簡易的な推定（実際のタイマーは実装されていない）
     ...
     * エラー報告の送信（開発環境用）
     */
    private reportImportError(errorData: ImportResult['data']): void {
        // 開発環境でのデバッグ用
     ...
     * 結果フィードバックアニメーション
     */
    animateResultFeedback(resultType: string, duration: number = 1000): AnimationResult {
        if (!this.feedbackAnimations[resultType]) {
    ...
    exportResult(format: string = 'json'): string | ImportResult {
        const result: ImportResult = {
            ...
            method: this.mainController.data.importMethod,
            error: this.mainController.data.error,
            stats: this.mainController.data.parsedData ? 
                this.generateImportStats(this.mainController.data.parsedData) : null
        } as any;
     ...
     * 結果をテキスト形式でフォーマット
     */
    formatResultAsText(result: any): string {
        const lines: string[] = [];

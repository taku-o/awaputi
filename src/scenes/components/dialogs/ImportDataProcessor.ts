...
export class ImportDataProcessor {
     ...
     * 確認ステップを描画
     */
    renderConfirmStep(context: CanvasRenderingContext2D, layout: Layout, y: number): void {
     ...
     * データプレビューを描画
     */
    renderDataPreview(context: CanvasRenderingContext2D, layout: Layout, y: number): void {
     ...
     * データを復元
     */
    async restoreData(importData: ImportData): Promise<void> {
     ...
     * プレイヤーデータの復元
     */
    async restorePlayerData(playerData: PlayerDataInterface, importedData: PlayerData): Promise<void> {
     ...
     * データサイズの検証
     */
    validateDataSize(data: any): DataSizeValidationResult {
     ...
     * バージョン互換性の確認
     */
    checkVersionCompatibility(dataVersion: string, currentVersion: string = '1.0.0'): VersionCompatibilityResult {
        // 簡易バージョン比較
     ...
     * データの完全性チェック
     */
    checkDataIntegrity(data: ImportData): DataIntegrityResult {
        const issues: string[] = [];

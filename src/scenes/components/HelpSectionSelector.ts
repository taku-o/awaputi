...
export class HelpSectionSelector {
    ...
    constructor(gameEngine: GameEngine, eventBus: ComponentEventBus, state: SceneState) {
        ...
        // アクセシビリティ設定
        this.accessibilitySettings = (state as any).accessibilitySettings || {};
    }
    ...
    render(context: CanvasRenderingContext2D, x: number, y: number, width: number): number {
        const buttonCount = this.sections.length;
    ...
    private renderSectionButton(
        ...
        // アクティブボタンのインジケーター
        if (isActive) {
            this.renderActiveIndicator(context, x, y + height - 3, width, 3);
        }
    }
     ...
     */
    handleClick(x: number, y: number, containerX: number, containerY: number, containerWidth: number): boolean {
     ...
     * @param containerWidth - コンテナ幅
     */
    handleHover(x: number, y: number, containerX: number, containerY: number, containerWidth: number): void {
        const relativeX = x - containerX;
        const relativeY = y - containerY;
     ...
     */
    private getButtonIndex(x: number, containerWidth: number): number {
        const buttonCount = this.sections.length;
        const totalSpacing = (buttonCount - 1) * this.buttonSpacing;
     ...
     */
    private selectSection(section: string, buttonIndex: number): void {
     ...
     */
    private startSectionAnimation(section: string): void {
     ...
     * @returns セクション情報の配列
     */
    getAvailableSections(): Array<{ id: string; label: string; active: boolean }> {

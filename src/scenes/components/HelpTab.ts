...
interface HelpContent {
    [sectionId: string]: HelpSectionContent;
}
...
export class HelpTab extends TabComponent {
    // ヘルプセクション
    ...
    private initializeHelpContent(): void {
        try {
            // 既存のAchievementHelpSystemからコンテンツを移行
            this.helpContent = {
                overview: {
                    title: '実績システム概要',
                    content: [
     ...
     * フォールバックコンテンツを作成
     */
    private createFallbackContent(): HelpContent {
        return {
            overview: {
                title: 'ヘルプ',
                content: [
     ...
     * ヘルプセクションセレクターを描画
     */
    private renderHelpSectionSelector(context: CanvasRenderingContext2D, x: number, y: number, width: number): number {
     ...
     * ヘルプコンテンツを描画
     */
    private renderHelpContent(context: CanvasRenderingContext2D, x: number, y: number, width: number, height: number): void {
        ...
        for (const line of currentSection.content) {
            ...
            } else if (line.startsWith('•') || line.startsWith('Q:') || line.startsWith('A:')) {
                // リストアイテムや質問回答
                if (currentY + this.lineHeight > y) {
     ...
     * セクションボタンクリック処理
     */
    private handleSectionClick(x: number, y: number): boolean {
        const buttonWidth = Math.min(100, (800 - this.contentPadding * 2) / this.helpSections.length - 10);
        const buttonY = 10;

...
export class ScenesDialogManager {
     ...
     * @returns レイアウト情報
     */
    private calculateDialogLayout(canvas: HTMLCanvasElement): DialogLayout {
     ...
     */
    private renderOverlay(context: CanvasRenderingContext2D, layout: DialogLayout): void {
        // 背景をダークオーバーレイで覆う
     ...
     */
    private renderDialogContent(context: CanvasRenderingContext2D, layout: DialogLayout, dialogInfo: DialogInfo): void {
     ...
     */
    private startAnimation(type: 'fade-in' | 'fade-out' | 'scale-in'): void {
        if (!this.animation.enabled) {
            return;
        }
     ...
     * @param layout - レイアウト情報
     */
    private updateAnimation(context: CanvasRenderingContext2D, layout: DialogLayout): void {

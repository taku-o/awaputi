...
interface ErrorInfo {
    type: 'listener_error' | 'emit_error';
    error: Error;
    event: string;
    listenerId?: string;
    data?: any;
...
export class ComponentEventBus {
    ...
    emit(event: string, data?: any): boolean {
        try {
            ...
            this.handleEmitError(error as Error, event, data);
            return false;
        }
    }
     ...
     */
    private handleListenerError(error: Error, event: string, listener: ListenerInfo): void {
     ...
     * @param data - イベントデータ
     */
    private handleEmitError(error: Error, event: string, data: any): void {
        const errorInfo: ErrorInfo = {
            type: 'emit_error',
            error,
            event,

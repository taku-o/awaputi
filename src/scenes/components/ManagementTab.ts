...
export class ManagementTab extends TabComponent {
     ...
     * ユーザー情報セクションの高さを推定
     */
    private estimateUserInfoHeight(): number {
        // 基本的な高さ + データ行数 * 行高
...
class DataManagementRenderer {
     ...
     */
    private renderButton(context: CanvasRenderingContext2D, x: number, y: number, width: number, height: number, button: Button, index: number): void {
     ...
     * データリセット処理
     */
    private async handleDataReset(): Promise<void> {
        // 確認ダイアログの表示は今後実装

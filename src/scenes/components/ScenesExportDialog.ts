...
// GameEngine interface for export dialog
...
export class ScenesExportDialog extends ScenesBaseDialog {
    private exportType: ExportType = 'playerData';
    ...
    // プレビューデータ
    private previewData: ExportData | null = null;
     ...
     * @returns ラベル
     */
    private getOptionLabel(key: string): string {
    ...
    private formatAsText(data: ExportData): string {
        ...
        if (data.playerData?.highScores && Object.keys(data.playerData.highScores).length > 0) {
            lines.push('ハイスコア:');
            Object.entries(data.playerData.highScores).forEach(([stage, score]) => {
    ...
    private collectPlayerData(): PlayerData {
        const playerData = (this.gameEngine as any).playerData as PlayerData || {};
    ...
    private collectGameSettings(): GameSettings {
        const gameSettings = (this.gameEngine as any).gameSettings as GameSettings || {};
     ...
     * @returns 実績データ
     */
    private collectAchievements(): { achievements: Achievement[] } {
        const achievements: Achievement[] = [];
        const options = this.exportOptions.achievements;
        
        if ((this.gameEngine as any).achievementManager) {
            const manager = (this.gameEngine as any).achievementManager as AchievementManager;

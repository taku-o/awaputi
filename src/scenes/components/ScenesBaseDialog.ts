...
export class ScenesBaseDialog {
    ...
    render(context: CanvasRenderingContext2D, layout: Partial<DialogLayout> | null = null): void {
        ...
        try {
            ...
            this.renderError(context, error as Error);
        }
    }
     ...
     * @param context - Canvas描画コンテキスト
     */
    protected renderButtons(context: CanvasRenderingContext2D): void {
     ...
     * @returns 処理されたかどうか
     */
    handleClick(x: number, y: number, layout: Partial<DialogLayout> | null = null): boolean {
        if (layout) {
            this.layout = { ...this.layout, ...layout };
        }
     ...
     * @param buttons - ボタン配列
     */
    setupButtons(buttons?: DialogButton[]): void {
     ...
     */
    protected getClickedButtonIndex(x: number, y: number): number {
        if (!this.buttons || this.buttons.length === 0) {
            return -1;
        }
     ...
     * @param error - エラーオブジェクト
     */
    protected handleRenderError(operation: string, error: Error): void {

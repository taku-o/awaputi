...
export class WCAGRuleEngine {
     ...
     */
    private setupTestRegistry(): void {
     ...
     */
    private testNameRoleValue(): TestResult {
        const issues: TestIssue[] = [];
        const warnings: TestWarning[] = [];
     ...
     */
    shouldRunGuideline(guidelineId: string): boolean {
        const guideline = this.guidelines[guidelineId];
     ...
     * Get test results
     */
    getTestResults(): Array<{test: string} & TestResult> {
        return Array.from(this.executionState.results.entries()).map(([test, result]) => ({
            test,
            ...result
        }));
    }

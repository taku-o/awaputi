...
export class KeyboardNavigationTester {
     ...
     * タブ順序テスト
     */
    private async test_tabOrder(options: TestOptions): Promise<TestResult> {
     ...
     * ブラウザショートカット競合テスト
     */
    private async test_browserConflicts(options: TestOptions): Promise<TestResult> {
        ...
        browserShortcuts.forEach(browserShortcut => {
            const conflictingShortcut = registeredShortcuts.find(s =>
     ...
     * エスケープルートの確認
     */
    private checkEscapeRoute(modal: HTMLElement): boolean {
        // Escapeキーハンドラーの存在確認
    ...
    private setupEventHandlers(): void {
        ...
        // フォーカスイベントの委譲
        this.stateManager.on('focusChange', (element: HTMLElement) => {
    ...
    private setupObservers(): void {
        ...
        this.observers.focus.observe(document.body, {
            attributes: true,
    ...
    private handleKeyboardEvent(event: KeyboardEvent): void {
        if (!this.navigationState.isActive) return;
    ...
    private handleFocusChange(element: HTMLElement): void {
        this.navigationState.currentElement = element;
     ...
     * ランドマーク間の循環
     */
    private cycleLandmarks(): void {
        const landmarks = this.stateManager.getLandmarks();
     ...
     * 平均フォーカス時間の更新
     */
    private updateAverageFocusTime(time: number): void {
    ...
    enableNavigation(): void {
        this.navigationState.isActive = true;
     ...
     * 特定のテストスイートの実行
     */
    async runTestSuiteByName(suiteName: keyof TestSuites, options: TestOptions = {}): Promise<SuiteResult> {
     ...
     * ショートカットの登録
     */
    registerShortcut(key: string, modifiers: string[], handler: Function, description?: string): void {
        this.eventHandler.registerShortcut({
            key,
            modifiers,
            handler,
    ...
    getCurrentFocus(): HTMLElement | null {
        return this.navigationState.currentElement;
    }
    ...
    getFocusHistory(): HTMLElement[] {
        return [...this.navigationState.focusHistory];
    }
    ...
    destroy(): void {
        ...
        // オブザーバーの停止
        if (this.observers.focus) {
            this.observers.focus.disconnect();
        }

...
interface LanguageAccessibilityConfig {
    enabled: boolean;
...
interface KeyboardLayouts {
    [language: string]: KeyboardLayout;
}

interface CulturalMetaphor {
    directions: {
        next: string;
        previous: string;
        forward: string;
...
interface CulturalMetaphors {
    [language: string]: CulturalMetaphor;
}

interface RTLAdaptation {
    applied: boolean;
...
interface NavigationKeyMapping {
    [key: string]: string;
}
...
interface AccessibilityManager {
    ...
    gameContentDescriber?: {
        updateGestureDescriptions: (gestures: Record<string, string>) => void;
    };
    eventSystem?: {
...
export class LanguageSpecificAccessibility {
     ...
     * 言語固有設定の適用
     */
    private applyLanguageSpecificSettings(): void {
        // RTL言語の場合の特別処理
    ...
    private applyRTLSupport(): void {
        ...
        this.rtlAdaptation.applied = true;
     ...
     * ゲーム要素のRTL適応
     */
    private applyRTLToGameElements(): void {
     ...
     * RTL用ナビゲーション適応
     */
    private adaptNavigationForRTL(): void {
        // 左右キーの動作を反転
     ...
     */
    private mapToUnifiedAction(languageAction: string): string | null {
     ...
     */
    private setupKeyboardLayoutSupport(): void {
        try {
            // キーボードイベントリスナーの設定
     ...
     * 言語固有のキーボード処理
     */
    private handleLanguageSpecificKeyboard(event: KeyboardEvent): void {
        try {
            // RTL言語での方向キー処理の調整
            if (this.isRTLLanguage(this.currentLanguage)) {
     ...
     * RTL言語での方向キー処理
     */
    private handleRTLKeyboardNavigation(event: KeyboardEvent): void {
     ...
     * キーボードレイアウトでの調整
     */
    private adjustForKeyboardLayout(event: KeyboardEvent): void {
        // 言語固有のキーマッピング調整
     ...
     * 文化的メタファーの初期化
     */
    private initializeCulturalMetaphors(): void {
        // 文化的メタファーの初期化処理
     ...
     * 意味に対応する色の取得
     */
    private getColorForMeaning(meaning: string): string {
     ...
     * ジェスチャーガイドラインの更新
     */
    private updateGestureGuidelines(): void {
        if (!this.currentMetaphors) return;
     ...
     * 混合方向テキストの検出
     */
    private containsMixedDirectionText(text: string): boolean {
    ...
    private getButtonDescription(element: ElementDescription, metaphors: CulturalMetaphor): string {
        ...
        if (this.currentLanguage === 'ja') {
            return `${element.text}ボタン - ${action}を実行します`;
     ...
     * ナビゲーション説明の生成
     */
    private getNavigationDescription(element: ElementDescription, metaphors: CulturalMetaphor): string {
        const directions = metaphors.directions;
        
        if (this.currentLanguage === 'ja') {
            return `ナビゲーション: ${directions.next}は右矢印、${directions.previous}は左矢印`;
        } else if (this.currentLanguage === 'ar') {
     ...
     * ステータス説明の生成
     */
    private getStatusDescription(element: ElementDescription, metaphors: CulturalMetaphor): string {
        const colors = metaphors.colors;
     ...
     * キーボードレイアウト情報の取得
     */
    getKeyboardLayoutInfo(language: string | null = null): KeyboardLayout {
     ...
     * 文化的メタファー情報の取得
     */
    getCulturalMetaphors(language: string | null = null): CulturalMetaphor {
        const lang = language || this.currentLanguage;
    ...
    getCurrentLanguageInfo(): LanguageInfo {
        return {
            ...
            keyboardLayout: this.currentLayout?.name || 'Unknown',

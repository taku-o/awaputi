...
interface SimulationResults {
    screenReader: ScreenReaderResults | null;
...
interface AriaValidationResults {
    timestamp: number;
    results: {
...
interface CompatibilityReport {
    screenReaders: Record<string, CompatibilityInfo>;
...
interface TTSEventHandlers {
    onStart: (announcement: Announcement) => void;
...
interface ScreenReaderEngineConfig {
    enabled: boolean;
...
export class ScreenReaderSimulator {
     ...
     */
    async runFullSimulation(readerType: string = 'all'): Promise<SimulationReport | null> {
     ...
     */
    async runReaderSimulation(readerType: string): Promise<ScreenReaderResults | null> {
     ...
     */
    async validateAriaAttributes(): Promise<AriaValidationResults | null> {
     ...
     * Process announcements through TTS
     */
    private async processAnnouncementsThroughTTS(): Promise<void> {
        ...
        for (const announcement of announcements) {
            try {
                await this.ttsController.announce(announcement.announcement || announcement.text, {
     ...
     */
    switchScreenReader(readerType: string): boolean {
     ...
     */
    getSimulationResults(): SimulationReport {
        return {
            simulationTime: this.stats.averageSimulationTime,
            results: this.results,
            stats: this.stats,
            performance: this.performance
        };
     ...
     */
    private saveSimulationResults(): void {
        try {
     ...
     */
    loadSimulationHistory(): SimulationHistoryEntry[] {
        try {
    ...
    setupEventListeners(): void {
        ...
        if (this.accessibilityManager?.eventSystem) {
            ...
            this.accessibilityManager.eventSystem.on('screen-reader-test-requested', (readerType: string) => {
     ...
     * Apply configuration
     */
    applyConfig(config: Partial<SimulatorConfig>): void {
        this.config = {
            ...this.config,
            ...config
        };
     ...
     */
    async testScreenReader(readerType: string = 'nvda'): Promise<TestResult> {
        try {

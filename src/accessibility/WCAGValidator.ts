...
export class WCAGValidator {
     ...
     * 完全なWCAG検証を実行
     */
    async runFullValidation(): Promise<ValidationReport | null> {
     ...
     */
    private processAuditResults(auditResults: AuditResults): void {
     ...
     * カテゴリーの検証
     */
    async validateCategory(categoryId: string): Promise<CategoryResult | null> {
     ...
     * ガイドラインの検証
     */
    async validateGuideline(categoryId: string, guidelineId: string, guideline: any): Promise<GuidelineResult | null> {
     ...
     * 個別テストの実行
     */
    async runTest(testName: string, level: string): Promise<TestResult | null> {
    ...
    generateValidationReport(type: ReportOptions['type'] = 'summary', format: ReportOptions['format'] = 'json'): ValidationReport {
        const auditResults: AuditResults = {
            ...
            categories: Object.fromEntries(
                Object.entries(this.results.categoryScores).map(([id, score]) => [
                    id,
                    {
                        id,
                        name: id,
    ...
    private scheduleRevalidation(): void {
        ...
        this.revalidationTimeout = window.setTimeout(() => {
            this.runQuickValidation();
        }, 5000); // 5秒後に実行
    }
     ...
     * リアルタイム検証の切り替え
     */
    toggleRealTimeValidation(enabled: boolean): void {
     ...
     * 設定の適用
     */
    applyConfig(config: Partial<ValidatorConfig>): void {
        this.config = {
            ...this.config,
            ...config
        };

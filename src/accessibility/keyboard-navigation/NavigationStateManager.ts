...
interface RestorationTestResults {
    passed: boolean;
    issues: TabOrderIssue[];
...
export class NavigationStateManager {
     ...
     * フォーカス変更の処理
     */
    private handleFocusChange(element: Element, type: string): void {
     ...
     * 論理的タブ順序の検証
     */
    private validateLogicalTabOrder(tabOrder: TabElement[]): TabOrderIssue[] {
     ...
     * 視覚的タブ順序のチェック
     */
    private checkVisualTabOrder(current: TabElement, next: TabElement): TabOrderIssue | null {
        try {
     ...
     * フォーカス包含テスト
     */
    async testFocusContainment(): Promise<ContainmentTestResults> {
     ...
     * モーダルフォーカス包含テスト
     */
    private async testModalFocusContainment(modal: Element): Promise<ContainmentResult> {
     ...
     * コンテナフォーカス包含テスト
     */
    private async testContainerFocusContainment(container: Element): Promise<ContainmentResult> {
     ...
     * フォーカス復元テスト
     */
    async testFocusRestoration(): Promise<RestorationTestResults> {
        const results: RestorationTestResults = {
            passed: true,
            issues: [],
            restorationTests: []
        };
     ...
     * 要素のフォーカス復元テスト
     */
    private async testElementFocusRestoration(trigger: Element, initialFocus: Element | null): Promise<RestorationTestResult> {
     ...
     * フォーカス包含のチェック
     */
    private checkFocusContainment(element: Element): void {
        try {
     ...
     * Escapeキーシミュレーション
     */
    private async simulateEscape(): Promise<SimulationResult> {
     ...
     * フォーカス履歴の取得
     */
    getFocusHistory(limit: number = 50): FocusEvent[] {
     ...
     * ナビゲーション統計の取得
     */
    getNavigationStats(): NavigationStatsInfo {
        return {
            ...this.stats,
     ...
     * フォーカス包含コンテナの追加
     */
    addFocusContainer(container: Element): void {
     ...
     * フォーカス包含コンテナの削除
     */
    removeFocusContainer(container: Element): void {

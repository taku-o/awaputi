...
interface DetailedResults {
    suite_results: Record<string, SuiteResult>;
    categorized_issues?: CategorizedIssues;
    statistics: Statistics;
}

interface Appendix {
    test_configuration: ReporterConfig;
...
export class KeyboardAccessibilityReporter {
     ...
     * テスト結果の処理と分析
     */
    processTestResults(results: Partial<TestResults>): TestResults | null {
        try {
            this.testResults = {
                ...this.testResults,
                ...results,
     ...
     * 問題のカテゴリ決定
     */
    private determineIssueCategory(issue: Issue): string {
     ...
     * WCAG参照の取得
     */
    private getWCAGReference(issue: Issue): string {
     ...
     * カテゴリ別推奨事項の生成
     */
    private generateCategoryRecommendation(category: string, issues: Issue[]): Recommendation | null {
     ...
     * カテゴリ別アクション生成
     */
    private generateCategoryActions(category: string, issues: Issue[]): string[] {
     ...
     * スコアベース推奨事項の生成
     */
    private generateScoreBasedRecommendations(): Recommendation[] {
        const recommendations: Recommendation[] = [];
     ...
     * WCAG参照情報の生成
     */
    private generateWCAGReferences(): Record<string, string> {
        return {
     ...
     * HTMLレポートの生成
     */
    generateHTMLReport(): string | null {
...
<body>
     ...
     * 推奨事項の取得
     */
    getRecommendations(priority: 'critical' | 'high' | 'medium' | 'low' | null = null): Recommendation[] {

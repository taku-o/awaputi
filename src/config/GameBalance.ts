...
export class OriginalBalanceHelper {
    ...
    static calculateScore(bubbleType: string, ageRatio: number = 0): number { const baseScore = ORIGINAL_BALANCE_CONFIG.scoring.baseScores[bubbleType] || 15;
        ...
        } else if (ageRatio > 0.9) { multiplier = ORIGINAL_BALANCE_CONFIG.scoring.ageBonus.lateBonus; }
        } else if (ageRatio >= 0.5 && ageRatio <= 0.7) { multiplier = ORIGINAL_BALANCE_CONFIG.scoring.ageBonus.midBonus; };
}

 ...
 * 互換性レイヤーが参照する基本設定
 */
export const ORIGINAL_BALANCE_CONFIG: OriginalBalanceConfig = { scoring: { baseScores: {
            normal: 15  ,
            stone: 25,
    iron: 40,
    diamond: 60,
...
export class OriginalBalanceHelper {
     ...
     */
    static calculateScore(bubbleType: string, ageRatio: number = 0): number { const baseScore = ORIGINAL_BALANCE_CONFIG.scoring.baseScores[bubbleType] || 15,
        let multiplier = 1,
        ...
        } else if (ageRatio > 0.9) { multiplier = ORIGINAL_BALANCE_CONFIG.scoring.ageBonus.lateBonus } else if (ageRatio >= 0.5 && ageRatio <= 0.7) { multiplier = ORIGINAL_BALANCE_CONFIG.scoring.ageBonus.midBonus }
     ...
     */
    static calculateItemCost(itemId: string, currentLevel: number): number { const baseCost = ORIGINAL_BALANCE_CONFIG.items.baseCosts[itemId] || 100,

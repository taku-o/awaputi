...
// ステータス型定義
interface EnvironmentalAudioStatus {
    enabled: boolean;
    disabled?: boolean;
    reason?: string;
...
export class EnvironmentalAudioManager {
    ...
    setVolume(volume: number): void {
        ...
        this.settings?.setVolume(volume, (volume: number) => {
            // アクティブな環境音の音量を更新
    ...
    setBiome(biomeId: string, options: BiomeOptions = {}): void {
        ...
        try {
            ...
            const {
                fadeTime = this.settings?.getSettings().fadeTime || 1.0,
                weather = null,
                timeOfDay = 'day',
                intensity = 1.0
            } = options;
    ...
    getStatus(): EnvironmentalAudioStatus {
        ...
        const additionalData = {
            ...
            activeLayers: this.transitionController?.getActiveLayerCount() || 0,
            availableBiomes: this.biomeDefinitionManager?.getAvailableBiomes().length || 0,
            availableWeatherEffects: this.biomeDefinitionManager?.getAvailableWeatherEffects().length || 0,

...
export class BGMPlayer {
    constructor(audioContext, bgmGainNode) {
        ...
        this.crossfadeTime = 0.5; // ループ時のクロスフェード時間
    ...
    async play(track, options = {}) {
        try {
            ...
            // 現在の再生を停止
            if (this.isPlaying) {
                await this.stop({ fadeOut: options.fadeOut !== false });
            }
            ...
            if (fadeInDuration > 0) {
                ...
                this.fadeGainNode.gain.setValueAtTime(volume, this.audioContext.currentTime);
            }
            ...
            getErrorHandler().handleError(error, 'BGM_PLAYER_ERROR', {
                operation: 'play',
    ...
    async _performSeamlessLoop() {
        try {
            ...
            newFadeGainNode.gain.linearRampToValueAtTime(this.fadeGainNode.gain.value, currentTime + crossfadeTime);
    ...
    async stop(options = {}) {
        try {
            ...
            // 統計更新
            if (this.startTime > 0) {
                this.playbackStats.totalPlayTime += this.audioContext.currentTime - this.startTime;
            }
    ...
    /**
     * 再生速度を設定
     * @param {number} rate - 再生速度 (0.25-4.0)
     */
    ...
    /**
     * クロスフェード時間を設定
     * @param {number} time - クロスフェード時間（秒）
     */
    setCrossfadeTime(time) {
        this.crossfadeTime = Math.max(0, Math.min(5, time));

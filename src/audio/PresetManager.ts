...
';'
    ...
    saveCurrentAsPreset(name: string, description: string = '', tags: string[] = [], isTemporary: boolean = false): string | null { try {
            ...
            // プリセットを保存
            if (isTemporary) { this.temporaryPresets.set(presetId, presetData); }
            } else {  this.userPresets.set(presetId, presetData); }
    ...
    duplicatePreset(sourcePresetId: string, newName: string, isTemporary: boolean = false): string | null { try {
            ...
            // プリセットを保存
            if (isTemporary) { this.temporaryPresets.set(newPresetId, newPresetData); }
            } else {  this.userPresets.set(newPresetId, newPresetData); }
                this._saveUserPresets(); }
            }

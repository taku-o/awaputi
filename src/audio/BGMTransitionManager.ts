...
export class BGMTransitionManager {
    ...
    private readonly transitionTypes = {''
        ...
        CROSSFADE: 'crossfade',
        CUT: 'cut',
        SMART_CROSSFADE: 'smart_crossfade' ,} as const;
    // 音量カーブタイプ
    ...
    private async _fadeOutInTransition(fromTrack: string, toTrack: string, options: TransitionOptions): Promise<void> { const { fadeOutDuration = this.defaultFadeOutDuration,
        ...
        // 現在のBGMをフェードアウト
        if (this.bgmSystem.isPlaying) { await this.fadeOut(fadeOutDuration, curve); }
    ...
    private _getKeyCompatibility(key1: string, key2: string): number { ''
        ...
        // 完全5度(7)、完全4度(5)、短3度(3)関係を高く評価 : undefined
        const harmonyWeights: Record<number, number> = { 0: 1.0, 7: 0.9, 5: 0.9, 3: 0.7, 4: 0.7, 8: 0.6, 9: 0.6  ,};
     ...
     * @param curve - カーブタイプ
     */
    async fadeOut(duration: number = this.defaultFadeOutDuration, curve: string = this.curveTypes.SMOOTH): Promise<void> { try {
     ...
     * @param settings - 設定
     */
    updateSettings(settings: TransitionSettings): void { try {
            ...
            if (settings.defaultFadeOutDuration !== undefined) { this.defaultFadeOutDuration = Math.max(0.1, Math.min(10, settings.defaultFadeOutDuration); }
     ...
     * @returns プロミス
     */
    private _wait(seconds: number): Promise<void> { return new Promise(resolve => setTimeout(resolve, seconds * 1000);

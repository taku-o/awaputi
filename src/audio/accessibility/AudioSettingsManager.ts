...
interface MainController {
    ...
    audioFeedbackManager: {
        updateColorIndicatorVisibility(visible: boolean): void;
        hapticSettings: {
            enabled: boolean;
            vibrationIntensity: number;
        };
...
export class AudioSettingsManager {
    ...
    public setupConfigWatchers(): void {
        ...
        // 触覚フィードバック設定の監視
        this.configManager.watch('audio', 'accessibility.hapticFeedback', (newValue: boolean) => {
        ...
        this.configManager.watch('audio', 'accessibility.vibrationIntensity', (newValue: number) => {
     ...
     * @param value - 設定値
     */
    public updateSetting(key: keyof AccessibilitySettings, value: any): void {
     ...
     * @returns 設定値
     */
    public getSetting(key: keyof AccessibilitySettings): any {
     ...
     */
    public exportSettings(): ExportedSettings {
        return {
            version: '1.0.0',
     ...
     * @returns インポート成功かどうか
     */
    public importSettings(settingsData: ExportedSettings): boolean {
        try {
     ...
     * @returns 検証結果
     */
    public validateSettings(settings: Partial<AccessibilitySettings>): ValidationResult {
        const errors: string[] = [];

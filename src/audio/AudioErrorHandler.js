...
export class AudioErrorHandler {
    ...
    updateErrorStats(error, errorType) {
        ...
        // 時間窓での集計（エラー頻度チェック用）
        const now = Date.now();
        if (now - this.errorState.lastErrorTime < 60000) { // 1分以内
    ...
    async recoverAudioDecode(error, context) {
        try {
            ...
            if (originalFormat !== 'wav') {
                ...
                return await this.audioManager.decodeAudioData(wavData);
            }
            
            // 品質を下げて再試行
            const lowerQualityData = await this.reduceAudioQuality(audioData);
            return await this.audioManager.decodeAudioData(lowerQualityData);
    ...
    async recoverAudioLoad(error, context) {
        try {
            ...
            if (retryCount < 3) {
                ...
                return await this.audioManager.loadAudioFile(url, { 
                    ...context, 
            ...
            if (fallbackUrl) {
                return await this.audioManager.loadAudioFile(fallbackUrl);
            }
    ...
    async recoverAudioPlay(error, context) {
        try {
            ...
            return await this.audioManager.recreateAndPlaySound(soundId, options);
    ...
    async recoverBGMGeneration(error, context) {
        try {
            ...
            return await this.audioManager.generateBGM(simplifiedConfig);
    ...
    generateRecommendations() {
        ...
        if (this.errorStats.errorsByType.get(this.errorTypes.MEMORY) > 0) {
    ...
    reset() {
        ...
        Object.keys(this.fallbackOptions).forEach(key => {
            this.fallbackOptions[key] = false;
        });

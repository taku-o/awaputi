...
export interface LoadTimeDataPoint {
    timestamp: number;
    avgLoadTime: number;
}

export interface RequestCountDataPoint {
    timestamp: number;
    requests: number;
}
/**
...
export class CacheStatistics {
     ...
     * パフォーマンスのスナップショットを収集
     */
    collectPerformanceSnapshot(): void {
        try {
     ...
     */
    updateOverallStatistics(snapshot: PerformanceSnapshot): void {
     ...
     * @param snapshot - パフォーマンススナップショット
     */
    updateTimeSeriesData(snapshot: PerformanceSnapshot): void {
     ...
     * @param snapshot - パフォーマンススナップショット  
     */
    updateCategoryStatistics(snapshot: PerformanceSnapshot): void {
        // AudioBuffer統計
     ...
     * @returns 推定Set操作数
     */
    estimateSetOperations(snapshot: PerformanceSnapshot): number {
        // エントリ数から推定（簡略化）
     ...
     * @returns 推定Delete操作数
     */
    estimateDeleteOperations(snapshot: PerformanceSnapshot): number {
     ...
     * @returns 傾向分析結果
     */
    analyzeTrends(timeRangeMs: number = 3600000): TrendAnalysis {
        // デフォルト1時間
        try {
     ...
     * @returns 統計サマリー
     */
    generateSummary(): StatisticsSummary {
     ...
     * @returns 推奨事項配列
     */
    generateRecommendations(trends: TrendAnalysis): Recommendation[] {
     ...
     * @returns エクスポートデータ
     */
    exportStatistics(format: 'json' | 'csv' = 'json'): string | null {
        try {
            const data = {

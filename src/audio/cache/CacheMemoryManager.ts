...
export class CacheMemoryManager {
     ...
     * メモリ監視を開始
     */
    startMemoryMonitoring(): void { try {
            // 既存の監視を停止
     ...
     * @returns メモリ使用量情報
     */
    getCurrentMemoryUsage(): MemoryUsageInfo { const bufferCacheUsage = this.mainController.audioBufferCache.currentSize;
     ...
     * 自動クリーンアップを実行
     */
    performAutomaticCleanup(): void { try {
            const now = Date.now();
     ...
     * 緊急クリーンアップを実行
     */
    performEmergencyCleanup(): void { try {
     ...
     */
    performSimpleOptimization(buffer: AudioBuffer): AudioBuffer { try {
            // ステレオからモノラルへの変換（簡略化）

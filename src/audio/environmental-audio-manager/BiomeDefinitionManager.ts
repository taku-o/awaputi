 ...
 */
export interface TimeRange { start: number, // Hour (0-23);
...
export class BiomeDefinitionManager {
    ...
    private _initializeBiomes(''';
            ...
            this.biomes.set('cave', { ''
                ...
                characteristics: { baseFrequency: 60)
                    richness: 0.7);
                    dynamics: 0.3 });
     ...
     * @private'
     */''
    private _initializeWeatherEffects(''';
     ...
     * バイオーム定義を取得
     */
    getBiome(biomeId: string): BiomeDefinition | undefined { return this.biomes.get(biomeId); }
    
    /**
     * 天候効果を取得
     */
    getWeatherEffect(weatherId: string): WeatherEffect | undefined { return this.weatherEffects.get(weatherId); }
    
    /**
     * 時間帯バリエーションを取得
     */
    getTimeVariation(timeId: string): TimeVariation | undefined { return this.timeVariations.get(timeId); }
    
    /**
     * 利用可能なバイオーム一覧を取得
     */
    getAvailableBiomes(): AvailableBiome[] { return Array.from(this.biomes.values().map(biome => ({
            id: biome.id);
            name: biome.name);
     ...
     * 利用可能な天候効果一覧を取得
     */
    getAvailableWeatherEffects(): AvailableWeatherEffect[] { return Array.from(this.weatherEffects.values().map(weather => ({
            id: weather.id);
            name: weather.name);
     ...
     */
    getAvailableTimeVariations(): AvailableTimeVariation[] { return Array.from(this.timeVariations.values().map(time => ({
            id: time.id);
            name: time.name);
     ...
     * 音の特性を取得'
     */''
    getSoundCharacteristics(soundType: string): SoundCharacteristics { const defaultCharacteristics: SoundCharacteristics = {
            frequency: 1.0;
            amplitude: 0.3;
            noiseLevel: 0.2;
            modulation: 0.1 };

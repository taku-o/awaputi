 ...
 */
export interface SystemStatus extends EnvironmentalSettings {
...
export class EnvironmentalAudioSettings {
     ...
     * 環境音を有効/無効に設定
     */
    setEnabled(enabled: boolean, callback: EnabledCallback | null = null): void {
        try {
     ...
     * 環境音音量を設定
     */
    setVolume(volume: number, callback: VolumeCallback | null = null): void {
        try {
     ...
     * パフォーマンスデータを更新
     */
    updatePerformanceData(activeLayers: number, cpuUsage: number = 0, memoryUsage: number = 0): void {
        this.performanceData = {

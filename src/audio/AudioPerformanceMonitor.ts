...
class PerformanceMetrics {
    ...
    addSample(cpu: number, memory: number, nodeCount: number, frameRate: number | null = null): void {
    ...
    getMax(metric: 'cpuTime' | 'memoryUsage' | 'audioNodeCount' | 'frameRate'): number {
        const values = this.getMetricArray(metric);
        return values.length > 0 ? Math.max(...values) : 0;
    }
    
    getMin(metric: 'cpuTime' | 'memoryUsage' | 'audioNodeCount' | 'frameRate'): number {
        const values = this.getMetricArray(metric);
    ...
    getVariance(metric: 'cpuTime' | 'memoryUsage' | 'audioNodeCount' | 'frameRate'): number {
        const values = this.getMetricArray(metric);
...
export class AudioPerformanceMonitor {
    ...
    private _applyPerformanceProfile(profileName: string): void {
        try {
            ...
            if (this.audioManager?.audioController) {
                (this.audioManager.audioController as any).setAudioQuality(profile.qualityLevel);
            }
     ...
     * @private
     */
    private _checkAndTriggerAlert(alertId: string, condition: boolean, alertData: any): void {
        try {
     ...
     */
    suppressAlert(alertId: string, duration: number = 60000): void {
        try {
     ...
     */
    updateMonitoringSettings(newSettings: Partial<MonitoringSettings>): void {
        try {
            // 設定をマージ

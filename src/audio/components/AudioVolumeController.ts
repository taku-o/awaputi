...
export class AudioVolumeController {
     ...
     * イコライザープリセットを設定
     */
    private setupEqualizerPresets(): void {
     ...
     * @param gainNode - 対象のゲインノード
     */
    cancelFade(gainNode: GainNode): void {
        try {
     ...
     * @returns ゲイン値（dB）
     */
    getEqualizerBandGain(bandIndex: number): number {
        try {
            if (bandIndex < 0 || bandIndex >= this.equalizer.bands.length) {
                throw new Error(`Invalid band index: ${bandIndex}`);
            }
     ...
     * @returns 周波数レスポンス
     */
    getEqualizerFrequencyResponse(frequencies: Float32Array): FrequencyResponse | null {
        try {
     ...
     * @returns イコライザー状態
     */
    getEqualizerStatus(): EqualizerStatus {
        return {
     ...
     * @returns 入力ノード
     */
    getEqualizerInput(): AudioNode | null {
     ...
     * @returns 出力ノード
     */
    getEqualizerOutput(): AudioNode | null {

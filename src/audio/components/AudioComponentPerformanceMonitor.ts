...
export class AudioComponentPerformanceMonitor {
    ...
    private setupConfigWatchers(): void {
        try {
            ...
            this.configManager.watch('performance', 'optimization.adaptiveMode', (newValue: boolean) => {
                if (newValue !== undefined) {
     ...
     * オーディオメトリクスを収集
     */
    private collectAudioMetrics(): void {
        try {
            // アクティブなオーディオノード数
     ...
     * @returns 処理負荷 (0-1)
     */
    private calculateAudioProcessingLoad(): number {
        try {
     ...
     * @returns アンダーラン数
     */
    private detectBufferUnderruns(): number {
        try {
            // 簡易的な検出
     ...
     */
    private calculateAverageLoad(metrics: MetricsHistoryEntry[]): void {
     ...
     * @param metrics - メトリクス配列
     */
    private calculatePeakLoad(metrics: MetricsHistoryEntry[]): void {
     ...
     * @param metrics - メトリクス配列
     */
    private calculateStabilityScore(metrics: MetricsHistoryEntry[]): void {
        if (metrics.length < 2) {
     ...
     * パフォーマンスグレードを決定
     */
    private determinePerformanceGrade(): void {
     ...
     * @param alert - アラート情報
     */
    private triggerAutoOptimization(alert: Alert): void {
        try {
            switch (alert.type) {
                case 'cpu':
     ...
     * @param currentTime - 現在時刻
     */
    private cleanupAlerts(currentTime: number): void {
     ...
     * @returns パフォーマンス履歴
     */
    getPerformanceHistory(timeWindow: number = 30000): MetricsHistoryEntry[] {
        const currentTime = performance.now();
    ...
    getStatus(): { [key: string]: any } {
        return {
            ...
            currentGrade: this.analysisData.performanceGrade
        };
    }

...
interface AudioManager {
    registerGainNodes?(gainNodes: GainNodeMap): void;
}
...
export class AudioChannelManager {
     ...
     * ゲインノード階層を作成
     */
    private createGainNodeHierarchy(): void {
        try {
            // マスターゲインノード
     ...
     * 初期音量を読み込み
     */
    private loadInitialVolumes(): void {
        try {
     ...
     * 音量をゲインノードに適用
     */
    private applyVolumeToGainNodes(): void {
        try {
            (Object.keys(this.gainNodes) as AudioChannel[]).forEach(channel => {
     ...
     */
    setVolume(channel: AudioChannel, volume: number, saveToConfig: boolean = true): number {
        try {
     ...
     * @param saveToConfig - 設定に保存するかどうか
     */
    setMute(channel: AudioChannel, muted: boolean, saveToConfig: boolean = true): boolean {
        try {
     ...
     * @returns ミュート状態
     */
    getMute(channel: AudioChannel): boolean {
        try {
            if (!(channel in this.muteStates)) {
                throw new Error(`Unknown audio channel: ${channel}`);
            }
     ...
     */
    adjustVolumeByStep(channel: AudioChannel, increase: boolean): number {
        try {
     ...
     */
    resetVolume(channel: AudioChannel | null = null): void {
        try {
            if (channel) {
                // 特定チャンネルをリセット
     ...
     * @param channel - チャンネル名（指定しない場合は全チャンネル）
     */
    resetMute(channel: AudioChannel | null = null): void {
        try {
            if (channel) {
                // 特定チャンネルをリセット
     ...
     * @returns ゲインノード
     */
    getGainNode(channel: AudioChannel): GainNode | null {
     ...
     * @param volume - 音量
     */
    private saveVolumeToConfig(channel: AudioChannel, volume: number): void {
        try {
     ...
     * @param muted - ミュート状態
     */
    private saveMuteToConfig(channel: AudioChannel, muted: boolean): void {
        try {
     ...
     */
    integrateWithAudioManager(audioManager: AudioManager): void {
        try {

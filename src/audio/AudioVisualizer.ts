...
export class AudioVisualizer {
    ...
    private setupConfigWatchers(): void {
        this.configManager.watch('audio', 'accessibility.visualFeedback', (enabled: boolean) => {
            this.setEnabled(enabled);
        });

        this.configManager.watch('audio', 'accessibility.highContrast', (enabled: boolean) => {
            this.highContrast = enabled;
            this.updateColorScheme();
        });

        this.configManager.watch('audio', 'accessibility.motionReduction', (enabled: boolean) => {
     ...
     * @private
     */
    private renderFrequencyBars(): void {
        if (!this.frequencyData || !this.ctx) return;
    ...
    private renderVolumeIndicator(): void {
        ...
        if (this.highContrast) {
            ...
            colorScheme.gradient.forEach((color, index) => {
                const rgbaColor = color.replace('#', '').match(/.{2}/g);
                if (rgbaColor && rgbaColor.length >= 3) {
                    const r = parseInt(rgbaColor[0], 16);
                    const g = parseInt(rgbaColor[1], 16);
                    const b = parseInt(rgbaColor[2], 16);
                    gradient.addColorStop(index / (colorScheme.gradient.length - 1), `rgba(${r}, ${g}, ${b}, 0.6)`);
                }
            });
        }
     ...
     * @private
     */
    private renderStereoScope(): void {
        if (!this.ctx) return;
     ...
     * @private
     */
    private updateAudioEvents(): void {
        // 音響レベルの変化を検出
     ...
     * @private
     */
    private renderAudioEvents(): void {
        if (this.motionReduction || !this.ctx) return; // モーション削減モード
     ...
     * @param type - 視覚化タイプ
     */
    setVisualizationType(type: VisualizationTypeKey): void {
     ...
     * @returns 統計情報
     */
    getStatistics(): VisualizerStatistics {
        return {

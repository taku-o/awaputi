...
export class SoundEffectSystem {
    ...
    playGameStateSound(state: GameState, options: PlaybackOptions = {}): any {
        ...
        try {
            ...
            return this.poolManager ? (this.poolManager as any).playSound('gamestate', state, playbackOptions) : null;
    ...
    stopAllSounds(): number {
        ...
        try {
            return this.poolManager ? (this.poolManager as any).stopAllSounds() : 0;
        } catch (error) {
    ...
    stopCategorySounds(category: SoundCategory): number {
        ...
        try {
            return this.poolManager ? (this.poolManager as any).stopCategory(category) : 0;
        } catch (error) {
     ...
     * エフェクト品質を設定
     */
    setEffectQuality(quality: string): void {
     ...
     * プール設定を更新
     */
    updatePoolConfig(config: any): void {
        if (this.poolManager) {
     ...
     * サウンドを再生成
     */
    async regenerateSounds(category: SoundCategory | null = null): Promise<void> {
        try {
            if (category) {
                // 特定カテゴリのサウンドのみ再生成
                switch (category) {

 ...
 * フィルターデータインターフェース
 */
interface FilterData extends BandConfig {
...
export class Equalizer {
     ...
     */
    private loadEqualizerSettings(): void {
        try {
     ...
     */
    setEnabled(enabled: boolean, saveToConfig: boolean = true): void {
        try {
     ...
     * @param saveToConfig - 設定に保存するか（デフォルト: true）
     */
    setBandGain(bandIndex: number, gain: number, saveToConfig: boolean = true): void {
        try {
     ...
     * @param saveToConfig - 設定に保存するか（デフォルト: true）
     */
    setAllBandGains(gains: number[], saveToConfig: boolean = true): void {
        try {
     ...
     * @param saveToConfig - 設定に保存するか（デフォルト: true）
     */
    reset(saveToConfig: boolean = true): void {
        try {
     ...
     * @param saveToConfig - 設定に保存するか（デフォルト: true）
     */
    applyPreset(presetName: string, saveToConfig: boolean = true): void {
        try {
     ...
     * @returns プリセット定義
     */
    getPresets(): { [key: string]: PresetDefinition } {
        return {
            flat: {
                name: 'フラット',
                description: '全周波数均等',
     ...
     * @returns 周波数レスポンスデータ
     */
    getFrequencyResponse(samplePoints: number = 256): FrequencyResponseData | null {
        try {

...
interface PlaybackInstance {
    sourceWrapper: AudioSourceWrapper;
    stop: () => void;
    category: string;
    soundKey: string;
    startTime: number;
...
export class SoundPoolManager {
     ...
     * 新しいオーディオソースを作成
     */
    private createNewSource(): AudioSourceWrapper {
        return {
            source: null,
     ...
     * サウンドをプールに追加
     */
    addToPool(category: SoundCategory, soundKey: string, audioBuffer: AudioBuffer, variations: AudioBuffer[] = []): void {
        try {
     ...
     * プールからサウンドを取得
     */
    private getFromPool(category: SoundCategory, soundKey: string, useVariation: boolean = true): AudioBuffer | null {
        try {
            const pool = this.categoryPools[category];
     ...
     * バリエーションを選択
     */
    private selectVariation(soundKey: string, soundEntry: SoundEntry): AudioBuffer {
     ...
     * オーディオソースを取得（プールから）
     */
    private getAudioSource(): AudioSourceWrapper | null {
        try {
            // プールから利用可能なソースを探す
     ...
     * オーディオソースを返却
     */
    private returnAudioSource(sourceWrapper: AudioSourceWrapper): void {
        try {
     ...
     * ソースラッパーを停止
     */
    private stopSourceWrapper(sourceWrapper: AudioSourceWrapper): void {
        try {
     ...
     * サウンドを再生
     */
    playSound(category: SoundCategory, soundKey: string, options: PlaybackOptions = {}): PlaybackInstance | null {
        try {
            // アクティブソース数をチェック
     ...
     * 再生オプションを適用
     */
    private applyPlaybackOptions(sourceWrapper: AudioSourceWrapper, options: PlaybackOptions): void {
        if (!sourceWrapper.source) return;
     ...
     * カテゴリ内の全サウンドを停止
     */
    stopCategory(category: SoundCategory): number {
        const stoppedSounds: AudioSourceWrapper[] = [];
    ...
    getPoolStatistics(): PoolStatistics {
        ...
        Object.keys(this.categoryPools).forEach(category => {
            const pool = this.categoryPools[category as SoundCategory];
     ...
     * カテゴリプールサイズを設定
     */
    setCategoryPoolSize(category: SoundCategory, maxSize: number): void {
        if (this.categoryPools[category]) {

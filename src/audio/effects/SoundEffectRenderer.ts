...
export class SoundEffectRenderer {
    ...
    private calculateEnvelope(sampleIndex: number, attackSamples: number, decaySamples: number, sustainSamples: number, releaseSamples: number, totalSamples: number): number { if (sampleIndex < attackSamples) {
        ...
        } else if (sampleIndex < attackSamples + decaySamples) { // Decay
            const decayProgress = (sampleIndex - attackSamples) / decaySamples;
            return 1 - (decayProgress * 0.3); // 70%まで減衰 }
        } else if (sampleIndex < attackSamples + decaySamples + sustainSamples) { // Sustain
            return 0.7; }
    ...
    private calculateSimpleEnvelope(sampleIndex: number, totalSamples: number, envelope: EnvelopeConfig | null = null): number { const progress = sampleIndex / totalSamples;
        ...
        if (envelope) { }
            ...
            if (progress < attackPoint) { return progress / attackPoint; }
            } else if (progress < decayPoint) { const decayProgress = (progress - attackPoint) / decay;
                return 1 - (decayProgress * 0.3); }
            } else if (progress < sustainPoint) { return 0.7; }
            } else {  const releaseProgress = (progress - sustainPoint) / release; }
                return 0.7 * (1 - releaseProgress); }
            }
        } else {  // デフォルトエンベロープ
            if (progress < 0.1) { }
                return progress / 0.1; }
            } else if (progress > 0.9) { return (1 - progress) / 0.1; }
            } else { return 1; }
            }
        }
    }
    ...
    private calculateNoteEnvelope(noteProgress: number): number { if (noteProgress < 0.1) {
        ...
        } else if (noteProgress > 0.8) { return (1 - noteProgress) / 0.2; }
        } else { return 1; }
        }
    }

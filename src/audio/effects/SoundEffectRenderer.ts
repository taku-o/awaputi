 ...
 * UIサウンド設定インターフェース
 */
interface UISoundConfig extends BaseSoundConfig {
    chord?: number[];
}
 ...
 * 実績サウンド設定インターフェース
 */
interface AchievementSoundConfig {
    baseFreq: number;
    duration: number;
    waveType: OscillatorType | 'noise';
    melody: number[];
 ...
 * レンダリング統計インターフェース
 */
interface RenderingStatistics {
    supportedBubbleTypes: number;
...
export class SoundEffectRenderer {
     ...
     * 泡破壊音を生成
     */
    generateBubbleSound(bubbleType: BubbleType, variation: number = 0): AudioBuffer | null {
        try {
     ...
     * UIサウンドを生成
     */
    generateUISound(uiType: UISoundType): AudioBuffer | null {
        try {
     ...
     * コンボサウンドを生成
     */
    generateComboSound(level: number): AudioBuffer | null {
        try {
     ...
     * 実績サウンドを生成
     */
    generateAchievementSound(rarity: AchievementRarity): AudioBuffer | null {
        try {
     ...
     * 波形を生成
     */
    private generateWaveform(waveType: OscillatorType | 'noise', frequency: number, time: number): number {
     ...
     * エンベロープを計算
     */
    private calculateEnvelope(sampleIndex: number, attackSamples: number, decaySamples: number, sustainSamples: number, releaseSamples: number, totalSamples: number): number {
     ...
     * シンプルなエンベロープを計算
     */
    private calculateSimpleEnvelope(sampleIndex: number, totalSamples: number, envelope: EnvelopeConfig | null = null): number {
     ...
     * 音符用エンベロープを計算
     */
    private calculateNoteEnvelope(noteProgress: number): number {
        if (noteProgress < 0.1) {
     ...
     * フォールバックサウンドを生成
     */
    private generateFallbackSound(duration: number): AudioBuffer | null {
        try {
            const buffer = this.audioContext.createBuffer(1, this.generationConfig.sampleRate * duration, this.generationConfig.sampleRate);
            const channelData = buffer.getChannelData(0);
     ...
     * 泡サウンド設定を更新
     */
    updateBubbleSoundConfig(bubbleType: BubbleType, config: Partial<BubbleSoundConfig>): void {

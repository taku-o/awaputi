    ...
    applyEffect(sourceNode: AudioNode, effectType: string, variation: EffectParameters = {}): ActiveEffectInstance | null {
        try {
            ...
            for (const effectName of chain) {
                const effect = this.effectNodes.get(effectName as EffectType);
                if (effect && this.effectConfig.effectTypes[effectName as EffectType]?.enabled) {
                    // エフェクトパラメータを適用
                    this.applyEffectParameters(effect, effectName as EffectType, variation);
                    // 接続

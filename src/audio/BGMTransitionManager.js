...
export class BGMTransitionManager {
    constructor(audioContext, bgmSystem) {
        ...
        // フェード設定
        this.defaultFadeInDuration = 2.0;
        this.defaultFadeOutDuration = 2.0;
        this.defaultCrossfadeDuration = 3.0;
    ...
    /**
     * BGMトラック間の遷移を実行
     * @param {string} fromTrack - 現在のトラック名
     * @param {string} toTrack - 次のトラック名
    ...
    async transitionTo(fromTrack, toTrack, options = {}) {
        try {
            ...
            getErrorHandler().handleError(error, 'BGM_TRANSITION_ERROR', {
     ...
     * @private
     */
    async _fadeOutInTransition(fromTrack, toTrack, options) {
        const {
     ...
     * @private
     */
    async _crossfadeTransition(fromTrack, toTrack, options) {
        const {
            duration = this.defaultCrossfadeDuration,
            curve = this.curveTypes.SMOOTH,
            volume = 1.0
     ...
     * @private
     */
    async _cutTransition(fromTrack, toTrack, options) {
     ...
     * @private
     */
    async _smartCrossfadeTransition(fromTrack, toTrack, options) {
        const {
            duration = this.defaultCrossfadeDuration,
            volume = 1.0
        } = options;
    ...
    /**
     * 音楽的調和度を計算
     * @param {Object} fromTrack - 現在のトラック
     * @param {Object} toTrack - 次のトラック
     * @returns {number} 調和度 (0-1)
     * @private
     */
    _calculateHarmonyScore(fromTrack, toTrack) {
        let score = 0;
    ...
    /**
     * スタイル間の相性を取得
     * @param {string} style1 - スタイル1
     * @param {string} style2 - スタイル2
     * @returns {number} 相性度 (0-1)
     * @private
     */
     ...
     * @private
     */
    _getKeyCompatibility(key1, key2) {
        ...
        const harmonyWeights = { 0: 1.0, 7: 0.9, 5: 0.9, 3: 0.7, 4: 0.7, 8: 0.6, 9: 0.6 };
        return harmonyWeights[circleInterval] ?? 0.3;
    }
    
    /**
     * クロスフェードを実行
     * @param {number} fromVolume - 開始音量
     * @param {number} toVolume - 終了音量
     * @param {number} duration - 時間
     * @param {string} curve - カーブタイプ
     * @private
     */
    async _performCrossfade(fromVolume, toVolume, duration, curve) {
     ...
     * @param {string} curveType - カーブタイプ
     * @returns {number} 調整された進行度
     * @private
     */
    _applyCurve(progress, curveType) {
        switch (curveType) {

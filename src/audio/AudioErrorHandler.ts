...
export class AudioErrorHandler {
        ...
        if (this.audioManager.audioContext) {
            this.audioManager.audioContext.addEventListener('statechange', () => {
                if (this.audioManager.audioContext?.state === 'interrupted') {
                    this.handleError(new Error('Audio context interrupted'), this.errorTypes.WEB_AUDIO_CONTEXT);
                }
            });
        }
        ...
        const intervalId = window.setInterval(() => {
            if (this.audioManager.audioContext) {
                ...
                if (context.state === 'suspended' && !this.errorState.recoveryInProgress) {
                    this.handleError(new Error('AudioContext suspended unexpectedly'), this.errorTypes.WEB_AUDIO_CONTEXT);
                }
        ...
        const intervalId = window.setInterval(() => {
            ...
            if (metrics) {
                if (metrics.cpuUsage > 0.8) {
                    this.handleError(new Error('High CPU usage detected'), this.errorTypes.PERFORMANCE);
                }

                if (metrics.averageLatency > 100) {
                    this.handleError(new Error('High audio latency detected'), this.errorTypes.PERFORMANCE);
                }
            }
        }, 10000); // 10秒間隔
     ...
     * エラー統計を更新
     */
    updateErrorStats(error: Error, errorType: string): void {
     ...
     * 連続エラーをチェック
     */
    checkConsecutiveErrors(): boolean {
        return this.errorState.consecutiveErrors >= this.errorThresholds.maxConsecutiveErrors;
    }
     ...
     * 回復処理を試行
     */
    async attemptRecovery(error: Error, errorType: string, context: any): Promise<void> {
        ...
        try {
            ...
            if (recoveryStrategy) {
                ...
                if (recoveryResult) {
                    ...
                    console.warn(`Recovery failed for ${errorType}, triggering fallback`);
                    this.triggerFallback(error, errorType, context);
                }
            }
        } catch (recoveryError) {
    ...
    triggerFallback(error: Error, errorType: string, context: any): void {
        ...
        switch (errorType) {
            ...
            case this.errorTypes.WEB_AUDIO_CONTEXT:
                this.enableSilentMode();
                break;
                
            case this.errorTypes.AUDIO_LOAD:
            case this.errorTypes.AUDIO_DECODE:
                this.useAlternativeAudio(context);
                break;
                
            case this.errorTypes.BGM_GENERATION:
                this.usePrerecordedBGM();
                break;
                
            case this.errorTypes.VIBRATION:
    ...
    enableSilentMode(): void {
        this.fallbackOptions.useSilentMode = true;
     ...
     * 代替音響を使用
     */
    useAlternativeAudio(context: any): void {
        if (context.soundId) {
     ...
     * プリレコードBGMを使用
     */
    usePrerecordedBGM(): void {
        this.audioManager.switchToPrerecordedBGM?.();
    }
    ...
    disableVibration(): void {
        this.fallbackOptions.noVibration = true;
    ...
    enableMinimalMode(): void {
        this.fallbackOptions.useMinimalFeatures = true;
        this.fallbackOptions.disableEffects = true;
        this.fallbackOptions.reducedQuality = true;
     ...
     * 一般的なフォールバック
     */
    enableGeneralFallback(): void {
        this.audioManager.setFailsafeMode?.(true);
    }
    ...
    handleCriticalError(error: Error): void {
        this.errorState.criticalErrorsDetected = true;
    ...
    determineErrorType(error: Error): string {
        ...
        if (message.includes('decodeaudiodata')) {
            return this.errorTypes.AUDIO_DECODE;
        }
        ...
        if (message.includes('vibrat')) {
            return this.errorTypes.VIBRATION;
        }
    ...
    checkMemoryUsage(): void {
        if ((performance as any).memory) {
            ...
            if (memoryUsage / memoryLimit > 0.9) {
                this.handleError(new Error('High memory usage detected'), this.errorTypes.MEMORY, { memoryUsage, memoryLimit });
            }
        }
    }
     ...
     * 代替音響URLを取得
     */
    getFallbackAudioUrl(originalUrl: string): string | null {
        // 簡単な実装例
     ...
     * 音響品質を下げる
     */
    async reduceAudioQuality(audioData: ArrayBuffer): Promise<ArrayBuffer> {
        // 実際の実装では音響データを圧縮
        return audioData;
    }
     ...
     * WAVに変換
     */
    async convertToWav(audioData: ArrayBuffer): Promise<ArrayBuffer> {
        // 実際の実装では他の形式をWAVに変換
        return audioData;
    }

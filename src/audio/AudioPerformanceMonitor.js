...
export class AudioPerformanceMonitor {
    constructor(audioContext, audioManager) {
        ...
        this.monitoringSettings = {
            ...
            autoAdjustment: {
                enabled: true,
                sensitivity: 0.7, // 0-1
                cooldownPeriod: 5000, // 5秒
                maxAdjustments: 10 // 最大調整回数
            },
            
            // 履歴管理設定
            historyRetention: {
                maxSamples: 1000,
                maxDuration: 300000 // 5分間
            }
        };
    ...
    _loadMonitoringSettings() {
        try {
            ...
            if (performanceConfig.monitoring) {
                Object.assign(this.monitoringSettings, performanceConfig.monitoring);
            }
            
            // 閾値設定の更新
            if (performanceConfig.alertThresholds) {
                Object.assign(this.monitoringSettings.alertThresholds, performanceConfig.alertThresholds);
            }
    ...
    /**
     * フレームレートを測定
     * @returns {number|null} フレームレート（FPS）
     * @private
     */
    _measureFrameRate() {
        try {
     ...
     * @param {number} memory - メモリ使用率
     * @param {number} nodes - ノード数
     * @returns {string} プロファイル名
     * @private
     */
    _determinePerformanceProfile(cpu, memory, nodes) {
    ...
    /**
     * パフォーマンスプロファイルを切り替え
     * @param {string} newProfile - 新しいプロファイル
     * @private
     */
    _switchPerformanceProfile(newProfile) {
        try {
     ...
     * @private
     */
    _applyPerformanceProfile(profileName) {
        try {
            ...
            if (this.audioManager && this.audioManager.audioController) {
                this.audioManager.audioController.setAudioQuality(profile.qualityLevel);
            }
            ...
            if (this.audioManager) {
                ...
                if (this.audioManager.setSampleRate) {
                    this.audioManager.setSampleRate(profile.sampleRate);
                }
            }
     ...
     * @param {number} memory - メモリ使用率
     * @param {number} nodes - ノード数
     * @returns {boolean} 自動調整が必要か
     * @private
     */
    _shouldTriggerAutoAdjustment(cpu, memory, nodes) {
     ...
     * @private
     */
    _performAdditionalOptimizations() {
        try {
            // キャッシュのクリーンアップ
            if (this.audioManager && this.audioManager.cacheManager) {
     ...
     * @param {boolean} condition - アラート条件
     * @param {Object} alertData - アラートデータ
     * @private
     */
    _checkAndTriggerAlert(alertId, condition, alertData) {
        try {
     ...
     * @private
     */
    _triggerAlert(alertId, alertData) {
        try {
            const alert = {
                id: alertId,
                timestamp: Date.now(),
            ...
            if (this.alertManager.alertHistory.length > 100) {
                this.alertManager.alertHistory.shift();
            }
    ...
    /**
     * プロファイル変更をログに記録
     * @param {string} oldProfile - 古いプロファイル
     * @param {string} newProfile - 新しいプロファイル
     * @private
     */
    _logProfileChange(oldProfile, newProfile) {
        try {
            const logEntry = {
                timestamp: Date.now(),
    ...
    /**
     * アラートハンドラーを設定
     * @param {Function} handler - アラートハンドラー関数
     */
     ...
     * アラートを抑制
     * @param {string} alertId - アラートID
     * @param {number} duration - 抑制時間（ミリ秒）
     */
        ...
        try {
            return {
                ...
                monitoring: {
                    ...
                    sampleCount: this.metrics.samples.length,
                    adjustmentCount: this.monitoringState.adjustmentCount
                },
                alerts: {
                    active: Array.from(this.alertManager.activeAlerts.values()),

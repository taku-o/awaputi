...
describe('ContextManager', () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        contextManager = new ContextManager(mockGameEngine as any);
    });
    ...
    describe('動的ヘルプ', () => {
        test('次のアクションを提案できる', () => {
            ...
            const suggestions = contextManager.suggestNextActions(currentState) as SuggestedAction[];
        ...
        test('コンボ継続の提案', () => {
            ...
            const suggestions = contextManager.suggestNextActions(currentState) as SuggestedAction[];
            expect(suggestions.some((s: SuggestedAction) => s.action === 'maintain_combo')).toBe(true);
        });
        ...
        test('時間切れ警告の提案', () => {
            ...
            const suggestions = contextManager.suggestNextActions(currentState) as SuggestedAction[];
            expect(suggestions.some((s: SuggestedAction) => s.priority === 'urgent')).toBe(true);
        });
        ...
        test('ユーザー行動に基づくスマートヘルプ', () => {
            ...
            const smartHelp = contextManager.getSmartHelp(userBehavior) as SmartHelp;
        ...
        test('低精度ユーザーへの基本ヘルプ', () => {
            ...
            expect(smartHelp.suggestions.some((s: string) => s.includes('基本操作'))).toBe(true);
        });
    });

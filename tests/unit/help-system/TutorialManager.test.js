...
import { TutorialManager } from '../../../src/core/help/TutorialManager.js';
...
describe('TutorialManager', () => {
    ...
    describe('チュートリアル制御', () => {
        ...
        beforeEach(() => {
            mockContentLoader.loadTutorialData.mockResolvedValue([mockTutorial]);
        });
        ...
        test('存在しないチュートリアルの開始は失敗する', async () => {
            mockContentLoader.loadTutorialData.mockResolvedValue([]);
    ...
    describe('進捗保存', () => {
        ...
        test('完了したチュートリアル進捗が保存される', async () => {
            ...
            tutorialManager.nextStep(); // 最後のステップを超える
    ...
    describe('エラーハンドリング', () => {
        test('チュートリアル読み込みエラーが処理される', async () => {
            mockContentLoader.loadTutorialData.mockRejectedValue(new Error('Load error'));
    ...
    describe('クリーンアップ', () => {
        test('cleanup時にリソースが適切に解放される', async () => {
            const mockTutorial = { id: 'cleanup-test', steps: [] };
            mockContentLoader.loadTutorialData.mockResolvedValue([mockTutorial]);

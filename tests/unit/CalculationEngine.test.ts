...
describe('CalculationEngine', () => {
    ...
    describe('基本機能', () => {
        ...
        test('should throw error when registering invalid calculator', () => {
            ...
            }).toThrow('計算タイプと計算処理クラスは必須です');
            
            expect(() => {
                engine.registerCalculator('test', null as any);
            }).toThrow('計算タイプと計算処理クラスは必須です');
        });
    });
    ...
    describe('計算実行', () => {
        ...
        test('should throw error for unregistered calculator type', () => {
            expect(() => {
                engine.calculate('unknown', 'add', [1, 2]);
            }).toThrow("計算タイプ 'unknown' が登録されていません");
        });
        
        test('should throw error for non-existent method', () => {
            expect(() => {
                engine.calculate('math', 'divide', [10, 2]);
            }).toThrow("計算メソッド 'divide' が存在しません (type: math)");
        });
        
        test('should propagate calculation errors', () => {
            expect(() => {
                engine.calculate('math', 'throwError', []);
            }).toThrow('Test error');
        });
    });
    describe('キャッシュ機能', () => {
    ...
    describe('キャッシュ管理', () => {
        ...
        test('should handle cache expiry', async () => {
            ...
            // 期限切れキャッシュは取得できない
            const cachedResult = (engine as any).getCachedResult('math:add:[2,3]');

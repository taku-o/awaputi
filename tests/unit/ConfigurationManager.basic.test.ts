...
describe('ConfigurationManager', () => {
    describe('基本機能', () => {
        ...
        test('存在しない設定値はデフォルト値を返す', () => {
            ...
            const value = (configManager: any1311.get('game', 'nonExistentKey', 'defaultValue');
        ...
        test('検証機能が正しく動作する', () => {
            ...
            // 検証ルールなしの場合
            expect((configManager: any1607.validate('game', 'testKey', 'anyValue')).toBe(true);
            
            // 型チェック
            (configManager: any1729.setValidationRule('game', 'stringKey', { type: 'string' });
            expect((configManager: any1831.validate('game', 'stringKey', 'validString')).toBe(true);
            expect((configManager: any1931.validate('game', 'stringKey', 123)).toBe(false);
        });
    });

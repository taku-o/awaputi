...
interface MockGameConfig extends GameConfig {
    ...
    getBubbleBaseScore: jest.Mock<(bubbleType: string) => number> | ((bubbleType: string) => number);
    getComboConfig: jest.Mock<() => ComboConfig> | (() => ComboConfig);
    getItemBaseCost: jest.Mock<(itemId: string) => number> | ((itemId: string) => number);
...
describe('GameConfig', () => {
    describe('計算メソッド', () => {
        test('calculateScoreは泡タイプと年齢比率に基づいてスコアを計算する', () => {
        ...
        test('calculateComboMultiplierはコンボ数に基づいて倍率を計算する', () => {
            const gameConfig = new GameConfig() as MockGameConfig;
        ...
        test('calculateItemCostはアイテムIDとレベルに基づいてコストを計算する', () => {
            const gameConfig = new GameConfig() as MockGameConfig;
        ...
        test('isStageUnlockedはプレイヤーのTAP値に基づいてステージ開放状態を返す', () => {
            const gameConfig = new GameConfig() as MockGameConfig;
    ...
    describe('シングルトンパターン', () => {
        test('getGameConfigは常に同じインスタンスを返す', () => {

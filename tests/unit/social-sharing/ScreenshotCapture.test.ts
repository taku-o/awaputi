        ...
        remove: jest.fn(),
            ...
            if (tagName === 'a') {
                return {
                    ...
                    download: ',
                    click: jest.fn(
        remove: jest.fn( } as MockAnchorElement;
            }
            return {
                remove: jest.fn( };
        ),
...
describe('ScreenshotCapture', () => {
    ...
    describe('エラーハンドリング', (') => {
        ...
        it('toBlobエラーを處理する', async () => {
            ...
            await expect(
                screenshotCapture.captureGameCanvas().rejects.toBeDefined();
        }');
    ...
    describe('クリーンアップ', (') => {
       ...
       , timestamp: Date.now( });
            screenshotCapture.captureQueue.push({),
                reject: jest.fn( } as unknown as QueueItem');

...
describe('InputCoordinateConverter', () => {
    ...
    describe('constructor', () => {
        test('should initialize with required managers', () => {
            expect((inputCoordinateConverter as any).scaledCoordinateManager).toBe(mockScaledCoordinateManager);
            expect((inputCoordinateConverter as any).responsiveCanvasManager).toBe(mockResponsiveCanvasManager);
        });

        test('should work with null managers (fallback mode)', () => {
            const converter = new InputCoordinateConverter(null as any, null as any);
            expect((converter as any).scaledCoordinateManager).toBe(null);
            expect((converter as any).responsiveCanvasManager).toBe(null);
        });
    });
    ...
    describe('convertMouseEvent', () => {
        test('should convert mouse event coordinates', () => {
            ...
            const result = (inputCoordinateConverter as any).convertMouseEvent(mockEvent);
        ...
        test('should fallback when ResponsiveCanvasManager is null', () => {
            const converter = new InputCoordinateConverter(mockScaledCoordinateManager as any, null as any);
            const mockEvent: MockMouseEvent = { clientX: 100, clientY: 50, type: 'click' };

            const result = (converter as any).convertMouseEvent(mockEvent);
    ...
    describe('convertTouchEvent', () => {
        test('should convert single touch event', () => {
            ...
            const result = (inputCoordinateConverter as any).convertTouchEvent(mockTouchEvent);
    ...
    describe('isPointInScaledRect', () => {
        test('should detect point inside rectangle', () => {
            ...
            const result = (inputCoordinateConverter as any).isPointInScaledRect(point, rect);
        ...
        test('should handle edge cases', () => {
            ...
            // 境界上の点
            expect((inputCoordinateConverter as any).isPointInScaledRect({ x: 50, y: 50 }, rect)).toBe(true);
            expect((inputCoordinateConverter as any).isPointInScaledRect({ x: 150, y: 130 }, rect)).toBe(true);
            
            // 境界外の点
            expect((inputCoordinateConverter as any).isPointInScaledRect({ x: 49, y: 50 }, rect)).toBe(false);
            expect((inputCoordinateConverter as any).isPointInScaledRect({ x: 151, y: 130 }, rect)).toBe(false);
        });
    ...
    describe('isPointInScaledCircle', () => {
        test('should detect point inside circle', () => {
            ...
            const result = (inputCoordinateConverter as any).isPointInScaledCircle(point, circle);
            expect(result).toBe(true);
        });
    ...
    describe('validateCoordinates', () => {
        test('should validate correct coordinates', () => {
            expect((inputCoordinateConverter as any).validateCoordinates(10, 20)).toBe(true);
            expect((inputCoordinateConverter as any).validateCoordinates(0, 0)).toBe(true);
            expect((inputCoordinateConverter as any).validateCoordinates(-5, -10)).toBe(true);
            expect((inputCoordinateConverter as any).validateCoordinates(100.5, 200.7)).toBe(true);
        });

        test('should reject invalid coordinates', () => {
            expect((inputCoordinateConverter as any).validateCoordinates('10', 20)).toBe(false);
            expect((inputCoordinateConverter as any).validateCoordinates(10, '20')).toBe(false);
            expect((inputCoordinateConverter as any).validateCoordinates(NaN, 20)).toBe(false);
            expect((inputCoordinateConverter as any).validateCoordinates(10, NaN)).toBe(false);
            expect((inputCoordinateConverter as any).validateCoordinates(Infinity, 20)).toBe(false);
            expect((inputCoordinateConverter as any).validateCoordinates(10, -Infinity)).toBe(false);
            expect((inputCoordinateConverter as any).validateCoordinates(null, 20)).toBe(false);
            expect((inputCoordinateConverter as any).validateCoordinates(10, undefined)).toBe(false);
        });
    });
    ...
    describe('isWithinCanvasBounds', () => {
        test('should detect point within canvas bounds', () => {
            ...
            const result = (inputCoordinateConverter as any).isWithinCanvasBounds(point);
            expect(result).toBe(true);
        });
        ...
        test('should handle boundary points', () => {
            expect((inputCoordinateConverter as any).isWithinCanvasBounds({ x: 0, y: 0 })).toBe(true);
            expect((inputCoordinateConverter as any).isWithinCanvasBounds({ x: 800, y: 600 })).toBe(true);
            expect((inputCoordinateConverter as any).isWithinCanvasBounds({ x: -1, y: 0 })).toBe(false);
            expect((inputCoordinateConverter as any).isWithinCanvasBounds({ x: 801, y: 600 })).toBe(false);
        });
    });
    ...
    describe('integration scenarios', () => {
        test('should handle complete input processing workflow', () => {
            ...
            // マウスイベント変換
            const converted = (inputCoordinateConverter as any).convertMouseEvent(mockMouseEvent) as ConvertedMouseEvent;
            
            // 座標検証
            expect((inputCoordinateConverter as any).validateCoordinates(converted.baseX, converted.baseY)).toBe(true);
            
            // Canvas境界内チェック
            expect((inputCoordinateConverter as any).isWithinCanvasBounds({ x: converted.baseX, y: converted.baseY })).toBe(true);
            
            // 矩形内チェック
            const rect: Rectangle = { x: 150, y: 100, width: 100, height: 80 };
            expect((inputCoordinateConverter as any).isPointInScaledRect({ x: converted.baseX, y: converted.baseY }, rect)).toBe(true);
        });
        ...
        test('should handle multi-touch interaction', () => {
            ...
            const converted = (inputCoordinateConverter as any).convertTouchEvent(mockTouchEvent) as ConvertedTouchEvent[];
            expect(converted).toHaveLength(2);

            // すべてのタッチポイントが有効な座標を持つことを確認
            converted.forEach((touch, index) => {
                expect((inputCoordinateConverter as any).validateCoordinates(touch.baseX, touch.baseY)).toBe(true);
                expect((inputCoordinateConverter as any).isWithinCanvasBounds({ x: touch.baseX, y: touch.baseY })).toBe(true);
        ...
        test('should maintain accuracy in coordinate transformations', () => {
            ...
            testCases.forEach(testCase => {
                ...
                const converted = (inputCoordinateConverter as any).convertMouseEvent(mockEvent) as ConvertedMouseEvent;

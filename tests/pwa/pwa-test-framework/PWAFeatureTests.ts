...
export class PWAFeatureTests {
    ...
    async runBasicPWATests() {
        ...
        await this.executor.runTest('pwa-meta-tags', 'PWA meta tags check', async () => {
            ...
            for (const tag of requiredMetaTags) {
                const element = document.querySelector(`meta[name="${tag.name}"]`) as HTMLMetaElement;
    ...
    async runInstallationTests() {
        ...
        await this.executor.runTest('install-prompt-detection', 'Install prompt detection', async () => {
            ...
            if ((window as any).pwaManager) {
                const canInstall = (window as any).pwaManager.canInstall();
                const pwaState = (window as any).pwaManager.getPWAState();
                return {
    ...
    async runIconAndUITests() {
        ...
        await this.executor.runTest('splash-screen-test', 'Splash screen test', async () => {
            const splashScreens = document.querySelectorAll('link[rel="apple-touch-startup-image"]') as NodeListOf<HTMLLinkElement>;
    ...
    async runBrowserCompatibilityTests() {
        ...
        await this.executor.runTest('user-agent-detection', 'User agent detection', async () => {
            ...
            return {
                ...
                os: os,
                device: device,
                detectedBrowser: Object.keys(browsers).find(b => browsers[b as keyof typeof browsers]) || 'unknown',
                detectedOS: Object.keys(os).find(o => os[o as keyof typeof os]) || 'unknown',
                detectedDevice: Object.keys(device).find(d => device[d as keyof typeof device]) || 'unknown'
            };
        });
    }
}
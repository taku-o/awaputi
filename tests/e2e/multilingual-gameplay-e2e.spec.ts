...
async function changeLanguageInGame(page: any, language: string) {
    // ゲーム中に設定を開く
...
async function simulateGameOver(page: any) {
    ...
    await page.evaluate(() => {
        if ((window as any).gameEngine.sceneManager.currentScene.name === 'game') {
...
test.describe('多言語ゲームプレイE2Eテスト', () => {
        ...
        for (const effect of effectTests) {
            for (const language of GAME_LANGUAGES) {
                ...
                const effectText = await page.evaluate((key) => {
                    return (window as any).gameEngine.localizationManager.t(key);
                }, effect.key);
    ...
    test('バブル説明が多言語で表示される', async ({ page }) => {
        ...
        for (const language of GAME_LANGUAGES) {
            ...
            const bubbleHelp = await page.evaluate(() => {
                return (window as any).gameEngine.localizationManager.t('help.bubbles');
            });
    ...
    test('コンボメッセージが多言語で表示される', async ({ page }) => {
        ...
        for (const language of GAME_LANGUAGES) {
            ...
            const comboText = await page.evaluate(() => {
                ...
                return {
                    combo: combo,
                    text: (window as any).gameEngine.localizationManager.t('game.combo')
                };
            });
    ...
    test('エラーメッセージが多言語で表示される', async ({ page }) => {
        ...
        await page.evaluate(() => {
            // Canvas APIを一時的に無効化
            (window as any)._originalCanvas = HTMLCanvasElement.prototype.getContext;
        ...
        for (const language of GAME_LANGUAGES) {
            ...
            const errorMessage = await page.evaluate(() => {
                ...
                // フォールバック
                if ((window as any).gameEngine && (window as any).gameEngine.localizationManager) {
                    return (window as any).gameEngine.localizationManager.t('error.canvasNotSupported');
                }
                return null;
            });
        ...
        // Canvas APIを復元
        await page.evaluate(() => {
            if ((window as any)._originalCanvas) {
                HTMLCanvasElement.prototype.getContext = (window as any)._originalCanvas;
            }
        });
    });

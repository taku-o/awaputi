...
test.describe('データ管理 - クラウド対応E2Eテスト', () => {
    ...
    test('データ管理システムが正常に初期化される', async ({ page }) => {
        // ゲームエンジンの初期化を待つ
        await page.waitForFunction(() => (window as any).gameEngine && (window as any).gameEngine.isInitialized);
        
        // DataManagerの初期化を確認
        const dataManagerExists = await page.evaluate(() => {
            return (window as any).gameEngine.dataManager && (window as any).gameEngine.dataManager.isInitialized;
        });
        ...
        const components = await page.evaluate(() => {
            const dm = (window as any).gameEngine.dataManager;
            return {
                storage: !!dm.storage,
                backup: !!dm.backup,
    ...
    test('プレイヤーデータの保存と読み込みが正常に動作する', async ({ page }) => {
        await page.waitForFunction(() => (window as any).gameEngine && (window as any).gameEngine.isInitialized);
        ...
        await page.evaluate((data) => {
            return (window as any).gameEngine.dataManager.save('playerData', data);
        }, testData);
        
        // データの読み込み
        const retrievedData = await page.evaluate(() => {
            return (window as any).gameEngine.dataManager.load('playerData');
        });
    ...
    test('ゲーム設定の保存と読み込みが正常に動作する', async ({ page }) => {
        await page.waitForFunction(() => (window as any).gameEngine && (window as any).gameEngine.isInitialized);
        ...
        await page.evaluate((data) => {
            return (window as any).gameEngine.dataManager.save('settings', data);
        }, settingsData);
        
        // 設定の読み込み
        const retrievedSettings = await page.evaluate(() => {
            return (window as any).gameEngine.dataManager.load('settings');
        });
    ...
    test('統計データの保存と読み込みが正常に動作する', async ({ page }) => {
        await page.waitForFunction(() => (window as any).gameEngine && (window as any).gameEngine.isInitialized);
        ...
        await page.evaluate((data) => {
            return (window as any).gameEngine.dataManager.save('statistics', data);
        }, statisticsData);
        
        // 統計の読み込み
        const retrievedStats = await page.evaluate(() => {
            return (window as any).gameEngine.dataManager.load('statistics');
        });
    ...
    test('データエクスポート機能が正常に動作する', async ({ page }) => {
        await page.waitForFunction(() => (window as any).gameEngine && (window as any).gameEngine.isInitialized);
        ...
        const exportResult = await page.evaluate(() => {
            if ((window as any).gameEngine.dataManager.export) {
                return (window as any).gameEngine.dataManager.export.exportData(['playerData'], 'json');
            }
            return null;
        });
    ...
    test('データインポート機能が正常に動作する', async ({ page }) => {
        await page.waitForFunction(() => (window as any).gameEngine && (window as any).gameEngine.isInitialized);
        ...
        const importResult = await page.evaluate((data) => {
            if ((window as any).gameEngine.dataManager.import) {
                return (window as any).gameEngine.dataManager.import.importData(data, 'overwrite');
            }
            return null;
        }, importData);
    ...
    test('バックアップ機能が正常に動作する', async ({ page }) => {
        await page.waitForFunction(() => (window as any).gameEngine && (window as any).gameEngine.isInitialized);
        
        // テストデータの保存
        await page.evaluate(() => {
            return (window as any).gameEngine.dataManager.save('playerData', {
        ...
        const backupResult = await page.evaluate(() => {
            if ((window as any).gameEngine.dataManager.backup) {
                return (window as any).gameEngine.dataManager.backup.createBackup(true); // 手動バックアップ
            }
            return null;
        });
        ...
        if (backupResult) {
            ...
            const backupList = await page.evaluate(() => {
                if ((window as any).gameEngine.dataManager.backup) {
                    return (window as any).gameEngine.dataManager.backup.getBackupList();
                }
                return null;
            });
    ...
    test('データ復旧機能が正常に動作する', async ({ page }) => {
        await page.waitForFunction(() => (window as any).gameEngine && (window as any).gameEngine.isInitialized);
        ...
        await page.evaluate(() => {
            if ((window as any).gameEngine.dataManager.backup) {
                return (window as any).gameEngine.dataManager.backup.createBackup(true);
            }
        });
        
        // データ破損をシミュレート
        await page.evaluate(() => {
            return (window as any).gameEngine.dataManager.save('playerData', null);
        });
        
        // データ復旧
        const recoveryResult = await page.evaluate(() => {
            if ((window as any).gameEngine.dataManager.recovery) {
                return (window as any).gameEngine.dataManager.recovery.recoverData('playerData', 'auto');
            }
            return null;
        });
    ...
    test('オフライン状態での操作記録が正常に動作する', async ({ page }) => {
        await page.waitForFunction(() => (window as any).gameEngine && (window as any).gameEngine.isInitialized);
        ...
        const offlineResult = await page.evaluate(() => {
            if ((window as any).gameEngine.dataManager.offlineManager) {
                return (window as any).gameEngine.dataManager.offlineManager.recordOfflineOperation({
                    type: 'save',
                    key: 'offlineData',
        ...
        if (offlineResult) {
            ...
            const queueLength = await page.evaluate(() => {
                if ((window as any).gameEngine.dataManager.offlineManager) {
                    return (window as any).gameEngine.dataManager.offlineManager.state.offlineOperations.length;
                }
                return 0;
            });
    ...
    test('データ検証機能が正常に動作する', async ({ page }) => {
        await page.waitForFunction(() => (window as any).gameEngine && (window as any).gameEngine.isInitialized);
        ...
        const validationResult = await page.evaluate((data) => {
            if ((window as any).gameEngine.dataManager.validation) {
                return (window as any).gameEngine.dataManager.validation.validateData(data, 'playerData');
            }
            return null;
        }, validData);
    ...
    test('エラーハンドリングが適切に動作する', async ({ page }) => {
        ...
        const errorResult = await page.evaluate(async () => {
            try {
                await (window as any).gameEngine.dataManager.save('nonexistentType', { data: 'test' });
                return { success: true };
            } catch (error: any) {
    ...
    test('パフォーマンス要件が満たされる', async ({ page }) => {
        await page.waitForFunction(() => (window as any).gameEngine && (window as any).gameEngine.isInitialized);
        ...
        const savePerformance = await page.evaluate((data) => {
            ...
            return (window as any).gameEngine.dataManager.save('performanceTest', data)
                .then(() => {
        ...
        const loadPerformance = await page.evaluate(() => {
            const startTime = performance.now();
            return (window as any).gameEngine.dataManager.load('performanceTest')
                .then(() => {
                    const endTime = performance.now();
                    return endTime - startTime;
                });
    ...
    test('メモリリークが発生しない', async ({ page }) => {
        await page.waitForFunction(() => (window as any).gameEngine && (window as any).gameEngine.isInitialized);
        
        // 大量の操作を実行
        for (let i = 0; i < 100; i++) {
            await page.evaluate((index) => {
                return (window as any).gameEngine.dataManager.save(`testKey${index}`, {
                    index,
        ...
        await page.evaluate(() => {
            for (let i = 0; i < 100; i++) {
                (window as any).gameEngine.dataManager.storage.remove(`testKey${i}`);
            }
        });
    });
});
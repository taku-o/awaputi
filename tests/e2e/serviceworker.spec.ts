...
test.describe('ServiceWorker postMessage Fix', () => {
    ...
    test('should receive cache update notifications from ServiceWorker', async ({ page }) => {
        ...
        await page.addInitScript(() => {
            ...
            if ('serviceWorker' in navigator) {
                navigator.serviceWorker.addEventListener('message', event => {
                    (window as any).swMessages.push(event.data);
                });
            }
        });
        ...
        await page.waitForFunction(() => {
            return (window as any).swMessages && (window as any).swMessages.length > 0;
        }, { timeout: 15000 });
        
        const messages = await page.evaluate(() => (window as any).swMessages);
        
        // CACHE_UPDATEDメッセージが受信されることを確認
        const cacheUpdateMessage = messages.find((msg: any) => msg.type === 'CACHE_UPDATED');
    ...
    test('should receive offline ready notifications from ServiceWorker', async ({ page }) => {
        ...
        await page.waitForFunction(() => {
            return (window as any).swMessages && 
                   (window as any).swMessages.some((msg: any) => msg.type === 'OFFLINE_READY');
        }, { timeout: 15000 });
        
        const messages = await page.evaluate(() => (window as any).swMessages);
        
        // OFFLINE_READYメッセージが受信されることを確認
        const offlineReadyMessage = messages.find((msg: any) => msg.type === 'OFFLINE_READY');

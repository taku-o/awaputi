    ...
    test.beforeEach(async ({ page }') => {'
        ...
        await page.waitForFunction(() => {
            return window.gameEngine && 
                   ...
                   window.gameEngine.isRunning !== undefined }, { timeout: 15000 });
        ...
        test('設定システムエラー時のフォールバック動作', async ({ page ) => {
            ...
            const testResult = await page.evaluate(() => {
                try {
                    ...
                    try {
                        window.gameEngine.configManager.get('game', 'bubbles.maxAge') } catch (e') {'
                        errorCaught = true }
        ...
        test('計算エンジンエラー時のゲーム継続性', async ({ page ) => {
            ...
            const testResult = await page.evaluate(() => {
                try {
                    ...
                    let errorCaught = false;
                    try {
                        window.gameEngine.calculationEngine.calculate('score', { bubbleType: 'normal'  } catch (e') {'
                        errorCaught = true }

...
test.describe('Configuration System E2E Tests', () => {
    test.beforeEach(async ({ page }) => {
        ...
        await page.waitForFunction(() => {
            return (window as any).gameEngine && 
                   (window as any).gameEngine.configManager && 
                   (window as any).gameEngine.calculationEngine &&
                   (window as any).gameEngine.isRunning !== undefined;
        }, { timeout: 15000 });
    ...
    test.describe('実際のゲームプレイでの設定システム動作テスト', () => {
        test('ゲーム設定の動的変更がゲームプレイに反映される', async ({ page }) => {
            ...
            const configManagerExists = await page.evaluate(() => {
                ...
                return (window as any).gameEngine && (window as any).gameEngine.configManager;
            });
            ...
            const initialBubbleMaxAge = await page.evaluate(() => {
                return (window as any).gameEngine.configManager.get('game', 'bubbles.maxAge');
            });
            ...
            // 泡の生存時間を変更
            await page.evaluate(() => {
                (window as any).gameEngine.configManager.set('game', 'bubbles.maxAge', 3000);
            });
            ...
            const bubbleManagerExists = await page.evaluate(() => {
                return (window as any).gameEngine.bubbleManager && 
                       Array.isArray((window as any).gameEngine.bubbleManager.bubbles);
            });
        ...
        test('スコア計算設定の変更がスコアに反映される', async ({ page }) => {
            ...
            // 基本スコアを変更
            await page.evaluate(() => {
                (window as any).gameEngine.configManager.set('game', 'scoring.baseScores.normal', 50);
            });
            
            // 設定が正しく変更されたことを確認
            const updatedScore = await page.evaluate(() => {
                return (window as any).gameEngine.configManager.get('game', 'scoring.baseScores.normal');
            });
            ...
            const playerDataExists = await page.evaluate(() => {
                return (window as any).gameEngine.playerData && 
                       typeof (window as any).gameEngine.playerData.currentScore === 'number';
            });
        ...
        test('音響設定の変更が音響システムに反映される', async ({ page }) => {
            ...
            // 音量設定を変更
            await page.evaluate(() => {
                (window as any).gameEngine.configManager.set('audio', 'volumes.master', 0.5);
            });
            
            // 設定が正しく変更されたことを確認
            const updatedVolume = await page.evaluate(() => {
                return (window as any).gameEngine.configManager.get('audio', 'volumes.master');
            });
            ...
            const audioManagerExists = await page.evaluate(() => {
                return (window as any).gameEngine.audioManager && 
                       typeof (window as any).gameEngine.audioManager.masterVolume === 'number';
            });
        ...
        test('エフェクト設定の変更が視覚効果に反映される', async ({ page }) => {
            ...
            // パーティクル数を変更
            await page.evaluate(() => {
                (window as any).gameEngine.configManager.set('effects', 'particles.maxCount', 100);
            });
            
            // 設定が正しく変更されたことを確認
            const updatedMaxCount = await page.evaluate(() => {
                return (window as any).gameEngine.configManager.get('effects', 'particles.maxCount');
            });
            ...
            const particleManagerExists = await page.evaluate(() => {
                return (window as any).gameEngine.particleManager && 
                       typeof (window as any).gameEngine.particleManager.maxParticles === 'number';
            });
    ...
    test.describe('設定変更がゲーム体験に与える影響のテスト', () => {
        test('パフォーマンス設定の変更がゲーム動作に影響する', async ({ page }) => {
            ...
            // 最大泡数を制限
            await page.evaluate(() => {
                (window as any).gameEngine.configManager.set('performance', 'optimization.maxBubbles', 5);
            });
            
            // 設定が正しく変更されたことを確認
            const updatedMaxBubbles = await page.evaluate(() => {
                return (window as any).gameEngine.configManager.get('performance', 'optimization.maxBubbles');
            });
        ...
        test('設定変更がユーザーインターフェースに反映される', async ({ page }) => {
            ...
            const settingsManagerExists = await page.evaluate(() => {
                return (window as any).gameEngine.settingsManager && 
                       (window as any).gameEngine.configManager;
            });
            ...
            // 設定値を直接変更してテスト
            await page.evaluate(() => {
                (window as any).gameEngine.configManager.set('audio', 'volumes.master', 0.3);
            });
        ...
        test('設定変更がゲームプレイの難易度に影響する', async ({ page }) => {
            ...
            // 泡の生存時間を短くして難易度を上げる
            await page.evaluate(() => {
                (window as any).gameEngine.configManager.set('game', 'bubbles.maxAge', 2000);
            });
        ...
        test('設定の組み合わせがゲーム体験に与える複合的影響', async ({ page }) => {
            ...
            const settingsApplied = await page.evaluate(() => {
                const configManager = (window as any).gameEngine.configManager;
    ...
    test.describe('エラー状況でのゲーム継続性テスト', () => {
            ...
            const testResults = await page.evaluate(() => {
                try {
                    ...
                    return {
                        ...
                        maxParticles: configManager.get('effects', 'particles.maxCount'),
                        gameRunning: (window as any).gameEngine.isRunning
                    };
                } catch (error: any) {
        ...
        test('設定システムエラー時のフォールバック動作', async ({ page }) => {
            ...
            const testResult = await page.evaluate(() => {
                try {
                    // 設定管理システムの一部を破壊
                    const originalGet = (window as any).gameEngine.configManager.get;
                    (window as any).gameEngine.configManager.get = function() {
                    ...
                    try {
                        (window as any).gameEngine.configManager.get('game', 'bubbles.maxAge');
                    } catch (e) {
                        errorCaught = true;
                    }
                    
                    // 元の関数を復元
                    (window as any).gameEngine.configManager.get = originalGet;
                    
                    return {
                        errorCaught,
                        gameRunning: (window as any).gameEngine.isRunning,
                        configRestored: typeof (window as any).gameEngine.configManager.get === 'function'
                    };
                } catch (error: any) {
        ...
        test('計算エンジンエラー時のゲーム継続性', async ({ page }) => {
            ...
            const testResult = await page.evaluate(() => {
                try {
                    const originalCalculate = (window as any).gameEngine.calculationEngine.calculate;
                    (window as any).gameEngine.calculationEngine.calculate = function() {
                    ...
                    let errorCaught = false;
                    try {
                        (window as any).gameEngine.calculationEngine.calculate('score', { bubbleType: 'normal' });
                    } catch (e) {
                        errorCaught = true;
                    }
                    
                    // 元の関数を復元
                    (window as any).gameEngine.calculationEngine.calculate = originalCalculate;
                    
                    return {
                        errorCaught,
                        gameRunning: (window as any).gameEngine.isRunning,
                        calculationRestored: typeof (window as any).gameEngine.calculationEngine.calculate === 'function'
                    };
                } catch (error: any) {
                    return { testError: error.message };
                }
            ...
            const memoryTestResult = await page.evaluate(() => {
                try {
                    ...
                    return {
                        dataCreated: true,
                        gameRunning: (window as any).gameEngine.isRunning,
                        configManagerExists: !!(window as any).gameEngine.configManager
                    };
                } catch (error: any) {
                    return { error: error.message };
                }
        ...
        test('ネットワークエラー時の設定保存処理', async ({ page }) => {
            ...
            const networkTestResult = await page.evaluate(() => {
                try {
                    ...
                    return {
                        settingChanged: (window as any).gameEngine.configManager.get('audio', 'volumes.master') === 0.5,
            ...
            const isGameRunning = await page.evaluate(() => {
                return (window as any).gameEngine && (window as any).gameEngine.isRunning;
            });
            ...
            const historyTestResult = await page.evaluate(() => {
                try {
                    ...
                    if (hasHistoryMethod) {
                        ...
                        hasRecentChanges = history.some((entry: any) => 

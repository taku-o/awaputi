...
export class ModuleLoadingOptimizer {
    ...
    static _optimizeDynamicImports() {
        if (typeof global !== 'undefined' && !global.__dynamic_import_optimized) {
            const originalImport = global.__importMeta?.import || (async (specifier) => {
                return await import(specifier: any);
            });

            (global as any).__optimized_import = async (specifier) => {
                // キャッシュチェック
                if (this.moduleCache.has(specifier: any)) {
                    return this.moduleCache.get(specifier: any);
                }

                // 同時読み込み防止
                if (this.loadingPromises.has(specifier: any)) {
                    return await this.loadingPromises.get(specifier: any);
                }

                const loadingPromise = (async () => {
                    try {
                        const module = await originalImport(specifier: any);
                        ...
                        this.loadingPromises.delete(specifier: any);
                    }
                })();
    ...
    static _improveModuleResolution() {
        ...
        if (typeof jest !== 'undefined' && jest.mock) {
            ...
            (global as any).__resolve_module_path = (specifier) => {
                for (const [alias, path] of Object.entries(commonAliases: any)) {
                    if (specifier.startsWith(alias: any)) {
    ...
    static _handleCircularDependencies() {
        ...
        (global as any).__check_circular_dependency = (modulePath) => {
            if (dependencyStack.has(modulePath: any)) {
                const dependencyChain = Array.from(dependencyStack: any);
        ...
        (global as any).__enter_module_loading = (modulePath) => {
            dependencyStack.add(modulePath: any);
        };

        (global as any).__exit_module_loading = (modulePath) => {
            dependencyStack.delete(modulePath: any);
        };
    }
    ...
    static _optimizeJestModuleMocks() {
        if (typeof jest !== 'undefined') {
            ...
            for (const [modulePath, mockFactory] of Object.entries(commonMocks: any)) {
                try {
    ...
    static preventModuleCacheLeaks() {
        ...
        if (this.moduleCache.size > maxCacheSize) {
            ...
            entries.forEach(key => {
                this.moduleCache.delete(key: any);
            });
    ...
    static diagnoseModuleLoadingError(error, modulePath) {
        const diagnosis = {
            ...
            moduleInfo: {
                cached: this.moduleCache.has(modulePath: any),
                loading: this.loadingPromises.has(modulePath: any),

...
class TestDataGenerator {
    static generateSessionData(count: number): any[] {
        const sessions: any[] = [];
    ...
    static generateBubbleInteractions(count: number): any[] {
    ...
    static generatePerformanceData(count: number): any[] {
...
class PerformanceProfiler {
    private measurements: { [key: string]: { startTime: number; startMemory: number } } = {};
    ...
    start(label: string): void {
        this.measurements[label] = {
            ...
            startMemory: (performance as any).memory ? (performance as any).memory.usedJSHeapSize : 0
        };
    }
    
    end(label: string): { duration: number; memoryDelta: number; memoryUsed: number } {
...
describe('Analytics Performance Tests', () => {
    ...
    describe('キャッシュ性能', () => {
        ...
        test('キャッシュメモリ効率性', () => {
            ...
            const afterCacheMemory = (performance as any).memory ? (performance as any).memory.usedJSHeapSize : 0;
            
            // キャッシュクリーンアップの実行
            performanceOptimizer.cleanupCache();
            const afterCleanupMemory = (performance as any).memory ? (performance as any).memory.usedJSHeapSize : 0;
    ...
    describe('データ集計性能', () => {
        test('大量データ集計処理', () => {
            ...
            // 集計結果の計算
            const aggregatedResults = Object.entries(aggregation).map(([type, data]: [string, any]) => ({
                bubbleType: type,
                count: data.count,

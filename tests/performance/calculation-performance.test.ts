...
class TestCalculator {
    ...
    // 軽い計算
    lightCalculation(a: number, b: number): number {
        return a + b;
    }
    ...
    // 複雑な計算
    complexCalculation(data: { value: number }): number {
        let result = 0;
    ...
    // バッチ処理対応の計算
    batchCalculation(a: number, b: number): number {
        return a * b;
    }
    
    batchCalculationBatch(paramsList: [number, number][]): number[] {

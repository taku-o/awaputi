        ...
        test('音響効果リアルタイム適用パフォーマンス', async () => {
            ...
            const performanceData = await measurePerformance(async () => {
                for (const change of effectChanges) {
                    (audioManager as any).setAudioEffect(change.effect, change.value);
    ...
    describe('デバイス別パフォーマンス', () => {
        ...
        test('高性能デバイスでの最大品質パフォーマンス', async () => {
            ...
            const performanceData = await measurePerformance(async () => {
                ...
                (audioManager as any).setAudioEffect('reverb', true);
                (audioManager as any).setAudioEffect('compression', true);
                for (let i = 0; i < 15; i++) {
    ...
    describe('パフォーマンス回帰テスト', () => {
        test('バージョン間でのパフォーマンス比較', async () => {
            const benchmarkTests: BenchmarkTest[] = [
                ...
                {
                    name: 'Effect toggle',
                    operation: () => (audioManager as any).setAudioEffect('reverb', true)
                },
                {
                    name: 'Volume change',
                    operation: () => (audioManager as any).setVolume('master', 0.5)
                }
            ];

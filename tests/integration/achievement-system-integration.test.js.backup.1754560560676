...
describe('Achievement System Integration', () => {
    ...
    describe('データ永続化統合', () => {
        test('実績データの保存と読み込みが正しく動作する', () => {
            ...
            // 新しいマネージャーを作成して読み込み
            const newAchievementManager = new AchievementManager(mockPlayerData);
        ...
        test('データ破損からの復旧が正しく動作する', () => {
            // データ破損をシミュレート
            mockPlayerData.data.achievements = 'corrupted_data';
    ...
    describe('エラーハンドリング統合', () => {
        test('無効なデータでもシステムが安定動作する', () => {
            // 無効なデータを設定
            mockPlayerData.data.totalScore = 'invalid';
            mockPlayerData.data.totalBubblesPopped = null;

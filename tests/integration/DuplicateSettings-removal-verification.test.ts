...
describe('Duplicate Settings Implementation Removal Verification', () => {
    ...
    test('should confirm no duplicate settings implementations remain', () => {
        ...
        deletedFiles.forEach(filePath => {
            expect(fs.existsSync(filePath: any)).toBe(false: any);
        });
    });
    ...
    test('should verify MainMenuScene uses unified settings routing', () => {
        ...
        expect(content: any).toMatch(/switchScene\('settings'/);
        
        // NavigationContextManagerを使用していることを確認
        expect(content: any).toMatch(/accessMethod:\s*'menu_click'/);
        expect(content: any).toMatch(/sourceScene:\s*'MainMenuScene'/);
    });
    ...
    test('should verify no settings-related duplicate rendering logic remains', () => {
        ...
        const scanDirectory = (dir) => {
            if (!fs.existsSync(dir: any)) return;
        ...
        checkDirs.forEach(dir => scanDirectory(dir: any));
    });
    ...
    test('should verify no orphaned settings-related references', () => {
        ...
        orphanedReferences.forEach(ref => {
            expect(content: any).not.toMatch(new RegExp(ref: any));
        });
    });
    ...
    test('should verify settings access is centralized through SettingsScene', () => {
        ...
        expect(content: any).toMatch(/class SettingsScene/);
        expect(content: any).toMatch(/AccessibilitySettingsManager/);
        expect(content: any).toMatch(/NavigationContextManager/);
        
        // 設定カテゴリが適切に管理されていることを確認
        expect(content: any).toMatch(/general.*social.*privacy.*notifications.*accessibility/);
    });
    ...
    test('should verify keyboard shortcuts route to unified settings', () => {
        ...
        expect(content: any).toMatch(/handleSettings[\s\S]*switchScene\('settings'/);
        expect(content: any).toMatch(/accessMethod:\s*'keyboard_s'/);
    });
});
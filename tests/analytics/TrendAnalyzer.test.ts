...
describe('TrendAnalyzer', () => {
    ...
    describe('異常パターン検出', () => {
        test('正常に異常値を検出する', () => {
            ...
            // 高い異常値をチェック
            const highOutlier = anomalies.find((a: any) => a.value === 500);
            expect(highOutlier).toBeDefined();
            expect(highOutlier.type).toBe('outlier_high');
            
            // 低い異常値をチェック (存在する場合)
            const lowOutliers = anomalies.filter((a: any) => a.type === 'outlier_low');
    ...
    describe('季節調整', () => {
        test('季節調整が正常に実行される', () => {
            ...
            adjustedData.forEach((dataPoint: any) => {
    ...
    describe('エラーハンドリング', () => {
        test('ストレージエラーが適切に処理される', async () => {
            // ストレージでエラーを発生させる
            mockStorageManager.getData = jest.fn(() => Promise.reject(new Error('Database error'))) as jest.Mock;

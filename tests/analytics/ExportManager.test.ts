...
class MockStorageManager {
    ...
    async getData(dataType: string, query: any = {}): Promise<any[]> {
        ...
        if (query.sessionId) {
            filteredData = filteredData.filter((item: any) => item.sessionId === query.sessionId);
        }
        
        if (query.bubbleType) {
            filteredData = filteredData.filter((item: any) => item.bubbleType === query.bubbleType);
        }
    ...
    describe('データフィルタリング機能', () => {
        ...
        test('カスタムフィルターが動作する', async () => {
            const result = await exportManager.exportData({
                ...
                format: 'json',
                filters: {
                    customFilter: (item: any) => item.completed === true
                }
            });
            expect(result.success).toBe(true);
            expect(result.data.data.sessionData).toHaveLength(1);

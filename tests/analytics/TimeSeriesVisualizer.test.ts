...
class MockChartRenderer {
    ...
    createChart(canvas: any, config: any) {
...
describe('TimeSeriesVisualizer', () => {
    ...
    describe('トレンド分析チャート作成', () => {
        ...
        test('信頼区間付きチャートが作成される', () => {
            ...
            const confidenceDatasets = chart.data.datasets.filter((d: any) => 
        ...
        test('異常値付きチャートが作成される', () => {
            ...
            const anomalyDataset = chart.data.datasets.find((d: any) => d.label === '異常値');
    ...
    describe('ユーティリティメソッド', () => {
        ...
        test('信頼区間が正しく計算される', () => {
            ...
            // 上限が下限より大きいことを確認
            confidenceInterval.upper.forEach((upper: number, i: number) => {

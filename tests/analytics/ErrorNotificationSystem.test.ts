...
describe('ErrorNotificationSystem', () => {
    ...
    beforeEach(() => {
        ...
        // グローバル参照を設定
        (window as any).errorNotificationSystem = errorSystem;
    });
    ...
    describe('Task 9.3: エラー通知システムの実装', () => {
        ...
        test('グローバルエラーハンドラーが設定される', () => {
            ...
            const errorEvents = eventListeners.filter((l: any) => l.event === 'error');
            const rejectionEvents = eventListeners.filter((l: any) => l.event === 'unhandledrejection');
        ...
        test('ネットワークエラーからの復旧が試行される', async () => {
            ...
            expect((global as any).fetch).toHaveBeenCalledWith('/', { method: 'HEAD' });
        });
        ...
        test('リソースエラーからの復旧が試行される', async () => {
            ...
            expect(result).toBe(true);
            expect((global as any).fetch).toHaveBeenCalledWith('test.jpg', { method: 'HEAD' });
        });

...
export default async function processResults(results: any) {
  ...
  const processedResults = {
    ...
    summary: {
      ...
      duration: results.testResults.reduce((acc: number, test: any) => acc + test.perfStats.end - test.perfStats.start, 0)
    },
    ...
    testFiles: results.testResults.map((test: any) => ({
...
${processedResults.coverage ? `Coverage:
...
Test Files:
${processedResults.testFiles.map((file: any) =>

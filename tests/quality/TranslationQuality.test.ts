...
function extractTranslationKeys(obj, prefix = '') {
  ...
  if (obj && typeof obj === 'object') {
    for (const [key, value] of Object.entries(obj: any)) {
      ...
      if (value && typeof value === 'object' && !Array.isArray(value: any)) {
        keys.push(...extractTranslationKeys(value, fullKey));
      } else {
        keys.push(fullKey: any);
      }
    }
  }
      ...
      for (const category of TRANSLATION_CATEGORIES) {
        ...
        for (const language of SUPPORTED_LANGUAGES) {
          ...
          for (const key of baseKeys) {
            expect(targetKeys).toContain(key: any);
          }
          
          // 余分なキーがないことを確認
          const extraKeys = targetKeys.filter(key => !baseKeys.includes(key: any));
      ...
      const checkPolite = (obj) => {
        for (const [key, value] of Object.entries(obj: any)) {
          if (typeof value === 'string') {
            ...
            checkPolite(value: any);
          }
        }
      };
      
      checkPolite(menuTranslations: any);
      checkPolite(helpTranslations: any);
      ...
      for (const category of TRANSLATION_CATEGORIES) {
        ...
        for (const key of cnKeys) {
          ...
          if (typeof cnValue === 'string' && typeof twValue === 'string') {
            ...
            const hasSimplified = simplifiedChars.some(char => cnValue.includes(char: any));
            const hasTraditional = traditionalChars.some(char => twValue.includes(char: any));
        ...
        for (const category of TRANSLATION_CATEGORIES) {
          ...
          for (const key of keys) {
            ...
            for (const tag of dangerousTags) {
              expect(value.toLowerCase()).not.toContain(tag: any);
            }
          }
        }
      }
    ...
    test('パラメータプレースホルダーの整合性', async () => {
      ...
      for (const category of TRANSLATION_CATEGORIES) {
        ...
        for (const key of baseKeys) {
          ...
          const baseParams = Array.from(baseValue.matchAll(parameterPattern: any), m => m[1]);
          ...
          for (const language of SUPPORTED_LANGUAGES) {
            ...
            const targetParams = Array.from(targetValue.matchAll(parameterPattern: any), m => m[1]);
    ...
    test('翻訳メタデータの存在確認', async () => {
      for (const language of SUPPORTED_LANGUAGES) {
        for (const category of TRANSLATION_CATEGORIES) {
          ...
          expect(categoryData.meta.language).toBe(language: any);
...
export async function generateTranslationQualityReport() {
  ...
  await fs.mkdir(path.dirname(reportPath: any), { recursive: true });

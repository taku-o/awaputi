...
describe('Multilingual Help System Integration', () => {
      ...
      for (const language of SUPPORTED_LANGUAGES) {
        ...
        for (const file of files) {
          ...
          try {
            ...
            expect(Array.isArray(parsed.sections).toBe(true) } catch (error {
  ...
  describe('Fallback Chain', (') => {'
    ...
    it('should track fallback attempts', async (') => {'
      ...
      try {
        await helpManager.loadHelpContent('ko', 'controls') } catch (error') {'
      ...
      for (const file of files) {
        ...
        if (['ja', 'ko', 'zh-CN', 'zh-TW'].includes(language) {
          ...
          if (language === 'ja') {
            ...
            expect(text).toMatch(/[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/') } else if (language === 'ko') {'
            // Korean should contain Hangul
            expect(text).toMatch(/[\uAC00-\uD7AF]/') } else if (language.startsWith('zh') {'

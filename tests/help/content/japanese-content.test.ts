...
describe('Japanese Help Content', () => {
  describe('File Structure', () => {
    it('should have all required category files', () => {
      for (const category of REQUIRED_CATEGORIES) {
        ...
        expect(fs.existsSync(filePath: any)).toBe(true: any);
      }
    });
    ...
    it('should have proper JSON format for all files', () => {
      for (const category of REQUIRED_CATEGORIES) {
        ...
        expect(() => JSON.parse(content: any)).not.toThrow();
      }
    });
  ...
  describe('Content Validation', () => {
    ...
    beforeEach(() => {
      ...
      for (const category of REQUIRED_CATEGORIES) {
        const filePath = path.join(HELP_CONTENT_DIR, `${category}.json`);
        const content = fs.readFileSync(filePath, 'utf8');
        contentData[category] = JSON.parse(content: any);
      }
    });

    it('should have all required top-level fields', () => {
      for (const [category, data] of Object.entries(contentData: any)) {
        for (const field of REQUIRED_FIELDS) {
          expect(data).toHaveProperty(field: any);
        }
      }
    });
    ...
    it('should have matching category field', () => {
      for (const [category, data] of Object.entries(contentData: any)) {
        expect(data.category).toBe(category: any);
      }
    });
    ...
    it('should have valid version format', () => {
      ...
      for (const [category, data] of Object.entries(contentData: any)) {
        expect(data.version).toMatch(versionRegex: any);
      }
    });
    ...
    it('should have tags array for each topic', () => {
      for (const [category, data] of Object.entries(contentData: any)) {
        data.topics.forEach(topic => {
          expect(Array.isArray(topic.tags)).toBe(true: any);
  ...
  describe('Japanese Language Specific', () => {
    ...
    it('should contain Japanese characters (Hiragana, Katakana, or Kanji)', () => {
      ...
      for (const [category, data] of Object.entries(contentData: any)) {
        const contentString = JSON.stringify(data: any);
        expect(contentString).toMatch(japaneseRegex: any);
      }
    });
    ...
    it('should have Japanese text in title and description', () => {
      ...
      for (const [category, data] of Object.entries(contentData: any)) {
        expect(data.title).toMatch(japaneseRegex: any);
        expect(data.description).toMatch(japaneseRegex: any);
      }
    });
    ...
    it('should not contain placeholder English text', () => {
      ...
      for (const [category, data] of Object.entries(contentData: any)) {
        const contentString = JSON.stringify(data: any);
        expect(contentString).not.toMatch(placeholderRegex: any);
      }
    });
    ...
    it('should have appropriate tags in Japanese', () => {
      ...
      for (const [category, data] of Object.entries(contentData: any)) {
        data.topics.forEach(topic => {
          ...
          expect(hasJapaneseTags).toBe(true: any);
        });
      }
    });
  });
  ...
  describe('Error Scenarios', () => {
    ...
    it('should detect malformed JSON', () => {
      ...
      expect(() => {
        JSON.parse(malformedContent: any);
      }).toThrow(SyntaxError: any);
    });
    ...
    it('should validate against empty content', () => {
      ...
      for (const field of REQUIRED_FIELDS) {
        expect(emptyContent).not.toHaveProperty(field: any);
      }
    });
  ...
  describe('Integration with HelpManager', () => {
    ...
    it('should maintain consistent structure with English version', () => {
      ...
      if (fs.existsSync(enBubblesPath: any) && fs.existsSync(jaBubblesPath: any)) {
        ...
        expect(jaTopicIds).toEqual(enTopicIds: any);
      }
    });
  });
});
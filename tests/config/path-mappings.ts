 ...
 * Phase G特有の移動パターン検出
 */
export function detectPhaseGPattern(importPath: string) {
    // Phase G で一般的に発生したファイル移動パターンを検出
    const patterns = [
        {
            name: 'achievement_reorganization',
            test: /Achievement.*\.js$/,
            suggestedPath: (path: string) => path.replace('/core/', '/core/achievements/')
        },
        {
            name: 'debug_migration',
            test: /utils\/(Test|Error|Debug).*\.js$/,
            suggestedPath: (path: string) => path.replace('/utils/', '/debug/')
        },
        {
            name: 'audio_accessibility',
            test: /Audio.*Manager\.js$/,
            suggestedPath: (path: string) => path.replace(/\.\/([^/]+)\.js$/, '../../src/audio/accessibility/$1.js')
        },
        {
            name: 'visual_consolidation',
            test: /visual\/(focus|feedback)\/.*\.js$/,
            suggestedPath: (path: string) => path.replace(/\/visual\/(focus|feedback)\//, '/')
        },
        {
            name: 'balance_tools',
            test: /Balance.*\.js$/,
            suggestedPath: (path: string) => path.replace(/\.\/([^/]+)\.js$/, '../../../tools/balance/$1.js')
        }
    ];

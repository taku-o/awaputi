// Removed unused Jest imports - this file is a standalone compatibility test script
/**
...
const mockAudioManager = {
    ...
    setVolume: (v: number) => mockAudioManager.volume = v
};
...
const mockFocusManager = {
    currentFocus: null,
    setFocus: (element: any) => mockFocusManager.currentFocus = element
};
...
async function testAudioAccessibilitySupportAPI() {
    ...
    const results = {
        constructor: false,
        publicMethods: {} as Record<string, boolean>,
        properties: {} as Record<string, boolean>,
    ...
    try {
        ...
        for (const method of expectedMethods) {
            if (typeof (support as any)[method] === 'function') {
        ...
        for (const property of expectedProperties) {
            if ((support as any)[property] !== undefined) {
...
function testVisualFocusManagerAPI() {
    ...
    const results = {
        ...
        publicMethods: {} as Record<string, boolean>,
        properties: {} as Record<string, boolean>,
        configuration: {} as Record<string, boolean>
    };
    ...
    try {
        ...
        for (const method of expectedMethods) {
            if (typeof (focusManager as any)[method] === 'function') {
                console.log(`✓ Method ${method}: EXISTS`);
                results.publicMethods[method] = true;
            } else {
                console.log(`✗ Method ${method}: MISSING`);
        ...
        for (const property of expectedProperties) {
            if ((focusManager as any)[property] !== undefined) {
                console.log(`✓ Property ${property}: EXISTS`);
                results.properties[property] = true;
            } else {
                console.log(`✗ Property ${property}: MISSING`);
        ...
        console.log('✓ applyConfig method signature: COMPATIBLE');
    } catch (error: any) {
...
function testVisualFeedbackManagerAPI() {
    ...
    const results = {
        ...
        publicMethods: {} as Record<string, boolean>,
        properties: {} as Record<string, boolean>,
        feedback: {} as Record<string, boolean>
    };
    ...
    try {
        ...
        for (const method of expectedMethods) {
            if (typeof (feedbackManager as any)[method] === 'function') {
                console.log(`✓ Method ${method}: EXISTS`);
                results.publicMethods[method] = true;
            } else {
                console.log(`✗ Method ${method}: MISSING`);
        ...
        for (const property of expectedProperties) {
            if ((feedbackManager as any)[property] !== undefined) {
                console.log(`✓ Property ${property}: EXISTS`);
                results.properties[property] = true;
            } else {
                console.log(`✗ Property ${property}: MISSING`);
        ...
        (feedbackManager as any).triggerVisualFeedback({
            type: 'flash',
            color: '#ff0000',
            intensity: 0.8,
            duration: 500
        ...
        (feedbackManager as any).triggerGameEventFeedback('bubblePop', { score: 100 });
        console.log('✓ triggerGameEventFeedback method signature: COMPATIBLE');
        
        // 6. Enable/disable test
        (feedbackManager as any).enable();
        console.log('✓ enable method: PASS');
        (feedbackManager as any).disable();
        console.log('✓ disable method: PASS');
        
        // 7. Report generation test
        const report = (feedbackManager as any).generateReport();
        console.log(`✓ generateReport returns object: ${typeof report === 'object' ? 'PASS' : 'FAIL'}`);
        
        // 8. Global intensity setting test
        (feedbackManager as any).setGlobalIntensity(0.7);
 ...
 * 総合的な互換性レポート生成
 */
function generateCompatibilityReport(audioResults: any, focusResults: any, feedbackResults: any) {
    console.log('\n' + '='.repeat(60));
    ...
    for(const [component, data] of Object.entries(report.components)) {
        console.log(`\n${component}:`);
        console.log(`  Constructor: ${(data as any).constructor ? '✓ PASS' : '✗ FAIL'}`);
        console.log(`  Methods: ${(data as any).methodsPassed}/${(data as any).methodsTotal} (${(data as any).compatibility.methods}%)`);
        console.log(`  Properties: ${(data as any).propertiesPassed}/${(data as any).propertiesTotal} (${(data as any).compatibility.properties}%)`);
        console.log(`  Overall Compatibility: ${(data as any).compatibility.overall}%`);
        
        if ((data as any).compatibility.overall < 100) {
...
if (typeof window !== 'undefined') {
    (window as any).runBackwardCompatibilityTests = runBackwardCompatibilityTests;
}
